...
class Entity {
    ...
    public function __construct(array $data = []) {
        ...
        if (false !== strpos($this->token, 'mebel')) {
            $this->setDescription('Соберем любой шкаф, и&nbsp;при этом не&nbsp;останется ни&nbsp;одной &laquo;лишней&raquo; детали. Занесем диван хоть на&nbsp;35-й&nbsp;этаж (а&nbsp;можем и на 36-й). Повесим все необходимые шкафчики на&nbsp;кухне в&nbsp;правильной последовательности (и&nbsp;обязательно уберём за&nbsp;собой весь мусор). Сборка и установка мебели осуществляются на следующий день после доставки.');
        } else if (false !== strpos($this->token, 'bitovaya-tehnika')) {
            $this->setDescription('Подключим стиральную и посудомоечную машину, подвесим телевизор на настенный кронштейн, установим любую кухонную технику,<br/>смонтируем и подключим проточный или накопительный водонагреватель.');
    ...
    public function getImageUrl($size = 1) {
        return $this->image
            ? ($this->getHost() . \App::config()->serviceCategory['url'][$size] . $this->image)
            : null;
    }
