...
class wp_xmlrpc_server extends IXR_Server {
	...
	function login_pass_ok($user_login, $user_pass) {
		if ( !get_option( 'enable_xmlrpc' ) ) {
			$this->error = new IXR_Error( 405, sprintf( wp__( 'XML-RPC services are disabled on this site.  An admin user can enable them at %s'),  admin_url('options-writing.php') ) );
			return false;
		}

		if (!user_pass_ok($user_login, $user_pass)) {
			$this->error = new IXR_Error(403, wp__('Bad login/pass combination.'));
			return false;
		}
		return true;
	}
	...
	function initialise_blog_option_info( ) {
		...
		$this->blog_options = array(
			// Read only options
			'software_name'		=> array(
				'desc'			=> wp__( 'Software Name' ),
				'readonly'		=> true,
				'value'			=> 'WordPress'
			),
			'software_version'	=> array(
				'desc'			=> wp__( 'Software Version' ),
				'readonly'		=> true,
				'value'			=> $wp_version
			),
			'blog_url'			=> array(
			...
			'blog_title'		=> array(
				'desc'			=> wp__( 'Site Title' ),
				'readonly'		=> false,
				'option'			=> 'blogname'
			),
			'blog_tagline'		=> array(
				'desc'			=> wp__( 'Site Tagline' ),
				'readonly'		=> false,
				'option'		=> 'blogdescription'
			),
			'date_format'		=> array(
				'desc'			=> wp__( 'Date Format' ),
				'readonly'		=> false,
				'option'		=> 'date_format'
			),
			'time_format'		=> array(
				'desc'			=> wp__( 'Time Format' ),
				'readonly'		=> false,
				'option'		=> 'time_format'
			),
			'users_can_register'	=> array(
				'desc'			=> wp__( 'Allow new users to sign up' ),
				'readonly'		=> false,
				'option'		=> 'users_can_register'
			),
			'thumbnail_size_w'	=> array(
				'desc'			=> wp__( 'Thumbnail Width' ),
				'readonly'		=> false,
				'option'		=> 'thumbnail_size_w'
			),
			'thumbnail_size_h'	=> array(
				'desc'			=> wp__( 'Thumbnail Height' ),
				'readonly'		=> false,
				'option'		=> 'thumbnail_size_h'
			),
			'thumbnail_crop'	=> array(
				'desc'			=> wp__( 'Crop thumbnail to exact dimensions' ),
				'readonly'		=> false,
				'option'		=> 'thumbnail_crop'
			),
			'medium_size_w'	=> array(
				'desc'			=> wp__( 'Medium size image width' ),
				'readonly'		=> false,
				'option'		=> 'medium_size_w'
			),
			'medium_size_h'	=> array(
				'desc'			=> wp__( 'Medium size image height' ),
				'readonly'		=> false,
				'option'		=> 'medium_size_h'
			),
			'large_size_w'	=> array(
				'desc'			=> wp__( 'Large size image width' ),
				'readonly'		=> false,
				'option'		=> 'large_size_w'
			),
			'large_size_h'	=> array(
				'desc'			=> wp__( 'Large size image height' ),
				'readonly'		=> false,
				'option'		=> 'large_size_h'
			)
		);
	...
	function wp_getPage($args) {
		...
		if ( !current_user_can( 'edit_page', $page_id ) )
			return new IXR_Error( 401, wp__( 'Sorry, you cannot edit this page.' ) );
		...
		// If the page doesn't exist indicate that.
			...
			return(new IXR_Error(404, wp__('Sorry, no such page.')));
		}
	}
	...
	function wp_getPages($args) {
		...
		if ( !current_user_can( 'edit_pages' ) )
			return new IXR_Error( 401, wp__( 'Sorry, you cannot edit pages.' ) );
	...
	function wp_newPage($args) {
		...
		if ( !current_user_can('publish_pages') )
			return(new IXR_Error(401, wp__('Sorry, you cannot add new pages.')));
	...
	function wp_deletePage($args) {
		...
		if ( !current_user_can('delete_page', $page_id) )
			return(new IXR_Error(401, wp__('Sorry, you do not have the right to delete this page.')));

		// Attempt to delete the page.
		$result = wp_delete_post($page_id);
		if ( !$result )
			return(new IXR_Error(500, wp__('Failed to delete the page.')));
	...
	function wp_editPage($args) {
		...
		if ( !current_user_can('edit_page', $page_id) )
			return(new IXR_Error(401, wp__('Sorry, you do not have the right to edit this page.')));
	...
	function wp_getAuthors($args) {
		...
		if ( !current_user_can('edit_posts') )
			return(new IXR_Error(401, wp__('Sorry, you cannot edit posts on this site.')));
	...
	function wp_getTags( $args ) {
		...
		if ( !current_user_can( 'edit_posts' ) )
			return new IXR_Error( 401, wp__( 'Sorry, you must be able to edit posts on this site in order to view tags.' ) );
	...
	function wp_newCategory($args) {
		...
		if ( !current_user_can('manage_categories') )
			return(new IXR_Error(401, wp__('Sorry, you do not have the right to add a category.')));
		...
		if ( is_wp_error( $cat_id ) ) {
			if ( 'term_exists' == $cat_id->get_error_code() )
				...
				return(new IXR_Error(500, wp__('Sorry, the new category failed.')));
		} elseif ( ! $cat_id ) {
			return(new IXR_Error(500, wp__('Sorry, the new category failed.')));
		}
	...
	function wp_deleteCategory($args) {
		...
		if ( !current_user_can('manage_categories') )
			return new IXR_Error( 401, wp__( 'Sorry, you do not have the right to delete a category.' ) );
	...
	function wp_suggestCategories($args) {
		...
		if ( !current_user_can( 'edit_posts' ) )
			return new IXR_Error( 401, wp__( 'Sorry, you must be able to edit posts to this site in order to view categories.' ) );
	...
	function wp_getComment($args) {
		...
		if ( !current_user_can( 'moderate_comments' ) )
			return new IXR_Error( 403, wp__( 'You are not allowed to moderate comments on this site.' ) );

		do_action('xmlrpc_call', 'wp.getComment');

		if ( ! $comment = get_comment($comment_id) )
			return new IXR_Error( 404, wp__( 'Invalid comment ID.' ) );
	...
	function wp_getComments($args) {
		...
		if ( !current_user_can( 'moderate_comments' ) )
			return new IXR_Error( 401, wp__( 'Sorry, you cannot edit comments.' ) );
	...
	function wp_editComment($args) {
		...
		if ( isset($content_struct['status']) ) {
			...
			if ( ! in_array($content_struct['status'], $statuses) )
				return new IXR_Error( 401, wp__( 'Invalid comment status.' ) );
		...
		if ( !$result )
			return new IXR_Error(500, wp__('Sorry, the comment could not be edited. Something wrong happened.'));
	...
	function wp_newComment($args) {
		...
		if ( !$user ) {
			...
			if ( $allow_anon && get_option('comment_registration') )
				return new IXR_Error( 403, wp__( 'You must be registered to comment' ) );
			else if ( !$allow_anon )
				return $this->error;
		} else {
			$logged_in = true;
		...
		if ( ! $post_id )
			return new IXR_Error( 404, wp__( 'Invalid post ID.' ) );
		...
		if ( $logged_in ) {
			...
			if ( get_option('require_name_email') ) {
				if ( 6 > strlen($comment['comment_author_email']) || '' == $comment['comment_author'] )
					return new IXR_Error( 403, wp__( 'Comment author name and email are required' ) );
				elseif ( !is_email($comment['comment_author_email']) )
					return new IXR_Error( 403, wp__( 'A valid email address is required' ) );
			}
		}
	...
	function wp_getCommentStatusList($args) {
		...
		if ( !current_user_can( 'moderate_comments' ) )
			return new IXR_Error( 403, wp__( 'You are not allowed access to details about this site.' ) );
	...
	function wp_getCommentCount( $args ) {
		...
		if ( !current_user_can( 'edit_posts' ) )
			return new IXR_Error( 403, wp__( 'You are not allowed access to details about comments.' ) );
	...
	function wp_setOptions( $args ) {
		...
		if ( !current_user_can( 'manage_options' ) )
			return new IXR_Error( 403, wp__( 'You are not allowed to update options.' ) );
	...
	function wp_getMediaItem($args) {
		...
		if ( !current_user_can( 'upload_files' ) )
			return new IXR_Error( 403, wp__( 'You are not allowed to upload files to this site.' ) );

		do_action('xmlrpc_call', 'wp.getMediaItem');

		if ( ! $attachment = get_post($attachment_id) )
			return new IXR_Error( 404, wp__( 'Invalid attachment ID.' ) );
	...
	function wp_getMediaLibrary($args) {
		...
		if ( !current_user_can( 'upload_files' ) )
			return new IXR_Error( 401, wp__( 'Sorry, you cannot upload files.' ) );
	...
	function blogger_getUserInfo($args) {
		...
		if ( !current_user_can( 'edit_posts' ) )
			return new IXR_Error( 401, wp__( 'Sorry, you do not have access to user data on this site.' ) );
	...
	function blogger_getPost($args) {
		...
		if ( !current_user_can( 'edit_post', $post_ID ) )
			return new IXR_Error( 401, wp__( 'Sorry, you cannot edit this post.' ) );
	...
	function blogger_getRecentPosts($args) {
		...
		if ( !$posts_list ) {
			$this->error = new IXR_Error(500, wp__('Either there are no posts, or something went wrong.'));
			return $this->error;
		}
	...
	function blogger_getTemplate($args) {
		...
		if ( !current_user_can('edit_themes') )
			return new IXR_Error(401, wp__('Sorry, this user can not edit the template.'));
	...
	function blogger_setTemplate($args) {
		...
		if ( !current_user_can('edit_themes') )
			return new IXR_Error(401, wp__('Sorry, this user cannot edit the template.'));
		...
		if ($f = fopen($filename, 'w+')) {
			...
			return new IXR_Error(500, wp__('Either the file is not writable, or something wrong happened. The file has not been updated.'));
		}
	...
	function blogger_newPost($args) {
		...
		if ( !current_user_can($cap) )
			return new IXR_Error(401, wp__('Sorry, you are not allowed to post on this site.'));
		...
		if ( !$post_ID )
			return new IXR_Error(500, wp__('Sorry, your entry could not be posted. Something wrong happened.'));
	...
	function blogger_editPost($args) {
		...
		if ( !$actual_post || $actual_post['post_type'] != 'post' )
			return new IXR_Error(404, wp__('Sorry, no such post.'));

		$this->escape($actual_post);

		if ( !current_user_can('edit_post', $post_ID) )
			return new IXR_Error(401, wp__('Sorry, you do not have the right to edit this post.'));

		extract($actual_post, EXTR_SKIP);

		if ( ('publish' == $post_status) && !current_user_can('publish_posts') )
			return new IXR_Error(401, wp__('Sorry, you do not have the right to publish this post.'));
		...
		if ( !$result )
			return new IXR_Error(500, wp__('For some strange yet very annoying reason, this post could not be edited.'));
	...
	function blogger_deletePost($args) {
		...
		if ( !current_user_can('delete_post', $post_ID) )
			return new IXR_Error(401, wp__('Sorry, you do not have the right to delete this post.'));

		$result = wp_delete_post($post_ID);

		if ( !$result )
			return new IXR_Error(500, wp__('For some strange yet very annoying reason, this post could not be deleted.'));
	...
	function mw_newPost($args) {
		...
		if ( !empty( $content_struct['post_type'] ) ) {
			if ( $content_struct['post_type'] == 'page' ) {
				...
				elseif ('publish' == $content_struct['page_status'])
					...
					$cap = 'edit_pages';
				$error_message = wp__( 'Sorry, you are not allowed to publish pages on this site.' );
				$post_type = 'page';
				...
				elseif ('publish' == $content_struct['post_status'])
					...
					$cap = 'edit_posts';
				$error_message = wp__( 'Sorry, you are not allowed to publish posts on this site.' );
				$post_type = 'post';
			} else {
				// No other post_type values are allowed here
				return new IXR_Error( 401, wp__( 'Invalid post type.' ) );
			}
		} else {
			if ( $publish )
				$cap  = 'publish_posts';
		...
		if ( isset( $content_struct['wp_post_format'] ) ) {
			...
			if ( !array_key_exists( $content_struct['wp_post_format'], get_post_format_strings() ) ) {
				return new IXR_Error( 404, wp__( 'Invalid post format' ) );
			}
		}
			...
			switch ( $post_type ) {
				case "post":
					if ( !current_user_can('edit_others_posts') )
						return(new IXR_Error(401, wp__('You are not allowed to post as this user')));
					break;
				case "page":
					if ( !current_user_can('edit_others_pages') )
						return(new IXR_Error(401, wp__('You are not allowed to create pages as this user')));
					break;
				default:
					return(new IXR_Error(401, wp__('Invalid post type.')));
					break;
	...
	function mw_editPost($args) {
		...
		if ( isset($content_struct['wp_author_id']) && ($user->ID != $content_struct['wp_author_id']) ) {
			switch ( $post_type ) {
				case 'post':
					if ( !current_user_can('edit_others_posts') )
						return(new IXR_Error(401, wp__('You are not allowed to change the post author as this user.')));
					break;
				case 'page':
					if ( !current_user_can('edit_others_pages') )
						return(new IXR_Error(401, wp__('You are not allowed to change the page author as this user.')));
					break;
				default:
					return(new IXR_Error(401, wp__('Invalid post type.')));
					break;
		...
		if ( ('publish' == $post_status) ) {
			if ( ( 'page' == $post_type ) && !current_user_can('publish_pages') )
				return new IXR_Error(401, wp__('Sorry, you do not have the right to publish this page.'));
		...
		if ( !$result )
			return new IXR_Error(500, wp__('Sorry, your entry could not be edited. Something wrong happened.'));
	...
	function mw_getCategories($args) {
		...
		if ( !current_user_can( 'edit_posts' ) )
			return new IXR_Error( 401, wp__( 'Sorry, you must be able to edit posts on this site in order to view categories.' ) );
	...
	function mw_newMediaObject($args) {
		...
		if ( !current_user_can('upload_files') ) {
			...
			$this->error = new IXR_Error(401, wp__('You are not allowed to upload files to this site.'));
			return $this->error;
		}
		...
		if ( ! empty($upload['error']) ) {
			$errorString = sprintf(wp__('Could not write file %1$s (%2$s)'), $name, $upload['error']);
	...
	function mt_getPostCategories($args) {
		...
		if ( !current_user_can( 'edit_post', $post_ID ) )
			return new IXR_Error( 401, wp__( 'Sorry, you can not edit this post.' ) );
	...
	function mt_publishPost($args) {
		...
		if ( !current_user_can('publish_posts') || !current_user_can('edit_post', $post_ID) )
			return new IXR_Error(401, wp__('Sorry, you cannot publish this post.'));
	...
	function pingback_ping($args) {
		...
		if ( !$pos1 )
			return new IXR_Error(0, wp__('Is there no link to us?'));
		...
		if ( $post_ID = url_to_postid($pagelinkedto) ) {
			...
			// TODO: Attempt to extract a post ID from the given URL
	  		return new IXR_Error(33, wp__('The specified target URL cannot be used as a target. It either doesn&#8217;t exist, or it is not a pingback-enabled resource.'));
		}
		$post_ID = (int) $post_ID;
		...
		if ( $post_ID == url_to_postid($pagelinkedfrom) )
			return new IXR_Error(0, wp__('The source URL and the target URL cannot both point to the same resource.'));
		...
		if ( $wpdb->get_results( $wpdb->prepare("SELECT * FROM $wpdb->comments WHERE comment_post_ID = %d AND comment_author_url = %s", $post_ID, $pagelinkedfrom) ) )
			return new IXR_Error( 48, wp__( 'The pingback has already been registered.' ) );
		...
		if ( !$linea )
	  		return new IXR_Error(16, wp__('The source URL does not exist.'));
		...
		$title = $matchtitle[1];
		if ( empty( $title ) )
			return new IXR_Error(32, wp__('We cannot find a title on that page.'));
		...
		if ( empty($context) ) // Link to target not found
			return new IXR_Error(17, wp__('The source URL does not contain a link to the target URL, and so cannot be used as a source.'));
		...
		return sprintf(wp__('Pingback from %1$s to %2$s registered. Keep the web talking! :-)'), $pagelinkedfrom, $pagelinkedto);
	}
	...
	function pingback_extensions_getPingbacks($args) {
		...
		if ( !$actual_post ) {
			// No such post = resource not found
	  		return new IXR_Error(32, wp__('The specified target URL does not exist.'));
		}
