...
class WP_User {
	...
	function __isset( $key ) {
		if ( 'id' == $key ) {
			_deprecated_argument( 'WP_User->id', '2.1', wp__( 'Use <code>WP_User->ID</code> instead.' ) );
			$key = 'ID';
		}
	...
	function has_cap( $cap ) {
		if ( is_numeric( $cap ) ) {
			_deprecated_argument( __FUNCTION__, '2.0', wp__('Usage of user levels by plugins and themes is deprecated. Use roles and capabilities instead.') );
