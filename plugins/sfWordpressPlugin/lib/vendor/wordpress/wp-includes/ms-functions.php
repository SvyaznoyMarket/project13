...
function add_user_to_blog( $blog_id, $user_id, $role ) {
	...
	if ( empty( $user->ID ) ) {
		...
		return new WP_Error('user_does_not_exist', wp__('That user does not exist.'));
	}
...
function create_empty_blog( $domain, $path, $weblog_title, $site_id = 1 ) {
	...
	if ( domain_exists($domain, $path, $site_id) )
		return wp__( '<strong>ERROR</strong>: Site URL already taken.' );
	...
	if ( ! $blog_id = insert_blog($domain, $path, $site_id) )
		return wp__( '<strong>ERROR</strong>: problem creating site entry.' );
...
function wpmu_validate_user_signup($user_name, $user_email) {
	...
	if ( $user_name != $orig_username || $user_name != $maybe[0] ) {
		$errors->add( 'user_name', wp__( 'Only lowercase letters (a-z) and numbers are allowed.' ) );
	...
	if ( empty( $user_name ) )
	   	$errors->add('user_name', wp__('Please enter a username'));
	...
	if ( in_array( $user_name, $illegal_names ) == true )
		$errors->add('user_name',  wp__('That username is not allowed'));

	if ( is_email_address_unsafe( $user_email ) )
		$errors->add('user_email',  wp__('You cannot use that email address to signup. We are having problems with them blocking some of our email. Please use another email provider.'));

	if ( strlen( $user_name ) < 4 )
		$errors->add('user_name',  wp__('Username must be at least 4 characters'));

	if ( strpos( ' ' . $user_name, '_' ) != false )
		$errors->add( 'user_name', wp__( 'Sorry, usernames may not contain the character &#8220;_&#8221;!' ) );

	// all numeric?
	$match = array();
	preg_match( '/[0-9]*/', $user_name, $match );
	if ( $match[0] == $user_name )
		$errors->add('user_name', wp__('Sorry, usernames must have letters too!'));

	if ( !is_email( $user_email ) )
		$errors->add('user_email', wp__('Please enter a correct email address'));

	$limited_email_domains = get_site_option( 'limited_email_domains' );
	if ( is_array( $limited_email_domains ) && empty( $limited_email_domains ) == false ) {
		$emaildomain = substr( $user_email, 1 + strpos( $user_email, '@' ) );
		if ( in_array( $emaildomain, $limited_email_domains ) == false )
			$errors->add('user_email', wp__('Sorry, that email address is not allowed!'));
	}

	// Check if the username has been used already.
	if ( username_exists($user_name) )
		$errors->add('user_name', wp__('Sorry, that username already exists!'));

	// Check if the email address has been used already.
	if ( email_exists($user_email) )
		$errors->add('user_email', wp__('Sorry, that email address is already used!'));
	...
	if ( $signup != null ) {
		...
		if ( $diff > 172800 )
			...
			$errors->add('user_name', wp__('That username is currently reserved but may be available in a couple of days.'));

		if ( $signup->active == 0 && $signup->user_email == $user_email )
			$errors->add('user_email_used', wp__('username and email used'));
	}
	...
	if ( $signup != null ) {
		...
		if ( $diff > 172800 )
			...
			$errors->add('user_email', wp__('That email address has already been used. Please check your inbox for an activation email. It will become available in a couple of days if you do nothing.'));
	}
...
function wpmu_validate_blog_signup($blogname, $blog_title, $user = '') {
	...
	if ( empty( $blogname ) )
		$errors->add('blogname', wp__('Please enter a site name'));

	if ( preg_match( '/[^a-z0-9]+/', $blogname ) )
		$errors->add('blogname', wp__('Only lowercase letters and numbers allowed'));

	if ( in_array( $blogname, $illegal_names ) == true )
		$errors->add('blogname',  wp__('That name is not allowed'));

	if ( strlen( $blogname ) < 4 && !is_super_admin() )
		$errors->add('blogname',  wp__('Site name must be at least 4 characters'));

	if ( strpos( ' ' . $blogname, '_' ) != false )
		$errors->add( 'blogname', wp__( 'Sorry, site names may not contain the character &#8220;_&#8221;!' ) );
	...
	if ( !is_subdomain_install() && $wpdb->get_var( $wpdb->prepare( "SELECT post_name FROM " . $wpdb->get_blog_prefix( $current_site->blog_id ) . "posts WHERE post_type = 'page' AND post_name = %s", $blogname ) ) )
		$errors->add( 'blogname', wp__( 'Sorry, you may not use that site name.' ) );

	// all numeric?
	$match = array();
	preg_match( '/[0-9]*/', $blogname, $match );
	if ( $match[0] == $blogname )
		$errors->add('blogname', wp__('Sorry, site names must have letters too!'));
	...
	if ( empty( $blog_title ) )
		$errors->add('blog_title', wp__('Please enter a site title'));
	...
	if ( domain_exists($mydomain, $path) )
		$errors->add('blogname', wp__('Sorry, that site already exists!'));

	if ( username_exists( $blogname ) ) {
		if ( is_object( $user ) == false || ( is_object($user) && ( $user->user_login != $blogname ) ) )
			$errors->add( 'blogname', wp__( 'Sorry, that site is reserved!' ) );
	}
	...
	if ( ! empty($signup) ) {
		...
		if ( $diff > 172800 )
			...
			$errors->add('blogname', wp__('That site is currently reserved but may be available in a couple days.'));
	}
...
function wpmu_signup_blog_notification($domain, $path, $title, $user, $user_email, $key, $meta = '') {
	...
	$message = sprintf(
		apply_filters( 'wpmu_signup_blog_notification_email',
			wp__( "To activate your blog, please click the following link:\n\n%s\n\nAfter you activate, you will receive *another email* with your login.\n\nAfter you activate, you can visit your site here:\n\n%s" ),
...
function wpmu_signup_user_notification($user, $user_email, $key, $meta = '') {
	...
	$message = sprintf(
		apply_filters( 'wpmu_signup_user_notification_email',
			wp__( "To activate your user, please click the following link:\n\n%s\n\nAfter you activate, you will receive *another email* with your login.\n\n" ),
...
function wpmu_activate_signup($key) {
	...
	if ( empty( $signup ) )
		return new WP_Error( 'invalid_key', wp__( 'Invalid activation key.' ) );

	if ( $signup->active ) {
		if ( empty( $signup->domain ) )
			return new WP_Error( 'already_active', wp__( 'The user is already active.' ), $signup );
		else
			return new WP_Error( 'already_active', wp__( 'The site is already active.' ), $signup );
	}
	...
	if ( ! $user_id )
		return new WP_Error('create_user', wp__('Could not create user'), $signup);
	...
	if ( empty($signup->domain) ) {
		...
		if ( isset( $user_already_exists ) )
			return new WP_Error( 'user_already_exists', wp__( 'That username is already activated.' ), $signup);
...
function wpmu_create_blog($domain, $path, $title, $user_id, $meta = '', $site_id = 1) {
	...
	// Check if the domain has been used already. We should return an error message.
	if ( domain_exists($domain, $path, $site_id) )
		return new WP_Error('blog_taken', wp__('Site already exists.'));

	if ( !defined('WP_INSTALLING') )
		define( 'WP_INSTALLING', true );

	if ( ! $blog_id = insert_blog($domain, $path, $site_id) )
		return new WP_Error('insert_blog', wp__('Could not create site.'));
...
Disable these notifications: %4s' ), $blogname, $siteurl, $_SERVER['REMOTE_ADDR'], $options_site_url);
	...
	wp_mail( $email, sprintf( wp__( 'New Site Registration: %s' ), $siteurl ), $msg );
	return true;
}
...
Disable these notifications: %3s'), $user->user_login, $_SERVER['REMOTE_ADDR'], $options_site_url);
	...
	wp_mail( $email, sprintf(wp__('New User Registration: %s'), $user->user_login), $msg );
	return true;
}
...
function install_blog($blog_id, $blog_title = '') {
	...
	if ( $wpdb->get_results("SELECT ID FROM $wpdb->posts") )
		die(wp__('<h1>Already Installed</h1><p>You appear to have already installed WordPress. To reinstall please clear your old database tables first.</p>') . '</body></html>');
...
function wpmu_welcome_notification($blog_id, $user_id, $password, $title, $meta = '') {
	...
	if ( $welcome_email == false )
		$welcome_email = stripslashes( wp__( 'Dear User,
...
--The Team @ SITE_NAME' ) );
	...
	$subject = apply_filters( 'update_welcome_subject', sprintf(wp__('New %1$s Site: %2$s'), $current_site->site_name, stripslashes( $title ) ) );
...
function wpmu_welcome_user_notification($user_id, $password, $meta = '') {
	...
	$subject = apply_filters( 'update_welcome_user_subject', sprintf(wp__('New %1$s User: %2$s'), $current_site->site_name, $user->user_login) );
	wp_mail($user->user_email, $subject, $message, $message_headers);
	return true;
}
...
function upload_is_file_too_big( $upload ) {
	...
	if ( strlen( $upload['bits'] )  > ( 1024 * get_site_option( 'fileupload_maxk', 1500 ) ) )
		return sprintf( wp__( 'This file is too big. Files must be less than %d KB in size.' ) . '<br />', get_site_option( 'fileupload_maxk', 1500 ));
...
function maybe_add_existing_user_to_blog() {
	...
	if ( empty( $details ) || is_wp_error( add_existing_user_to_blog( $details ) ) )
		wp_die( sprintf(wp__('An error occurred adding you to this site. Back to the <a href="%s">homepage</a>.'), site_url() ) );

	wp_die( sprintf(wp__('You have been added to this site. Please visit the <a href="%s">homepage</a> or <a href="%s">log in</a> using your username and password.'), site_url(), admin_url() ), wp__('Success') );
}
