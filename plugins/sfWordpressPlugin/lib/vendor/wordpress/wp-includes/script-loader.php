...
function wp_default_scripts( &$scripts ) {
	...
	$scripts->localize( 'common', 'commonL10n', array(
		'warnDelete' => wp__("You are about to permanently delete the selected items.\n  'Cancel' to stop, 'OK' to delete.")
	) );
	...
	$scripts->localize( 'quicktags', 'quicktagsL10n', array(
		'wordLookup' => wp__('Enter a word to look up:'),
		'dictionaryLookup' => esc_attr(wp__('Dictionary lookup')),
		'lookup' => esc_attr(wp__('lookup')),
		'closeAllOpenTags' => esc_attr(wp__('Close all open tags')),
		'closeTags' => esc_attr(wp__('close tags')),
		'enterURL' => wp__('Enter the URL'),
		'enterImageURL' => wp__('Enter the URL of the image'),
		'enterImageDescription' => wp__('Enter a description of the image'),
		'fullscreen' => wp__('fullscreen'),
		'toggleFullscreen' => esc_attr( wp__('Toggle fullscreen mode') )
	) );
	...
	$scripts->localize( 'wp-ajax-response', 'wpAjax', array(
		'noPerm' => wp__('You do not have permission to do that.'),
		'broken' => wp__('An unidentified error has occurred.')
	) );
	...
	$scripts->localize( 'thickbox', 'thickboxL10n', array(
			...
			'of' => wp__('of'),
			'close' => wp__('Close'),
			'noiframes' => wp__('This feature requires inline frames. You have iframes disabled or your browser does not support them.'),
	...
	if ( empty($max_upload_size) )
		$max_upload_size = wp__('not configured');

	// error message for both plupload and swfupload
	$uploader_l10n = array(
		'queue_limit_exceeded' => wp__('You have attempted to queue too many files.'),
		'file_exceeds_size_limit' => wp__('%s exceeds the maximum upload size for this site.'),
		'zero_byte_file' => wp__('This file is empty. Please try another.'),
		'invalid_filetype' => wp__('This file type is not allowed. Please try another.'),
		'not_an_image' => wp__('This file is not an image. Please try another.'),
		'image_memory_exceeded' => wp__('Memory exceeded. Please try another smaller file.'),
		'image_dimensions_exceeded' => wp__('This is larger than the maximum size. Please try another.'),
		'default_error' => wp__('An error occurred in the upload. Please try again later.'),
		'missing_upload_url' => wp__('There was a configuration error. Please contact the server administrator.'),
		'upload_limit_exceeded' => wp__('You may only upload 1 file.'),
		'http_error' => wp__('HTTP error.'),
		'upload_failed' => wp__('Upload failed.'),
		'big_upload_failed' => wp__('Please try uploading this file with the %1$sbrowser uploader%2$s.'),
		'big_upload_queued' => wp__('%s exceeds the maximum upload size for the multi-file uploader when used in your browser.'),
		'io_error' => wp__('IO error.'),
		'security_error' => wp__('Security error.'),
		'file_cancelled' => wp__('File canceled.'),
		'upload_stopped' => wp__('Upload stopped.'),
		'dismiss' => wp__('Dismiss'),
		'crunching' => wp__('Crunching&hellip;'),
		'deleted' => wp__('moved to the trash.'),
		'error_uploading' => wp__('&#8220;%s&#8221; has failed to upload.')
	);
	...
	$scripts->localize( 'password-strength-meter', 'pwsL10n', array(
		'empty' => wp__('Strength indicator'),
		'short' => wp__('Very weak'),
		'bad' => wp__('Weak'),
		/* translators: password strength */
		'good' => _x('Medium', 'password strength'),
		'strong' => wp__('Strong'),
		'mismatch' => wp__('Mismatch')
	) );
	...
	$scripts->localize( 'wplink', 'wpLinkL10n', array(
		'title' => wp__('Insert/edit link'),
		'update' => wp__('Update'),
		'save' => wp__('Add Link'),
		'noTitle' => wp__('(no title)'),
		'noMatchesFound' => wp__('No matches found.')
	) );
	...
	if ( is_admin() ) {
		...
		$scripts->localize( 'ajaxcat', 'catL10n', array(
			'add' => esc_attr(wp__('Add')),
			'how' => wp__('Separate multiple categories with commas.')
		) );
		...
		$scripts->localize( 'admin-comments', 'adminCommentsL10n', array(
			...
			'replyApprove' => wp__( 'Approve and Reply' ),
			'reply' => wp__( 'Reply' )
		) );
		...
		$scripts->localize( 'post', 'postL10n', array(
			'ok' => wp__('OK'),
			'cancel' => wp__('Cancel'),
			'publishOn' => wp__('Publish on:'),
			'publishOnFuture' =>  wp__('Schedule for:'),
			'publishOnPast' => wp__('Published on:'),
			'showcomm' => wp__('Show more comments'),
			'endcomm' => wp__('No more comments found.'),
			'publish' => wp__('Publish'),
			'schedule' => wp__('Schedule'),
			'update' => wp__('Update'),
			'savePending' => wp__('Save as Pending'),
			'saveDraft' => wp__('Save Draft'),
			'private' => wp__('Private'),
			'public' => wp__('Public'),
			'publicSticky' => wp__('Public, Sticky'),
			'password' => wp__('Password Protected'),
			'privatelyPublished' => wp__('Privately Published'),
			'published' => wp__('Published')
		) );
		...
		$scripts->localize( 'inline-edit-post', 'inlineEditL10n', array(
			'error' => wp__('Error while saving the changes.'),
			'ntdeltitle' => wp__('Remove From Bulk Edit'),
			'notitle' => wp__('(no title)')
		) );
		...
		$scripts->localize( 'plugin-install', 'plugininstallL10n', array(
			'plugin_information' => wp__('Plugin Information:'),
			'ays' => wp__('Are you sure you want to install this plugin?')
		) );
		...
		$scripts->localize( 'image-edit', 'imageEditL10n', array(
			'error' => wp__( 'Could not load the preview image. Please reload the page and try again.' )
		));
		...
		$scripts->localize( 'set-post-thumbnail', 'setPostThumbnailL10n', array(
			'setThumbnail' => wp__( 'Use as featured image' ),
			'saving' => wp__( 'Saving...' ),
			'error' => wp__( 'Could not set that as the thumbnail image. Try a different attachment.' ),
			'done' => wp__( 'Done' )
		) );
		...
		$scripts->localize( 'nav-menu', 'navMenuL10n', array(
			...
			'warnDeleteMenu' => wp__( "You are about to permanently delete this menu. \n 'Cancel' to stop, 'OK' to delete." ),
			'saveAlert' => wp__('The changes you made will be lost if you navigate away from this page.')
		) );
