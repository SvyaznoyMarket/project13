...
function is_archive() {
	...
	if ( ! isset( $wp_query ) ) {
		_doing_it_wrong( __FUNCTION__, wp__( 'Conditional query tags do not work before the query is run. Before then, they always return false.' ), '3.1' );
		return false;
	}
...
class WP_Query {
	...
	function &get_posts() {
		...
		if ( isset( $q['caller_get_posts'] ) ) {
			_deprecated_argument( 'WP_Query', '3.1', wp__( '"caller_get_posts" is deprecated. Use "ignore_sticky_posts" instead.' ) );
