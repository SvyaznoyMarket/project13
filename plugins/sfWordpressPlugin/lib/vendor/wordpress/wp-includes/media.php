...
function wp_load_image( $file ) {
	...
	if ( ! file_exists( $file ) )
		return sprintf(wp__('File &#8220;%s&#8221; doesn&#8217;t exist?'), $file);

	if ( ! function_exists('imagecreatefromstring') )
		return wp__('The GD image library is not installed.');
	...
	if ( !is_resource( $image ) )
		return sprintf(wp__('File &#8220;%s&#8221; is not an image.'), $file);
...
function image_resize( $file, $max_w, $max_h, $crop = false, $suffix = null, $dest_path = null, $jpeg_quality = 90 ) {
	...
	if ( !$size )
		return new WP_Error('invalid_image', wp__('Could not read image size'), $file);
	list($orig_w, $orig_h, $orig_type) = $size;

	$dims = image_resize_dimensions($orig_w, $orig_h, $max_w, $max_h, $crop);
	if ( !$dims )
		return new WP_Error( 'error_getting_dimensions', wp__('Could not calculate resized image dimensions') );
	...
	if ( IMAGETYPE_GIF == $orig_type ) {
		if ( !imagegif( $newimage, $destfilename ) )
			return new WP_Error('resize_path_invalid', wp__( 'Resize path invalid' ));
