...
class WP_Widget {
	...
	function form($instance) {
		echo '<p class="no-options-widget">' . wp__('There are no options for this widget.') . '</p>';
		return 'noform';
	}
...
function register_sidebars($number = 1, $args = array()) {
	...
	for ( $i = 1; $i <= $number; $i++ ) {
		...
		if ( $number > 1 )
			$_args['name'] = isset($args['name']) ? sprintf($args['name'], $i) : sprintf(wp__('Sidebar %d'), $i);
		else
			$_args['name'] = isset($args['name']) ? $args['name'] : wp__('Sidebar');
