...
function get_category_parents( $id, $link = false, $separator = '/', $nicename = false, $visited = array() ) {
	...
	if ( $link )
		$chain .= '<a href="' . get_category_link( $parent->term_id ) . '" title="' . esc_attr( sprintf( wp__( "View all posts in %s" ), $parent->name ) ) . '">'.$name.'</a>' . $separator;
	else
...
function get_the_category_list( $separator = '', $parents='', $post_id = false ) {
	...
	if ( empty( $categories ) )
		return apply_filters( 'the_category', wp__( 'Uncategorized' ), $separator, $parents );
		...
		foreach ( $categories as $category ) {
			...
			switch ( strtolower( $parents ) ) {
				case 'multiple':
					...
					$thelist .= '<a href="' . get_category_link( $category->term_id ) . '" title="' . esc_attr( sprintf( wp__( "View all posts in %s" ), $category->name ) ) . '" ' . $rel . '>' . $category->name.'</a></li>';
					break;
				case 'single':
					$thelist .= '<a href="' . get_category_link( $category->term_id ) . '" title="' . esc_attr( sprintf( wp__( "View all posts in %s" ), $category->name ) ) . '" ' . $rel . '>';
					if ( $category->parent )
		...
		foreach ( $categories as $category ) {
			...
			switch ( strtolower( $parents ) ) {
				case 'multiple':
					if ( $category->parent )
						$thelist .= get_category_parents( $category->parent, true, $separator );
					$thelist .= '<a href="' . get_category_link( $category->term_id ) . '" title="' . esc_attr( sprintf( wp__( "View all posts in %s" ), $category->name ) ) . '" ' . $rel . '>' . $category->name.'</a>';
					break;
				case 'single':
					$thelist .= '<a href="' . get_category_link( $category->term_id ) . '" title="' . esc_attr( sprintf( wp__( "View all posts in %s" ), $category->name ) ) . '" ' . $rel . '>';
					if ( $category->parent )
...
function wp_list_categories( $args = '' ) {
	$defaults = array(
		'show_option_all' => '', 'show_option_none' => wp__('No categories'),
		...
		'hierarchical' => true, 'title_li' => wp__( 'Categories' ),
		'echo' => 1, 'depth' => 0,
		'taxonomy' => 'category'
	);
...
class Walker_Category extends Walker {
	...
	function start_el(&$output, $category, $depth, $args) {
		...
		if ( $use_desc_for_title == 0 || empty($category->description) )
			$link .= 'title="' . esc_attr( sprintf(wp__( 'View all posts filed under %s' ), $cat_name) ) . '"';
		else
		...
		if ( !empty($feed_image) || !empty($feed) ) {
			...
			if ( empty($feed) ) {
				$alt = ' alt="' . sprintf(wp__( 'Feed for all posts filed under %s' ), $cat_name ) . '"';
			} else {
				$title = ' title="' . $feed . '"';
				$alt = ' alt="' . $feed . '"';
				$name = $feed;
