...
function wp_authenticate_username_password($user, $username, $password) {
	...
	if ( empty($username) || empty($password) ) {
		...
		if ( empty($username) )
			$error->add('empty_username', wp__('<strong>ERROR</strong>: The username field is empty.'));

		if ( empty($password) )
			$error->add('empty_password', wp__('<strong>ERROR</strong>: The password field is empty.'));
	...
	if ( !$userdata )
		return new WP_Error('invalid_username', sprintf(wp__('<strong>ERROR</strong>: Invalid username. <a href="%s" title="Password Lost and Found">Lost your password</a>?'), wp_lostpassword_url()));

	if ( is_multisite() ) {
		// Is user marked as spam?
		if ( 1 == $userdata->spam)
			return new WP_Error('invalid_username', wp__('<strong>ERROR</strong>: Your account has been marked as a spammer.'));

		// Is a user's blog marked as spam?
		if ( !is_super_admin( $userdata->ID ) && isset($userdata->primary_blog) ) {
			$details = get_blog_details( $userdata->primary_blog );
			if ( is_object( $details ) && $details->spam == 1 )
				return new WP_Error('blog_suspended', wp__('Site Suspended.'));
		}
	}
	...
	if ( !wp_check_password($password, $userdata->user_pass, $userdata->ID) )
		return new WP_Error( 'incorrect_password', sprintf( wp__( '<strong>ERROR</strong>: The password you entered for the username <strong>%1$s</strong> is incorrect. <a href="%2$s" title="Password Lost and Found">Lost your password</a>?' ),
...
function wp_authenticate_cookie($user, $username, $password) {
	...
	if ( empty($username) && empty($password) ) {
		...
		if ( !empty($_COOKIE[$auth_cookie]) )
			return new WP_Error('expired_session', wp__('Please log in again.'));
...
function wp_insert_user($userdata) {
	...
	if ( empty($user_login) )
		return new WP_Error('empty_user_login', wp__('Cannot create a user with an empty login name.') );

	if ( !$update && username_exists( $user_login ) )
		return new WP_Error('existing_user_login', wp__('This username is already registered.') );
	...
	if ( !$update && ! defined( 'WP_IMPORTING' ) && email_exists($user_email) )
		return new WP_Error('existing_user_email', wp__('This email address is already registered.') );
...
function _wp_get_user_contactmethods( $user = null ) {
	$user_contactmethods = array(
		'aim' => wp__('AIM'),
		'yim' => wp__('Yahoo IM'),
		'jabber' => wp__('Jabber / Google Talk')
	);
