...
function get_theme_data( $theme_file ) {
	...
	if ( $theme_data['Author'] == '' ) {
		$theme_data['Author'] = $theme_data['AuthorName'] = wp__('Anonymous');
	} else {
...
function get_themes() {
	...
	foreach ( (array) $theme_files as $theme_file ) {
		...
		if ( !is_readable("$theme_root/$theme_file") ) {
			$wp_broken_themes[$theme_file] = array('Name' => $theme_file, 'Title' => $theme_file, 'Description' => wp__('File not readable.'));
			continue;
		}
			...
			if ( file_exists("$theme_root/$parent_dir/$template/index.php") ) {
				...
				if ( isset($theme_files[$template]) && file_exists( $theme_files[$template]['theme_root'] . "/$template/index.php" ) ) {
					...
					if ( empty( $parent_template) )
						$wp_broken_themes[$name] = array('Name' => $name, 'Title' => $title, 'Description' => wp__('Template is missing.'), 'error' => 'no_template');
					else
						$wp_broken_themes[$name] = array('Name' => $name, 'Title' => $title, 'Description' => sprintf( wp__('The parent theme is missing. Please install the "%s" parent theme.'),  $parent_template ), 'error' => 'no_parent', 'parent' => $parent_template );
					continue;
				}
			...
			if ( is_dir($theme_root . '/' . $theme_dir) && is_readable($theme_root . '/' . $theme_dir) ) {
				...
				if ( !$found_stylesheet ) { // look for themes in that dir
					...
					if ( !$found_subdir_themes )
						$wp_broken_themes[$theme_dir] = array('Name' => $theme_dir, 'Title' => $theme_dir, 'Description' => wp__('Stylesheet is missing.'));
				}
			}
		}
		@closedir( $themes_dir );
