...
function wp_allow_comment($commentdata) {
	...
	if ( $wpdb->get_var($dupe) ) {
		...
		if ( defined('DOING_AJAX') )
			die( wp__('Duplicate comment detected; it looks as though you&#8217;ve already said that!') );

		wp_die( wp__('Duplicate comment detected; it looks as though you&#8217;ve already said that!') );
	}
...
function check_comment_flood_db( $ip, $email, $date ) {
	...
	if ( $lasttime = $wpdb->get_var( $wpdb->prepare( "SELECT `comment_date_gmt` FROM `$wpdb->comments` WHERE `comment_date_gmt` >= %s AND ( `comment_author_IP` = %s OR `comment_author_email` = %s ) ORDER BY `comment_date_gmt` DESC LIMIT 1", $hour_ago, $ip, $email ) ) ) {
		...
		if ( $flood_die ) {
			...
			if ( defined('DOING_AJAX') )
				die( wp__('You are posting comments too quickly.  Slow down.') );

			wp_die( wp__('You are posting comments too quickly.  Slow down.'), '', array('response' => 403) );
		}
	}
}
...
function wp_set_comment_status($comment_id, $comment_status, $wp_error = false) {
	...
	if ( !$wpdb->update( $wpdb->comments, array('comment_approved' => $status), array('comment_ID' => $comment_id) ) ) {
		if ( $wp_error )
			return new WP_Error('db_update_error', wp__('Could not update comment status'), $wpdb->last_error);
		else
			return false;
	}
