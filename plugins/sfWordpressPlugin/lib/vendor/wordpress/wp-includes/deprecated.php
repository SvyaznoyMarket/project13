...
function start_wp() {
	...
	_deprecated_function( __FUNCTION__, '1.5', wp__('new WordPress Loop') );
	...
	foreach ( (array) $results as $row ) {
		...
		if ( $show_updated )
			if (substr($row->link_updated_f, 0, 2) != '00')
				$title .= ' ('.wp__('Last updated') . ' ' . date(get_option('links_updated_date_format'), $row->link_updated_f + (get_option('gmt_offset') * 3600)) . ')';
 ...
 *
 * The filter 'js_escape' is also applied by wp_esc_js()
 *
 * @since 2.0.4
 * @deprecated 2.8.0
 * @deprecated Use wp_esc_js()
 * @see wp_esc_js()
 *
...
function js_escape( $text ) {
	_deprecated_function( __FUNCTION__, '2.8', 'wp_esc_js()' );
...
function get_boundary_post_rel_link($title = '%title', $in_same_cat = false, $excluded_categories = '', $start = true) {
	...
	if ( empty($post->post_title) )
		$post->post_title = $start ? wp__('First Post') : wp__('Last Post');
...
function wp_admin_bar_dashboard_view_site_menu( $wp_admin_bar ) {
	...
	if ( 0 != $user_id ) {
		if ( is_admin() )
			$wp_admin_bar->add_menu( array( 'id' => 'view-site', 'title' => wp__( 'Visit Site' ), 'href' => home_url() ) );
		elseif ( is_multisite() )
			$wp_admin_bar->add_menu( array( 'id' => 'dashboard', 'title' => wp__( 'Dashboard' ), 'href' => get_dashboard_url( $user_id ) ) );
		else
			$wp_admin_bar->add_menu( array( 'id' => 'dashboard', 'title' => wp__( 'Dashboard' ), 'href' => admin_url() ) );
	}
}
