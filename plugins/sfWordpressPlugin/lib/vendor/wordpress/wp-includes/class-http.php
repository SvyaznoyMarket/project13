...
class WP_Http {
	...
	function request( $url, $args = array() ) {
		...
		if ( empty( $url ) || empty( $arrURL['scheme'] ) )
			return new WP_Error('http_request_failed', wp__('A valid URL was not provided.'));

		if ( $this->block_request( $url ) )
			return new WP_Error( 'http_request_failed', wp__( 'User has blocked requests through HTTP.' ) );
		...
		if ( $r['stream'] ) {
			...
			if ( ! is_writable( dirname( $r['filename'] ) ) )
				return new WP_Error( 'http_request_failed', wp__( 'Destination directory for file streaming does not exist or is not writable.' ) );
		}
	...
	private function _dispatch_request( $url, $args ) {
		...
		if ( !$class )
			return new WP_Error( 'http_failure', wp__( 'There are no HTTP transports available which can complete the requested request.' ) );
	...
	function request($url, $args = array()) {
		...
		if ( $r['stream'] ) {
			if ( ! WP_DEBUG )
				...
				$stream_handle = fopen( $r['filename'], 'w+' );
			if ( ! $stream_handle )
				return new WP_Error( 'http_request_failed', sprintf( wp__( 'Could not open handle for fopen() to %s' ), $r['filename'] ) );
		...
		if ( isset($arrHeaders['headers']['location']) && 0 !== $r['_redirection'] ) {
			if ( $r['redirection']-- > 0 ) {
				...
				return new WP_Error('http_request_failed', wp__('Too many redirects.'));
			}
		}
...
class WP_Http_Streams {
	...
	function request($url, $args = array()) {
		...
		if ( false === $arrURL )
			return new WP_Error('http_request_failed', sprintf(wp__('Malformed URL: %s'), $url));
		...
		if ( ! $handle )
			return new WP_Error('http_request_failed', sprintf(wp__('Could not open handle for fopen() to %s'), $url));
