...
class Walker_Nav_Menu extends Walker {
	...
	function start_el(&$output, $item, $depth, $args) {
        ...
        # Hack
        $isSelectedItem = in_array('current-menu-item', $classes);
        ...
        if($isSelectedItem)
        {
            $item_output .= '<strong class="orange"'. $attributes .'>';
...
function wp_nav_menu( $args = array() ) {
    ...
    $currentPage = isset($args['currentPage'])?$args['currentPage']:Null;
	...
	_wp_menu_item_classes_by_context( $menu_items, $currentPage);
    ...
    $nav_menu = '<h2>' . $menu->name . '</h2>' . $nav_menu;
 ...
 */
function _wp_menu_item_classes_by_context( &$menu_items, $currentPage ) {
	global $wp_query;
	...
	foreach ( (array) $menu_items as $key => $menu_item ) {
		...
		// if the menu item corresponds to the currently-requested URL
			...
			} elseif ($currentPage == $item_url || ($item_url == $front_page_url && is_front_page()) ) {
				$classes[] = 'current-menu-item';
				$classes[] = 'orange';
			}
