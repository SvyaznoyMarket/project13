...
function wp_authenticate($username, $password) {
	...
	if ( $user == null ) {
		...
		$user = new WP_Error('authentication_failed', wp__('<strong>ERROR</strong>: Invalid username or incorrect password.'));
	}
...
function check_admin_referer($action = -1, $query_arg = '_wpnonce') {
	if ( -1 == $action )
		_doing_it_wrong( __FUNCTION__, wp__( 'You should specify a nonce action to be verified by using the first parameter.' ), '3.2' );
...
function wp_notify_postauthor( $comment_id, $comment_type = '' ) {
	...
	if ('comment' == $comment_type) {
		$notify_message  = sprintf( wp__( 'New comment on your post "%s"' ), $post->post_title ) . "\r\n";
		/* translators: 1: comment author, 2: author IP, 3: author domain */
		$notify_message .= sprintf( wp__('Author : %1$s (IP: %2$s , %3$s)'), $comment->comment_author, $comment->comment_author_IP, $comment_author_domain ) . "\r\n";
		$notify_message .= sprintf( wp__('E-mail : %s'), $comment->comment_author_email ) . "\r\n";
		$notify_message .= sprintf( wp__('URL    : %s'), $comment->comment_author_url ) . "\r\n";
		$notify_message .= sprintf( wp__('Whois  : http://whois.arin.net/rest/ip/%s'), $comment->comment_author_IP ) . "\r\n";
		$notify_message .= wp__('Comment: ') . "\r\n" . $comment->comment_content . "\r\n\r\n";
		$notify_message .= wp__('You can see all comments on this post here: ') . "\r\n";
		/* translators: 1: blog name, 2: post title */
		$subject = sprintf( wp__('[%1$s] Comment: "%2$s"'), $blogname, $post->post_title );
	} elseif ('trackback' == $comment_type) {
		$notify_message  = sprintf( wp__( 'New trackback on your post "%s"' ), $post->post_title ) . "\r\n";
		/* translators: 1: website name, 2: author IP, 3: author domain */
		$notify_message .= sprintf( wp__('Website: %1$s (IP: %2$s , %3$s)'), $comment->comment_author, $comment->comment_author_IP, $comment_author_domain ) . "\r\n";
		$notify_message .= sprintf( wp__('URL    : %s'), $comment->comment_author_url ) . "\r\n";
		$notify_message .= wp__('Excerpt: ') . "\r\n" . $comment->comment_content . "\r\n\r\n";
		$notify_message .= wp__('You can see all trackbacks on this post here: ') . "\r\n";
		/* translators: 1: blog name, 2: post title */
		$subject = sprintf( wp__('[%1$s] Trackback: "%2$s"'), $blogname, $post->post_title );
	} elseif ('pingback' == $comment_type) {
		$notify_message  = sprintf( wp__( 'New pingback on your post "%s"' ), $post->post_title ) . "\r\n";
		/* translators: 1: comment author, 2: author IP, 3: author domain */
		$notify_message .= sprintf( wp__('Website: %1$s (IP: %2$s , %3$s)'), $comment->comment_author, $comment->comment_author_IP, $comment_author_domain ) . "\r\n";
		$notify_message .= sprintf( wp__('URL    : %s'), $comment->comment_author_url ) . "\r\n";
		$notify_message .= wp__('Excerpt: ') . "\r\n" . sprintf('[...] %s [...]', $comment->comment_content ) . "\r\n\r\n";
		$notify_message .= wp__('You can see all pingbacks on this post here: ') . "\r\n";
		/* translators: 1: blog name, 2: post title */
		$subject = sprintf( wp__('[%1$s] Pingback: "%2$s"'), $blogname, $post->post_title );
	}
	$notify_message .= get_permalink($comment->comment_post_ID) . "#comments\r\n\r\n";
	$notify_message .= sprintf( wp__('Permalink: %s'), get_permalink( $comment->comment_post_ID ) . '#comment-' . $comment_id ) . "\r\n";
	if ( EMPTY_TRASH_DAYS )
		$notify_message .= sprintf( wp__('Trash it: %s'), admin_url("comment.php?action=trash&c=$comment_id") ) . "\r\n";
	else
		$notify_message .= sprintf( wp__('Delete it: %s'), admin_url("comment.php?action=delete&c=$comment_id") ) . "\r\n";
	$notify_message .= sprintf( wp__('Spam it: %s'), admin_url("comment.php?action=spam&c=$comment_id") ) . "\r\n";
...
function wp_notify_moderator($comment_id) {
	...
	{
		case 'trackback':
			$notify_message  = sprintf( wp__('A new trackback on the post "%s" is waiting for your approval'), $post->post_title ) . "\r\n";
			$notify_message .= get_permalink($comment->comment_post_ID) . "\r\n\r\n";
			$notify_message .= sprintf( wp__('Website : %1$s (IP: %2$s , %3$s)'), $comment->comment_author, $comment->comment_author_IP, $comment_author_domain ) . "\r\n";
			$notify_message .= sprintf( wp__('URL    : %s'), $comment->comment_author_url ) . "\r\n";
			$notify_message .= wp__('Trackback excerpt: ') . "\r\n" . $comment->comment_content . "\r\n\r\n";
			break;
		case 'pingback':
			$notify_message  = sprintf( wp__('A new pingback on the post "%s" is waiting for your approval'), $post->post_title ) . "\r\n";
			$notify_message .= get_permalink($comment->comment_post_ID) . "\r\n\r\n";
			$notify_message .= sprintf( wp__('Website : %1$s (IP: %2$s , %3$s)'), $comment->comment_author, $comment->comment_author_IP, $comment_author_domain ) . "\r\n";
			$notify_message .= sprintf( wp__('URL    : %s'), $comment->comment_author_url ) . "\r\n";
			$notify_message .= wp__('Pingback excerpt: ') . "\r\n" . $comment->comment_content . "\r\n\r\n";
			break;
		default: //Comments
			$notify_message  = sprintf( wp__('A new comment on the post "%s" is waiting for your approval'), $post->post_title ) . "\r\n";
			$notify_message .= get_permalink($comment->comment_post_ID) . "\r\n\r\n";
			$notify_message .= sprintf( wp__('Author : %1$s (IP: %2$s , %3$s)'), $comment->comment_author, $comment->comment_author_IP, $comment_author_domain ) . "\r\n";
			$notify_message .= sprintf( wp__('E-mail : %s'), $comment->comment_author_email ) . "\r\n";
			$notify_message .= sprintf( wp__('URL    : %s'), $comment->comment_author_url ) . "\r\n";
	...
	$notify_message .= sprintf( wp__('Approve it: %s'),  admin_url("comment.php?action=approve&c=$comment_id") ) . "\r\n";
	if ( EMPTY_TRASH_DAYS )
		$notify_message .= sprintf( wp__('Trash it: %s'), admin_url("comment.php?action=trash&c=$comment_id") ) . "\r\n";
	else
		$notify_message .= sprintf( wp__('Delete it: %s'), admin_url("comment.php?action=delete&c=$comment_id") ) . "\r\n";
	...
	$subject = sprintf( wp__('[%1$s] Please moderate: "%2$s"'), $blogname, $post->post_title );
	$message_headers = '';
...
function wp_password_change_notification(&$user) {
	...
	if ( $user->user_email != get_option('admin_email') ) {
		$message = sprintf(wp__('Password Lost and Changed for user: %s'), $user->user_login) . "\r\n";
		// The blogname option is escaped with esc_html on the way into the database in sanitize_option
		// we want to reverse this for the plain text arena of emails.
		$blogname = wp_specialchars_decode(get_option('blogname'), ENT_QUOTES);
		wp_mail(get_option('admin_email'), sprintf(wp__('[%s] Password Lost/Changed'), $blogname), $message);
	}
}
endif;
...
function wp_new_user_notification($user_id, $plaintext_pass = '') {
	...
	$message  = sprintf(wp__('New user registration on your site %s:'), $blogname) . "\r\n\r\n";
	$message .= sprintf(wp__('Username: %s'), $user_login) . "\r\n\r\n";
	$message .= sprintf(wp__('E-mail: %s'), $user_email) . "\r\n";

	@wp_mail(get_option('admin_email'), sprintf(wp__('[%s] New User Registration'), $blogname), $message);

	if ( empty($plaintext_pass) )
		return;

	$message  = sprintf(wp__('Username: %s'), $user_login) . "\r\n";
	$message .= sprintf(wp__('Password: %s'), $plaintext_pass) . "\r\n";
	$message .= wp_login_url() . "\r\n";

	wp_mail($user_email, sprintf(wp__('[%s] Your username and password'), $blogname), $message);
