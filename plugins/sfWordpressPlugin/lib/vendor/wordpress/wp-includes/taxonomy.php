...
function create_initial_taxonomies() {
	...
	register_taxonomy( 'nav_menu', 'nav_menu_item', array(
		...
		'hierarchical' => false,
		'labels' => array(
			'name' => wp__( 'Navigation Menus' ),
			'singular_name' => wp__( 'Navigation Menu' ),
		),
		'query_var' => false,
		'rewrite' => false,
		'show_ui' => false,
	...
	register_taxonomy( 'link_category', 'link', array(
		'hierarchical' => false,
		'labels' => array(
			'name' => wp__( 'Link Categories' ),
			'singular_name' => wp__( 'Link Category' ),
			'search_items' => wp__( 'Search Link Categories' ),
			'popular_items' => null,
			'all_items' => wp__( 'All Link Categories' ),
			'edit_item' => wp__( 'Edit Link Category' ),
			'update_item' => wp__( 'Update Link Category' ),
			'add_new_item' => wp__( 'Add New Link Category' ),
			'new_item_name' => wp__( 'New Link Category Name' ),
...
function get_taxonomy_labels( $tax ) {
	...
	$nohier_vs_hier_defaults = array(
		...
		'search_items' => array( wp__( 'Search Tags' ), wp__( 'Search Categories' ) ),
		'popular_items' => array( wp__( 'Popular Tags' ), null ),
		'all_items' => array( wp__( 'All Tags' ), wp__( 'All Categories' ) ),
		'parent_item' => array( null, wp__( 'Parent Category' ) ),
		'parent_item_colon' => array( null, wp__( 'Parent Category:' ) ),
		'edit_item' => array( wp__( 'Edit Tag' ), wp__( 'Edit Category' ) ),
		'view_item' => array( wp__( 'View Tag' ), wp__( 'View Category' ) ),
		'update_item' => array( wp__( 'Update Tag' ), wp__( 'Update Category' ) ),
		'add_new_item' => array( wp__( 'Add New Tag' ), wp__( 'Add New Category' ) ),
		'new_item_name' => array( wp__( 'New Tag Name' ), wp__( 'New Category Name' ) ),
		'separate_items_with_commas' => array( wp__( 'Separate tags with commas' ), null ),
		'add_or_remove_items' => array( wp__( 'Add or remove tags' ), null ),
		'choose_from_most_used' => array( wp__( 'Choose from the most used tags' ), null ),
	);
...
function get_objects_in_term( $term_ids, $taxonomies, $args = array() ) {
	...
	foreach ( (array) $taxonomies as $taxonomy ) {
		if ( ! taxonomy_exists( $taxonomy ) )
			return new WP_Error( 'invalid_taxonomy', wp__( 'Invalid Taxonomy' ) );
	}
...
function &get_term($term, $taxonomy, $output = OBJECT, $filter = 'raw') {
	...
	if ( empty($term) ) {
		$error = new WP_Error('invalid_term', wp__('Empty Term'));
		return $error;
	}

	if ( ! taxonomy_exists($taxonomy) ) {
		$error = new WP_Error('invalid_taxonomy', wp__('Invalid Taxonomy'));
		return $error;
	}
...
function wp_insert_term( $term, $taxonomy, $args = array() ) {
	...
	if ( is_int($term) && 0 == $term )
		return new WP_Error('invalid_term_id', wp__('Invalid term ID'));
	...
	if ( $term_id = term_exists($slug) ) {
		...
		if ( is_taxonomy_hierarchical($taxonomy) && $existing_term['name'] == $name && $exists = term_exists( (int) $term_id, $taxonomy ) ) {
			...
			if ( in_array($name, $siblings) ) {
				return new WP_Error('term_exists', wp__('A term with the name provided already exists with this parent.'), $exists['term_id']);
			} else {
				$slug = wp_unique_term_slug($slug, (object) $args);
				if ( false === $wpdb->insert( $wpdb->terms, compact( 'name', 'slug', 'term_group' ) ) )
					return new WP_Error('db_insert_error', wp__('Could not insert term into the database'), $wpdb->last_error);
			...
			// Same name, same slug.
			return new WP_Error('term_exists', wp__('A term with the name provided already exists.'), $exists['term_id']);
		}
	} else {
...
function wp_update_term( $term_id, $taxonomy, $args = array() ) {
	...
	if ( $id && ($id != $term_id) ) {
		...
		if ( $empty_slug || ( $parent != $term['parent']) )
			...
			return new WP_Error('duplicate_term_slug', sprintf(wp__('The slug &#8220;%s&#8221; is already in use by another term'), $slug));
	}
	do_action( 'edit_terms', $term_id );
...
function get_term_link( $term, $taxonomy = '') {
	...
	if ( !is_object($term) )
		$term = new WP_Error('invalid_term', wp__('Empty Term'));
...
function is_object_in_term( $object_id, $taxonomy, $terms = null ) {
	if ( !$object_id = (int) $object_id )
		return new WP_Error( 'invalid_object', wp__( 'Invalid object ID' ) );
