...
function get_search_form($echo = true) {
	...
	<div><label class="screen-reader-text" for="s">' . wp__('Search for:') . '</label>
...
function wp_loginout($redirect = '', $echo = true) {
	if ( ! is_user_logged_in() )
		$link = '<a href="' . esc_url( wp_login_url($redirect) ) . '">' . wp__('Log in') . '</a>';
	else
		$link = '<a href="' . esc_url( wp_logout_url($redirect) ) . '">' . wp__('Log out') . '</a>';
...
function wp_login_form( $args = array() ) {
	$defaults = array( 'echo' => true,
						'redirect' => ( is_ssl() ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'], // Default redirect is back to the current page
	 					'form_id' => 'loginform',
						'label_username' => wp__( 'Username' ),
						'label_password' => wp__( 'Password' ),
						'label_remember' => wp__( 'Remember Me' ),
						'label_log_in' => wp__( 'Log In' ),
...
function wp_register( $before = '<li>', $after = '</li>', $echo = true ) {
	...
	if ( ! is_user_logged_in() ) {
		if ( get_option('users_can_register') )
			$link = $before . '<a href="' . site_url('wp-login.php?action=register', 'login') . '">' . wp__('Register') . '</a>' . $after;
		else
			$link = '';
	} else {
		$link = $before . '<a href="' . admin_url() . '">' . wp__('Site Admin') . '</a>' . $after;
	}
...
function get_bloginfo( $show = '', $filter = 'raw' ) {
	...
	switch( $show ) {
		...
		case 'siteurl' : // DEPRECATED
			_deprecated_argument( __FUNCTION__, '2.2', sprintf( wp__('The <code>%s</code> option is deprecated for the family of <code>bloginfo()</code> functions.' ), $show ) . ' ' . sprintf( wp__( 'Use the <code>%s</code> option instead.' ), 'url'  ) );
		case 'url' :
			$output = home_url();
			break;
		case 'wpurl' :
		...
		case 'language':
			...
			break;
		case 'text_direction':
			//_deprecated_argument( __FUNCTION__, '2.2', sprintf( wp__('The <code>%s</code> option is deprecated for the family of <code>bloginfo()</code> functions.' ), $show ) . ' ' . sprintf( wp__( 'Use the <code>%s</code> function instead.' ), 'is_rtl()'  ) );
...
function wp_title($sep = '&raquo;', $display = true, $seplocation = '') {
	...
	if ( is_search() ) {
		...
		$title = sprintf(wp__('Search Results %1$s %2$s'), $t_sep, strip_tags($search));
	}

	// If it's a 404 page
	if ( is_404() ) {
		$title = wp__('Page not found');
	}
	...
	if ( 'monthly' == $type ) {
		...
		if ( $arcresults ) {
			...
			foreach ( (array) $arcresults as $arcresult ) {
				...
				$text = sprintf(wp__('%1$s %2$d'), $wp_locale->get_month($arcresult->month), $arcresult->year);
				if ( $show_post_count )
...
function get_calendar($initial = true, $echo = true) {
	...
	if ( $previous ) {
		$calendar_output .= "\n\t\t".'<td colspan="3" id="prev"><a href="' . get_month_link($previous->year, $previous->month) . '" title="' . esc_attr( sprintf(wp__('View posts for %1$s %2$s'), $wp_locale->get_month($previous->month), date('Y', mktime(0, 0 , 0, $previous->month, 1, $previous->year)))) . '">&laquo; ' . $wp_locale->get_month_abbrev($wp_locale->get_month($previous->month)) . '</a></td>';
	} else {
	...
	if ( $next ) {
		$calendar_output .= "\n\t\t".'<td colspan="3" id="next"><a href="' . get_month_link($next->year, $next->month) . '" title="' . esc_attr( sprintf(wp__('View posts for %1$s %2$s'), $wp_locale->get_month($next->month), date('Y', mktime(0, 0 , 0, $next->month, 1, $next->year))) ) . '">' . $wp_locale->get_month_abbrev($wp_locale->get_month($next->month)) . ' &raquo;</a></td>';
	} else {
...
function feed_links( $args = array() ) {
	...
	$defaults = array(
		...
		'feedtitle'	=> wp__('%1$s %2$s Feed'),
		/* translators: %s: blog title, 2: separator (raquo) */
		'comstitle'	=> wp__('%1$s %2$s Comments Feed'),
	);
...
function feed_links_extra( $args = array() ) {
	$defaults = array(
		...
		'singletitle' => wp__('%1$s %2$s %3$s Comments Feed'),
		/* translators: 1: blog name, 2: separator(raquo), 3: category name */
		'cattitle'    => wp__('%1$s %2$s %3$s Category Feed'),
		/* translators: 1: blog name, 2: separator(raquo), 3: tag name */
		'tagtitle'    => wp__('%1$s %2$s %3$s Tag Feed'),
		/* translators: 1: blog name, 2: separator(raquo), 3: author name  */
		'authortitle' => wp__('%1$s %2$s Posts by %3$s Feed'),
		/* translators: 1: blog name, 2: separator(raquo), 3: search phrase */
		'searchtitle' => wp__('%1$s %2$s Search Results for &#8220;%3$s&#8221; Feed'),
	);
...
function paginate_links( $args = '' ) {
	$defaults = array(
		...
		'show_all' => false,
		'prev_next' => true,
		'prev_text' => wp__('&laquo; Previous'),
		'next_text' => wp__('Next &raquo;'),
		'end_size' => 1,
		'mid_size' => 2,
		'type' => 'plain',
	...
	for ( $n = 1; $n <= $total; $n++ ) :
		...
		if ( $n == $current ) :
			...
			elseif ( $dots && !$show_all ) :
				$page_links[] = '<span class="page-numbers dots">' . wp__( '&hellip;' ) . '</span>';
				$dots = false;
			endif;
		endif;
	endfor;
 ...
 * <code>
 * wp_admin_css_color('classic', wp__('Classic'), admin_url("css/colors-classic.css"),
