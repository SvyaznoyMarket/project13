...
final class _WP_Editors {
	...
	public static function editor( $content, $editor_id, $settings = array() ) {
		...
		if ( self::$this_quicktags && self::$this_tinymce ) {
			...
			$buttons .= '<a id="' . $editor_id . '-html" class="hide-if-no-js wp-switch-editor switch-html" onclick="switchEditors.switchto(this);">' . wp__('HTML') . "</a>\n";
			$buttons .= '<a id="' . $editor_id . '-tmce" class="hide-if-no-js wp-switch-editor switch-tmce" onclick="switchEditors.switchto(this);">' . wp__('Visual') . "</a>\n";
		}
...
<?php
	...
	public static function wp_fullscreen_html() {
		...
		$save = isset($post->post_status) && $post->post_status == 'publish' ? wp__('Update') : wp__('Save');
	?>
	...
	<?php
		...
		$buttons = array(
			...
			'bold' => array( 'title' => wp__('Bold (Ctrl + B)'), 'onclick' => 'fullscreen.b();', 'both' => false ),
			'italic' => array( 'title' => wp__('Italic (Ctrl + I)'), 'onclick' => 'fullscreen.i();', 'both' => false ),
			'0' => 'separator',
			'bullist' => array( 'title' => wp__('Unordered list (Alt + Shift + U)'), 'onclick' => 'fullscreen.ul();', 'both' => false ),
			'numlist' => array( 'title' => wp__('Ordered list (Alt + Shift + O)'), 'onclick' => 'fullscreen.ol();', 'both' => false ),
			'1' => 'separator',
			'blockquote' => array( 'title' => wp__('Blockquote (Alt + Shift + Q)'), 'onclick' => 'fullscreen.blockquote();', 'both' => false ),
			'image' => array( 'title' => wp__('Insert/edit image (Alt + Shift + M)'), 'onclick' => "fullscreen.medialib();", 'both' => true ),
			'2' => 'separator',
			'link' => array( 'title' => wp__('Insert/edit link (Alt + Shift + A)'), 'onclick' => 'fullscreen.link();', 'both' => true ),
			'unlink' => array( 'title' => wp__('Unlink (Alt + Shift + S)'), 'onclick' => 'fullscreen.unlink();', 'both' => false ),
			'3' => 'separator',
			'help' => array( 'title' => wp__('Help (Alt + Shift + H)'), 'onclick' => 'fullscreen.help();', 'both' => false )
		);
	...
	<div id="wp-fullscreen-wrap" style="width:<?php echo $dfw_width; ?>px;">
		...
		<label id="wp-fullscreen-title-prompt-text" for="wp-fullscreen-title"><?php echo apply_filters( 'enter_title_here', wp__( 'Enter title here' ), $post ); ?></label>
		...
		<div id="wp-fullscreen-status">
			<div id="wp-fullscreen-count"><?php printf( wp__( 'Word count: %s' ), '<span class="word-count">0</span>' ); ?></div>
	...
	public static function wp_link_query( $args = array() ) {
		...
		foreach ( $posts as $post ) {
			if ( 'post' == $post->post_type )
				$info = mysql2date( wp__( 'Y/m/d' ), $post->post_date );
			else
