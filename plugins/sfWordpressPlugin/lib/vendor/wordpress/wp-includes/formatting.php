	...
	switch ( $option ) {
		case 'admin_email':
			...
			if ( !is_email($value) ) {
				...
				if ( function_exists('add_settings_error') )
					add_settings_error('admin_email', 'invalid_admin_email', wp__('The email address entered did not appear to be a valid email address. Please enter a valid email address.'));
			}
			break;
		...
		case 'new_admin_email':
			...
			if ( !is_email($value) ) {
				$value = get_option( $option ); // Resets option to stored value in the case of failed sanitization
				if ( function_exists('add_settings_error') )
					add_settings_error('new_admin_email', 'invalid_admin_email', wp__('The email address entered did not appear to be a valid email address. Please enter a valid email address.'));
			}
			break;
		...
		case 'siteurl':
			if ( (bool)preg_match( '#http(s?)://(.+)#i', $value) ) {
				...
				$value = get_option( $option ); // Resets option to stored value in the case of failed sanitization
				if ( function_exists('add_settings_error') )
					add_settings_error('siteurl', 'invalid_siteurl', wp__('The WordPress address you entered did not appear to be a valid URL. Please enter a valid URL.'));
			}
			break;
		...
		case 'home':
			if ( (bool)preg_match( '#http(s?)://(.+)#i', $value) ) {
				...
				$value = get_option( $option ); // Resets option to stored value in the case of failed sanitization
				if ( function_exists('add_settings_error') )
					add_settings_error('home', 'invalid_home', wp__('The Site address you entered did not appear to be a valid URL. Please enter a valid URL.'));
			}
			break;
		...
		case 'timezone_string':
			...
			if ( ! in_array( $value, $allowed_zones ) && ! empty( $value ) ) {
				$value = get_option( $option ); // Resets option to stored value in the case of failed sanitization
				if ( function_exists('add_settings_error') )
					add_settings_error('timezone_string', 'invalid_timezone_string', wp__('The timezone you have entered is not valid. Please select a valid timezone.') );
			}
			break;
...
function wp_sprintf_l($pattern, $args) {
	...
	$l = apply_filters('wp_sprintf_l', array(
		...
		'between_last_two' => wp__(', and '),
		/* translators: used between only two list items, there is a space after the and */
		'between_only_two' => wp__(' and '),
		));
