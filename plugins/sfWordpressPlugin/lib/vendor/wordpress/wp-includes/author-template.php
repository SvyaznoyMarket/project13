...
function the_author( $deprecated = '', $deprecated_echo = true ) {
	...
	if ( $deprecated_echo !== true )
		_deprecated_argument( __FUNCTION__, '1.5', wp__('Use <code>get_the_author()</code> instead if you do not want the value echoed.') );
	if ( $deprecated_echo )
		echo get_the_author();
...
function get_the_author_link() {
	if ( get_the_author_meta('url') ) {
		return '<a href="' . get_the_author_meta('url') . '" title="' . esc_attr( sprintf(wp__("Visit %s&#8217;s website"), get_the_author()) ) . '" rel="external">' . get_the_author() . '</a>';
	} else {
		return get_the_author();
	}
}
...
function the_author_posts_link($deprecated = '') {
	...
	$link = sprintf(
		...
		esc_attr( sprintf( wp__( 'Posts by %s' ), get_the_author() ) ),
		get_the_author()
	);
...
function wp_list_authors($args = '') {
	...
	foreach ( $authors as $author_id ) {
		...
		$link = '<a href="' . get_author_posts_url( $author->ID, $author->user_nicename ) . '" title="' . esc_attr( sprintf(wp__("Posts by %s"), $author->display_name) ) . '">' . $name . '</a>';
