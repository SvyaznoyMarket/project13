...
function create_initial_post_types() {
	...
	register_post_type( 'nav_menu_item', array(
		'labels' => array(
			'name' => wp__( 'Navigation Menu Items' ),
			'singular_name' => wp__( 'Navigation Menu Item' ),
		),
		'public' => false,
		'_builtin' => true, /* internal use only. don't use this when registering your own post type. */
		'hierarchical' => false,
...
function set_post_format( $post, $format ) {
	...
	if ( empty($post) )
		return new WP_Error('invalid_post', wp__('Invalid post'));
...
function get_post_statuses( ) {
	$status = array(
		'draft'			=> wp__('Draft'),
		'pending'		=> wp__('Pending Review'),
		'private'		=> wp__('Private'),
		'publish'		=> wp__('Published')
	);
...
function register_post_type($post_type, $args = array()) {
	...
	if ( strlen( $post_type ) > 20 )
			return new WP_Error( 'post_type_too_long', wp__( 'Post types cannot exceed 20 characters in length' ) );
...
function get_post_type_labels( $post_type_object ) {
	$nohier_vs_hier_defaults = array(
		...
		'add_new_item' => array( wp__('Add New Post'), wp__('Add New Page') ),
		'edit_item' => array( wp__('Edit Post'), wp__('Edit Page') ),
		'new_item' => array( wp__('New Post'), wp__('New Page') ),
		'view_item' => array( wp__('View Post'), wp__('View Page') ),
		'search_items' => array( wp__('Search Posts'), wp__('Search Pages') ),
		'not_found' => array( wp__('No posts found.'), wp__('No pages found.') ),
		'not_found_in_trash' => array( wp__('No posts found in Trash.'), wp__('No pages found in Trash.') ),
		'parent_item_colon' => array( null, wp__('Parent Page:') ),
		'all_items' => array( wp__( 'All Posts' ), wp__( 'All Pages' ) )
	);
...
function wp_get_recent_posts( $args = array(), $output = ARRAY_A ) {
	...
	if ( is_numeric( $args ) ) {
		_deprecated_argument( __FUNCTION__, '3.1', wp__( 'Passing an integer number of posts is deprecated. Pass an array of arguments instead.' ) );
...
function wp_insert_post($postarr, $wp_error = false) {
	...
	if ( apply_filters( 'wp_insert_post_empty_content', $maybe_empty, $postarr ) ) {
		if ( $wp_error )
			return new WP_Error( 'empty_content', wp__( 'Content, title, and excerpt are empty.' ) );
		else
			return 0;
	}
	...
	if ( $update ) {
		...
		if ( false === $wpdb->update( $wpdb->posts, $data, $where ) ) {
			if ( $wp_error )
				return new WP_Error('db_update_error', wp__('Could not update post in the database'), $wpdb->last_error);
			else
				return 0;
		}
	} else {
		...
		if ( false === $wpdb->insert( $wpdb->posts, $data ) ) {
			if ( $wp_error )
				return new WP_Error('db_insert_error', wp__('Could not insert post into the database'), $wpdb->last_error);
			else
				return 0;
		}
	...
	if ( !empty($page_template) && 'page' == $data['post_type'] ) {
		...
		if ( 'default' != $page_template && !in_array($page_template, $page_templates) ) {
			if ( $wp_error )
				return new WP_Error('invalid_page_template', wp__('The page template is invalid.'));
			else
				return 0;
		}
...
function _wp_post_revision_fields( $post = null, $autosave = false ) {
	...
	if ( !$fields ) {
		...
		$fields = array(
			'post_title' => wp__( 'Title' ),
			'post_content' => wp__( 'Content' ),
			'post_excerpt' => wp__( 'Excerpt' ),
		);
...
function _wp_put_post_revision( $post = null, $autosave = false ) {
	...
	if ( isset($post['post_type']) && 'revision' == $post['post_type'] )
		return new WP_Error( 'post_type', wp__( 'Cannot create a revision of a revision' ) );
...
function _show_post_preview() {
	...
	if ( isset($_GET['preview_id']) && isset($_GET['preview_nonce']) ) {
		...
		if ( false == wp_verify_nonce( $_GET['preview_nonce'], 'post_preview_' . $id ) )
			wp_die( wp__('You do not have permission to preview drafts.') );
