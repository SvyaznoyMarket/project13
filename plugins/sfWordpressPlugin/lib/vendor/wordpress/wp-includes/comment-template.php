...
function comments_number( $zero = false, $one = false, $more = false, $deprecated = '' ) {
	...
	if ( $number > 1 )
		$output = str_replace('%', number_format_i18n($number), ( false === $more ) ? wp__('% Comments') : $more);
	elseif ( $number == 0 )
		$output = ( false === $zero ) ? wp__('No Comments') : $zero;
	else // must be one
		$output = ( false === $one ) ? wp__('1 Comment') : $one;
...
function comment_type($commenttxt = false, $trackbacktxt = false, $pingbacktxt = false) {
	...
	if ( false === $trackbacktxt ) $trackbacktxt = wp__( 'Trackback' );
	if ( false === $pingbacktxt ) $pingbacktxt = wp__( 'Pingback' );
...
function trackback_url( $deprecated_echo = true ) {
	if ( $deprecated_echo !== true )
		_deprecated_argument( __FUNCTION__, '2.5', wp__('Use <code>get_trackback_url()</code> instead if you do not want the value echoed.') );
	if ( $deprecated_echo )
...
function comments_popup_link( $zero = false, $one = false, $more = false, $css_class = '', $none = false ) {
	...
	if ( false === $zero ) $zero = wp__( 'No Comments' );
	if ( false === $one ) $one = wp__( '1 Comment' );
	if ( false === $more ) $more = wp__( '% Comments' );
	if ( false === $none ) $none = wp__( 'Comments Off' );
	...
	if ( post_password_required() ) {
		echo wp__('Enter your password to view comments.');
		return;
	}
	...
	echo ' title="' . esc_attr( sprintf( wp__('Comment on %s'), $title ) ) . '">';
...
function get_comment_reply_link($args = array(), $comment = null, $post = null) {
	...
	$defaults = array('add_below' => 'comment', 'respond_id' => 'respond', 'reply_text' => wp__('Reply'),
		'login_text' => wp__('Log in to Reply'), 'depth' => 0, 'before' => '', 'after' => '');
...
function get_post_reply_link($args = array(), $post = null) {
	...
	$defaults = array('add_below' => 'post', 'respond_id' => 'respond', 'reply_text' => wp__('Leave a Comment'),
		'login_text' => wp__('Log in to leave a Comment'), 'before' => '', 'after' => '');
...
function get_cancel_comment_reply_link($text = '') {
	if ( empty($text) )
		$text = wp__('Click here to cancel reply.');
...
function comment_form_title( $noreplytext = false, $replytext = false, $linktoparent = TRUE ) {
	...
	if ( false === $noreplytext ) $noreplytext = wp__( 'Leave a Reply' );
	if ( false === $replytext ) $replytext = wp__( 'Leave a Reply to %s' );
...
?>
		...
		<?php printf(wp__('<cite class="fn">%s</cite> <span class="says">says:</span>'), get_comment_author_link()) ?>
		</div>
...
<?php endif; ?>
		...
		<div class="comment-meta commentmetadata"><a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ) ?>">
			<?php
				...
				printf( wp__('%1$s at %2$s'), get_comment_date(),  get_comment_time()) ?></a><?php edit_comment_link(wp__('(Edit)'),'&nbsp;&nbsp;','' );
			?>
		</div>
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$fields =  array(
		'author' => '<p class="comment-form-author">' . '<label for="author">' . wp__( 'Name' ) . '</label> ' . ( $req ? '<span class="required">*</span>' : '' ) .
		            '<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>',
		'email'  => '<p class="comment-form-email"><label for="email">' . wp__( 'Email' ) . '</label> ' . ( $req ? '<span class="required">*</span>' : '' ) .
		            '<input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>',
		'url'    => '<p class="comment-form-url"><label for="url">' . wp__( 'Website' ) . '</label>' .
		            '<input id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></p>',
	);

	$required_text = sprintf( ' ' . wp__('Required fields are marked %s'), '<span class="required">*</span>' );
	$defaults = array(
		'fields'               => apply_filters( 'comment_form_default_fields', $fields ),
		'comment_field'        => '<p class="comment-form-comment"><label for="comment">' . _x( 'Comment', 'noun' ) . '</label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>',
		'must_log_in'          => '<p class="must-log-in">' .  sprintf( wp__( 'You must be <a href="%s">logged in</a> to post a comment.' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'logged_in_as'         => '<p class="logged-in-as">' . sprintf( wp__( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>' ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'comment_notes_before' => '<p class="comment-notes">' . wp__( 'Your email address will not be published.' ) . ( $req ? $required_text : '' ) . '</p>',
		'comment_notes_after'  => '<p class="form-allowed-tags">' . sprintf( wp__( 'You may use these <abbr title="HyperText Markup Language">HTML</abbr> tags and attributes: %s' ), ' <code>' . allowed_tags() . '</code>' ) . '</p>',
		'id_form'              => 'commentform',
		'id_submit'            => 'submit',
		'title_reply'          => wp__( 'Leave a Reply' ),
		'title_reply_to'       => wp__( 'Leave a Reply to %s' ),
		'cancel_reply_link'    => wp__( 'Cancel reply' ),
		'label_submit'         => wp__( 'Post Comment' ),
	);
