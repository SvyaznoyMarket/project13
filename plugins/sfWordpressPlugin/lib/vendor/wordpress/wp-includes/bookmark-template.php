	...
	foreach ( (array) $bookmarks as $bookmark ) {
		...
		if ( $show_updated )
			if ( '00' != substr($bookmark->link_updated_f, 0, 2) ) {
				$title .= ' (';
				$title .= sprintf(wp__('Last updated: %s'), date(get_option('links_updated_date_format'), $bookmark->link_updated_f + (get_option('gmt_offset') * 3600)));
				$title .= ')';
			}
...
function wp_list_bookmarks($args = '') {
	$defaults = array(
		...
		'categorize' => 1, 'title_li' => wp__('Bookmarks'),
