...
function wp_update_nav_menu_object( $menu_id = 0, $menu_data = array() ) {
	...
	if (
		...
		return new WP_Error( 'menu_exists', sprintf( wp__('The menu name <strong>%s</strong> conflicts with another menu name. Please try another.'), esc_html( $menu_data['menu-name'] ) ) );
...
function wp_update_nav_menu_item( $menu_id = 0, $menu_item_db_id = 0, $menu_item_data = array() ) {
	...
	if ( ! empty( $menu_item_db_id ) && ! is_nav_menu_item( $menu_item_db_id ) )
		return new WP_Error('update_nav_menu_item_failed', wp__('The given object ID is not that of a menu item.'));
	...
	if ( isset( $menu_item->post_type ) ) {
		if ( 'nav_menu_item' == $menu_item->post_type ) {
			...
			if ( 'post_type' == $menu_item->type ) {
				...
				$menu_item->type_label = wp__('Custom');
