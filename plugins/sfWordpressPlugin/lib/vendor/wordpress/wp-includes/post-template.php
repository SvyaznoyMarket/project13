...
function get_the_title( $id = 0 ) {
	...
	if ( !is_admin() ) {
		if ( !empty($post->post_password) ) {
			$protected_title_format = apply_filters('protected_title_format', wp__('Protected: %s'));
			$title = sprintf($protected_title_format, $title);
		} else if ( isset($post->post_status) && 'private' == $post->post_status ) {
			$private_title_format = apply_filters('private_title_format', wp__('Private: %s'));
...
function get_the_excerpt( $deprecated = '' ) {
	...
	if ( post_password_required($post) ) {
		$output = wp__('There is no excerpt because this is a protected post.');
		return $output;
	}
...
function wp_link_pages($args = '') {
	$defaults = array(
		'before' => '<p>' . wp__('Pages:'), 'after' => '</p>',
		'link_before' => '', 'link_after' => '',
		'next_or_number' => 'number', 'nextpagelink' => wp__('Next page'),
		'previouspagelink' => wp__('Previous page'), 'pagelink' => '%',
		'echo' => 1
	);
...
function wp_get_attachment_link( $id = 0, $size = 'thumbnail', $permalink = false, $icon = false, $text = false ) {
	...
	if ( empty( $_post ) || ( 'attachment' != $_post->post_type ) || ! $url = wp_get_attachment_url( $_post->ID ) )
		return wp__( 'Missing Attachment' );
...
function get_the_password_form() {
	...
	<p>' . wp__("This post is password protected. To view it please enter your password below:") . '</p>
	<p><label for="' . $label . '">' . wp__("Password:") . ' <input name="post_password" id="' . $label . '" type="password" size="20" /></label> <input type="submit" name="Submit" value="' . esc_attr__("Submit") . '" /></p>
	</form>
	';
...
function wp_post_revision_title( $revision, $link = true ) {
	...
	/* translators: 1: date */
	$autosavef = wp__( '%1$s [Autosave]' );
	/* translators: 1: date */
	$currentf  = wp__( '%1$s [Current Revision]' );
	...
	foreach ( $revisions as $revision ) {
		...
		if ( 'form-table' == $format ) {
			...
			if ( $post->ID != $revision->ID && $can_edit_post )
				$actions = '<a href="' . wp_nonce_url( add_query_arg( array( 'revision' => $revision->ID, 'action' => 'restore' ) ), "restore-post_$post->ID|$revision->ID" ) . '">' . wp__( 'Restore' ) . '</a>';
			else
				$actions = '';
