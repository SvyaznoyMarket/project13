...
function wp_protect_special_option( $option ) {
	...
	if ( in_array( $option, $protected ) )
		wp_die( sprintf( wp__( '%s is a protected WP option and may not be modified' ), esc_html( $option ) ) );
}
...
function do_feed() {
	...
	$hook = 'do_feed_' . $feed;
	if ( !has_action($hook) ) {
		$message = sprintf( wp__( 'ERROR: %s is not a valid feed template.' ), esc_html($feed));
...
function wp_upload_dir( $time = null ) {
	...
	if ( ! wp_mkdir_p( $uploads['path'] ) ) {
		$message = sprintf( wp__( 'Unable to create directory %s. Is its parent directory writable by the server?' ), $uploads['path'] );
...
function wp_upload_bits( $name, $deprecated, $bits, $time = null ) {
	...
	if ( empty( $name ) )
		return array( 'error' => wp__( 'Empty filename' ) );

	$wp_filetype = wp_check_filetype( $name );
	if ( !$wp_filetype['ext'] )
		return array( 'error' => wp__( 'Invalid file type' ) );
	...
	if ( ! wp_mkdir_p( dirname( $new_file ) ) ) {
		$message = sprintf( wp__( 'Unable to create directory %s. Is its parent directory writable by the server?' ), dirname( $new_file ) );
		return array( 'error' => $message );
	}

	$ifp = @ fopen( $new_file, 'wb' );
	if ( ! $ifp )
		return array( 'error' => sprintf( wp__( 'Could not write file %s' ), $new_file ) );
...
function wp_explain_nonce( $action ) {
	if ( $action !== -1 && preg_match( '/([a-z]+)-([a-z]+)(_(.+))?/', $action, $matches ) ) {
		...
		$trans = array();
		$trans['update']['attachment'] = array( wp__( 'Your attempt to edit this attachment: &#8220;%s&#8221; has failed.' ), 'get_the_title' );

		$trans['add']['category']      = array( wp__( 'Your attempt to add this category has failed.' ), false );
		$trans['delete']['category']   = array( wp__( 'Your attempt to delete this category: &#8220;%s&#8221; has failed.' ), 'get_cat_name' );
		$trans['update']['category']   = array( wp__( 'Your attempt to edit this category: &#8220;%s&#8221; has failed.' ), 'get_cat_name' );

		$trans['delete']['comment']    = array( wp__( 'Your attempt to delete this comment: &#8220;%s&#8221; has failed.' ), 'use_id' );
		$trans['unapprove']['comment'] = array( wp__( 'Your attempt to unapprove this comment: &#8220;%s&#8221; has failed.' ), 'use_id' );
		$trans['approve']['comment']   = array( wp__( 'Your attempt to approve this comment: &#8220;%s&#8221; has failed.' ), 'use_id' );
		$trans['update']['comment']    = array( wp__( 'Your attempt to edit this comment: &#8220;%s&#8221; has failed.' ), 'use_id' );
		$trans['bulk']['comments']     = array( wp__( 'Your attempt to bulk modify comments has failed.' ), false );
		$trans['moderate']['comments'] = array( wp__( 'Your attempt to moderate comments has failed.' ), false );

		$trans['add']['bookmark']      = array( wp__( 'Your attempt to add this link has failed.' ), false );
		$trans['delete']['bookmark']   = array( wp__( 'Your attempt to delete this link: &#8220;%s&#8221; has failed.' ), 'use_id' );
		$trans['update']['bookmark']   = array( wp__( 'Your attempt to edit this link: &#8220;%s&#8221; has failed.' ), 'use_id' );
		$trans['bulk']['bookmarks']    = array( wp__( 'Your attempt to bulk modify links has failed.' ), false );

		$trans['add']['page']          = array( wp__( 'Your attempt to add this page has failed.' ), false );
		$trans['delete']['page']       = array( wp__( 'Your attempt to delete this page: &#8220;%s&#8221; has failed.' ), 'get_the_title' );
		$trans['update']['page']       = array( wp__( 'Your attempt to edit this page: &#8220;%s&#8221; has failed.' ), 'get_the_title' );

		$trans['edit']['plugin']       = array( wp__( 'Your attempt to edit this plugin file: &#8220;%s&#8221; has failed.' ), 'use_id' );
		$trans['activate']['plugin']   = array( wp__( 'Your attempt to activate this plugin: &#8220;%s&#8221; has failed.' ), 'use_id' );
		$trans['deactivate']['plugin'] = array( wp__( 'Your attempt to deactivate this plugin: &#8220;%s&#8221; has failed.' ), 'use_id' );
		$trans['upgrade']['plugin']    = array( wp__( 'Your attempt to update this plugin: &#8220;%s&#8221; has failed.' ), 'use_id' );

		$trans['add']['post']          = array( wp__( 'Your attempt to add this post has failed.' ), false );
		$trans['delete']['post']       = array( wp__( 'Your attempt to delete this post: &#8220;%s&#8221; has failed.' ), 'get_the_title' );
		$trans['update']['post']       = array( wp__( 'Your attempt to edit this post: &#8220;%s&#8221; has failed.' ), 'get_the_title' );

		$trans['add']['user']          = array( wp__( 'Your attempt to add this user has failed.' ), false );
		$trans['delete']['users']      = array( wp__( 'Your attempt to delete users has failed.' ), false );
		$trans['bulk']['users']        = array( wp__( 'Your attempt to bulk modify users has failed.' ), false );
		$trans['update']['user']       = array( wp__( 'Your attempt to edit this user: &#8220;%s&#8221; has failed.' ), 'get_the_author_meta', 'display_name' );
		$trans['update']['profile']    = array( wp__( 'Your attempt to modify the profile for: &#8220;%s&#8221; has failed.' ), 'get_the_author_meta', 'display_name' );

		$trans['update']['options']    = array( wp__( 'Your attempt to edit your settings has failed.' ), false );
		$trans['update']['permalink']  = array( wp__( 'Your attempt to change your permalink structure to: %s has failed.' ), 'use_id' );
		$trans['edit']['file']         = array( wp__( 'Your attempt to edit this file: &#8220;%s&#8221; has failed.' ), 'use_id' );
		$trans['edit']['theme']        = array( wp__( 'Your attempt to edit this theme file: &#8220;%s&#8221; has failed.' ), 'use_id' );
		$trans['switch']['theme']      = array( wp__( 'Your attempt to switch to this theme: &#8220;%s&#8221; has failed.' ), 'use_id' );

		$trans['log']['out']           = array( sprintf( wp__( 'You are attempting to log out of %s' ), get_bloginfo( 'sitename' ) ), false );
		...
		return apply_filters( 'explain_nonce_' . $verb . '-' . $noun, wp__( 'Are you sure you want to do this?' ), isset($matches[4]) ? $matches[4] : '' );
	} else {
		return apply_filters( 'explain_nonce_' . $action, wp__( 'Are you sure you want to do this?' ) );
	}
}
...
function wp_nonce_ays( $action ) {
	$title = wp__( 'WordPress Failure Notice' );
	$html = esc_html( wp_explain_nonce( $action ) );
	if ( 'log-out' == $action )
		$html .= "</p><p>" . sprintf( wp__( "Do you really want to <a href='%s'>log out</a>?"), wp_logout_url() );
	elseif ( wp_get_referer() )
		$html .= "</p><p><a href='" . esc_url( remove_query_arg( 'updated', wp_get_referer() ) ) . "'>" . wp__( 'Please try again.' ) . "</a>";
...
function _default_wp_die_handler( $message, $title = '', $args = array() ) {
	...
	$have_gettext = function_exists('wp__');
	...
	if ( isset( $r['back_link'] ) && $r['back_link'] ) {
		$back_text = $have_gettext? wp__('&laquo; Back') : '&laquo; Back';
	...
	if ( !function_exists( 'did_action' ) || !did_action( 'admin_head' ) ) :
		...
		if ( empty($title) )
			$title = $have_gettext ? wp__('WordPress &rsaquo; Error') : 'WordPress &rsaquo; Error';
...
function wp_widgets_add_menu() {
	global $submenu;
	$submenu['themes.php'][7] = array( wp__( 'Widgets' ), 'edit_theme_options', 'widgets.php' );
...
function _deprecated_function( $function, $version, $replacement = null ) {
	...
	if ( WP_DEBUG && apply_filters( 'deprecated_function_trigger_error', true ) ) {
		if ( ! is_null($replacement) )
			trigger_error( sprintf( wp__('%1$s is <strong>deprecated</strong> since version %2$s! Use %3$s instead.'), $function, $version, $replacement ) );
		else
			trigger_error( sprintf( wp__('%1$s is <strong>deprecated</strong> since version %2$s with no alternative available.'), $function, $version ) );
	}
}
...
function _deprecated_file( $file, $version, $replacement = null, $message = '' ) {
	...
	if ( WP_DEBUG && apply_filters( 'deprecated_file_trigger_error', true ) ) {
		...
		if ( ! is_null( $replacement ) )
			trigger_error( sprintf( wp__('%1$s is <strong>deprecated</strong> since version %2$s! Use %3$s instead.'), $file, $version, $replacement ) . $message );
		else
			trigger_error( sprintf( wp__('%1$s is <strong>deprecated</strong> since version %2$s with no alternative available.'), $file, $version ) . $message );
	}
}
/**
...
function _deprecated_argument( $function, $version, $message = null ) {
	...
	if ( WP_DEBUG && apply_filters( 'deprecated_argument_trigger_error', true ) ) {
		if ( ! is_null( $message ) )
			trigger_error( sprintf( wp__('%1$s was called with an argument that is <strong>deprecated</strong> since version %2$s! %3$s'), $function, $version, $message ) );
		else
			trigger_error( sprintf( wp__('%1$s was called with an argument that is <strong>deprecated</strong> since version %2$s with no alternative available.'), $function, $version ) );
	}
}
...
function _doing_it_wrong( $function, $message, $version ) {
	...
	if ( WP_DEBUG && apply_filters( 'doing_it_wrong_trigger_error', true ) ) {
		$version = is_null( $version ) ? '' : sprintf( wp__( '(This message was added in version %s.)' ), $version );
		$message .= ' ' . wp__( 'Please see <a href="http://codex.wordpress.org/Debugging_in_WordPress">Debugging in WordPress</a> for more information.' );
		trigger_error( sprintf( wp__( '%1$s was called <strong>incorrectly</strong>. %2$s %3$s' ), $function, $message, $version ) );
	}
}
...
function wp_timezone_choice( $selected_zone ) {
	...
	if ( empty( $selected_zone ) ) {
		$structure[] = '<option selected="selected" value="">' . wp__( 'Select a city' ) . '</option>';
	}
	...
	if ( 'UTC' === $selected_zone )
		$selected = 'selected="selected" ';
	$structure[] = '<option ' . $selected . 'value="' . esc_attr( 'UTC' ) . '">' . wp__('UTC') . '</option>';
	$structure[] = '</optgroup>';
