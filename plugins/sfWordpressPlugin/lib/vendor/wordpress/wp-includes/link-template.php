		...
		if ( ! empty( $excluded_categories ) ) {
			if ( ! is_array( $excluded_categories ) ) {
				...
				if ( strpos( $excluded_categories, ' and ' ) !== false ) {
					_deprecated_argument( __FUNCTION__, '3.3', sprintf( wp__( 'Use commas instead of %s to separate excluded categories.' ), "'and'" ) );
...
function get_adjacent_post_rel_link($title = '%title', $in_same_cat = false, $excluded_categories = '', $previous = true) {
	...
	if ( empty($post->post_title) )
		$post->post_title = $previous ? wp__('Previous Post') : wp__('Next Post');
...
function adjacent_post_link($format, $link, $in_same_cat = false, $excluded_categories = '', $previous = true) {
	...
	if ( empty($post->post_title) )
		$title = $previous ? wp__('Previous Post') : wp__('Next Post');
...
function get_previous_posts_link( $label = null ) {
	...
	if ( null === $label )
		$label = wp__( '&laquo; Previous Page' );
...
function get_posts_nav_link( $args = array() ) {
	...
	if ( !is_singular() ) {
		$defaults = array(
			'sep' => ' &#8212; ',
			'prelabel' => wp__('&laquo; Previous Page'),
			'nxtlabel' => wp__('Next Page &raquo;'),
		);
...
function get_next_comments_link( $label = '', $max_page = 0 ) {
	...
	if ( empty($label) )
		$label = wp__('Newer Comments &raquo;');
...
function get_previous_comments_link( $label = '' ) {
	...
	if ( empty($label) )
		$label = wp__('&laquo; Older Comments');
 ...
 *
 * Call like the_shortlink(wp__('Shortlinkage FTW'))
 *
 * @since 3.0.0
 *
...
function the_shortlink( $text = '', $title = '', $before = '', $after = '' ) {
	...
	if ( empty( $text ) )
		$text = wp__('This is the short link.');
