...
class AtomServer {
	...
	function handle_request() {
		...
		if ( !get_option( 'enable_app' ) )
			$this->forbidden( sprintf( wp__( 'AtomPub services are disabled on this site.  An admin user can enable them at %s' ), admin_url('options-writing.php') ) );
	...
	function get_service() {
		...
		if ( !current_user_can( 'edit_posts' ) )
			$this->auth_required( wp__( 'Sorry, you do not have the right to access this site.' ) );
...
EOD;
	...
	function create_post() {
		...
		if ( !current_user_can($cap) )
			$this->auth_required(wp__('Sorry, you do not have the right to edit/publish new posts.'));
		...
		if ( !$postID )
			$this->internal_error(wp__('Sorry, your entry could not be posted. Something wrong happened.'));
	...
	function get_post($postID) {
		...
		if ( !current_user_can( 'edit_post', $postID ) )
			$this->auth_required( wp__( 'Sorry, you do not have the right to access this post.' ) );
	...
	function put_post($postID) {
		...
		if ( !current_user_can('edit_post', $entry['ID']) )
			$this->auth_required(wp__('Sorry, you do not have the right to edit this post.'));
		...
		if ( !$result )
			$this->internal_error(wp__('For some strange yet very annoying reason, this post could not be edited.'));
	...
	function delete_post($postID) {
		...
		if ( !current_user_can('edit_post', $postID) )
			$this->auth_required(wp__('Sorry, you do not have the right to delete this post.'));
		...
		if ( $entry['post_type'] == 'attachment' ) {
			...
			if ( !$result ) {
				$this->internal_error(wp__('For some strange yet very annoying reason, this post could not be deleted.'));
			}
	...
	function get_attachment($postID = null) {
		if ( !current_user_can( 'upload_files' ) )
			$this->auth_required( wp__( 'Sorry, you do not have permission to upload files.' ) );
	...
	function create_attachment() {
		...
		if ( !current_user_can('upload_files') )
			$this->auth_required(wp__('You do not have permission to upload files.'));
	...
	function delete_attachment($postID) {
		...
		if ( !isset($location) || 'attachment' != $entry['post_type'] || empty($filetype['ext']) )
			$this->internal_error(wp__('Error occurred while accessing post metadata for file location.'));
