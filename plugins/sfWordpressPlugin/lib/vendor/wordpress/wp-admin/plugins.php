...
if ( !current_user_can('activate_plugins') || !ENABLE_PLUGIN_MANAGE )
	wp_die( wp__( 'You do not have sufficient permissions to manage plugins for this site.' ) );
	...
	switch ( $action ) {
		case 'activate':
			if ( ! current_user_can('activate_plugins') )
				wp_die(wp__('You do not have sufficient permissions to activate plugins for this site.'));
		...
		case 'deactivate':
			if ( ! current_user_can('activate_plugins') )
				wp_die(wp__('You do not have sufficient permissions to deactivate plugins for this site.'));
		...
		case 'delete-selected':
			if ( ! current_user_can('delete_plugins') )
				wp_die(wp__('You do not have sufficient permissions to delete plugins for this site.'));
			...
			<div class="wrap">
					...
					<ul class="ul-disc">
						...
						foreach ( $plugin_info as $plugin ) {
							if ( $plugin['is_uninstallable'] ) {
								...
								echo '<li>', sprintf( wp__( '<strong>%1$s</strong> by <em>%2$s</em> (will also <strong>delete its data</strong>)' ), esc_html($plugin['Name']), esc_html($plugin['AuthorName']) ), '</li>';
								$data_to_delete = true;
							} else {
								/* translators: 1: plugin name, 2: plugin author */
								echo '<li>', sprintf( wp__('<strong>%1$s</strong> by <em>%2$s</em>' ), esc_html($plugin['Name']), esc_html($plugin['AuthorName']) ), '</li>';
							}
						}
						?>
					</ul>
				...
				<form method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>" style="display:inline;">
					...
					<?php submit_button( $data_to_delete ? wp__( 'Yes, Delete these files and data' ) : wp__( 'Yes, Delete these files' ), 'button', 'submit', false ); ?>
				</form>
				<form method="post" action="<?php echo esc_url(wp_get_referer()); ?>" style="display:inline;">
					<?php submit_button( wp__( 'No, Return me to the plugin list' ), 'button', 'submit', false ); ?>
				</form>
...
'title'		=> wp__('Overview'),
'content'	=>
	'<p>' . wp__('Plugins extend and expand the functionality of WordPress. Once a plugin is installed, you may activate it or deactivate it here.') . '</p>' .
	'<p>' . sprintf(wp__('You can find additional plugins for your site by using the <a href="%1$s">Plugin Browser/Installer</a> functionality or by browsing the <a href="%2$s" target="_blank">WordPress Plugin Directory</a> directly and installing new plugins manually. To manually install a plugin you generally just need to upload the plugin file into your <code>/wp-content/plugins</code> directory. Once a plugin has been installed, you can activate it here.'), 'plugin-install.php', 'http://wordpress.org/extend/plugins/') . '</p>'
) );
get_current_screen()->add_help_tab( array(
'id'		=> 'compatibility-problems',
'title'		=> wp__('Troubleshooting'),
'content'	=>
	...
	'<p>' . sprintf( wp__('If something goes wrong with a plugin and you can&#8217;t use WordPress, delete or rename that file in the <code>%s</code> directory and it will be automatically deactivated.'), WP_PLUGIN_DIR) . '</p>'
) );

get_current_screen()->set_help_sidebar(
	'<p><strong>' . wp__('For more information:') . '</strong></p>' .
	'<p>' . wp__('<a href="http://codex.wordpress.org/Managing_Plugins#Plugin_Management" target="_blank">Documentation on Managing Plugins</a>') . '</p>' .
...
if ( !empty($invalid) )
	foreach ( $invalid as $plugin_file => $error )
		echo '<div id="message" class="error"><p>' . sprintf(wp__('The plugin <code>%s</code> has been <strong>deactivated</strong> due to an error: %s'), esc_html($plugin_file), $error->get_error_message()) . '</p></div>';
?>

<?php if ( isset($_GET['error']) ) :

	if ( isset( $_GET['main'] ) )
		$errmsg = wp__( 'You cannot delete a plugin while it is active on the main site.' );
	elseif ( isset($_GET['charsout']) )
		$errmsg = sprintf(wp__('The plugin generated %d characters of <strong>unexpected output</strong> during activation.  If you notice &#8220;headers already sent&#8221; messages, problems with syndication feeds or other issues, try deactivating or removing this plugin.'), $_GET['charsout']);
	else
		$errmsg = wp__('Plugin could not be activated because it triggered a <strong>fatal error</strong>.');
	?>
...
<?php elseif ( isset($_GET['deleted']) ) :
		...
		<div id="message" class="updated"><p><?php printf( wp__('Plugin could not be deleted due to an error: %s'), $delete_result->get_error_message() ); ?></p></div>
		<?php else : ?>
...
<?php $wp_list_table->search_box( wp__( 'Search Installed Plugins' ), 'plugin' ); ?>
</form>
