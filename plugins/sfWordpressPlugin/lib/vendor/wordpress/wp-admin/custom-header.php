...
class Custom_Image_Header {
	...
	function init() {
		...
		$this->page = $page = add_theme_page(wp__('Header'), wp__('Header'), 'edit_theme_options', 'custom-header', array(&$this, 'admin_page'));
	...
	function help() {
		get_current_screen()->add_help_tab( array(
			...
			'title'   => wp__('Overview'),
			'content' =>
				'<p>' . wp__( 'You can set a custom image header for your site. Simply upload the image and crop it, and the new header will go live immediately.' ) . '</p>' .
				'<p>' . wp__( 'If you want to discard your custom header and go back to the default included in your theme, click on the buttons to remove the custom image and restore the original header image.' ) . '</p>' .
				'<p>' . wp__( 'Some themes come with additional header images bundled. If you see multiple images displayed, select the one you&#8217;d like and click the Save Changes button.' ) . '</p>'
		) );

		get_current_screen()->set_help_sidebar(
			'<p><strong>' . wp__( 'For more information:' ) . '</strong></p>' .
			'<p>' . wp__( '<a href="http://codex.wordpress.org/Appearance_Header_Screen" target="_blank">Documentation on Custom Header</a>' ) . '</p>' .
	...
	function show_header_selector( $type = 'default' ) {
		...
		if ( 1 < count( $headers ) ) {
			...
			echo wp__( '<strong>Random:</strong> Show a different image on each page.' );
			echo '</label>';
			echo '</div>';
		}
...
<p><?php printf( wp__( 'Header updated. <a href="%s">Visit your site</a> to see how it looks.' ), home_url( '/' ) ); ?></p>
</div>
<?php } ?>
...
<td>
	...
	<?php printf( wp__( 'Images of exactly <strong>%1$d &times; %2$d pixels</strong> will be used as-is.' ), HEADER_IMAGE_WIDTH, HEADER_IMAGE_HEIGHT ); ?></p>
	...
	<p>
		...
		<?php submit_button( wp__( 'Upload' ), 'button', 'submit', false ); ?>
	</p>
	</form>
</td>
</tr>
...
<td>
	...
	<?php submit_button( wp__( 'Remove Header Image' ), 'button', 'removeheader', false ); ?>
</td>
</tr>
	<?php endif;
...
<td>
	...
	<?php submit_button( wp__( 'Restore Original Header Image' ), 'button', 'resetheader', false ); ?>
</td>
</tr>
	<?php endif; ?>
</tbody>
...
<td>
	...
	<?php submit_button( wp__( 'Restore Original Header Text' ), 'button', 'resettext', false ); ?>
</td>
</tr>
	<?php } ?>
...
<?php }
	...
	function step_2() {
		...
		if ( isset($file['error']) )
			wp_die( $file['error'],  wp__( 'Image Upload Error' ) );
		...
		if ( $width == HEADER_IMAGE_WIDTH && $height == HEADER_IMAGE_HEIGHT ) {
			...
			if ( is_wp_error( $image ) )
				wp_die( wp__( 'Image could not be processed.  Please go back and try again.' ), wp__( 'Image Processing Error' ) );
...
<form method="post" action="<?php echo esc_attr(add_query_arg('step', 3)); ?>">
	...
	<?php submit_button( wp__( 'Crop and Publish' ) ); ?>
	</p>
</form>
</div>
		<?php
	...
	function admin_page() {
		if ( ! current_user_can('edit_theme_options') )
			wp_die(wp__('You do not have permission to customize headers.'));
		$step = $this->step();
		if ( 1 == $step )
			$this->step_1();
		elseif ( 2 == $step )
