...
class Custom_Background {
	...
	function init() {
		...
		$this->page = $page = add_theme_page(wp__('Background'), wp__('Background'), 'edit_theme_options', 'custom-background', array(&$this, 'admin_page'));
	...
	function admin_load() {
		get_current_screen()->add_help_tab( array(
			...
			'title'   => wp__('Overview'),
			'content' =>
				'<p>' . wp__( 'You can customize the look of your site without touching any of your theme&#8217;s code by using a custom background. Your background can be an image or a color.' ) . '</p>' .
				'<p>' . wp__( 'To use a background image, simply upload it, then choose your display options below. You can display a single instance of your image, or tile it to fill the screen. You can have your background fixed in place, so your site content moves on top of it, or you can have it scroll with your site.' ) . '</p>' .
				'<p>' . wp__( 'You can also choose a background color. If you know the hexadecimal code for the color you want, enter it in the Background Color field. If not, click on the Select a Color link, and a color picker will allow you to choose the exact shade you want.' ) . '</p>' .
				'<p>' . wp__( 'Don&#8217;t forget to click on the Save Changes button when you are finished.' ) . '</p>'
		) );

		get_current_screen()->set_help_sidebar(
			'<p><strong>' . wp__( 'For more information:' ) . '</strong></p>' .
			'<p>' . wp__( '<a href="http://codex.wordpress.org/Appearance_Background_Screen" target="_blank">Documentation on Custom Background</a>' ) . '</p>' .
...
<p><?php printf( wp__( 'Background updated. <a href="%s">Visit your site</a> to see how it looks.' ), home_url( '/' ) ); ?></p>
</div>
<?php }
...
<?php submit_button( wp__( 'Remove Background Image' ), 'button', 'remove-background', false ); ?><br/>
...
<?php submit_button( wp__( 'Restore Original Image' ), 'button', 'reset-background', false ); ?><br/>
