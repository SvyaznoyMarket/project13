...
if ( ! current_theme_supports( 'menus' ) && ! current_theme_supports( 'widgets' ) )
	wp_die( wp__( 'Your theme does not support navigation menus or widgets.' ) );
...
switch ( $action ) {
	...
	case 'delete-menu-item':
		...
		if ( is_nav_menu_item( $menu_item_id ) && wp_delete_post( $menu_item_id, true ) )
			$messages[] = '<div id="message" class="updated"><p>' . wp__('The menu item has been successfully deleted.') . '</p></div>';
		break;
	case 'delete':
		...
		if ( is_nav_menu( $nav_menu_selected_id ) ) {
			...
			if ( is_wp_error($delete_nav_menu) ) {
				...
				$messages[] = '<div id="message" class="updated"><p>' . wp__('The menu has been successfully deleted.') . '</p></div>';
	...
	case 'update':
		...
		if ( 0 == $nav_menu_selected_id ) {
			...
			if ( $new_menu_title ) {
				...
				if ( is_wp_error( $_nav_menu_selected_id ) ) {
					...
					$messages[] = '<div id="message" class="updated"><p>' . sprintf( wp__('The <strong>%s</strong> menu has been successfully created.'), $nav_menu_selected_title ) . '</p></div>';
				}
			} else {
				$messages[] = '<div id="message" class="error"><p>' . wp__('Please enter a valid menu name.') . '</p></div>';
			}
		...
		// update existing menu
			...
			if ( ! is_wp_error( $_menu_object ) ) {
				...
				$messages[] = '<div id="message" class="updated"><p>' . sprintf( wp__('The <strong>%s</strong> menu has been updated.'), $nav_menu_selected_title ) . '</p></div>';
...
if ( ! current_theme_supports( 'menus' ) && ! wp_get_nav_menus() )
	$messages[] = '<div id="message" class="updated"><p>' . wp__('The current theme does not natively support menus, but you can use the &#8220;Custom Menu&#8221; widget to add any menus you create here to the theme&#8217;s sidebar.') . '</p></div>';

get_current_screen()->add_help_tab( array(
'id'		=> 'overview',
'title'		=> wp__('Overview'),
'content'	=>
	'<p>' . wp__('This feature allows you to use a custom menu in place of your theme&#8217;s default menus.') . '</p>' .
	...
	'<p>' . wp__('If your theme does not support the custom menus feature yet (the default themes, Twenty Eleven and Twenty Ten, do), you can learn about adding this support by following the Documentation link to the side.') . '</p>'
) );
get_current_screen()->add_help_tab( array(
'id'		=> 'create-menus',
'title'		=> wp__('Create Menus'),
'content'	=>
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . wp__('<a href="http://codex.wordpress.org/Appearance_Menus_Screen" target="_blank">Documentation on Menus</a>') . '</p>' .
		...
		<div id="menu-management">
			<div id="select-nav-menu-container" class="hide-if-js">
				<form id="select-nav-menu" action="">
					...
					<?php submit_button( wp__( 'Select' ), 'secondary', 'select_menu', false ); ?>
				</form>
			</div>
						...
						<div id="submitpost" class="submitbox">
							<div class="major-publishing-actions">
								...
								<div class="auto-add-pages">
									<label class="howto"><input type="checkbox"<?php checked( $auto_add ); ?> name="auto-add-pages" value="1" /> <?php printf( wp__('Automatically add new top-level pages' ), esc_url( admin_url( 'edit.php?post_type=page' ) ) ); ?></label>
								</div>
								<?php endif; ?>
								<br class="clear" />
								<div class="publishing-action">
									<?php submit_button( empty( $nav_menu_selected_id ) ? wp__( 'Create Menu' ) : wp__( 'Save Menu' ), 'button-primary menu-save', 'save_menu', false, array( 'id' => 'save_menu_header' ) ); ?>
					...
					<div id="post-body">
						<div id="post-body-content">
							...
							if ( isset( $edit_markup ) ) {
								...
								echo '<p>' . wp__('To create a custom menu, give it a name above and click Create Menu. Then choose items like pages, categories or custom links from the left column to add to this menu.') . '</p>';
								echo '<p>' . wp__('After you have added your items, drag and drop to put them in the order you want. You can also click each item to reveal additional configuration options.') . '</p>';
								echo '<p>' . wp__('When you have finished building your custom menu, make sure you click the Save Menu button.') . '</p>';
								echo '</div>';
							}
							?>
					...
					<div id="nav-menu-footer">
						...
						<div class="publishing-action">
							...
							if ( ! empty( $nav_menu_selected_id ) )
								submit_button( wp__( 'Save Menu' ), 'button-primary menu-save', 'save_menu', false, array( 'id' => 'save_menu_footer' ) );
							?>
						</div>
						</div>
					</div><!-- /#nav-menu-footer -->
