...
if ( !defined('WP_ALLOW_REPAIR') ) {
	echo '<p>'.wp__('To allow use of this page to automatically repair database problems, please add the following line to your wp-config.php file.  Once this line is added to your config, reload this page.')."</p><code>define('WP_ALLOW_REPAIR', true);</code>";
	...
	foreach ( $tables as $table ) {
		...
		if ( 'OK' == $check->Msg_text ) {
			...
			printf( wp__( 'The %s table is okay.' ), $table );
		} else {
			/* translators: 1: table name, 2: error message, */
			printf( wp__( 'The %1$s table is not okay. It is reporting the following error: %2$s.  WordPress will attempt to repair this table&hellip;' ) , $table, "<code>$check->Msg_text</code>" );
			...
			if ( 'OK' == $check->Msg_text ) {
				/* translators: %s: table name */
				printf( wp__( 'Successfully repaired the %s table.' ), $table );
			} else {
				/* translators: 1: table name, 2: error message, */
				echo sprintf( wp__( 'Failed to repair the  %1$s table. Error: %2$s' ), $table, "<code>$check->Msg_text</code>" ) . '<br />';
		...
		if ( $okay && $optimize ) {
			...
			if ( 'Table is already up to date' == $check->Msg_text )  {
				/* translators: %s: table name */
				printf( wp__( 'The %s table is already optimized.' ), $table );
			} else {
				...
				if ( 'OK' == $check->Msg_text || 'Table is already up to date' == $check->Msg_text ) {
					/* translators: %s: table name */
					printf( wp__( 'Successfully optimized the %s table.' ), $table );
				} else {
					/* translators: 1: table name, 2: error message, */
					printf( wp__( 'Failed to optimize the %1$s table. Error: %2$s' ), $table, "<code>$check->Msg_text</code>" );
				}
			}
		}
		echo '</p>';
	...
	if ( !empty($problems) ) {
		printf('<p>'.wp__('Some database problems could not be repaired. Please copy-and-paste the following list of errors to the <a href="%s">WordPress support forums</a> to get additional assistance.').'</p>', 'http://wordpress.org/support/forum/3');
		...
		echo '<p>'.wp__('Repairs complete.  Please remove the following line from wp-config.php to prevent this page from being used by unauthorized users.')."</p><code>define('WP_ALLOW_REPAIR', true);</code>";
	}
} else {
