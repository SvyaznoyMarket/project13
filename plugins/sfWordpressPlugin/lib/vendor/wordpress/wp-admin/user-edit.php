...
elseif ( ! $user_id && ! IS_PROFILE_PAGE )
	wp_die(wp__( 'Invalid user ID.' ) );
elseif ( ! get_userdata( $user_id ) )
	wp_die( wp__('Invalid user ID.') );

wp_enqueue_script('user-profile');

$title = IS_PROFILE_PAGE ? wp__('Profile') : wp__('Edit User');
...
$profile_help = '<p>' . wp__('Your profile contains information about you (your &#8220;account&#8221;) as well as some personal options related to using WordPress.') . '</p>' .
	...
	'<p>' . wp__('Your username cannot be changed, but you can use other fields to enter your real name or a nickname, and change which name to display on your posts.') . '</p>' .
	'<p>' . wp__('Required fields are indicated; the rest are optional. Profile information will only be displayed if your theme is set up to do so.') . '</p>' .
	'<p>' . wp__('Remember to click the Update Profile button when you are finished.') . '</p>';
...
get_current_screen()->set_help_sidebar(
    ...
    '<p>' . wp__('<a href="http://codex.wordpress.org/Users_Your_Profile_Screen" target="_blank">Documentation on User Profiles</a>') . '</p>' .
...
if ( is_multisite() && ! current_user_can( 'manage_network_users' ) && $user_id != $current_user->ID && ! apply_filters( 'enable_edit_any_user_configuration', true ) )
	wp_die( wp__( 'You do not have permission to edit this user.' ) );
...
if ( $user_role )
	echo '<option value="">' . wp__('&mdash; No role for this site &mdash;') . '</option>';
else
	echo '<option value="" selected="selected">' . wp__('&mdash; No role for this site &mdash;') . '</option>';
?>
</select></td></tr>
...
<tr>
	...
	<p><?php printf( wp__('There is a pending change of your e-mail to <code>%1$s</code>. <a href="%2$s">Cancel</a>'), $new_email['newemail'], esc_url( self_admin_url( 'profile.php?dismiss=' . $current_user->ID . '_new_email' ) ) ); ?></p>
	</div>
	<?php endif; ?>
	</td>
</tr>
...
<?php submit_button( IS_PROFILE_PAGE ? wp__('Update Profile') : wp__('Update User') ); ?>
