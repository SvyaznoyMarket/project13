 ...
 * @var string
 */
$submitbutton_text = wp__('Edit Comment');
$toprow_title = sprintf(wp__('Editing Comment # %s'), $comment->comment_ID);
...
$stamp = wp__('Submitted on: <b>%1$s</b>');
...
<?php echo "<a class='submitdelete deletion' href='" . wp_nonce_url("comment.php?action=" . ( !EMPTY_TRASH_DAYS ? 'deletecomment' : 'trashcomment' ) . "&amp;c=$comment->comment_ID&amp;_wp_original_http_referer=" . urlencode(wp_get_referer()), 'delete-comment_' . $comment->comment_ID) . "'>" . ( !EMPTY_TRASH_DAYS ? wp__('Delete Permanently') : wp__('Move to Trash') ) . "</a>\n"; ?>
</div>
<div id="publishing-action">
<?php submit_button( wp__( 'Update' ), 'primary', 'save', false, array( 'tabindex' => '4' ) ); ?>
</div>
<div class="clear"></div>
</div>
</div>
...
<tr valign="top">
	...
	<?php
		if ( $comment->comment_author_email ) {
			printf( wp__( 'E-mail (%s):' ), get_comment_author_email_link( wp__( 'send e-mail' ), '', '' ) );
		} else {
			_e( 'E-mail:' );
		}
?></td>
...
<tr valign="top">
	...
	<?php
		if ( ! empty( $comment->comment_author_url ) && 'http://' != $comment->comment_author_url ) {
			$link = '<a href="' . $comment->comment_author_url . '" rel="external nofollow" target="_blank">' . wp__('visit site') . '</a>';
			printf( wp__( 'URL (%s):' ), apply_filters('get_comment_author_link', $link ) );
		} else {
			_e( 'URL:' );
		} ?></td>
