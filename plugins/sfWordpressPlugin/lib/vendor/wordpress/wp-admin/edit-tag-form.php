...
<?php } ?>
		...
		<?php if($taxonomy == 'bank') { ?>
            <tr class="form-field">
                <th scope="row" valign="top"><label for="description">Условия кредитования</label></th>
        ...
        <?php } elseif($taxonomy == 'category') { ?>
            <tr class="form-field">
                <th scope="row" valign="top"><label for="description">Текст пункта выпадающего списка</label></th>
                <td><textarea name="description" id="description" rows="5" cols="50" style="width: 97%;"><?php echo $tag->description; // textarea_escaped ?></textarea><br />
                </td>
            </tr>
        <?php } elseif(is_taxonomy_hierarchical($taxonomy)) { ?>
            <tr class="form-field">
                ...
                <td>
                    <?php wp_dropdown_categories(array('hide_empty' => 0, 'hide_if_empty' => false, 'name' => 'parent', 'orderby' => 'name', 'taxonomy' => $taxonomy, 'selected' => $tag->parent, 'exclude_tree' => $tag->term_id, 'hierarchical' => true, 'show_option_none' => wp__('None'))); ?><br />
        ...
        <?php if(in_array($taxonomy, array('category', 'bank'))) { ?>
            <tr class="form-field form-required">
            <th scope="row" valign="top"><label for="priority"><?php _ex('Приоритет показа', 'Приоритет показа'); ?></label></th>
            <td><input name="priority" id="priority" type="text" value="<?php if ( isset( $tag->priority ) ) echo esc_attr($tag->priority); ?>" size="40" />
                <p class="priority"><?php _e('Определяет порядок вывода терма'); ?></p>
            </td>
        </tr>
        <?php } ?>
