...
get_current_screen()->add_help_tab( array(
	'id'      => 'overview',
	'title'   => wp__('Overview'),
	'content' => '<p>' . wp__('This screen provides many options for controlling the management and display of comments and links to your posts/pages. So many, in fact, they won&#8217;t all fit here! :) Use the documentation links to get information on what each discussion setting does.') . '</p>' .
		'<p>' . wp__('You must click the Save Changes button at the bottom of the screen for new settings to take effect.') . '</p>',
) );

get_current_screen()->set_help_sidebar(
	'<p><strong>' . wp__('For more information:') . '</strong></p>' .
	'<p>' . wp__('<a href="http://codex.wordpress.org/Settings_Discussion_Screen" target="_blank">Documentation on Discussion Settings</a>') . '</p>' .
...
<br />
<small><em><?php echo '(' . wp__('These settings may be overridden for individual articles.') . ')'; ?></em></small>
</fieldset></td>
</tr>
<tr valign="top">
...
<?php if ( !get_option( 'users_can_register' ) && is_multisite() ) echo ' ' . wp__( '(Signup has been disabled. Only members of this site can comment.)' ); ?>
</label>
<br />

<label for="close_comments_for_old_posts">
<input name="close_comments_for_old_posts" type="checkbox" id="close_comments_for_old_posts" value="1" <?php checked('1', get_option('close_comments_for_old_posts')); ?> />
<?php printf( wp__('Automatically close comments on articles older than %s days'), '</label><input name="close_comments_days_old" type="text" id="close_comments_days_old" value="' . esc_attr(get_option('close_comments_days_old')) . '" class="small-text" />'); ?>
<br />
...
printf( wp__('Enable threaded (nested) comments %s levels deep'), $thread_comments_depth );
...
$default_comments_page .= '>' . wp__('last') . '</option><option value="oldest"';
if ( 'oldest' == get_option('default_comments_page') ) $default_comments_page .= ' selected="selected"';
$default_comments_page .= '>' . wp__('first') . '</option></select>';

printf( wp__('Break comments into pages with %1$s top level comments per page and the %2$s page displayed by default'), '</label><label for="comments_per_page"><input name="comments_per_page" type="text" id="comments_per_page" value="' . esc_attr(get_option('comments_per_page')) . '" class="small-text" />', $default_comments_page );
...
$comment_order .= '>' . wp__('older') . '</option><option value="desc"';
if ( 'desc' == get_option('comment_order') ) $comment_order .= ' selected="selected"';
$comment_order .= '>' . wp__('newer') . '</option></select>';

printf( wp__('Comments should be displayed with the %s comments at the top of each page'), $comment_order );
...
<p><label for="comment_max_links"><?php printf(wp__('Hold a comment in the queue if it contains %s or more links. (A common characteristic of comment spam is a large number of hyperlinks.)'), '<input name="comment_max_links" type="text" id="comment_max_links" value="' . esc_attr(get_option('comment_max_links')) . '" class="small-text" />' ); ?></label></p>
...
<?php
	$yesorno = array( 0 => wp__( 'Don&#8217;t show Avatars' ), 1 => wp__( 'Show Avatars' ) );
...
$ratings = array(
	...
	'G' => wp__('G &#8212; Suitable for all audiences'),
	/* translators: Content suitability rating: http://bit.ly/89QxZA */
	'PG' => wp__('PG &#8212; Possibly offensive, usually for audiences 13 and above'),
	/* translators: Content suitability rating: http://bit.ly/89QxZA */
	'R' => wp__('R &#8212; Intended for adult audiences above 17'),
	/* translators: Content suitability rating: http://bit.ly/89QxZA */
	'X' => wp__('X &#8212; Even more mature than above')
);
...
$avatar_defaults = array(
	'mystery' => wp__('Mystery Man'),
	'blank' => wp__('Blank'),
	'gravatar_default' => wp__('Gravatar Logo'),
	'identicon' => wp__('Identicon (Generated)'),
	'wavatar' => wp__('Wavatar (Generated)'),
	'monsterid' => wp__('MonsterID (Generated)'),
	'retro' => wp__('Retro (Generated)')
);
