...
if ( 'update' == $action ) {
	...
	if ( !isset( $whitelist_options[ $option_page ] ) )
		wp_die( wp__( '<strong>ERROR</strong>: options page not found.' ) );

	if ( 'options' == $option_page ) {
		if ( is_multisite() && ! is_super_admin() )
			wp_die( wp__( 'You do not have sufficient permissions to modify unregistered settings for this site.' ) );
	...
	if ( $options ) {
		foreach ( $options as $option ) {
			if ( $unregistered )
				_deprecated_argument( 'options.php', '2.7', sprintf( wp__( 'The <code>%1$s</code> setting is unregistered. Unregistered settings are deprecated. See http://codex.wordpress.org/Settings_API' ), $option, $option_page ) );
	...
	if ( !count( get_settings_errors() ) )
		add_settings_error('general', 'settings_updated', wp__('Settings saved.'), 'updated');
...
<?php submit_button( wp__( 'Save Changes' ), 'primary', 'Update' ); ?>
