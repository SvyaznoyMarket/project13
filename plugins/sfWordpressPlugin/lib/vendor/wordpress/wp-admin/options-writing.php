...
$title = wp__('Writing Settings');
$parent_file = 'options-general.php';

get_current_screen()->add_help_tab( array(
	'id'      => 'overview',
	'title'   => wp__('Overview'),
	'content' => '<p>' . wp__('You can submit content in several different ways; this screen holds the settings for all of them. The top section controls the editor within the dashboard, while the rest control external publishing methods. For more information on any of these methods, use the documentation links.') . '</p>' .
		'<p>' . wp__('You must click the Save Changes button at the bottom of the screen for new settings to take effect.') . '</p>',
) );

get_current_screen()->add_help_tab( array(
	'id'      => 'options-press',
	'title'   => wp__('Press This'),
...
if ( is_multisite() ) {
	$post_email_help = '<p>' . wp__('Due to security issues, you cannot use Post By Email on Multisite Installs.') . '</p>';
} else {
	$post_email_help = '<p>' . wp__('Post via email settings allow you to send your WordPress install an email with the content of your post.  You must set up a secret e-mail account with POP3 access to use this, and any mail received at this address will be posted, so it&#8217;s a good idea to keep this address very secret.') . '</p>';
}

get_current_screen()->add_help_tab( array(
	'id'      => 'options-postemail',
	'title'   => wp__('Post Via Email'),
	'content' => $post_email_help,
) );

get_current_screen()->add_help_tab( array(
	'id'      => 'options-remote',
	'title'   => wp__('Remote Publishing'),
	'content' => '<p>' . wp__('Remote Publishing allows you to use an external editor (like the iOS or Android app) to write your posts.') . '</p>',
) );

get_current_screen()->add_help_tab( array(
	'id'      => 'options-services',
	'title'   => wp__('Update Services'),
	'content' => '<p>' . wp__('If desired, WordPress will automatically alert various services of your new posts.') . '</p>',
) );

get_current_screen()->set_help_sidebar(
	'<p><strong>' . wp__('For more information:') . '</strong></p>' .
	'<p>' . wp__('<a href="http://codex.wordpress.org/Settings_Writing_Screen" target="_blank">Documentation on Writing Settings</a>') . '</p>' .
...
<p><?php printf(wp__('To post to WordPress from a desktop blogging client or remote website that uses the Atom Publishing Protocol or one of the XML-RPC publishing interfaces you must enable them below.')) ?></p>
<table class="form-table">
<tr valign="top">
...
<?php else : ?>
	...
	<p><?php printf(wp__('WordPress is not notifying any <a href="http://codex.wordpress.org/Update_Services">Update Services</a> because of your site&#8217;s <a href="%s">privacy settings</a>.'), 'options-privacy.php'); ?></p>
