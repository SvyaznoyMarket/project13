...
if ( ! is_user_logged_in() ) {
	...
	if ( isset( $_POST['action'] ) && $_POST['action'] == 'autosave' ) {
		...
		$message = sprintf( wp__('<strong>ALERT: You are logged out!</strong> Could not save draft. <a href="%s" target="_blank">Please log in again.</a>'), wp_login_url() );
...
case 'dim-comment' : // On success, die with time() instead of 1
	...
	if ( !$comment = get_comment( $id ) ) {
		$x = new WP_Ajax_Response( array(
			'what' => 'comment',
			'id' => new WP_Error('invalid_comment', sprintf(wp__('Comment %d does not exist'), $id))
		) );
		$x->send();
	}
...
case 'add-tag' :
	...
	if ( !$tag || is_wp_error($tag) || (!$tag = get_term( $tag['term_id'], $taxonomy )) ) {
		$message = wp__('An error has occurred. Please reload the page and try again.');
...
case 'get-tagcloud' :
	...
	if ( empty( $tags ) )
		die( isset( $tax->no_tagcloud ) ? $tax->no_tagcloud : wp__('No tags found!') );
...
case 'replyto-comment' :
	...
	elseif ( in_array($status, array('draft', 'pending', 'trash') ) )
		die( wp__('ERROR: you are replying to a comment on a draft post.') );
	...
	if ( $user->ID ) {
		...
		die( wp__('Sorry, you must be logged in to reply to a comment.') );
	}

	if ( '' == $comment_content )
		die( wp__('ERROR: please type a comment.') );
	...
	if ( isset($_POST['metakeyselect']) || isset($_POST['metakeyinput']) ) {
		...
		if ( $post->post_status == 'auto-draft' ) {
			...
			if ( $pid = edit_post() ) {
				...
				if ( !$mid = add_meta( $pid ) )
					die(wp__('Please provide a custom field value.'));
			} else {
				die('0');
			}
		...
		if ( '' == trim($key) )
			die(wp__('Please provide a custom field name.'));
		if ( '' == trim($value) )
			die(wp__('Please provide a custom field value.'));
...
case 'add-user' :
	...
	$x = new WP_Ajax_Response( array(
		...
		'supplemental' => array(
			'show-link' => sprintf(wp__( 'User <a href="#%s">%s</a> added' ), "user-$user_id", $user_object->user_login),
...
case 'autosave' : // The name of this action is hardcoded in edit_post()
	...
	$draft_saved_date_format = wp__('g:i:s a');
	/* translators: %s: date and time */
	$message = sprintf( wp__('Draft saved at %s.'), date_i18n( $draft_saved_date_format ) );

	$supplemental = array();
	if ( isset($login_grace_period) )
		$alert .= sprintf( wp__('Your login has expired. Please open a new browser window and <a href="%s" target="_blank">log in again</a>. '), add_query_arg( 'interim-login', 1, wp_login_url() ) );
	...
	if ( $last = wp_check_post_lock( $post->ID ) ) {
		...
		$last_user_name = $last_user ? $last_user->display_name : wp__( 'Someone' );
		$data = wp__( 'Autosave disabled.' );

		$supplemental['disable_autosave'] = 'disable';
		$alert .= sprintf( wp__( '%s is currently editing this article. If you update it, you will overwrite the changes.' ), esc_html( $last_user_name ) );
	}

	if ( 'page' == $post->post_type ) {
		if ( !current_user_can('edit_page', $post_ID) )
			die(wp__('You are not allowed to edit this page.'));
	} else {
		if ( !current_user_can('edit_post', $post_ID) )
			die(wp__('You are not allowed to edit this post.'));
	}
...
case 'inline-save':
	...
	if ( $last = wp_check_post_lock( $post_ID ) ) {
		$last_user = get_userdata( $last );
		$last_user_name = $last_user ? $last_user->display_name : wp__( 'Someone' );
		printf( $_POST['post_type'] == 'page' ? wp__( 'Saving is disabled: %s is currently editing this page.' ) : wp__( 'Saving is disabled: %s is currently editing this post.' ),	esc_html( $last_user_name ) );
		exit;
	}
...
case 'find_posts':
	...
	$html = '<table class="widefat" cellspacing="0"><thead><tr><th class="found-radio"><br /></th><th>'.wp__('Title').'</th><th>'.wp__('Date').'</th><th>'.wp__('Status').'</th></tr></thead><tbody>';
	foreach ( $posts as $post ) {
		...
		if ( '0000-00-00 00:00:00' == $post->post_date ) {
			...
			$time = mysql2date(wp__('Y/m/d'), $post->post_date);
		}
...
case 'save-widget' :
	...
	$error = '<p>' . wp__('An error has occurred. Please reload the page and try again.') . '</p>';
...
case 'wp-fullscreen-save-post' :
	...
	if ( $last_id = get_post_meta($post_id, '_edit_last', true) ) {
		...
		$last_edited = sprintf( wp__('Last edited by %1$s on %2$s at %3$s'), esc_html( $last_user->display_name ), $last_date, $last_time );
	} else {
		$last_edited = sprintf( wp__('Last edited on %1$s at %2$s'), $last_date, $last_time );
	}
