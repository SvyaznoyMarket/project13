...
if ( !current_user_can('manage_network_themes') )
	wp_die( wp__( 'You do not have sufficient permissions to manage network themes.' ) );
	...
	switch ( $action ) {
		...
		case 'delete-selected':
			if ( ! current_user_can( 'delete_themes' ) )
				wp_die( wp__('You do not have sufficient permissions to delete themes for this site.') );
			check_admin_referer( 'bulk-themes' );
			...
			<div class="wrap">
				...
				<p><?php echo _n( 'You are about to remove the following theme:', 'You are about to remove the following themes:', $themes_to_delete ); ?></p>
					<ul class="ul-disc">
						<?php foreach ( $theme_info as $theme )
							echo '<li>', sprintf( wp__('<strong>%1$s</strong> by <em>%2$s</em>' ), esc_html( $theme['Name'] ), esc_html( $theme['AuthorName'] ) ), '</li>'; /* translators: 1: theme name, 2: theme author */ ?>
					</ul>
				...
				<form method="post" action="<?php echo esc_url(wp_get_referer()); ?>" style="display:inline;">
					<?php submit_button( wp__( 'No, Return me to the theme list' ), 'button', 'submit', false ); ?>
				</form>
...
get_current_screen()->add_help_tab( array(
	...
	'title'   => wp__('Overview'),
	'content' =>
		'<p>' . wp__('This screen enables and disables the inclusion of themes available to choose in the Appearance menu for each site. It does not activate or deactivate which theme a site is currently using.') . '</p>' .
		'<p>' . wp__('If the network admin disables a theme that is in use, it can still remain selected on that site. If another theme is chosen, the disabled theme will not appear in the site&#8217;s Appearance > Themes screen.') . '</p>' .
		'<p>' . wp__('Themes can be enabled on a site by site basis by the network admin on the Edit Site screen (which has a Themes tab); get there via the Edit action link on the All Sites screen. Only network admins are able to install or edit themes.') . '</p>'
) );

get_current_screen()->set_help_sidebar(
	'<p><strong>' . wp__('For more information:') . '</strong></p>' .
	'<p>' . wp__('<a href="http://codex.wordpress.org/Network_Admin_Themes_Screen" target="_blank">Documentation on Network Themes</a>') . '</p>' .
...
if ( isset( $_GET['enabled'] ) ) {
	...
	echo '<div class="error"><p>' . wp__( 'You cannot delete a theme while it is active on the main site.' ) . '</p></div>';
}
