...
get_current_screen()->add_help_tab( array(
	...
	'content' =>
		...
		'<p>' . wp__('If a version update to core has not happened, clicking this button won&#8217;t affect anything.') . '</p>' .
		'<p>' . wp__('If this process fails for any reason, users logging in to their sites will force the same update.') . '</p>'
) );

get_current_screen()->set_help_sidebar(
	'<p><strong>' . wp__('For more information:') . '</strong></p>' .
	'<p>' . wp__('<a href="http://codex.wordpress.org/Network_Admin_Updates_Screen" target="_blank">Documentation on Update Network</a>') . '</p>' .
...
echo '<div class="wrap">';
screen_icon('tools');
echo '<h2>' . wp__( 'Update Network' ) . '</h2>';
	...
	case "upgrade":
		...
		foreach ( (array) $blogs as $details ) {
			...
			if ( is_wp_error( $response ) )
				wp_die( sprintf( wp__( 'Warning! Problem updating %1$s. Your server may not be able to connect to sites running on it. Error message: <em>%2$s</em>' ), $siteurl, $response->get_error_message() ) );
