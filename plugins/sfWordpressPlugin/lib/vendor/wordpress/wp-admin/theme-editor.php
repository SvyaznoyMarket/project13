...
if ( !current_user_can('edit_themes') )
	wp_die('<p>'.wp__('You do not have sufficient permissions to edit templates for this site.').'</p>');

$title = wp__("Edit Themes");
...
'title'		=> wp__('Overview'),
'content'	=>
	'<p>' . wp__('You can use the Theme Editor to edit the individual CSS and PHP files which make up your theme.') . '</p>
	<p>' . wp__('Begin by choosing a theme to edit from the dropdown menu and clicking Select. A list then appears of all the template files. Clicking once on any file name causes the file to appear in the large Editor box.') . '</p>
	<p>' . wp__('For PHP files, you can use the Documentation dropdown to select from functions recognized in that file. Lookup takes you to a web page with reference material about that particular function.') . '</p>
	<p>' . wp__('After typing in your edits, click Update File.') . '</p>
	<p>' . wp__('<strong>Advice:</strong> think very carefully about your site crashing if you are live-editing the theme currently in use.') . '</p>
	<p>' . wp__('Upgrading to a newer version of the same theme will override changes made here. To avoid this, consider creating a <a href="http://codex.wordpress.org/Child_Themes" target="_blank">child theme</a> instead.') . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . wp__('<a href="http://codex.wordpress.org/Theme_Development" target="_blank">Documentation on Theme Development</a>') . '</p>' .
	'<p>' . wp__('<a href="http://codex.wordpress.org/Using_Themes" target="_blank">Documentation on Using Themes</a>') . '</p>' .
	'<p>' . wp__('<a href="http://codex.wordpress.org/Editing_Files" target="_blank">Documentation on Editing Files</a>') . '</p>' .
	'<p>' . wp__('<a href="http://codex.wordpress.org/Template_Tags" target="_blank">Documentation on Template Tags</a>') . '</p>' .
...
if ( ! isset($themes[$theme]) )
	wp_die(wp__('The requested theme does not exist.'));
...
?>
	...
	<p class="howto"><?php printf( wp__( 'This child theme inherits templates from a parent theme, %s.' ), $themes[$theme]['Parent Theme'] ); ?></p>
	<?php endif; ?>
	<ul>
<?php
...
<?php
	if ( is_writeable( $file ) ) :
		submit_button( wp__( 'Update File' ), 'primary', 'submit', true, array( 'tabindex' => '2' ) );
	else : ?>
...
<?php
		...
		echo '<div class="error"><p>' . wp__('Oops, no such file exists! Double check the name and try again, merci.') . '</p></div>';
	}
?>
<br class="clear" />
</div>
