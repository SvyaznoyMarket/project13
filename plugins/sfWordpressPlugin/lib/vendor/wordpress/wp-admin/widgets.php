...
get_current_screen()->add_help_tab( array(
'id'		=> 'removing-reusing',
'title'		=> wp__('Removing and Reusing'),
'content'	=>
	'<p>' . wp__('If you want to remove the widget but save its setting for possible future use, just drag it into the Inactive Widgets area. You can add them back anytime from there. This is especially helpful when you switch to a theme with fewer or different widget areas.') . '</p>
	<p>' . wp__('Widgets may be used multiple times. You can give each widget a title, to display on your site, but it&#8217;s not required.') . '</p>
	<p>' . wp__('Enabling Accessibility Mode, via Screen Options, allows you to use Add and Edit buttons instead of using drag and drop.') . '</p>'
) );
get_current_screen()->add_help_tab( array(
'id'		=> 'missing-widgets',
'title'		=> wp__('Missing Widgets'),
'content'	=>
	'<p>' . wp__('Many themes show some sidebar widgets by default until you edit your sidebars, but they are not automatically displayed in your sidebar management tool. After you make your first widget change, you can re-add the default widgets by adding them from the Available Widgets area.') . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . wp__('<a href="http://codex.wordpress.org/Appearance_Widgets_Screen" target="_blank">Documentation on Widgets</a>') . '</p>' .
	...
	if ( !isset( $wp_registered_sidebars[ $sidebar_id ] ) ) {
		if ( ! empty( $widgets ) ) { // register the inactive_widgets area as sidebar
			register_sidebar(array(
				'name' => wp__( 'Inactive Sidebar (not used)' ),
				'id' => $sidebar_id,
				'class' => 'inactive-sidebar orphan-sidebar',
				'description' => wp__( 'This sidebar is no longer available and does not show anywhere on your site. Remove each of the widgets below to fully remove this inactive sidebar.' ),
				'before_widget' => '',
				'after_widget' => '',
				'before_title' => '',
				'after_title' => '',
...
register_sidebar(array(
	'name' => wp__('Inactive Widgets'),
	'id' => 'wp_inactive_widgets',
	'class' => 'inactive-sidebar',
	'description' => wp__( 'Drag widgets here to remove them from the sidebar but keep their settings.' ),
	'before_widget' => '',
	'after_widget' => '',
	'before_title' => '',
	'after_title' => '',
...
if ( isset($_GET['editwidget']) && $_GET['editwidget'] ) {
	...
	<h3><?php printf( wp__( 'Widget %s' ), $name ); ?></h3>
...
<?php
	if ( is_callable( $control_callback ) )
		...
		echo '<p>' . wp__('There are no options for this widget.') . "</p>\n"; ?>
	</div>
...
<?php
	foreach ( $wp_registered_sidebars as $sbname => $sbvalue ) {
		...
		if ( 'wp_inactive_widgets' == $sbname || 'orphaned_widgets' == substr( $sbname, 0, 16 ) ) {
			...
			echo "\t\t<option value=''>" . wp__('&mdash; Select &mdash;') . "</option>\n";
			for ( $i = 1; $i <= $j; $i++ ) {
...
<?php
		...
		submit_button( wp__( 'Delete' ), 'button alignleft', 'removewidget', false );
	}
	submit_button( wp__( 'Save Widget' ), 'button-primary alignright', 'savewidget', false ); ?>
...
$errors = array(
	wp__('Error while saving.'),
	wp__('Error in displaying the widget settings form.')
);
