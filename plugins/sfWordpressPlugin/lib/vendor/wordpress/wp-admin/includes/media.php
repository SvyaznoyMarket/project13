...
function media_upload_tabs() {
	$_default_tabs = array(
		'type' => wp__('From Computer'), // handler action suffix => tab text
		'type_url' => wp__('From URL'),
		'gallery' => wp__('Gallery'),
		'library' => wp__('Media Library')
	);
...
function update_gallery_tab($tabs) {
	...
	$tabs['gallery'] = sprintf(wp__('Gallery (%s)'), "<span id='attachments-count'>$attachments</span>");
...
function media_buttons($editor_id = 'content') {
	$context = apply_filters('media_buttons_context', wp__('Upload/Insert %s'));
...
function wp_media_upload_handler() {
	...
	if ( isset($_POST['save']) ) {
		$errors['upload_notice'] = wp__('Saved.');
...
function image_align_input_fields( $post, $checked = '' ) {
	...
	$alignments = array('none' => wp__('None'), 'left' => wp__('Left'), 'center' => wp__('Center'), 'right' => wp__('Right'));
...
function image_size_input_fields( $post, $check = '' ) {
		...
		$size_names = apply_filters( 'image_size_names_choose', array('thumbnail' => wp__('Thumbnail'), 'medium' => wp__('Medium'), 'large' => wp__('Large'), 'full' => wp__('Full Size')) );
...
function image_link_input_fields($post, $url_type = '') {
	...
	<button type='button' class='button urlnone' title=''>" . wp__('None') . "</button>
	<button type='button' class='button urlfile' title='" . esc_attr($file) . "'>" . wp__('File URL') . "</button>
	<button type='button' class='button urlpost' title='" . esc_attr($link) . "'>" . wp__('Attachment Post URL') . "</button>
";
}
...
function image_attachment_fields_to_edit($form_fields, $post) {
	if ( substr($post->post_mime_type, 0, 5) == 'image' ) {
		...
		$form_fields['image_alt'] = array(
			'value' => $alt,
			'label' => wp__('Alternate Text'),
			'helps' => wp__('Alt text for the image, e.g. &#8220;The Mona Lisa&#8221;')
		);

		$form_fields['align'] = array(
			'label' => wp__('Alignment'),
			'input' => 'html',
...
function image_attachment_fields_to_save($post, $attachment) {
	if ( substr($post['post_mime_type'], 0, 5) == 'image' ) {
		if ( strlen(trim($post['post_title'])) == 0 ) {
			...
			$post['errors']['post_title']['errors'][] = wp__('Empty Title filled from filename.');
		}
	}
...
function get_attachment_fields_to_edit($post, $errors = null) {
	...
	$form_fields = array(
		...
		'url'          => array(
			...
			'helps'      => wp__('Enter a link URL or click above for presets.')
		),
		'menu_order'   => array(
			'label'      => wp__('Order'),
		...
		'image_url'	=> array(
			...
			'helps'      => wp__('Location of the uploaded file.')
		)
	);
...
function get_media_item( $attachment_id, $args = null ) {
	...
	<table class='slidetoggle describe $class'>
		...
		<tr valign='top'>
			...
			<td>
			<p><strong>" . wp__('File name:') . "</strong> $filename</p>
			<p><strong>" . wp__('File type:') . "</strong> $post->post_mime_type</p>
			<p><strong>" . wp__('Upload date:') . "</strong> " . mysql2date( get_option('date_format'), $post->post_date ). '</p>';
			if ( !empty( $media_dims ) )
				$item .= "<p><strong>" . wp__('Dimensions:') . "</strong> $media_dims</p>\n";
	...
	if ( $send )
		$send = get_submit_button( wp__( 'Insert into Post' ), 'button', "send[$attachment_id]", false );
	if ( $delete && current_user_can( 'delete_post', $attachment_id ) ) {
		if ( !EMPTY_TRASH_DAYS ) {
			$delete = "<a href='" . wp_nonce_url( "post.php?action=delete&amp;post=$attachment_id", 'delete-attachment_' . $attachment_id ) . "' id='del[$attachment_id]' class='delete'>" . wp__( 'Delete Permanently' ) . '</a>';
		} elseif ( !MEDIA_TRASH ) {
			$delete = "<a href='#' class='del-link' onclick=\"document.getElementById('del_attachment_$attachment_id').style.display='block';return false;\">" . wp__( 'Delete' ) . "</a>
			 <div id='del_attachment_$attachment_id' class='del-attachment' style='display:none;'>" . sprintf( wp__( 'You are about to delete <strong>%s</strong>.' ), $filename ) . "
			 <a href='" . wp_nonce_url( "post.php?action=delete&amp;post=$attachment_id", 'delete-attachment_' . $attachment_id ) . "' id='del[$attachment_id]' class='button'>" . wp__( 'Continue' ) . "</a>
			 <a href='#' class='button' onclick=\"this.parentNode.style.display='none';return false;\">" . wp__( 'Cancel' ) . "</a>
			 </div>";
		} else {
			$delete = "<a href='" . wp_nonce_url( "post.php?action=trash&amp;post=$attachment_id", 'trash-attachment_' . $attachment_id ) . "' id='del[$attachment_id]' class='delete'>" . wp__( 'Move to Trash' ) . "</a>
			<a href='" . wp_nonce_url( "post.php?action=untrash&amp;post=$attachment_id", 'untrash-attachment_' . $attachment_id ) . "' id='undo[$attachment_id]' class='undo hidden'>" . wp__( 'Undo' ) . "</a>";
		}
	} else {
		$delete = '';
	}
...
if ( is_multisite() && !is_upload_space_available() ) {
	echo '<p>' . sprintf( wp__( 'Sorry, you have filled your storage quota (%s MB).' ), get_space_allowed() ) . '</p>';
	return;
}
...
$plupload_init = array(
	...
	'filters' => array( array('title' => wp__( 'Allowed Files' ), 'extensions' => '*') ),
	'multipart' => true,
...
<?php do_action('pre-html-upload-ui'); ?>
	<p id="async-upload-wrap">
		...
		<?php submit_button( wp__( 'Upload' ), 'button', 'html-upload', false ); ?>
...
<span class="max-upload-size"><?php printf( wp__( 'Maximum upload file size: %d%s.' ), esc_html($upload_size_unit), esc_html($sizes[$u]) ); ?></span>
<?php
...
<?php submit_button( wp__( 'Save all changes' ), 'button', 'save', false ); ?>
</p>
</form>
<?php
}
...
<p class="ml-submit">
<?php submit_button( wp__( 'Save all changes' ), 'button savebutton', 'save', false, array( 'id' => 'save-all', 'style' => 'display: none;' ) ); ?>
<input type="hidden" name="post_id" id="post_id" value="<?php echo (int) $post_id; ?>" />
...
<p id="media-search" class="search-box">
	...
	<?php submit_button( wp__( 'Search Media' ), 'button', '', false ); ?>
</p>
...
if ( empty($_GET['post_mime_type']) || $_GET['post_mime_type'] == 'all' )
	...
	$class = '';
$type_links[] = "<li><a href='" . esc_url(add_query_arg(array('post_mime_type'=>'all', 'paged'=>false, 'm'=>false))) . "'$class>".wp__('All Types')."</a>";
...
$page_links = paginate_links( array(
	...
	'format' => '',
	'prev_text' => wp__('&laquo;'),
	'next_text' => wp__('&raquo;'),
...
<?php submit_button( wp__( 'Filter &#187;' ), 'secondary', 'post-query-submit', false ); ?>
...
<p class="ml-submit">
<?php submit_button( wp__( 'Save all changes' ), 'button savebutton', 'save', false ); ?>
<input type="hidden" name="post_id" id="post_id" value="<?php echo (int) $post_id; ?>" />
</p>
</form>
<?php
	...
	if ( !apply_filters( 'disable_captions', '' ) ) {
		...
		<tr class="image-only">
			<th valign="top" scope="row" class="label">
				<span class="alignleft"><label for="caption">' . wp__('Image Caption') . '</label></span>
			</th>
...
';
	...
	return '
	<p class="media-types"><label><input type="radio" name="media_type" value="image" id="image-only"' . checked( 'image-only', $view, false ) . ' /> ' . wp__( 'Image' ) . '</label> &nbsp; &nbsp; <label><input type="radio" name="media_type" value="generic" id="not-image"' . checked( 'not-image', $view, false ) . ' /> ' . wp__( 'Audio, Video, or Other File' ) . '</label></p>
	<table class="describe ' . $table_class . '"><tbody>
		<tr>
			<th valign="top" scope="row" class="label" style="width:130px;">
				<span class="alignleft"><label for="src">' . wp__('URL') . '</label></span>
		...
		<tr>
			<th valign="top" scope="row" class="label">
				<span class="alignleft"><label for="title">' . wp__('Title') . '</label></span>
				<span class="alignright"><abbr title="required" class="required">*</abbr></span>
			</th>
			<td class="field"><input id="title" name="title" value="" type="text" aria-required="true" /></td>
		</tr>

		<tr class="not-image"><td></td><td><p class="help">' . wp__('Link text, e.g. &#8220;Ransom Demands (PDF)&#8221;') . '</p></td></tr>

		<tr class="image-only">
			<th valign="top" scope="row" class="label">
				<span class="alignleft"><label for="alt">' . wp__('Alternate Text') . '</label></span>
			</th>
			<td class="field"><input id="alt" name="alt" value="" type="text" aria-required="true" />
			<p class="help">' . wp__('Alt text for the image, e.g. &#8220;The Mona Lisa&#8221;') . '</p></td>
		</tr>
		' . $caption . '
		<tr class="align image-only">
			<th valign="top" scope="row" class="label"><p><label for="align">' . wp__('Alignment') . '</label></p></th>
			<td class="field">
				...
				<label for="align-none" class="align image-align-none-label">' . wp__('None') . '</label>
				...
				<label for="align-left" class="align image-align-left-label">' . wp__('Left') . '</label>
				...
				<label for="align-center" class="align image-align-center-label">' . wp__('Center') . '</label>
				...
				<label for="align-right" class="align image-align-right-label">' . wp__('Right') . '</label>
			</td>
		</tr>

		<tr class="image-only">
			<th valign="top" scope="row" class="label">
				<span class="alignleft"><label for="url">' . wp__('Link Image To:') . '</label></span>
			</th>
			<td class="field"><input id="url" name="url" value="" type="text" /><br />

			<button type="button" class="button" value="" onclick="document.forms[0].url.value=null">' . wp__('None') . '</button>
			<button type="button" class="button" value="" onclick="document.forms[0].url.value=document.forms[0].src.value">' . wp__('Link to image') . '</button>
			<p class="help">' . wp__('Enter a link URL or click above for presets.') . '</p></td>
		</tr>
		<tr class="image-only">
			<td></td>
			<td>
		...
		<tr class="not-image">
			<td></td>
			<td>
				' . get_submit_button( wp__( 'Insert into Post' ), 'button', 'insertonlybutton', false ) . '
			</td>
		</tr>
	</tbody></table>
';
...
<?php
	...
	printf( wp__( 'Scale images to match the large size selected in %1$simage options%2$s (%3$d &times; %4$d).' ), $a, $end, (int) get_option( 'large_size_w', '1024' ), (int) get_option( 'large_size_h', '1024' ) );
?>
</label></p>
<?php
}
