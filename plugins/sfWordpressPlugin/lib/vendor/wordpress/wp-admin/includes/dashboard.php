...
function wp_dashboard_setup() {
	...
	if ( $response && $response['upgrade'] ) {
		...
		if ( $response['insecure'] )
			wp_add_dashboard_widget( 'dashboard_browser_nag', wp__( 'You are using an insecure browser!' ), 'wp_dashboard_browser_nag' );
		else
			wp_add_dashboard_widget( 'dashboard_browser_nag', wp__( 'Your browser is out of date!' ), 'wp_dashboard_browser_nag' );
	}

	// Right Now
	if ( is_blog_admin() && current_user_can('edit_posts') )
		wp_add_dashboard_widget( 'dashboard_right_now', wp__( 'Right Now' ), 'wp_dashboard_right_now' );

	if ( is_network_admin() )
		wp_add_dashboard_widget( 'network_dashboard_right_now', wp__( 'Right Now' ), 'wp_network_dashboard_right_now' );
	...
	if ( is_blog_admin() && current_user_can('moderate_comments') ) {
		...
		$recent_comments_title = wp__( 'Recent Comments' );
	...
	if ( is_blog_admin() && current_user_can('publish_posts') ) {
		...
		wp_add_dashboard_widget( 'dashboard_incoming_links', wp__( 'Incoming Links' ), 'wp_dashboard_incoming_links', 'wp_dashboard_incoming_links_control' );
	}

	// WP Plugins Widget
	if ( ( ! is_multisite() && is_blog_admin() && current_user_can( 'install_plugins' ) ) || ( is_network_admin() && current_user_can( 'manage_network_plugins' ) && current_user_can( 'install_plugins' ) ) )
		wp_add_dashboard_widget( 'dashboard_plugins', wp__( 'Plugins' ), 'wp_dashboard_plugins' );

	// QuickPress Widget
	if ( is_blog_admin() && current_user_can('edit_posts') )
		wp_add_dashboard_widget( 'dashboard_quick_press', wp__( 'QuickPress' ), 'wp_dashboard_quick_press' );

	// Recent Drafts
	if ( is_blog_admin() && current_user_can('edit_posts') )
		wp_add_dashboard_widget( 'dashboard_recent_drafts', wp__('Recent Drafts'), 'wp_dashboard_recent_drafts' );

	// Primary feed (Dev Blog) Widget
	if ( !isset( $widget_options['dashboard_primary'] ) ) {
		$update = true;
		$widget_options['dashboard_primary'] = array(
			'link' => apply_filters( 'dashboard_primary_link',  wp__( 'http://wordpress.org/news/' ) ),
			'url' => apply_filters( 'dashboard_primary_feed',  wp__( 'http://wordpress.org/news/feed/' ) ),
			'title' => apply_filters( 'dashboard_primary_title', wp__( 'WordPress Blog' ) ),
			'items' => 2,
			'show_summary' => 1,
			'show_author' => 0,
			'show_date' => 1,
	...
	if ( !isset( $widget_options['dashboard_secondary'] ) ) {
		...
		$widget_options['dashboard_secondary'] = array(
			'link' => apply_filters( 'dashboard_secondary_link',  wp__( 'http://planet.wordpress.org/' ) ),
			'url' => apply_filters( 'dashboard_secondary_feed',  wp__( 'http://planet.wordpress.org/feed/' ) ),
			'title' => apply_filters( 'dashboard_secondary_title', wp__( 'Other WordPress News' ) ),
			'items' => 5,
			'show_summary' => 0,
			'show_author' => 0,
			'show_date' => 0,
	...
	foreach ( $dashboard_widgets as $widget_id ) {
		$name = empty( $wp_registered_widgets[$widget_id]['all_link'] ) ? $wp_registered_widgets[$widget_id]['name'] : $wp_registered_widgets[$widget_id]['name'] . " <a href='{$wp_registered_widgets[$widget_id]['all_link']}' class='edit-box open-box'>" . wp__('View all') . '</a>';
...
function wp_add_dashboard_widget( $widget_id, $widget_name, $callback, $control_callback = null ) {
	...
	if ( $control_callback && current_user_can( 'edit_dashboard' ) && is_callable( $control_callback ) ) {
		...
		if ( isset( $_GET['edit'] ) && $widget_id == $_GET['edit'] ) {
			...
			$widget_name .= ' <span class="postbox-title-action"><a href="' . esc_url( $url ) . '">' . wp__( 'Cancel' ) . '</a></span>';
			$callback = '_wp_dashboard_control_callback';
		} else {
			list($url) = explode( '#', add_query_arg( 'edit', $widget_id ), 2 );
			$widget_name .= ' <span class="postbox-title-action"><a href="' . esc_url( "$url#$widget_id" ) . '" class="edit-box open-box">' . wp__( 'Configure' ) . '</a></span>';
		}
	}
	...
	*/
	...
	if ( empty( $ct->stylesheet_dir ) ) {
		if ( ! is_multisite() || is_super_admin() )
			echo '<span class="error-message">' . wp__('ERROR: The themes directory is either empty or doesn&#8217;t exist. Please check your installation.') . '</span>';
		...
		if ( current_user_can( 'switch_themes' ) )
			printf( wp__('Theme <span class="b"><a href="themes.php">%1$s</a></span>'), $ct->title );
		else
			printf( wp__('Theme <span class="b">%1$s</span>'), $ct->title );
	}
	echo '</p>';

	// Check if search engines are blocked.
	if ( !is_network_admin() && !is_user_admin() && current_user_can('manage_options') && '1' != get_option('blog_public') ) {
		$title = apply_filters('privacy_on_link_title', wp__('Your site is asking search engines not to index its content') );
		$content = apply_filters('privacy_on_link_text', wp__('Search Engines Blocked') );
...
function wp_network_dashboard_right_now() {
	...
	if ( current_user_can('create_sites') )
		$actions['create-site'] = '<a href="' . network_admin_url('site-new.php') . '">' . wp__( 'Create a New Site' ) . '</a>';
	if ( current_user_can('create_users') )
		$actions['create-user'] = '<a href="' . network_admin_url('user-new.php') . '">' . wp__( 'Create a New User' ) . '</a>';
	...
	$sentence = sprintf( wp__( 'You have %1$s and %2$s.' ), $blog_text, $user_text );
...
?>
	...
	<form name="searchform" action="<?php echo network_admin_url('users.php'); ?>" method="get">
		<p>
			...
			<?php submit_button( wp__( 'Search Users' ), 'button', 'submit', false, array( 'id' => 'submit_users' ) ); ?>
		</p>
	</form>
	...
	<form name="searchform" action="<?php echo network_admin_url('sites.php'); ?>" method="get">
		<p>
			<input type="text" name="s" value="" size="17" />
			<?php submit_button( wp__( 'Search Sites' ), 'button', 'submit', false, array( 'id' => 'submit_sites' ) ); ?>
		</p>
	</form>
<?php
	...
	if ( 'post' === strtolower( $_SERVER['REQUEST_METHOD'] ) && isset( $_POST['action'] ) && 0 === strpos( $_POST['action'], 'post-quickpress' ) && (int) $_POST['post_ID'] ) {
		...
		if ( 'post-quickpress-publish' == $_POST['action'] ) {
			if ( current_user_can('publish_posts') )
				printf( '<div class="updated"><p>' . wp__( 'Post published. <a href="%s">View post</a> | <a href="%s">Edit post</a>' ) . '</p></div>', esc_url( $view ), $edit );
			else
				printf( '<div class="updated"><p>' . wp__( 'Post submitted. <a href="%s">Preview post</a> | <a href="%s">Edit post</a>' ) . '</p></div>', esc_url( add_query_arg( 'preview', 1, $view ) ), $edit );
		} else {
			printf( '<div class="updated"><p>' . wp__( 'Draft saved. <a href="%s">Preview post</a> | <a href="%s">Edit post</a>' ) . '</p></div>', esc_url( add_query_arg( 'preview', 1, $view ) ), $edit );
		...
		printf('<p class="textright">' . wp__('You can also try %s, easy blogging from anywhere on the Web.') . '</p>', '<a href="' . esc_url( admin_url( 'tools.php' ) ) . '">' . wp__('Press This') . '</a>' );
...
?>
	...
	<form name="post" action="<?php echo esc_url( admin_url( 'post.php' ) ); ?>" method="post" id="quick-press">
		...
		<p class="submit">
			...
			<?php submit_button( wp__( 'Save Draft' ), 'button', 'save', false, array( 'id' => 'save-post', 'tabindex'=> 4 ) ); ?>
...
function wp_dashboard_recent_drafts( $drafts = false ) {
	...
	if ( $drafts && is_array( $drafts ) ) {
		...
		foreach ( $drafts as $draft ) {
			...
			$item = "<h4><a href='$url' title='" . sprintf( wp__( 'Edit &#8220;%s&#8221;' ), esc_attr( $title ) ) . "'>" . esc_html($title) . "</a> <abbr title='" . get_the_time(wp__('Y/m/d g:i:s A'), $draft) . "'>" . get_the_time( get_option( 'date_format' ), $draft ) . '</abbr></h4>';
...
function _wp_dashboard_recent_comments_row( &$comment, $show_date = true ) {
	...
	if ( current_user_can( 'edit_comment', $comment->comment_ID ) ) {
		...
		$actions['approve'] = "<a href='$approve_url' class='dim:the-comment-list:comment-$comment->comment_ID:unapproved:e7e7d3:e7e7d3:new=approved vim-a' title='" . esc_attr__( 'Approve this comment' ) . "'>" . wp__( 'Approve' ) . '</a>';
		$actions['unapprove'] = "<a href='$unapprove_url' class='dim:the-comment-list:comment-$comment->comment_ID:unapproved:e7e7d3:e7e7d3:new=unapproved vim-u' title='" . esc_attr__( 'Unapprove this comment' ) . "'>" . wp__( 'Unapprove' ) . '</a>';
		$actions['edit'] = "<a href='comment.php?action=editcomment&amp;c={$comment->comment_ID}' title='" . esc_attr__('Edit comment') . "'>". wp__('Edit') . '</a>';
		$actions['reply'] = '<a onclick="commentReply.open(\''.$comment->comment_ID.'\',\''.$comment->comment_post_ID.'\');return false;" class="vim-r hide-if-no-js" title="'.esc_attr__('Reply to this comment').'" href="#">' . wp__('Reply') . '</a>';
		...
		if ( !EMPTY_TRASH_DAYS )
			$actions['delete'] = "<a href='$delete_url' class='delete:the-comment-list:comment-$comment->comment_ID::trash=1 delete vim-d vim-destructive'>" . wp__('Delete Permanently') . '</a>';
		else
...
?>
		...
		<div id="comment-<?php echo $comment->comment_ID; ?>" <?php comment_class( array( 'comment-item', wp_get_comment_status($comment->comment_ID) ) ); ?>>
			...
			<h4 class="comment-meta">
				<?php printf( /* translators: 1: comment author, 2: post link, 3: notification if the comment is pending */wp__( 'From %1$s on %2$s%3$s' ),
					'<cite class="comment-author">' . get_comment_author_link() . '</cite>', $comment_post_link.' '.$comment_link, ' <span class="approve">' . wp__( '[Pending]' ) . '</span>' ); ?>
			</h4>
...
function wp_dashboard_recent_comments_control() {
	...
	echo '<p><label for="comments-number">' . wp__('Number of comments to show:') . '</label>';
...
function wp_dashboard_incoming_links_output() {
	...
	if ( is_wp_error($rss) ) {
		if ( is_admin() || current_user_can('manage_options') ) {
			echo '<p>';
			printf(wp__('<strong>RSS Error</strong>: %s'), $rss->get_error_message());
			echo '</p>';
		}
		return;
	}
	...
	if ( !$rss->get_item_quantity() ) {
		echo '<p>' . wp__('This dashboard widget queries <a href="http://blogsearch.google.com/">Google Blog Search</a> so that when another blog links to your site it will show up here. It has found no incoming links&hellip; yet. It&#8217;s okay &#8212; there is no rush.') . "</p>\n";
		$rss->__destruct();
		unset($rss);
		return;
	}
	...
	foreach ( $rss->get_items(0, $items) as $item ) {
		...
		if ( $link )
			...
			$text = wp__( '%1$s linked here <a href="%2$s">saying</a>, "%3$s"' );
		else
			/* translators: incoming links feed, %1$s is other person, %3$s is content */
			$text = wp__( '%1$s linked here saying, "%3$s"' );
...
function wp_dashboard_plugins_output() {
	...
	foreach ( array( 'popular' => wp__('Most Popular'), 'new' => wp__('Newest Plugins'), 'updated' => wp__('Recently Updated') ) as $feed => $label ) {
		...
		echo "<h4>$label</h4>\n";
		echo "<h5><a href='$link'>$title</a></h5>&nbsp;<span>(<a href='$ilink' class='thickbox' title='$title'>" . wp__( 'Install' ) . "</a>)</span>\n";
...
function wp_dashboard_cached_rss_widget( $widget_id, $callback, $check_urls = array() ) {
	$loading = '<p class="widget-loading hide-if-no-js">' . wp__( 'Loading&#8230;' ) . '</p><p class="hide-if-js">' . wp__( 'This widget requires JavaScript.' ) . '</p>';
...
function wp_dashboard_rss_control( $widget_id, $form_inputs = array() ) {
	...
	if ( 'POST' == $_SERVER['REQUEST_METHOD'] && isset($_POST['widget-rss'][$number]) ) {
		...
		if ( !$widget_options[$widget_id]['title'] && isset($_POST['widget-rss'][$number]['title']) ) {
			...
			if ( is_wp_error($rss) ) {
				$widget_options[$widget_id]['title'] = htmlentities(wp__('Unknown Feed'));
			} else {
...
function wp_dashboard_quota() {
	...
	<table>
		<tr class="first">
			<td class="first b b-posts"><?php printf( wp__( '<a href="%1$s" title="Manage Uploads" class="musublink">%2$sMB</a>' ), esc_url( admin_url( 'upload.php' ) ), $quota ); ?></td>
	...
	<table>
		<tr class="first">
			<td class="b b-comments"><?php printf( wp__( '<a href="%1$s" title="Manage Uploads" class="musublink">%2$sMB (%3$s%%)</a>' ), esc_url( admin_url( 'upload.php' ) ), $used, $percentused ); ?></td>
...
function wp_dashboard_browser_nag() {
	...
	if ( $response ) {
		if ( $response['insecure'] ) {
			$msg = sprintf( wp__( "It looks like you're using an insecure version of <a href='%s'>%s</a>. Using an outdated browser makes your computer unsafe. For the best WordPress experience, please update your browser." ), esc_attr( $response['update_url'] ), esc_html( $response['name'] ) );
		} else {
			$msg = sprintf( wp__( "It looks like you're using an old version of <a href='%s'>%s</a>. For the best WordPress experience, please update your browser." ), esc_attr( $response['update_url'] ), esc_html( $response['name'] ) );
		}
		...
		$notice .= '<p>' . sprintf( wp__( '<a href="%1$s" class="update-browser-link">Update %2$s</a> or learn how to <a href="%3$s" class="browse-happy-link">browse happy</a>' ), esc_attr( $response['update_url'] ), esc_html( $response['name'] ), 'http://browsehappy.com/' ) . '</p>';
		$notice .= '<p class="hide-if-no-js"><a href="" class="dismiss">' . wp__( 'Dismiss' ) . '</a></p>';
...
function wp_welcome_panel() {
	...
	<div class="wp-badge"><?php printf( wp__( 'Version %s' ), $display_version ); ?></div>
	...
	<div class="welcome-panel-column">
		...
		<ul>
		<li><?php echo sprintf(	wp__( '<a href="%s">Choose your privacy setting</a>' ), esc_url( admin_url('options-privacy.php') ) ); ?></li>
		<li><?php echo sprintf( wp__( '<a href="%s">Select your tagline and time zone</a>' ), esc_url( admin_url('options-general.php') ) ); ?></li>
		<li><?php echo sprintf( wp__( '<a href="%s">Turn comments on or off</a>' ), esc_url( admin_url('options-discussion.php') ) ); ?></li>
		<li><?php echo sprintf( wp__( '<a href="%s">Fill in your profile</a>' ), esc_url( admin_url('profile.php') ) ); ?></li>
		</ul>
	</div>
	<div class="welcome-panel-column">
		...
		<ul>
		<li><?php echo sprintf( wp__( 'View the <a href="%1$s">sample page</a> and <a href="%2$s">post</a>' ), esc_url( get_permalink( 2 ) ), esc_url( get_permalink( 1 ) ) ); ?></li>
		<li><?php echo sprintf( wp__( 'Delete the <a href="%1$s">sample page</a> and <a href="%2$s">post</a>' ), esc_url( admin_url('edit.php?post_type=page') ), esc_url( admin_url('edit.php') ) ); ?></li>
		<li><?php echo sprintf( wp__( '<a href="%s">Create an About Me page</a>' ), esc_url( admin_url('edit.php?post_type=page') ) ); ?></li>
		<li><?php echo sprintf( wp__( '<a href="%s">Write your first post</a>' ), esc_url( admin_url('post-new.php') ) ); ?></li>
		</ul>
	</div>
	<div class="welcome-panel-column welcome-panel-last">
		...
		if ( empty ( $ct->stylesheet_dir ) ) :
			echo '<p>';
			printf( wp__( '<a href="%s">Install a theme</a> to get started customizing your site.' ), esc_url( admin_url( 'themes.php' ) ) );
			echo '</p>';
		else:
			$customize_links = array();
			if ( 'twentyeleven' == $ct->stylesheet )
				$customize_links[] = sprintf( wp__( '<a href="%s">Choose light or dark</a>' ), esc_url( admin_url( 'themes.php?page=theme_options' ) ) );

			if ( current_theme_supports( 'custom-background' ) )
				$customize_links[] = sprintf( wp__( '<a href="%s">Set a background color</a>' ), esc_url( admin_url( 'themes.php?page=custom-background' ) ) );

			if ( current_theme_supports( 'custom-header' ) )
				$customize_links[] = sprintf( wp__( '<a href="%s">Select a new header image</a>' ), esc_url( admin_url( 'themes.php?page=custom-header' ) ) );

			if ( current_theme_supports( 'widgets' ) )
				$customize_links[] = sprintf( wp__( '<a href="%s">Add some widgets</a>' ), esc_url( admin_url( 'widgets.php' ) ) );

			if ( ! empty( $customize_links ) ) {
				echo '<p>';
				printf( wp__( 'Use the current theme &mdash; %1$s &mdash; or <a href="%2$s">choose a new one</a>. If you stick with %3$s, here are a few ways to make your site look unique.' ), $ct->title, esc_url( admin_url( 'themes.php' ) ), $ct->title );
				echo '</p>';
			?>
			<ul>
			...
			<?php
				...
				echo '<p>';
				printf( wp__( 'Use the current theme &mdash; %1$s &mdash; or <a href="%2$s">choose a new one</a>.' ), $ct->title, esc_url( admin_url( 'themes.php' ) ) );
				echo '</p>';
			}
		endif; ?>
	</div>
	</div>
	<p class="welcome-panel-dismiss"><?php printf( wp__( 'Already know what you&#8217;re doing? <a href="%s">Dismiss this message</a>.' ), esc_url( admin_url( '?welcome=0' ) ) ); ?></p>
	</div>
	</div>
	<?php
}
