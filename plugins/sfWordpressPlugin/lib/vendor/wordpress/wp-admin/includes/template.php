...
<div class="hidden" id="trash-undo-holder">
	<div class="trash-undo-inside"><?php printf(wp__('Comment by %s moved to the trash.'), '<strong></strong>'); ?> <span class="undo untrash"><a href="#"><?php _e('Undo'); ?></a></span></div>
</div>
<div class="hidden" id="spam-undo-holder">
	<div class="spam-undo-inside"><?php printf(wp__('Comment by %s marked as spam.'), '<strong></strong>'); ?> <span class="undo unspam"><a href="#"><?php _e('Undo'); ?></a></span></div>
</div>
<?php
}
...
function _list_meta_row( $entry, &$count ) {
	...
	$r .= "\n\t\t<td class='left'><label class='screen-reader-text' for='meta[{$entry['meta_id']}][key]'>" . wp__( 'Key' ) . "</label><input name='meta[{$entry['meta_id']}][key]' id='meta[{$entry['meta_id']}][key]' tabindex='6' type='text' size='20' value='{$entry['meta_key']}' />";

	$r .= "\n\t\t<div class='submit'>";
	$r .= get_submit_button( wp__( 'Delete' ), "delete:the-list:meta-{$entry['meta_id']}::_ajax_nonce=$delete_nonce deletemeta", "deletemeta[{$entry['meta_id']}]", false, array( 'tabindex' => '6' ) );
	$r .= "\n\t\t";
	$r .= get_submit_button( wp__( 'Update' ), "add:the-list:meta-{$entry['meta_id']}::_ajax_nonce-add-meta=$update_nonce updatemeta" , 'updatemeta', false, array( 'tabindex' => '6' ) );
	$r .= "</div>";
	$r .= wp_nonce_field( 'change-meta', '_ajax_nonce', false, false );
	$r .= "</td>";

	$r .= "\n\t\t<td><label class='screen-reader-text' for='meta[{$entry['meta_id']}][value]'>" . wp__( 'Value' ) . "</label><textarea name='meta[{$entry['meta_id']}][value]' id='meta[{$entry['meta_id']}][value]' tabindex='6' rows='2' cols='30'>{$entry['meta_value']}</textarea></td>\n\t</tr>";
	return $r;
}
...
<?php submit_button( wp__( 'Add Custom Field' ), 'add:the-list:newmeta', 'addmeta', false, array( 'id' => 'addmetasub', 'tabindex' => '9' ) ); ?>
...
function touch_time( $edit = 1, $for_post = 1, $tab_index = 0, $multi = 0 ) {
	...
	// echo '<label for="timestamp" style="display: block;"><input type="checkbox" class="checkbox" name="edit_date" value="1" id="timestamp"'.$tab_index_attribute.' /> '.wp__( 'Edit timestamp' ).'</label><br />';
	...
	printf(wp__('%1$s%2$s, %3$s @ %4$s : %5$s'), $month, $day, $year, $hour, $minute);
...
<p>
<label for="upload"><?php _e( 'Choose a file from your computer:' ); ?></label> (<?php printf( wp__('Maximum size: %s' ), $size ); ?>)
<input type="file" id="upload" name="import" size="25" />
<input type="hidden" name="action" value="save" />
<input type="hidden" name="max_file_size" value="<?php echo $bytes; ?>" />
</p>
<?php submit_button( wp__('Upload file and import'), 'button' ); ?>
</form>
<?php
	endif;
}
...
function add_settings_section($id, $title, $callback, $page) {
	...
	if ( 'misc' == $page ) {
		_deprecated_argument( __FUNCTION__, '3.0', wp__( 'The miscellaneous options group has been removed. Use another settings group.' ) );
		$page = 'general';
	}
...
?>
	<div id="find-posts" class="find-box" style="display:none;">
		...
		<div class="find-box-buttons">
			...
			<?php submit_button( wp__( 'Select' ), 'button-primary alignright', 'find-posts-submit', false ); ?>
		</div>
	</div>
<?php
}
...
function _post_states($post) {
	...
	if ( !empty($post->post_password) )
		$post_states['protected'] = wp__('Password protected');
	if ( 'private' == $post->post_status && 'private' != $post_status )
		$post_states['private'] = wp__('Private');
	if ( 'draft' == $post->post_status && 'draft' != $post_status )
		$post_states['draft'] = wp__('Draft');
	if ( 'pending' == $post->post_status && 'pending' != $post_status )
		/* translators: post state */
		$post_states['pending'] = _x('Pending', 'post state');
	if ( is_sticky($post->ID) )
		$post_states['sticky'] = wp__('Sticky');
...
function _media_states( $post ) {
	...
	if ( current_theme_supports( 'custom-header') ) {
		...
		if ( ! empty( $meta_header ) && $meta_header == $stylesheet )
			$media_states[] = wp__( 'Header Image' );
	}

	if ( current_theme_supports( 'custom-background') ) {
		$meta_background = get_post_meta($post->ID, '_wp_attachment_is_custom_background', true );
		if ( ! empty( $meta_background ) && $meta_background == $stylesheet )
			$media_states[] = wp__( 'Background Image' );
	}
...
function get_submit_button( $text = NULL, $type = 'primary', $name = 'submit', $wrap = true, $other_attributes = NULL ) {
	...
	endswitch;
	$text = ( NULL == $text ) ? wp__( 'Save Changes' ) : $text;
...
final class WP_Internal_Pointers {
	...
	public static function pointer_wp330_toolbar() {
		$content  = '<h3>' . wp__( 'New Feature: Toolbar' ) . '</h3>';
		$content .= '<p>' .  wp__( 'We&#8217;ve combined the admin bar and the old Dashboard header into one persistent toolbar. Hover over the toolbar items to see what&#8217;s new.' ) . '</p>';

		if ( is_multisite() && is_super_admin() )
			$content .= '<p>' . wp__( 'Network Admin is now located in the My Sites menu.' ) . '</p>';
	...
	public static function pointer_wp330_media_uploader() {
		$content  = '<h3>' . wp__( 'Updated Media Uploader' ) . '</h3>';
		$content .= '<p>' . wp__( 'The single media icon now launches the uploader for all file types, and the new drag and drop interface makes uploading a breeze.' ) . '</p>';
	...
	public static function pointer_wp330_saving_widgets() {
		$content  = '<h3>' . wp__( 'New Feature: Saving Widgets' ) . '</h3>';
		$content .= '<p>' . wp__( 'If you change your mind and revert to your previous theme, we&#8217;ll put the widgets back the way you had them.' ). '</p>';
...
function convert_to_screen( $hook_name ) {
	if ( ! class_exists( 'WP_Screen' ) ) {
		_doing_it_wrong( 'convert_to_screen(), add_meta_box()', wp__( "Likely direct inclusion of wp-admin/includes/template.php in order to use add_meta_box(). This is very wrong. Hook the add_meta_box() call into the add_meta_boxes action instead." ), '3.3' );
