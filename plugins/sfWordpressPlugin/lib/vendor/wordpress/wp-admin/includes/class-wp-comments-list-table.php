...
class WP_Comments_List_Table extends WP_List_Table {
	...
	function get_bulk_actions() {
		...
		if ( in_array( $comment_status, array( 'all', 'approved' ) ) )
			$actions['unapprove'] = wp__( 'Unapprove' );
		if ( in_array( $comment_status, array( 'all', 'moderated', 'spam' ) ) )
			$actions['approve'] = wp__( 'Approve' );
...
<?php
	...
	function column_comment( $comment ) {
		...
		if ( ( abs( time() - $ptime ) ) < 86400 )
			$ptime = sprintf( wp__( '%s ago' ), human_time_diff( $ptime ) );
		else
			$ptime = mysql2date( wp__( 'Y/m/d \a\t g:i A' ), $comment->comment_date );
		...
		printf( wp__( 'Submitted on <a href="%1$s">%2$s at %3$s</a>' ), $comment_url,
			/* translators: comment date format. See http://php.net/date */ get_comment_date( wp__( 'Y/m/d' ) ),
		...
		if ( $comment->comment_parent ) {
			...
			printf( ' | '.wp__( 'In reply to <a href="%1$s">%2$s</a>.' ), $parent_link, $name );
		}
		...
		if ( $user_can ) {
			...
			if ( $comment_status && 'all' != $comment_status ) { // not looking at all comments
				if ( 'approved' == $the_comment_status )
					$actions['unapprove'] = "<a href='$unapprove_url' class='delete:the-comment-list:comment-$comment->comment_ID:e7e7d3:action=dim-comment&amp;new=unapproved vim-u vim-destructive' title='" . esc_attr__( 'Unapprove this comment' ) . "'>" . wp__( 'Unapprove' ) . '</a>';
				else if ( 'unapproved' == $the_comment_status )
					$actions['approve'] = "<a href='$approve_url' class='delete:the-comment-list:comment-$comment->comment_ID:e7e7d3:action=dim-comment&amp;new=approved vim-a vim-destructive' title='" . esc_attr__( 'Approve this comment' ) . "'>" . wp__( 'Approve' ) . '</a>';
			} else {
			...
			if ( 'spam' != $the_comment_status && 'trash' != $the_comment_status ) {
				...
				$actions['untrash'] = "<a href='$untrash_url' class='delete:the-comment-list:comment-$comment->comment_ID:66cc66:untrash=1 vim-z vim-destructive'>" . wp__( 'Restore' ) . '</a>';
			}

			if ( 'spam' == $the_comment_status || 'trash' == $the_comment_status || !EMPTY_TRASH_DAYS ) {
				$actions['delete'] = "<a href='$delete_url' class='delete:the-comment-list:comment-$comment->comment_ID::delete=1 delete vim-d vim-destructive'>" . wp__( 'Delete Permanently' ) . '</a>';
			} else {
			...
			if ( 'spam' != $the_comment_status && 'trash' != $the_comment_status ) {
				...
				$actions['quickedit'] = '<a onclick="commentReply.open( \''.$comment->comment_ID.'\',\''.$post->ID.'\',\'edit\' );return false;" class="vim-q" title="'.esc_attr__( 'Quick Edit' ).'" href="#">' . wp__( 'Quick&nbsp;Edit' ) . '</a>';
				$actions['reply'] = '<a onclick="commentReply.open( \''.$comment->comment_ID.'\',\''.$post->ID.'\' );return false;" class="vim-r" title="'.esc_attr__( 'Reply to this comment' ).'" href="#">' . wp__( 'Reply' ) . '</a>';
			}
	...
	function column_date( $comment ) {
		return get_comment_date( wp__( 'Y/m/d \a\t g:ia' ) );
	}
