...
class WP_Terms_List_Table extends WP_List_Table {
	...
	function get_columns() {
		...
		if ( 'link_category' == $taxonomy ) {
			$columns['links'] = wp__( 'Links' );
		} else {
			$post_type_object = get_post_type_object( $post_type );
			$columns['posts'] = $post_type_object ? $post_type_object->labels->name : wp__( 'Posts' );
		}
	...
	function column_name( $tag ) {
		...
		$out = '<strong><a class="row-title" href="' . $edit_link . '" title="' . esc_attr( sprintf( wp__( 'Edit &#8220;%s&#8221;' ), $name ) ) . '">' . $name . '</a></strong><br />';

		$actions = array();
		if ( current_user_can( $tax->cap->edit_terms ) ) {
			$actions['edit'] = '<a href="' . $edit_link . '">' . wp__( 'Edit' ) . '</a>';
			$actions['inline hide-if-no-js'] = '<a href="#" class="editinline">' . wp__( 'Quick&nbsp;Edit' ) . '</a>';
		}
		if ( current_user_can( $tax->cap->delete_terms ) && $tag->term_id != $default_term )
			$actions['delete'] = "<a class='delete-tag' href='" . wp_nonce_url( "edit-tags.php?action=delete&amp;taxonomy=$taxonomy&amp;tag_ID=$tag->term_id", 'delete-tag_' . $tag->term_id ) . "'>" . wp__( 'Delete' ) . "</a>";
		$actions['view'] = '<a href="' . get_term_link( $tag ) . '">' . wp__( 'View' ) . '</a>';
