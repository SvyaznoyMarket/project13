...
class WP_User_Search {
	...
	function query() {
		...
		if ( $this->results )
			...
			$this->search_errors = new WP_Error('no_matching_users_found', wp__('No matching users were found!'));
	}
	...
	function do_paging() {
		if ( $this->total_users_for_query > $this->users_per_page ) { // have to page the results
			...
			if ( $this->paging_text ) {
				$this->paging_text = sprintf( '<span class="displaying-num">' . wp__( 'Displaying %s&#8211;%s of %s' ) . '</span>%s',
