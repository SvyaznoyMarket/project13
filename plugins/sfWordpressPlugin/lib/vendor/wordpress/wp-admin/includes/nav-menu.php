...
class Walker_Nav_Menu_Edit extends Walker_Nav_Menu  {
	...
	function start_el(&$output, $item, $depth, $args) {
		...
		if ( ! empty( $item->_invalid ) ) {
			...
			$title = sprintf( wp__( '%s (Invalid)' ), $item->title );
		} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf( wp__('%s (Pending)'), $item->title );
		}
				...
				<div class="menu-item-actions description-wide submitbox">
					<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
						<p class="link-to-original">
							<?php printf( wp__('Original: %s'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						</p>
					<?php endif; ?>
...
function wp_nav_menu_setup() {
	// Register meta boxes
	if ( wp_get_nav_menus() )
		add_meta_box( 'nav-menu-theme-locations', wp__( 'Theme Locations' ), 'wp_nav_menu_locations_meta_box' , 'nav-menus', 'side', 'default' );
	add_meta_box( 'add-custom-links', wp__('Custom Links'), 'wp_nav_menu_item_link_meta_box', 'nav-menus', 'side', 'default' );
...
function wp_nav_menu_locations_meta_box() {
	...
	if ( ! current_theme_supports( 'menus' ) ) {
		...
		echo '<p class="howto">' . wp__('The current theme does not natively support menus, but you can use the &#8220;Custom Menu&#8221; widget to add any menus you create here to the theme&#8217;s sidebar.') . '</p>';
		return;
	}
	...
	<p class="button-controls">
		...
		<?php submit_button( wp__( 'Save' ), 'primary', 'nav-menu-locations', false, disabled( $nav_menu_selected_id, 0, false ) ); ?>
	</p>
	<?php
}
	...
	<div id="posttype-<?php echo $post_type_name; ?>" class="posttypediv">
		...
		?>" id="tabs-panel-posttype-<?php echo $post_type_name; ?>-search">
			...
			<p class="quick-search-wrap">
				...
				<img class="waiting" src="<?php echo esc_url( admin_url( 'images/wpspin_light.gif' ) ); ?>" alt="" />
				<?php submit_button( wp__( 'Search' ), 'quick-search-submit button-secondary hide-if-js', 'submit', false, array( 'id' => 'submit-quick-search-posttype-' . $post_type_name ) ); ?>
			</p>
	...
	<div id="taxonomy-<?php echo $taxonomy_name; ?>" class="taxonomydiv">
		...
		?>" id="tabs-panel-search-taxonomy-<?php echo $taxonomy_name; ?>">
			...
			<p class="quick-search-wrap">
				...
				<img class="waiting" src="<?php echo esc_url( admin_url( 'images/wpspin_light.gif' ) ); ?>" alt="" />
				<?php submit_button( wp__( 'Search' ), 'quick-search-submit button-secondary hide-if-js', 'submit', false, array( 'id' => 'submit-quick-search-taxonomy-' . $taxonomy_name ) ); ?>
			</p>
...
function wp_get_nav_menu_to_edit( $menu_id = 0 ) {
	...
	if ( is_nav_menu( $menu ) ) {
		...
		$result .= '<p>' . wp__('Select menu items (pages, categories, links) from the boxes at left to begin building your custom menu.') . '</p>';
		$result .= '</div>';
		...
		if ( class_exists( $walker_class_name ) )
			...
			return new WP_Error( 'menu_walker_not_exist', sprintf( wp__('The Walker class named <strong>%s</strong> does not exist.'), $walker_class_name ) );
		...
		if ( $some_pending_menu_items )
			$result .= '<div class="updated inline"><p>' . wp__('Click Save Menu to make pending menu items public.') . '</p></div>';

		if ( $some_invalid_menu_items )
			$result .= '<div class="error inline"><p>' . wp__('There are some invalid menu items. Please check or delete them.') . '</p></div>';
...
function wp_nav_menu_manage_columns() {
	return array(
		'_title' => wp__('Show advanced menu properties'),
		'cb' => '<input type="checkbox" />',
		'link-target' => wp__('Link Target'),
		'css-classes' => wp__('CSS Classes'),
		'xfn' => wp__('Link Relationship (XFN)'),
