...
function wp_image_editor($post_id, $msg = false) {
	...
	if ( is_array($meta) && isset($meta['width']) )
		...
		die( wp__('Image data does not exist. Please re-upload the image.') );
...
<?php } ?>
	...
	<div class="imgedit-group-top">
		...
		<p><?php printf( wp__('Original dimensions %s'), $meta['width'] . '&times;' . $meta['height'] ); ?></p>
		<div class="imgedit-submit">
...
<?php if ( $can_restore ) { ?>
	...
	<div class="imgedit-group-top">
		...
		if ( !defined('IMAGE_EDIT_OVERWRITE') || !IMAGE_EDIT_OVERWRITE )
			echo ' '.wp__('Previously edited copies of the image will not be deleted.');
...
function wp_restore_image($post_id) {
	...
	if ( !is_array($backup_sizes) ) {
		$msg->error = wp__('Cannot load image metadata.');
		return $msg;
	}
	...
	if ( !wp_update_attachment_metadata($post_id, $meta) || !update_post_meta( $post_id, '_wp_attachment_backup_sizes', $backup_sizes) ) {
		$msg->error = wp__('Cannot save image metadata.');
		return $msg;
	}

	if ( !$restored )
		$msg->error = wp__('Image metadata is inconsistent.');
	else
		$msg->msg = wp__('Image restored successfully.');
...
function wp_save_image($post_id) {
	...
	if ( !is_resource($img) ) {
		$return->error = wp_esc_js( wp__('Unable to create new image.') );
		return $return;
	}
	...
	if ( $scale && $fwidth > 0 && $fheight > 0 ) {
		...
		if ( !$scaled ) {
			$return->error = wp_esc_js( wp__('Error while saving the scaled image. Please reload the page and try again.') );
			return $return;
		}
		...
		$return->error = wp_esc_js( wp__('Nothing to save, the image has not changed.') );
		return $return;
	}
	...
	if ( !is_array($meta) ) {
		$return->error = wp_esc_js( wp__('Image data does not exist. Please re-upload the image.') );
		return $return;
	}
	...
	if ( !wp_save_image_file($new_path, $img, $post->post_mime_type, $post_id) ) {
		$return->error = wp_esc_js( wp__('Unable to save the image.') );
		return $return;
	}
	...
	$return->msg = wp_esc_js( wp__('Image saved') );
	return $return;
}
