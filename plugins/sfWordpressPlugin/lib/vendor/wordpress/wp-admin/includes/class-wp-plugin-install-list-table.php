...
class WP_Plugin_Install_List_Table extends WP_List_Table {
	...
	function prepare_items() {
		...
		$tabs = array();
		$tabs['dashboard'] = wp__( 'Search' );
		if ( 'search' == $tab )
			$tabs['search']	= wp__( 'Search Results' );
		$tabs['upload'] = wp__( 'Upload' );
		...
		if ( is_wp_error( $api ) )
			wp_die( $api->get_error_message() . '</p> <p class="hide-if-no-js"><a href="#" onclick="document.location.reload(); return false;">' . wp__( 'Try again' ) . '</a>' );
	...
	function display_rows() {
		...
		foreach ( (array) $this->items as $plugin ) {
			...
			if ( ! empty( $plugin['author'] ) )
				$author = ' <cite>' . sprintf( wp__( 'By %s' ), $author ) . '.</cite>';
			...
			$action_links[] = '<a href="' . self_admin_url( 'plugin-install.php?tab=plugin-information&amp;plugin=' . $plugin['slug'] .
								...
								esc_attr( sprintf( wp__( 'More information about %s' ), $name ) ) . '">' . wp__( 'Details' ) . '</a>';
			...
			if ( current_user_can( 'install_plugins' ) || current_user_can( 'update_plugins' ) ) {
				...
				switch ( $status['status'] ) {
					case 'install':
						if ( $status['url'] )
							$action_links[] = '<a class="install-now" href="' . $status['url'] . '" title="' . esc_attr( sprintf( wp__( 'Install %s' ), $name ) ) . '">' . wp__( 'Install Now' ) . '</a>';
						break;
					case 'update_available':
						if ( $status['url'] )
							$action_links[] = '<a href="' . $status['url'] . '" title="' . esc_attr( sprintf( wp__( 'Update to version %s' ), $status['version'] ) ) . '">' . sprintf( wp__( 'Update Now' ), $status['version'] ) . '</a>';
						break;
					case 'latest_installed':
					case 'newer_installed':
						$action_links[] = '<span title="' . esc_attr__( 'This plugin is already installed and is up to date' ) . ' ">' . wp__( 'Installed' ) . '</span>';
						break;
				}
			}
