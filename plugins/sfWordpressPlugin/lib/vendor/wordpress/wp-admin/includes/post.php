		...
		if ( !current_user_can( $ptype->cap->edit_others_posts ) ) {
			if ( 'page' == $post_data['post_type'] ) {
				return new WP_Error( 'edit_others_pages', $update ?
					wp__( 'You are not allowed to edit pages as this user.' ) :
					wp__( 'You are not allowed to create pages as this user.' )
				);
			} else {
				return new WP_Error( 'edit_others_posts', $update ?
					wp__( 'You are not allowed to edit posts as this user.' ) :
					wp__( 'You are not allowed to post as this user.' )
				);
			}
		}
	}
...
function edit_post( $post_data = null ) {
	...
	if ( !current_user_can( $ptype->cap->edit_post, $post_ID ) ) {
		if ( 'page' == $post_data['post_type'] )
			wp_die( wp__('You are not allowed to edit this page.' ));
		else
...
function bulk_edit_posts( $post_data = null ) {
	...
	if ( !current_user_can( $ptype->cap->edit_posts ) ) {
		if ( 'page' == $ptype->name )
			wp_die( wp__('You are not allowed to edit pages.'));
		else
			wp_die( wp__('You are not allowed to edit posts.'));
	}
...
function get_default_post_to_edit( $post_type = 'post', $create_in_db = false ) {
	...
	if ( $create_in_db ) {
		...
		$post_id = wp_insert_post( array( 'post_title' => wp__( 'Auto Draft' ), 'post_type' => $post_type, 'post_status' => 'auto-draft' ) );
		$post = get_post( $post_id );
...
function wp_write_post() {
	...
	if ( !current_user_can( $ptype->cap->edit_posts ) ) {
		if ( 'page' == $ptype->name )
			return new WP_Error( 'edit_pages', wp__( 'You are not allowed to create pages on this site.' ) );
		else
			return new WP_Error( 'edit_posts', wp__( 'You are not allowed to create posts or drafts on this site.' ) );
	}
...
function get_post_mime_types() {
	$post_mime_types = array(	//	array( adj, noun )
		'image' => array(wp__('Images'), wp__('Manage Images'), _n_noop('Image <span class="count">(%s)</span>', 'Images <span class="count">(%s)</span>')),
		'audio' => array(wp__('Audio'), wp__('Manage Audio'), _n_noop('Audio <span class="count">(%s)</span>', 'Audio <span class="count">(%s)</span>')),
		'video' => array(wp__('Video'), wp__('Manage Video'), _n_noop('Video <span class="count">(%s)</span>', 'Video <span class="count">(%s)</span>')),
	);
...
function get_sample_permalink_html( $id, $new_title = null, $new_slug = null ) {
	...
	if ( 'publish' == $post->post_status ) {
		...
		$title = wp__('Click to edit this part of the permalink');
	} else {
		$title = wp__('Temporary permalink. Click to edit this part.');
	}

	if ( false === strpos($permalink, '%postname%') && false === strpos($permalink, '%pagename%') ) {
		$return = '<strong>' . wp__('Permalink:') . "</strong>\n" . '<span id="sample-permalink">' . $permalink . "</span>\n";
		if ( '' == get_option( 'permalink_structure' ) && current_user_can( 'manage_options' ) && !( 'page' == get_option('show_on_front') && $id == get_option('page_on_front') ) )
			$return .= '<span id="change-permalinks"><a href="options-permalink.php" class="button" target="_blank">' . wp__('Change Permalinks') . "</a></span>\n";
		if ( isset($view_post) )
	...
	$return .= '<span id="edit-slug-buttons"><a href="#post_name" class="edit-slug button hide-if-no-js" onclick="editPermalink(' . $id . '); return false;">' . wp__('Edit') . "</a></span>\n";
...
function _admin_notice_post_locked() {
	...
	$last_user_name = $last_user ? $last_user->display_name : wp__('Somebody');

	switch ($post->post_type) {
		case 'post':
			$message = wp__( 'Warning: %s is currently editing this post' );
			break;
		case 'page':
			$message = wp__( 'Warning: %s is currently editing this page' );
			break;
		default:
			$message = wp__( 'Warning: %s is currently editing this.' );
	}
...
function post_preview() {
	...
	if ( 'auto-draft' == $status )
		wp_die( wp__('Preview not available. Please save as a draft first.') );
