...
class WP_Filesystem_SSH2 extends WP_Filesystem_Base {
	...
	function __construct($opt='') {
		...
		if ( ! extension_loaded('ssh2') ) {
			$this->errors->add('no_ssh2_ext', wp__('The ssh2 PHP extension is not available'));
			return false;
		}
		if ( !function_exists('stream_get_contents') ) {
			$this->errors->add('ssh2_php_requirement', wp__('The ssh2 PHP extension is available, however, we require the PHP5 function <code>stream_get_contents()</code>'));
			return false;
		}
		...
		if ( empty($opt['hostname']) )
			$this->errors->add('empty_hostname', wp__('SSH2 hostname is required'));
		else
		...
		if ( !empty ($opt['public_key']) && !empty ($opt['private_key']) ) {
			...
			$this->errors->add('empty_username', wp__('SSH2 username is required'));
		}
		...
		if ( empty ($opt['password']) ) {
			if ( !$this->keys )	//password can be blank if we are using keys
				$this->errors->add('empty_password', wp__('SSH2 password is required'));
		} else {
	...
	function connect() {
		...
		if ( ! $this->link ) {
			$this->errors->add('connect', sprintf(wp__('Failed to connect to SSH2 Server %1$s:%2$s'), $this->options['hostname'], $this->options['port']));
			return false;
		}

		if ( !$this->keys ) {
			if ( ! @ssh2_auth_password($this->link, $this->options['username'], $this->options['password']) ) {
				$this->errors->add('auth', sprintf(wp__('Username/Password incorrect for %s'), $this->options['username']));
				return false;
			}
		} else {
			if ( ! @ssh2_auth_pubkey_file($this->link, $this->options['username'], $this->options['public_key'], $this->options['private_key'], $this->options['password'] ) ) {
				$this->errors->add('auth', sprintf(wp__('Public and Private keys incorrect for %s'), $this->options['username']));
				return false;
			}
		}
	...
	function run_command( $command, $returnbool = false) {
		...
		if ( ! ($stream = ssh2_exec($this->link, $command)) ) {
			$this->errors->add('command', sprintf(wp__('Unable to perform command: %s'), $command));
		} else {
