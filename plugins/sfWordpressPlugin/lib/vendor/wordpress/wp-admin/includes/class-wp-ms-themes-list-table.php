...
class WP_MS_Themes_List_Table extends WP_List_Table {
	...
	function get_bulk_actions() {
		...
		$actions = array();
		if ( 'enabled' != $status )
			$actions['enable-selected'] = $this->is_site_themes ? wp__( 'Enable' ) : wp__( 'Network Enable' );
		if ( 'disabled' != $status )
			$actions['disable-selected'] = $this->is_site_themes ? wp__( 'Disable' ) : wp__( 'Network Disable' );
		if ( ! $this->is_site_themes ) {
	...
	function single_row( $key, $theme ) {
		...
		if ( empty( $theme['enabled'] ) )
			$actions['enable'] = '<a href="' . esc_url( wp_nonce_url($url . 'action=enable&amp;theme=' . $theme_key . '&amp;paged=' . $page . '&amp;s=' . $s, 'enable-theme_' . $theme_key) ) . '" title="' . esc_attr__('Enable this theme') . '" class="edit">' . ( $this->is_site_themes ? wp__( 'Enable' ) : wp__( 'Network Enable' ) ) . '</a>';
		else
			$actions['disable'] = '<a href="' . esc_url( wp_nonce_url($url . 'action=disable&amp;theme=' . $theme_key . '&amp;paged=' . $page . '&amp;s=' . $s, 'disable-theme_' . $theme_key) ) . '" title="' . esc_attr__('Disable this theme') . '">' . ( $this->is_site_themes ? wp__( 'Disable' ) : wp__( 'Network Disable' ) ) . '</a>';

		if ( current_user_can('edit_themes') )
			$actions['edit'] = '<a href="' . esc_url('theme-editor.php?theme=' . urlencode( $theme['Name'] )) . '" title="' . esc_attr__('Open this theme in the Theme Editor') . '" class="edit">' . wp__('Edit') . '</a>';
		...
		if ( empty( $theme['enabled'] ) && current_user_can( 'delete_themes' ) && ! $this->is_site_themes && $theme_key != get_option( 'stylesheet' ) && $theme_key != get_option( 'template' ) )
			$actions['delete'] = '<a href="' . esc_url( wp_nonce_url( 'themes.php?action=delete-selected&amp;checked[]=' . $theme_key . '&amp;theme_status=' . $context . '&amp;paged=' . $page . '&amp;s=' . $s, 'bulk-themes' ) ) . '" title="' . esc_attr__( 'Delete this theme' ) . '" class="delete">' . wp__( 'Delete' ) . '</a>';
		...
		$checkbox = "<input type='checkbox' name='checked[]' value='" . esc_attr( $theme_key ) . "' id='" . $checkbox_id . "' /><label class='screen-reader-text' for='" . $checkbox_id . "' >" . wp__('Select') . " " . $theme['Name'] . "</label>";
			...
			switch ( $column_name ) {
				...
				case 'description':
					...
					if ( !empty( $theme['Version'] ) )
						$theme_meta[] = sprintf( wp__( 'Version %s' ), $theme['Version'] );

					if ( !empty( $theme['Author'] ) )
						$theme_meta[] = sprintf( wp__( 'By %s' ), $theme['Author'] );

					if ( !empty( $theme['Theme URI'] ) )
						$theme_meta[] = '<a href="' . $theme['Theme URI'] . '" title="' . esc_attr__( 'Visit theme homepage' ) . '">' . wp__( 'Visit Theme Site' ) . '</a>';
