...
function check_upload_size( $file ) {
	...
	if ( $space_left < $file_size )
		$file['error'] = sprintf( wp__( 'Not enough space to upload. %1$s KB needed.' ), number_format( ($file_size - $space_left) /1024 ) );
	if ( $file_size > ( 1024 * get_site_option( 'fileupload_maxk', 1500 ) ) )
		$file['error'] = sprintf(wp__('This file is too big. Files must be less than %1$s KB in size.'), get_site_option( 'fileupload_maxk', 1500 ) );
	if ( upload_is_user_over_quota( false ) ) {
		$file['error'] = wp__( 'You have used your space quota. Please delete files before uploading.' );
	}
	if ( $file['error'] != '0' && !isset($_POST['html-upload']) )
		wp_die( $file['error'] . ' <a href="javascript:history.go(-1)">' . wp__( 'Back' ) . '</a>' );
...
function update_option_new_admin_email( $old_value, $value ) {
	...
	$content = apply_filters( 'new_admin_email_content', wp__( "Dear user,
...
###SITEURL### "), $new_admin_email );
	...
	wp_mail( $value, sprintf( wp__( '[%s] New Admin Email Address' ), get_option( 'blogname' ) ), $content );
}
...
function send_confirmation_on_profile_email() {
	...
	if ( $current_user->user_email != $_POST['email'] ) {
		if ( !is_email( $_POST['email'] ) ) {
			$errors->add( 'user_email', wp__( "<strong>ERROR</strong>: The e-mail address isn't correct." ), array( 'form-field' => 'email' ) );
			return;
		}

		if ( $wpdb->get_var( $wpdb->prepare( "SELECT user_email FROM {$wpdb->users} WHERE user_email=%s", $_POST['email'] ) ) ) {
			$errors->add( 'user_email', wp__( "<strong>ERROR</strong>: The e-mail address is already used." ), array( 'form-field' => 'email' ) );
		...
		$content = apply_filters( 'new_user_email_content', wp__( "Dear user,
...
###SITEURL###" ), $new_user_email );
		...
		wp_mail( $_POST['email'], sprintf( wp__( '[%s] New Email Address' ), get_option( 'blogname' ) ), $content );
...
function new_user_email_admin_notice() {
	if ( strpos( $_SERVER['PHP_SELF'], 'profile.php' ) && isset( $_GET['updated'] ) && $email = get_option( get_current_user_id() . '_new_email' ) )
		echo "<div class='update-nag'>" . sprintf( wp__( "Your email address has not been updated yet. Please check your inbox at %s for a confirmation email." ), $email['newemail'] ) . "</div>";
}
...
function display_space_usage() {
	...
	?>
	<strong><?php printf( wp__( 'Used: %1s%% of %2s' ), number_format( $percentused ), $space ); ?></strong>
	<?php
}
...
function _access_denied_splash() {
	...
	if ( empty( $blogs ) )
		wp_die( sprintf( wp__( 'You attempted to access the "%1$s" dashboard, but you do not currently have privileges on this site. If you believe you should be able to access the "%1$s" dashboard, please contact your network administrator.' ), $blog_name ) );

	$output = '<p>' . sprintf( wp__( 'You attempted to access the "%1$s" dashboard, but you do not currently have privileges on this site. If you believe you should be able to access the "%1$s" dashboard, please contact your network administrator.' ), $blog_name ) . '</p>';
	$output .= '<p>' . wp__( 'If you reached this screen by accident and meant to visit one of your own sites, here are some shortcuts to help you find your way.' ) . '</p>';

	$output .= '<h3>' . wp__('Your Sites') . '</h3>';
	$output .= '<table>';
	...
	foreach ( $blogs as $blog ) {
		...
		$output .= "</td>";
		$output .= "<td valign='top'>";
		$output .= "<a href='" . esc_url( get_admin_url( $blog->userblog_id ) ) . "'>" . wp__( 'Visit Dashboard' ) . "</a> | <a href='" . esc_url( get_home_url( $blog->userblog_id ) ). "'>" . wp__( 'View Site' ) . "</a>" ;
		$output .= "</td>";
		$output .= "</tr>";
	}
	$output .= '</table>';
...
function mu_dropdown_languages( $lang_files = array(), $current = '' ) {
	...
	foreach ( (array) $lang_files as $val ) {
		...
		if ( $code_lang == 'en_US' ) { // American English
			$flag = true;
			$ae = wp__( 'American English' );
			$output[$ae] = '<option value="' . esc_attr( $code_lang ) . '"' . selected( $current, $code_lang, false ) . '> ' . $ae . '</option>';
		} elseif ( $code_lang == 'en_GB' ) { // British English
			$flag = true;
			$be = wp__( 'British English' );
	...
	if ( $flag === false ) // WordPress english
		$output[] = '<option value=""' . selected( $current, '', false ) . '>' . wp__( 'English' ) . "</option>";
...
function secret_salt_warning() {
	...
	if ( $out != '' ) {
		$msg  = wp__( 'Warning! WordPress encrypts user cookies, but you must add the following lines to <strong>wp-config.php</strong> for it to be more secure.' );
		$msg .= '<br/>' . wp__( "Before the line <code>/* That's all, stop editing! Happy blogging. */</code> please add this code:" );
...
function site_admin_notice() {
	...
	if ( get_site_option( 'wpmu_upgrade_site' ) != $wp_db_version )
		echo "<div class='update-nag'>" . sprintf( wp__( 'Thank you for Updating! Please visit the <a href="%s">Update Network</a> page to update all your sites.' ), esc_url( network_admin_url( 'upgrade.php' ) ) ) . "</div>";
}
...
function ms_deprecated_blogs_file() {
	...
	if ( ! file_exists( WP_CONTENT_DIR . '/blogs.php' ) )
		return;
	echo '<div class="update-nag">' . sprintf( wp__( 'The <code>%1$s</code> file is deprecated. Please remove it and update your server rewrite rules to use <code>%2$s</code> instead.' ), 'wp-content/blogs.php', 'wp-includes/ms-files.php' ) . '</div>';
}
