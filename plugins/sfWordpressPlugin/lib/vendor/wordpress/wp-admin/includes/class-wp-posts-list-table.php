...
class WP_Posts_List_Table extends WP_List_Table {
	...
	function get_bulk_actions() {
		...
		if ( $this->is_trash )
			$actions['untrash'] = wp__( 'Restore' );
		else
			$actions['edit'] = wp__( 'Edit' );

		if ( $this->is_trash || !EMPTY_TRASH_DAYS )
			$actions['delete'] = wp__( 'Delete Permanently' );
		else
			$actions['trash'] = wp__( 'Move to Trash' );
		...
		if ( 'top' == $which && !is_singular() ) {
			...
			if ( is_object_in_taxonomy( $post_type_object->name, 'category' ) ) {
				$dropdown_options = array(
					'show_option_all' => wp__( 'View all categories' ),
					'hide_empty' => 0,
					'hierarchical' => 1,
					'show_count' => 0,
					'orderby' => 'name',
			...
			submit_button( wp__( 'Filter' ), 'secondary', false, false, array( 'id' => 'post-query-submit' ) );
		}

		if ( $this->is_trash && current_user_can( $post_type_object->cap->edit_others_posts ) ) {
			submit_button( wp__( 'Empty Trash' ), 'button-secondary apply', 'delete_all', false );
		}
?>
		</div>
<?php
...
?>
			<td <?php echo $attributes ?>><strong><?php if ( $can_edit_post && $post->post_status != 'trash' ) { ?><a class="row-title" href="<?php echo $edit_link; ?>" title="<?php echo esc_attr( sprintf( wp__( 'Edit &#8220;%s&#8221;' ), $title ) ); ?>"><?php echo $pad; echo $title ?></a><?php } else { echo $pad; echo $title; }; _post_states( $post ); echo isset( $parent_name ) ? ' | ' . $post_type_object->labels->parent_item_colon . ' ' . esc_html( $parent_name ) : ''; ?></strong>
<?php
				}
				else {
					$attributes = 'class="post-title page-title column-title"' . $style;
?>
			<td <?php echo $attributes ?>><strong><?php if ( $can_edit_post && $post->post_status != 'trash' ) { ?><a class="row-title" href="<?php echo $edit_link; ?>" title="<?php echo esc_attr( sprintf( wp__( 'Edit &#8220;%s&#8221;' ), $title ) ); ?>"><?php echo $title ?></a><?php } else { echo $title; }; _post_states( $post ); ?></strong>
<?php
					if ( 'excerpt' == $mode ) {
						the_excerpt();
					}
				...
				if ( $can_edit_post && 'trash' != $post->post_status ) {
					$actions['edit'] = '<a href="' . get_edit_post_link( $post->ID, true ) . '" title="' . esc_attr( wp__( 'Edit this item' ) ) . '">' . wp__( 'Edit' ) . '</a>';
					$actions['inline hide-if-no-js'] = '<a href="#" class="editinline" title="' . esc_attr( wp__( 'Edit this item inline' ) ) . '">' . wp__( 'Quick&nbsp;Edit' ) . '</a>';
				}
				if ( current_user_can( $post_type_object->cap->delete_post, $post->ID ) ) {
					if ( 'trash' == $post->post_status )
						$actions['untrash'] = "<a title='" . esc_attr( wp__( 'Restore this item from the Trash' ) ) . "' href='" . wp_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $post->ID ) ), 'untrash-' . $post->post_type . '_' . $post->ID ) . "'>" . wp__( 'Restore' ) . "</a>";
					elseif ( EMPTY_TRASH_DAYS )
						$actions['trash'] = "<a class='submitdelete' title='" . esc_attr( wp__( 'Move this item to the Trash' ) ) . "' href='" . get_delete_post_link( $post->ID ) . "'>" . wp__( 'Trash' ) . "</a>";
					if ( 'trash' == $post->post_status || !EMPTY_TRASH_DAYS )
						$actions['delete'] = "<a class='submitdelete' title='" . esc_attr( wp__( 'Delete this item permanently' ) ) . "' href='" . get_delete_post_link( $post->ID, '', true ) . "'>" . wp__( 'Delete Permanently' ) . "</a>";
				}
				if ( $post_type_object->public ) {
					if ( in_array( $post->post_status, array( 'pending', 'draft', 'future' ) ) ) {
						if ( $can_edit_post )
							$actions['view'] = '<a href="' . esc_url( add_query_arg( 'preview', 'true', get_permalink( $post->ID ) ) ) . '" title="' . esc_attr( sprintf( wp__( 'Preview &#8220;%s&#8221;' ), $title ) ) . '" rel="permalink">' . wp__( 'Preview' ) . '</a>';
					} elseif ( 'trash' != $post->post_status ) {
						$actions['view'] = '<a href="' . get_permalink( $post->ID ) . '" title="' . esc_attr( sprintf( wp__( 'View &#8220;%s&#8221;' ), $title ) ) . '" rel="permalink">' . wp__( 'View' ) . '</a>';
					}
				}
			...
			case 'date':
				if ( '0000-00-00 00:00:00' == $post->post_date && 'date' == $column_name ) {
					$t_time = $h_time = wp__( 'Unpublished' );
					$time_diff = 0;
				} else {
					$t_time = get_the_time( wp__( 'Y/m/d g:i:s A' ) );
					...
					if ( $time_diff > 0 && $time_diff < 24*60*60 )
						$h_time = sprintf( wp__( '%s ago' ), human_time_diff( $time ) );
					else
						$h_time = mysql2date( wp__( 'Y/m/d' ), $m_time );
				}
				...
				if ( 'publish' == $post->post_status ) {
					...
					if ( $time_diff > 0 )
						echo '<strong class="attention">' . wp__( 'Missed schedule' ) . '</strong>';
					else
						_e( 'Scheduled' );
				} else {
					_e( 'Last Modified' );
	...
	<form method="get" action=""><table style="display: none"><tbody id="inlineedit">
		...
		<fieldset class="inline-edit-col-left"><div class="inline-edit-col">
			<h4><?php echo $bulk ? wp__( 'Bulk Edit' ) : wp__( 'Quick Edit' ); ?></h4>
	<?php
	...
	<?php endif; // $bulk
		...
		if ( post_type_supports( $screen->post_type, 'author' ) ) :
			...
			if ( is_super_admin() || current_user_can( $post_type_object->cap->edit_others_posts ) ) :
				...
				if ( $bulk )
					$users_opt['show_option_none'] = wp__( '&mdash; No Change &mdash;' );

				if ( $authors = wp_dropdown_users( $users_opt ) ) :
					$authors_dropdown  = '<label class="inline-edit-author">';
					$authors_dropdown .= '<span class="title">' . wp__( 'Author' ) . '</span>';
	...
	?>
			...
			<div class="inline-edit-group">
				...
				<em style="margin:5px 10px 0 0" class="alignleft">
					...
					echo wp__( '&ndash;OR&ndash;' );
					?>
				</em>
				<label class="alignleft inline-edit-private">
					<input type="checkbox" name="keep_private" value="private" />
					<span class="checkbox-title"><?php echo wp__( 'Private' ); ?></span>
				</label>
			</div>
	...
	<?php
		$dropdown_args = array(
			...
			'name'              => 'post_parent',
			'show_option_none'  => wp__( 'Main Page (no parent)' ),
			'option_none_value' => 0,
			'sort_column'       => 'menu_order, post_title',
		);

		if ( $bulk )
			$dropdown_args['show_option_no_change'] =  wp__( '&mdash; No Change &mdash;' );
	...
	?>
		<p class="submit inline-edit-save">
			...
			<?php } else {
				submit_button( wp__( 'Update' ), 'button-primary alignright', 'bulk_edit', false, array( 'accesskey' => 's' ) );
			} ?>
