...
class WP_Media_List_Table extends WP_List_Table {
	...
	function get_bulk_actions() {
		...
		if ( $this->detached )
			$actions['attach'] = wp__( 'Attach to a post' );
...
<?php
		...
		if ( $this->detached ) {
			submit_button( wp__( 'Scan for lost attachments' ), 'secondary', 'find_detached', false );
...
?>
				<a href="<?php echo get_edit_post_link( $post->ID, true ); ?>" title="<?php echo esc_attr( sprintf( wp__( 'Edit &#8220;%s&#8221;' ), $att_title ) ); ?>">
					<?php echo $thumb; ?>
				</a>
...
?>
		<td <?php echo $attributes ?>><strong>
			...
			<a href="<?php echo get_edit_post_link( $post->ID, true ); ?>"
				title="<?php echo esc_attr( sprintf( wp__( 'Edit &#8220;%s&#8221;' ), $att_title ) ); ?>">
	...
	case 'date':
		if ( '0000-00-00 00:00:00' == $post->post_date && 'date' == $column_name ) {
			...
			if ( ( abs( $t_diff = time() - $time ) ) < 86400 ) {
				if ( $t_diff < 0 )
					$h_time = sprintf( wp__( '%s from now' ), human_time_diff( $time ) );
				else
...
?>
			<td <?php echo $attributes ?>><strong>
				...
				<?php echo get_the_time( wp__( 'Y/m/d' ) ); ?>
			</td>
<?php
		} else {
?>
	...
	function _get_row_actions( $post, $att_title ) {
		...
		if ( $this->detached ) {
			if ( current_user_can( 'edit_post', $post->ID ) )
				$actions['edit'] = '<a href="' . get_edit_post_link( $post->ID, true ) . '">' . wp__( 'Edit' ) . '</a>';
			if ( current_user_can( 'delete_post', $post->ID ) )
				if ( EMPTY_TRASH_DAYS && MEDIA_TRASH ) {
					$actions['trash'] = "<a class='submitdelete' href='" . wp_nonce_url( "post.php?action=trash&amp;post=$post->ID", 'trash-attachment_' . $post->ID ) . "'>" . wp__( 'Trash' ) . "</a>";
				} else {
					$delete_ays = !MEDIA_TRASH ? " onclick='return showNotice.warn();'" : '';
					$actions['delete'] = "<a class='submitdelete'$delete_ays href='" . wp_nonce_url( "post.php?action=delete&amp;post=$post->ID", 'delete-attachment_' . $post->ID ) . "'>" . wp__( 'Delete Permanently' ) . "</a>";
				}
			$actions['view'] = '<a href="' . get_permalink( $post->ID ) . '" title="' . esc_attr( sprintf( wp__( 'View &#8220;%s&#8221;' ), $att_title ) ) . '" rel="permalink">' . wp__( 'View' ) . '</a>';
			if ( current_user_can( 'edit_post', $post->ID ) )
				$actions['attach'] = '<a href="#the-list" onclick="findPosts.open( \'media[]\',\''.$post->ID.'\' );return false;" class="hide-if-no-js">'.wp__( 'Attach' ).'</a>';
		}
		else {
			...
			if ( current_user_can( 'delete_post', $post->ID ) ) {
				if ( $this->is_trash )
					$actions['untrash'] = "<a class='submitdelete' href='" . wp_nonce_url( "post.php?action=untrash&amp;post=$post->ID", 'untrash-attachment_' . $post->ID ) . "'>" . wp__( 'Restore' ) . "</a>";
