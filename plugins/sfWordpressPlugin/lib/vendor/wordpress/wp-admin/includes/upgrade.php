...
function wp_install( $blog_title, $user_name, $user_email, $public, $deprecated = '', $user_password = '' ) {
	...
	if ( !$user_id && empty($user_password) ) {
		...
		$message = wp__('<strong><em>Note that password</em></strong> carefully! It is a <em>random</em> password that was generated just for you.');
		$user_id = wp_create_user($user_name, $user_password, $user_email);
		update_user_option($user_id, 'default_password_nag', true, true);
		$email_password = true;
	} else if ( !$user_id ) {
		// Password has been provided
		$message = '<em>'.wp__('Your chosen password.').'</em>';
		$user_id = wp_create_user($user_name, $user_password, $user_email);
	} else {
		$message =  wp__('User already exists. Password inherited.');
	}
	...
	wp_new_blog_notification($blog_title, $guessurl, $user_id, ($email_password ? $user_password : wp__('The password you chose during the install.') ) );
...
function wp_install_defaults($user_id) {
	...
	if ( is_multisite() ) {
		...
		if ( empty($first_post) )
			$first_post = stripslashes( wp__( 'Welcome to <a href="SITE_URL">SITE_NAME</a>. This is your first post. Edit or delete it, then start blogging!' ) );

		$first_post = str_replace( "SITE_URL", esc_url( network_home_url() ), $first_post );
		$first_post = str_replace( "SITE_NAME", $current_site->site_name, $first_post );
	} else {
		$first_post = wp__('Welcome to WordPress. This is your first post. Edit or delete it, then start blogging!');
	}
	...
	// Default comment
	$first_comment_author = wp__('Mr WordPress');
	$first_comment_url = 'http://wordpress.org/';
	$first_comment = wp__('Hi, this is a comment.<br />To delete a comment, just log in and view the post&#039;s comments. There you will have the option to edit or delete them.');
	if ( is_multisite() ) {
	...
	// First Page
	$first_page = sprintf( wp__( "This is an example page. It's different from a blog post because it will stay in one place and will show up in your site navigation (in most themes). Most people start with an About page that introduces them to potential site visitors. It might say something like this:
...
As a new WordPress user, you should go to <a href=\"%s\">your dashboard</a> to delete this page and create new pages for your content. Have fun!" ), admin_url() );
	...
	$wpdb->insert( $wpdb->posts, array(
								...
								'post_excerpt' => '',
								'post_title' => wp__( 'Sample Page' ),
								/* translators: Default page slug */
								'post_name' => wp__( 'sample-page' ),
								'post_modified' => $now,
								'post_modified_gmt' => $now_gmt,
								'guid' => $first_post_guid,
								'post_type' => 'page',
...
function wp_new_blog_notification($blog_title, $blog_url, $user_id, $password) {
	...
	$message = sprintf(wp__("Your new WordPress site has been successfully set up at:
...
"), $blog_url, $name, $password);
	...
	@wp_mail($email, wp__('New WordPress Site'), $message);
}
endif;
