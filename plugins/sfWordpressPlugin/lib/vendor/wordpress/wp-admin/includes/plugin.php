...
function get_plugin_data( $plugin_file, $markup = true, $translate = true ) {
	...
	if ( empty( $plugin_data['Network'] ) && ! empty( $plugin_data['_sitewide'] ) ) {
		_deprecated_argument( __FUNCTION__, '3.0', sprintf( wp__( 'The <code>%1$s</code> plugin header is deprecated. Use <code>%2$s</code> instead.' ), 'Site Wide Only: true', 'Network: true' ) );
...
function _get_plugin_data_markup_translate($plugin_file, $plugin_data, $markup = true, $translate = true) {
	...
	if ( $markup ) {
		...
		if ( ! empty($plugin_data['Author']) )
			$plugin_data['Description'] .= ' <cite>' . sprintf( wp__('By %s'), $plugin_data['Author'] ) . '.</cite>';
	}
...
function _get_dropins() {
	$dropins = array(
		'advanced-cache.php' => array( wp__( 'Advanced caching plugin.'       ), 'WP_CACHE' ), // WP_CACHE
		'db.php'             => array( wp__( 'Custom database class.'         ), true ), // auto on load
		'db-error.php'       => array( wp__( 'Custom database error message.' ), true ), // auto on error
		'install.php'        => array( wp__( 'Custom install script.'         ), true ), // auto on install
		'maintenance.php'    => array( wp__( 'Custom maintenance message.'    ), true ), // auto on maintenance
		'object-cache.php'   => array( wp__( 'External object cache.'         ), true ), // auto on load
	);

	if ( is_multisite() ) {
		$dropins['sunrise.php'       ] = array( wp__( 'Executed before Multisite is loaded.' ), 'SUNRISE' ); // SUNRISE
		$dropins['blog-deleted.php'  ] = array( wp__( 'Custom site deleted message.'   ), true ); // auto on deleted blog
		$dropins['blog-inactive.php' ] = array( wp__( 'Custom site inactive message.'  ), true ); // auto on inactive blog
		$dropins['blog-suspended.php'] = array( wp__( 'Custom site suspended message.' ), true ); // auto on archived or spammed blog
	}
...
function activate_plugin( $plugin, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( !in_array($plugin, $current) ) {
		...
		if ( ob_get_length() > 0 ) {
			$output = ob_get_clean();
			return new WP_Error('unexpected_output', wp__('The plugin generated unexpected output.'), $output);
		}
		ob_end_clean();
	}
...
function activate_plugins( $plugins, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( !empty($errors) )
		return new WP_Error('plugins_invalid', wp__('One of the plugins is invalid.'), $errors);
...
function delete_plugins($plugins, $redirect = '' ) {
	...
	if ( empty($plugins_dir) )
		return new WP_Error('fs_no_plugins_dir', wp__('Unable to locate WordPress Plugin directory.'));
	...
	if ( ! empty($errors) )
		return new WP_Error('could_not_remove_plugin', sprintf(wp__('Could not fully remove the plugin(s) %s.'), implode(', ', $errors)) );
...
function validate_plugin($plugin) {
	if ( validate_file($plugin) )
		return new WP_Error('plugin_invalid', wp__('Invalid plugin path.'));
	if ( ! file_exists(WP_PLUGIN_DIR . '/' . $plugin) )
		return new WP_Error('plugin_not_found', wp__('Plugin file does not exist.'));

	$installed_plugins = get_plugins();
	if ( ! isset($installed_plugins[$plugin]) )
		return new WP_Error('no_plugin_header', wp__('The plugin does not have a valid header.'));
	return 0;
}
