...
<form id="search-themes" method="get" action="">
	...
	<?php submit_button( wp__( 'Search' ), 'button', 'search', false ); ?>
</form>
<?php
}
...
<br class="clear" />
<?php submit_button( wp__( 'Find Themes' ), 'button', 'search' ); ?>
</form>
<?php
}
...
<form method="post" enctype="multipart/form-data" action="<?php echo self_admin_url('update.php?action=upload-theme') ?>">
	...
	<?php submit_button( wp__( 'Install Now' ), 'button', 'install-theme-submit', false ); ?>
</form>
	<?php
}
...
function display_theme($theme, $actions = null, $show_details = true) {
	...
	if ( !is_array($actions) ) {
		...
		$actions[] = '<a href="' . self_admin_url('theme-install.php?tab=theme-information&amp;theme=' . $theme->slug .
										'&amp;TB_iframe=true&amp;tbWidth=500&amp;tbHeight=385') . '" class="thickbox thickbox-preview onclick" title="' . esc_attr(sprintf(wp__('Install &#8220;%s&#8221;'), $name)) . '">' . wp__('Install') . '</a>';
		if ( !is_network_admin() )
			$actions[] = '<a href="' . $preview_link . '" class="thickbox thickbox-preview onclick previewlink" title="' . esc_attr(sprintf(wp__('Preview &#8220;%s&#8221;'), $name)) . '">' . wp__('Preview') . '</a>';
...
<a class='thickbox thickbox-preview screenshot'
	...
	title='<?php echo esc_attr(sprintf(wp__('Preview &#8220;%s&#8221;'), $name)); ?>'>
...
<p><strong><?php _e('Last Updated:') ?></strong> <span title="<?php echo $theme->last_updated ?>"><?php printf( wp__('%s ago'), human_time_diff(strtotime($theme->last_updated)) ) ?></span></p>
<?php endif; if ( ! empty($theme->requires) ) : ?>
<p><strong><?php _e('Requires WordPress Version:') ?></strong> <?php printf(wp__('%s or higher'), $theme->requires) ?></p>
...
function install_theme_information() {
	...
	iframe_header( wp__('Theme Install') );

	if ( empty($api->download_link) ) {
		echo '<div id="message" class="error"><p>' . wp__('<strong>ERROR:</strong> This theme is currently not available. Please try again later.') . '</p></div>';
		iframe_footer();
		exit;
	}

	if ( !empty($api->tested) && version_compare($GLOBALS['wp_version'], $api->tested, '>') )
		echo '<div class="updated"><p>' . wp__('<strong>Warning:</strong> This theme has <strong>not been tested</strong> with your current version of WordPress.') . '</p></div>';
	else if ( !empty($api->requires) && version_compare($GLOBALS['wp_version'], $api->requires, '<') )
		echo '<div class="updated"><p>' . wp__('<strong>Warning:</strong> This theme has not been marked as <strong>compatible</strong> with your version of WordPress.') . '</p></div>';
...
<p><?php printf(wp__('by %s'), $api->author); ?></p>
<p><?php printf(wp__('Version: %s'), $api->version); ?></p>

<?php
$buttons = '<a class="button" id="cancel" href="#" onclick="tb_close();return false;">' . wp__('Cancel') . '</a> ';

switch ( $type ) {
default:
case 'install':
	if ( current_user_can('install_themes') ) :
	$buttons .= '<a class="button-primary" id="install" href="' . wp_nonce_url(self_admin_url('update.php?action=install-theme&theme=' . $api->slug), 'install-theme_' . $api->slug) . '" target="_parent">' . wp__('Install Now') . '</a>';
	endif;
	break;
case 'update_available':
	if ( current_user_can('update_themes') ) :
	$buttons .= '<a class="button-primary" id="install"	href="' . wp_nonce_url(self_admin_url('update.php?action=upgrade-theme&theme=' . $update_file), 'upgrade-theme_' . $update_file) . '" target="_parent">' . wp__('Install Update Now') . '</a>';
	endif;
	break;
case 'newer_installed':
	if ( current_user_can('install_themes') || current_user_can('update_themes') ) :
	?><p><?php printf(wp__('Newer version (%s) is installed.'), $newer_version); ?></p><?php
	endif;
	break;
