...
function plugins_api($action, $args = null) {
	...
	if ( false === $res ) {
		...
		if ( is_wp_error($request) ) {
			$res = new WP_Error('plugins_api_failed', wp__('An Unexpected HTTP Error occurred during the API request.'), $request->get_error_message() );
		} else {
			$res = unserialize( wp_remote_retrieve_body( $request ) );
			if ( false === $res )
				$res = new WP_Error('plugins_api_failed', wp__('An unknown error occurred.'), wp_remote_retrieve_body( $request ) );
		}
...
function install_dashboard() {
	?>
	<p><?php printf( wp__( 'Plugins extend and expand the functionality of WordPress. You may automatically install plugins from the <a href="http://wordpress.org/extend/plugins/">WordPress Plugin Directory</a> or upload a plugin in .zip format via <a href="%s">this page</a>.' ), self_admin_url( 'plugin-install.php?tab=upload' ) ); ?></p>
	...
	if ( is_wp_error($api_tags) ) {
		...
		foreach ( (array)$api_tags as $tag )
			$tags[ $tag['name'] ] = (object) array(
									...
									'count' => $tag['count'] );
		echo wp_generate_tag_cloud($tags, array( 'single_text' => wp__('%s plugin'), 'multiple_text' => wp__('%s plugins') ) );
	}
...
function install_search_form(){
	...
	?><form id="search-plugins" method="get" action="">
		...
		<?php submit_button( wp__( 'Search Plugins' ), 'button', 'plugin-search-input', false ); ?>
	</form><?php
}
...
function install_plugin_information() {
	...
	iframe_header( wp__('Plugin Install') );
	...
	<div class="alignright fyi">
		...
		switch ( $status['status'] ) {
			case 'install':
				if ( $status['url'] )
					echo '<a href="' . $status['url'] . '" target="_parent">' . wp__('Install Now') . '</a>';
				break;
			case 'update_available':
				if ( $status['url'] )
					echo '<a href="' . $status['url'] . '" target="_parent">' . wp__('Install Update Now') .'</a>';
				break;
			case 'newer_installed':
				echo '<a>' . sprintf(wp__('Newer Version (%s) Installed'), $status['version']) . '</a>';
				break;
			case 'latest_installed':
				echo '<a>' . wp__('Latest Version Installed') . '</a>';
				break;
		}
		?>
		</p>
...
<?php endif; if ( ! empty($api->last_updated) ) : ?>
			<li><strong><?php _e('Last Updated:') ?></strong> <span title="<?php echo $api->last_updated ?>"><?php
							printf( wp__('%s ago'), human_time_diff(strtotime($api->last_updated)) ) ?></span></li>
<?php endif; if ( ! empty($api->requires) ) : ?>
			<li><strong><?php _e('Requires WordPress Version:') ?></strong> <?php printf(wp__('%s or higher'), $api->requires) ?></li>
...
<?php endif; ?>
	...
	<?php
		if ( !empty($api->tested) && version_compare( substr($GLOBALS['wp_version'], 0, strlen($api->tested)), $api->tested, '>') )
			echo '<div class="updated"><p>' . wp__('<strong>Warning:</strong> This plugin has <strong>not been tested</strong> with your current version of WordPress.') . '</p></div>';

		else if ( !empty($api->requires) && version_compare( substr($GLOBALS['wp_version'], 0, strlen($api->requires)), $api->requires, '<') )
			echo '<div class="updated"><p>' . wp__('<strong>Warning:</strong> This plugin has <strong>not been marked as compatible</strong> with your version of WordPress.') . '</p></div>';
