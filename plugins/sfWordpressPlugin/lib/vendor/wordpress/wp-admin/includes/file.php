...
$wp_file_descriptions = array(
	'index.php' => wp__( 'Main Index Template' ),
	'style.css' => wp__( 'Stylesheet' ),
	'editor-style.css' => wp__( 'Visual Editor Stylesheet' ),
	'editor-style-rtl.css' => wp__( 'Visual Editor RTL Stylesheet' ),
	'rtl.css' => wp__( 'RTL Stylesheet' ),
	'comments.php' => wp__( 'Comments' ),
	'comments-popup.php' => wp__( 'Popup Comments' ),
	'footer.php' => wp__( 'Footer' ),
	'header.php' => wp__( 'Header' ),
	'sidebar.php' => wp__( 'Sidebar' ),
	'archive.php' => wp__( 'Archives' ),
	'author.php' => wp__( 'Author Template' ),
	'tag.php' => wp__( 'Tag Template' ),
	'category.php' => wp__( 'Category Template' ),
	'page.php' => wp__( 'Page Template' ),
	'search.php' => wp__( 'Search Results' ),
	'searchform.php' => wp__( 'Search Form' ),
	'single.php' => wp__( 'Single Post' ),
	'404.php' => wp__( '404 Template' ),
	'link.php' => wp__( 'Links Template' ),
	'functions.php' => wp__( 'Theme Functions' ),
	'attachment.php' => wp__( 'Attachment Template' ),
	'image.php' => wp__('Image Attachment Template'),
	'video.php' => wp__('Video Attachment Template'),
	'audio.php' => wp__('Audio Attachment Template'),
	'application.php' => wp__('Application Attachment Template'),
	'my-hacks.php' => wp__( 'my-hacks.php (legacy hacks support)' ),
	'.htaccess' => wp__( '.htaccess (for rewrite rules )' ),
	// Deprecated files
	'wp-layout.css' => wp__( 'Stylesheet' ),
	'wp-comments.php' => wp__( 'Comments Template' ),
	'wp-comments-popup.php' => wp__( 'Popup Comments Template' ),
);
...
function get_file_description( $file ) {
	...
	elseif ( file_exists( $file ) && is_file( $file ) ) {
		...
		if ( preg_match( '|Template Name:(.*)$|mi', $template_data, $name ))
			return sprintf( wp__( '%s Page Template' ), _cleanup_header_comment($name[1]) );
	}
...
function validate_file_to_edit( $file, $allowed_files = '' ) {
	...
	switch ( $code ) {
		case 1 :
			wp_die( wp__('Sorry, can&#8217;t edit files with &#8220;..&#8221; in the name. If you are trying to edit a file in your WordPress home directory, you can just type the name of the file in.' ));

		//case 2 :
		//	wp_die( wp__('Sorry, can&#8217;t call files with their real path.' ));

		case 3 :
			wp_die( wp__('Sorry, that file cannot be edited.' ));
	}
}
...
function wp_handle_upload( &$file, $overrides = false, $time = null ) {
	...
	$upload_error_strings = array( false,
		wp__( "The uploaded file exceeds the upload_max_filesize directive in php.ini." ),
		wp__( "The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form." ),
		wp__( "The uploaded file was only partially uploaded." ),
		wp__( "No file was uploaded." ),
		'',
		wp__( "Missing a temporary folder." ),
		wp__( "Failed to write file to disk." ),
		wp__( "File upload stopped by extension." ));
	...
	if ( $test_form && (!isset( $_POST['action'] ) || ($_POST['action'] != $action ) ) )
		return call_user_func($upload_error_handler, $file, wp__( 'Invalid form submission.' ));
	...
	if ( $test_size && !($file['size'] > 0 ) ) {
		if ( is_multisite() )
			$error_msg = wp__( 'File is empty. Please upload something more substantial.' );
		else
			$error_msg = wp__( 'File is empty. Please upload something more substantial. This error could also be caused by uploads being disabled in your php.ini or by post_max_size being defined as smaller than upload_max_filesize in php.ini.' );
		return call_user_func($upload_error_handler, $file, $error_msg);
	}

	// A properly uploaded file will pass this test. There should be no reason to override this one.
	if ( $test_upload && ! @ is_uploaded_file( $file['tmp_name'] ) )
		return call_user_func($upload_error_handler, $file, wp__( 'Specified file failed upload test.' ));
	...
	if ( $test_type ) {
		...
		if ( ( !$type || !$ext ) && !current_user_can( 'unfiltered_upload' ) )
			return call_user_func($upload_error_handler, $file, wp__( 'Sorry, this file type is not permitted for security reasons.' ));
	...
	if ( false === @ move_uploaded_file( $file['tmp_name'], $tmp_file ) )
		return $upload_error_handler( $file, sprintf( wp__('The uploaded file could not be moved to %s.' ), $uploads['path'] ) );
...
function wp_handle_sideload( &$file, $overrides = false ) {
	...
	// Courtesy of php.net, the strings that describe the error indicated in $_FILES[{form field}]['error'].
	$upload_error_strings = array( false,
		wp__( "The uploaded file exceeds the <code>upload_max_filesize</code> directive in <code>php.ini</code>." ),
		wp__( "The uploaded file exceeds the <em>MAX_FILE_SIZE</em> directive that was specified in the HTML form." ),
		wp__( "The uploaded file was only partially uploaded." ),
		wp__( "No file was uploaded." ),
		'',
		wp__( "Missing a temporary folder." ),
	...
	// A correct form post will pass this test.
	if ( $test_form && (!isset( $_POST['action'] ) || ($_POST['action'] != $action ) ) )
		return $upload_error_handler( $file, wp__( 'Invalid form submission.' ));
	...
	if ( $test_size && !(filesize($file['tmp_name']) > 0 ) )
		return $upload_error_handler( $file, wp__( 'File is empty. Please upload something more substantial. This error could also be caused by uploads being disabled in your php.ini.' ));

	// A properly uploaded file will pass this test. There should be no reason to override this one.
	if (! @ is_file( $file['tmp_name'] ) )
		return $upload_error_handler( $file, wp__( 'Specified file does not exist.' ));
	...
	if ( $test_type ) {
		...
		if ( ( !$type || !$ext ) && !current_user_can( 'unfiltered_upload' ) )
			return $upload_error_handler( $file, wp__( 'Sorry, this file type is not permitted for security reasons.' ));
...
function download_url( $url, $timeout = 300 ) {
	...
	if ( ! $url )
		return new WP_Error('http_no_url', wp__('Invalid URL Provided.'));

	$tmpfname = wp_tempnam($url);
	if ( ! $tmpfname )
		return new WP_Error('http_no_file', wp__('Could not create Temporary file.'));
...
function unzip_file($file, $to) {
	...
	if ( ! $wp_filesystem || !is_object($wp_filesystem) )
		return new WP_Error('fs_unavailable', wp__('Could not access filesystem.'));
...
function _unzip_file_ziparchive($file, $to, $needed_dirs = array() ) {
	...
	if ( true !== $zopen )
		return new WP_Error('incompatible_archive', wp__('Incompatible Archive.'));

	for ( $i = 0; $i < $z->numFiles; $i++ ) {
		if ( ! $info = $z->statIndex($i) )
			return new WP_Error('stat_failed', wp__('Could not retrieve file from archive.'));
	...
	foreach ( $needed_dirs as $_dir ) {
		if ( ! $wp_filesystem->mkdir($_dir, FS_CHMOD_DIR) && ! $wp_filesystem->is_dir($_dir) ) // Only check to see if the Dir exists upon creation failure. Less I/O this way.
			return new WP_Error('mkdir_failed', wp__('Could not create directory.'), $_dir);
	}
	unset($needed_dirs);
	...
	for ( $i = 0; $i < $z->numFiles; $i++ ) {
		...
		if ( false === $contents )
			return new WP_Error('extract_failed', wp__('Could not extract file from archive.'), $info['name']);

		if ( ! $wp_filesystem->put_contents( $to . $info['name'], $contents, FS_CHMOD_FILE) )
			return new WP_Error('copy_failed', wp__('Could not copy file.'), $to . $info['filename']);
	}
...
function _unzip_file_pclzip($file, $to, $needed_dirs = array()) {
	...
	if ( !is_array($archive_files) )
		return new WP_Error('incompatible_archive', wp__('Incompatible Archive.'), $archive->errorInfo(true));

	if ( 0 == count($archive_files) )
		return new WP_Error('empty_archive', wp__('Empty archive.'));
	...
	foreach ( $archive_files as $file ) {
		...
		if ( ! $wp_filesystem->put_contents( $to . $file['filename'], $file['content'], FS_CHMOD_FILE) )
			return new WP_Error('copy_failed', wp__('Could not copy file.'), $to . $file['filename']);
	}
	return true;
}
	...
	foreach ( (array) $dirlist as $filename => $fileinfo ) {
		...
		if ( 'f' == $fileinfo['type'] ) {
			if ( ! $wp_filesystem->copy($from . $filename, $to . $filename, true, FS_CHMOD_FILE) ) {
				...
				if ( ! $wp_filesystem->copy($from . $filename, $to . $filename, true, FS_CHMOD_FILE) )
					return new WP_Error('copy_failed', wp__('Could not copy file.'), $to . $filename);
			}
		} elseif ( 'd' == $fileinfo['type'] ) {
			if ( !$wp_filesystem->is_dir($to . $filename) ) {
				if ( !$wp_filesystem->mkdir($to . $filename, FS_CHMOD_DIR) )
					return new WP_Error('mkdir_failed', wp__('Could not create directory.'), $to . $filename);
			}
...
function request_filesystem_credentials($form_post, $type = '', $error = false, $context = false, $extra_fields = null) {
	...
	if ( $error ) {
		$error_string = wp__('<strong>ERROR:</strong> There was an error connecting to the server, Please verify the settings are correct.');
...
<p><?php
	$label_user = wp__('Username');
	$label_pass = wp__('Password');
	_e('To perform the requested action, WordPress needs to access your web server.');
	echo ' ';
	if ( ( isset( $types['ftp'] ) || isset( $types['ftps'] ) ) ) {
		if ( isset( $types['ssh'] ) ) {
			_e('Please enter your FTP or SSH credentials to proceed.');
			$label_user = wp__('FTP/SSH Username');
			$label_pass = wp__('FTP/SSH Password');
		} else {
			_e('Please enter your FTP credentials to proceed.');
			$label_user = wp__('FTP Username');
			$label_pass = wp__('FTP Password');
		}
		echo ' ';
	}
...
submit_button( wp__( 'Proceed' ), 'button', 'upgrade' );
?>
</div>
</form>
<?php
