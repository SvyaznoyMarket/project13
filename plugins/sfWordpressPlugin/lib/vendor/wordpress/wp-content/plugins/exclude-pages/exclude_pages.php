...
function ep_admin_sidebar_wp25() {
	...
	echo ' />';
	echo '			'.wp__( 'Include this page in lists of pages', EP_TD ).'</label>';
	echo '		<input type="hidden" name="ep_ctrl_present" value="1" /></p>';
	if ( $nearest_excluded_ancestor !== false ) {
		echo '<p class="ep_exclude_alert"><em>';
		printf( wp__( 'N.B. An ancestor of this page is excluded, so this page is too (<a href="%1$s" title="%2$s">edit ancestor</a>).', EP_TD), "post.php?action=edit&amp;post=$nearest_excluded_ancestor", wp__( 'edit the excluded ancestor', EP_TD ) );
		echo '</em></p>';
	}
	...
	if ( ep_has_menu() ) {
		...
		if ( current_user_can( 'edit_theme_options' ) )
			printf( wp__( 'N.B. This page can still appear in explicitly created <a href="%1$s">menus</a> (<a id="ep_toggle_more" href="#ep_explain_more">explain more</a>)', EP_TD),
				"nav-menus.php" );
		else
		...
		if ( current_user_can( 'edit_theme_options' ) )
			printf( wp__( 'WordPress provides a simple function for you to maintain your site <a href="%1$s">menus</a>. If you create a menu which includes this page, the checkbox above will not have any effect on the visibility of that menu item.', EP_TD),
				"nav-menus.php" );
		else
...
function ep_admin_init() {
	...
	global $wp_version;
	add_meta_box('ep_admin_meta_box', wp__( 'Exclude Pages', EP_TD ), 'ep_admin_sidebar_wp25', 'page', 'side', 'low');
