...
function akismet_manage_page() {
	...
	$count = sprintf(wp__('Akismet Spam (%s)'), akismet_spam_count());
	if ( isset( $submenu['edit-comments.php'] ) )
		add_submenu_page('edit-comments.php', wp__('Akismet Spam'), $count, 'moderate_comments', 'akismet-admin', 'akismet_caught' );
	elseif ( function_exists('add_management_page') )
		add_management_page(wp__('Akismet Spam'), $count, 'moderate_comments', 'akismet-admin', 'akismet_caught');
}
...
function akismet_caught() {
	...
	if (isset($_POST['submit']) && 'recover' == $_POST['action'] && ! empty($_POST['not_spam'])) {
		...
		if ( function_exists('current_user_can') && !current_user_can('moderate_comments') )
			die(wp__('You do not have sufficient permission to moderate comments.'));
...
if ( isset( $_GET['recovered'] ) ) {
	...
	echo '<div class="updated"><p>' . sprintf(wp__('%1$s comments recovered.'), $i) . "</p></div>";
}

if (isset( $_GET['deleted'] ) )
	echo '<div class="updated"><p>' . wp__('All spam deleted.') . '</p></div>';
...
if ( $count ) {
?>
<p><?php printf(wp__('Akismet has caught <strong>%1$s spam</strong> for you since you first installed it.'), number_format_i18n($count) ); ?></p>
<?php
}
...
if ( 0 == $spam_count ) {
	echo '<p>'.wp__('You have no spam currently in the queue. Must be your lucky day. :)').'</p>';
	echo '</div>';
} else {
	echo '<p>'.wp__('You can delete all of the spam from your database with a single click. This operation cannot be undone, so you may wish to check to ensure that no legitimate comments got through first. Spam is automatically deleted after 15 days, so don&#8217;t sweat it.').'</p>';
?>
<?php if ( !isset( $_POST['s'] ) ) { ?>
<form method="post" action="<?php echo attribute_escape( add_query_arg( 'noheader', 'true' ) ); ?>">
<?php akismet_nonce_field($akismet_nonce) ?>
<input type="hidden" name="action" value="delete" />
<?php printf(wp__('There are currently %1$s comments identified as spam.'), $spam_count); ?>&nbsp; &nbsp; <input type="submit" class="button delete" name="Submit" value="<?php _e('Delete all'); ?>" />
...
<p>  <input type="text" name="s" value="<?php if (isset($_POST['s'])) echo attribute_escape($_POST['s']); ?>" size="17" />
  <input type="submit" class="button" name="submit" value="<?php echo attribute_escape(wp__('Search Spam &raquo;')) ?>"  />  </p>
</form>
<?php if ( $total > 50 ) {
...
if ( 1 < $page ) {
	...
	$r .=  '<a class="prev" href="' . clean_url(add_query_arg( $args )) . '">'. wp__('&laquo; Previous Page') .'</a>' . "\n";
}
...
if ( ( $page ) * 50 < $total || -1 == $total ) {
	$args['apage'] = $page + 1;
	$r .=  '<a class="next" href="' . clean_url(add_query_arg($args)) . '">'. wp__('Next Page &raquo;') .'</a>' . "\n";
}
echo "<p>$r</p>";
?>
...
?>
<p class="submit">
<input type="submit" name="submit" value="<?php echo attribute_escape(wp__('De-spam marked comments &raquo;')); ?>" />
</p>
...
<?php printf(wp__('There are currently %1$s comments identified as spam.'), $spam_count); ?>&nbsp; &nbsp; <input type="submit" name="Submit" class="button" value="<?php echo attribute_escape(wp__('Delete all')); ?>" />
...
if ( 'moderation.php' == $pagenow ) {
	function akismet_recheck_button( $page ) {
		...
		if ( isset( $submenu['edit-comments.php'] ) )
			...
			$link = 'edit.php';
		$button = "<a href='$link?page=akismet-admin&amp;recheckqueue=true&amp;noheader=true' style='display: block; width: 100px; position: absolute; right: 7%; padding: 5px; font-size: 14px; text-decoration: underline; background: #fff; border: 1px solid #ccc;'>" . wp__('Recheck Queue for Spam') . "</a>";
