		...
		if ( intval($akismet_last_comment['comment_post_ID']) == intval($comment->comment_post_ID)
			...
			&& $akismet_last_comment['comment_author_email'] == $comment->comment_author_email ) {
				...
				if ( $akismet_last_comment['akismet_result'] == 'true' ) {
					...
					akismet_update_comment_history( $comment->comment_ID, wp__('Akismet caught this comment as spam'), 'check-spam' );
					if ( $comment->comment_approved != 'spam' )
						akismet_update_comment_history( $comment->comment_ID, sprintf( wp__('Comment status was changed to %s'), $comment->comment_approved), 'status-changed'.$comment->comment_approved );
				} elseif ( $akismet_last_comment['akismet_result'] == 'false' ) {
					update_comment_meta( $comment->comment_ID, 'akismet_result', 'false' );
					akismet_update_comment_history( $comment->comment_ID, wp__('Akismet cleared this comment'), 'check-ham' );
					if ( $comment->comment_approved == 'spam' ) {
						if ( wp_blacklist_check($comment->comment_author, $comment->comment_author_email, $comment->comment_author_url, $comment->comment_content, $comment->comment_author_IP, $comment->comment_agent) )
							akismet_update_comment_history( $comment->comment_ID, wp__('Comment was caught by wp_blacklist_check'), 'wp-blacklisted' );
						else
							akismet_update_comment_history( $comment->comment_ID, sprintf( wp__('Comment status was changed to %s'), $comment->comment_approved), 'status-changed-'.$comment->comment_approved );
					}
				// abnormal result: error
					...
					akismet_update_comment_history( $comment->comment_ID, sprintf( wp__('Akismet was unable to check this comment (response: %s), will automatically retry again later.'), $akismet_last_comment['akismet_result']), 'check-error' );
				}
...
function akismet_cron_recheck() {
	...
	foreach ( (array) $comment_errors as $comment_id ) {
		...
		$msg = '';
		if ( $status == 'true' ) {
			$msg = wp__( 'Akismet caught this comment as spam during an automatic retry.' );
		} elseif ( $status == 'false' ) {
			$msg = wp__( 'Akismet cleared this comment during an automatic retry.' );
		}
