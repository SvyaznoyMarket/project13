...
function akismet_admin_init() {
    ...
    if ( !function_exists('is_multisite') && version_compare( $wp_version, '3.0', '<' ) ) {
        ...
        function akismet_version_warning() {
            echo "
            <div id='akismet-warning' class='updated fade'><p><strong>".sprintf(wp__('Akismet %s requires WordPress 3.0 or higher.'), AKISMET_VERSION) ."</strong> ".sprintf(wp__('Please <a href="%s">upgrade WordPress</a> to a current version, or <a href="%s">downgrade to version 2.4 of the Akismet plugin</a>.'), 'http://codex.wordpress.org/Upgrading_WordPress', 'http://wordpress.org/extend/plugins/akismet/download/'). "</p></div>
            ";
        }
    ...
    add_meta_box('akismet-status', wp__('Comment History'), 'akismet_comment_status_meta_box', 'comment', 'normal');
...
function akismet_config_page() {
	if ( function_exists('add_submenu_page') )
		add_submenu_page('plugins.php', wp__('Akismet Configuration'), wp__('Akismet Configuration'), 'manage_options', 'akismet-key-config', 'akismet_conf');
}

function akismet_plugin_action_links( $links, $file ) {
	if ( $file == plugin_basename( dirname(__FILE__).'/akismet.php' ) ) {
		$links[] = '<a href="plugins.php?page=akismet-key-config">'.wp__('Settings').'</a>';
	}
...
function akismet_conf() {
	...
	$messages = array(
		'new_key_empty' => array('color' => 'aa0', 'text' => wp__('Your key has been cleared.')),
		'new_key_valid' => array('color' => '4AB915', 'text' => wp__('Your key has been verified. Happy blogging!')),
		'new_key_invalid' => array('color' => '888', 'text' => wp__('The key you entered is invalid. Please double-check it.')),
		'new_key_failed' => array('color' => '888', 'text' => wp__('The key you entered could not be verified because a connection to akismet.com could not be established. Please check your server configuration.')),
		'no_connection' => array('color' => '888', 'text' => wp__('There was a problem connecting to the Akismet server. Please check your server configuration.')),
		'key_empty' => array('color' => 'aa0', 'text' => sprintf(wp__('Please enter an API key. (<a href="%s" style="color:#fff">Get your key.</a>)'), 'http://akismet.com/get/')),
		'key_valid' => array('color' => '4AB915', 'text' => wp__('This key is valid.')),
		'key_failed' => array('color' => 'aa0', 'text' => wp__('The key below was previously validated but a connection to akismet.com can not be established at this time. Please check your server configuration.')),
		'bad_home_url' => array('color' => '888', 'text' => sprintf( wp__('Your WordPress home URL %s is invalid.  Please fix the <a href="%s">home option</a>.'), esc_html( get_bloginfo('url') ), admin_url('options.php#home') ) ),
	);
?>
...
<?php if ( !$wpcom_api_key ) { ?>
	<p><?php printf(wp__('For many people, <a href="%1$s">Akismet</a> will greatly reduce or even completely eliminate the comment and trackback spam you get on your site. If one does happen to get through, simply mark it as "spam" on the moderation screen and Akismet will learn from the mistakes. If you don\'t have an API key yet, you can get one at <a href="%2$s">Akismet.com</a>.'), 'http://akismet.com/', 'http://akismet.com/get/'); ?></p>
...
<?php
	if ( !function_exists('fsockopen') || !function_exists('gethostbynamel') ) {
		?>
			...
			<p><?php echo sprintf( wp__('Your web host or server administrator has disabled PHP\'s <code>fsockopen</code> or <code>gethostbynamel</code> functions.  <strong>Akismet cannot work correctly until this is fixed.</strong>  Please contact your web host or firewall administrator and give them <a href="%s" target="_blank">this information about Akismet\'s system requirements</a>.'), 'http://blog.akismet.com/akismet-hosting-faq/'); ?></p>
		<?php
	} else {
		...
		if ( is_array($servers) && count($servers) > 0 ) {
			...
			?>
				...
				<p><?php echo sprintf( wp__('A DNS problem or firewall is preventing all access from your web server to Akismet.com.  <strong>Akismet cannot work correctly until this is fixed.</strong>  Please contact your web host or firewall administrator and give them <a href="%s" target="_blank">this information about Akismet and firewalls</a>.'), 'http://blog.akismet.com/akismet-hosting-faq/'); ?></p>
			<?php
		}
	}
...
<?php
	...
	?>
		...
		<td style="padding: 0 .5em; font-weight:bold; color: #fff; background-color: <?php echo $color; ?>"><?php echo ($status ? wp__('Accessible') : wp__('Re-trying') ); ?></td>
...
?>
	...
	<p><?php if ( get_option('akismet_connectivity_time') ) echo sprintf( wp__('Last checked %s ago.'), human_time_diff( get_option('akismet_connectivity_time') ) ); ?></p>
	<p class="submit"><input type="submit" name="check" value="<?php _e('Check network status &raquo;'); ?>" /></p>
	<p><?php printf( wp__('<a href="%s" target="_blank">Click here</a> to confirm that <a href="%s" target="_blank">Akismet.com is up</a>.'), 'http://status.automattic.com/9931/136079/Akismet-API', 'http://status.automattic.com/9931/136079/Akismet-API' ); ?></p>
</form>
...
function akismet_stats_page() {
	if ( function_exists('add_submenu_page') )
		add_submenu_page('index.php', wp__('Akismet Stats'), wp__('Akismet Stats'), 'manage_options', 'akismet-stats-display', 'akismet_stats_display');
...
function akismet_admin_warnings() {
	...
	if ( !get_option('wordpress_api_key') && !$wpcom_api_key && !isset($_POST['submit']) ) {
		function akismet_warning() {
			echo "
			<div id='akismet-warning' class='updated fade'><p><strong>".wp__('Akismet is almost ready.')."</strong> ".sprintf(wp__('You must <a href="%1$s">enter your Akismet API key</a> for it to work.'), "plugins.php?page=akismet-key-config")."</p></div>
			";
		}
		...
		function akismet_warning() {
			global $wpdb;
				...
				if ( $waiting > 0 )
					echo "
			<div id='akismet-warning' class='updated fade'><p><strong>".wp__('Akismet has detected a problem.')."</strong> ".sprintf(_n('A server or network problem prevented Akismet from checking %d comment. It has been temporarily held for moderation and will be automatically re-checked in %s.', 'A server or network problem prevented Akismet from checking %d comments. They have been temporarily held for moderation and will be automatically re-checked in %s.', $waiting), number_format_i18n( $waiting ), $next_check)."</p></div>
			";
		}
		add_action('admin_notices', 'akismet_warning');
		return;
...
function akismet_comment_row_action( $a, $comment ) {
	...
	if ( !$user_result || $user_result == $akismet_result ) {
		...
		if ( $akismet_result == 'true' && $comment_status != 'spam' && $comment_status != 'trash' )
			$desc = wp__( 'Flagged as spam by Akismet' );
		elseif ( $akismet_result == 'false' && $comment_status == 'spam' )
			$desc = wp__( 'Cleared by Akismet' );
	} else {
		$who = get_comment_meta( $comment->comment_ID, 'akismet_user', true );
		if ( $user_result == 'true' )
			$desc = sprintf( wp__('Flagged as spam by %s'), $who );
		else
			$desc = sprintf( wp__('Un-spammed by %s'), $who );
	}
	...
	if ( $akismet_result ) {
		...
		foreach ( $a as $k => $item ) {
			$b[ $k ] = $item;
			if ( $k == 'edit' )
				$b['history'] = '<a href="comment.php?action=editcomment&amp;c='.$comment->comment_ID.'#akismet-status" title="'. esc_attr__( 'View comment history' ) . '"> '. wp__('History') . '</a>';
		}
...
function akismet_comment_status_meta_box($comment) {
	...
	if ( $history ) {
		...
		foreach ( $history as $row ) {
			...
			echo '<div style="margin-bottom: 13px;"><span style="color: #999;" alt="' . $time . '" title="' . $time . '">' . sprintf( wp__('%s ago'), human_time_diff( $row['time'] ) ) . '</span> - ';
...
function akismet_comment_column_row( $column, $comment_id ) {
	...
	if ( $history ) {
		...
		foreach ( $history as $row ) {
			echo '<dt>' . sprintf( wp__('%s ago'), human_time_diff( $row['time'] ) ) . '</dt>';
...
function akismet_rightnow() {
	...
	if ( $count = get_option('akismet_spam_count') ) {
		...
		$intro = sprintf( wp__('<a href="%1$s">Akismet</a> blocks spam from getting to your blog. '), 'http://akismet.com/' );
	}
	...
	if ( $queue_count = akismet_spam_count() ) {
		...
		$queue_text = sprintf( wp__( "There's nothing in your <a href='%1\$s'>spam queue</a> at the moment." ), $esc_url($link) );
	}
...
function akismet_check_for_spam_button($comment_status) {
	...
	echo "</div><div class='alignleft'><a class='button-secondary checkforspam' href='$link'>" . wp__('Check for Spam') . "</a>";
}
...
function akismet_submit_nonspam_comment ( $comment_id ) {
	...
	if ( $comment->reporter ) {
		akismet_update_comment_history( $comment_id, sprintf( wp__('%s reported this comment as not spam'), $comment->reporter ), 'report-ham' );
...
function akismet_submit_spam_comment ( $comment_id ) {
	...
	if ( $comment->reporter ) {
		akismet_update_comment_history( $comment_id, sprintf( wp__('%s reported this comment as spam'), $comment->reporter ), 'report-spam' );
...
function akismet_transition_comment_status( $new_status, $old_status, $comment ) {
	...
	if ( !get_comment_meta( $comment->comment_ID, 'akismet_rechecking' ) )
		akismet_update_comment_history( $comment->comment_ID, sprintf( wp__('%s changed the comment status to %s'), $reporter, $new_status ), 'status-' . $new_status );
}
...
function akismet_recheck_queue() {
	...
	foreach ( (array) $moderation as $c ) {
		...
		if ( 'true' == $response[1] ) {
			...
			akismet_update_comment_history( $c['comment_ID'], wp__('Akismet re-checked and caught this comment as spam'), 'check-spam' );
		
		} elseif ( 'false' == $response[1] ) {
			update_comment_meta( $c['comment_ID'], 'akismet_result', 'false' );
			akismet_update_comment_history( $c['comment_ID'], wp__('Akismet re-checked and cleared this comment'), 'check-ham' );
		// abnormal result: error
		} else {
			update_comment_meta( $c['comment_ID'], 'akismet_result', 'error' );
			akismet_update_comment_history( $c['comment_ID'], sprintf( wp__('Akismet was unable to re-check this comment (response: %s)'), $response[1]), 'check-error' );
		}
