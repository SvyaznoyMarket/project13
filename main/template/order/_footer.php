<div class="footer__order">
    <ul class="footer__list">
        <li class="footer__list__title">
            Мы обо всем позаботимся!
        </li>

        <li class="footer__list__item mEnter">
            <div class="title">Ответы на вопросы</div>
            <span class="span__text">Наш Контакт-cENTER<br/><strong class="strong"><?= \App::config()->company['phone'] ?></strong><br/> 24 часа в&nbsp;сутки / 7 дней в&nbsp;неделю. Звонок бесплатный. Радость в&nbsp;подарок :)</span>
        </li>

        <li class="footer__list__item mSafeOder">
            <div class="title">Безопасные покупки</div>
            <span class="span__text">Вы приобретаете качественный товар. Получаете и оплачиваете любым удобным для&nbsp;Вас способом.</span>
        </li>
     
        <li class="footer__list__item mOrder">
            <div class="title">Сопровождение заказа</div>
            <span class="span__text">После оформления заказа с&nbsp;Вами свяжется специалист нашего Контакт-сENTER для подтверждения заказа.</span>
        </li>

        <li class="footer__list__item mDelivery">
            <div class="title">Собственная служба доставки</div>
            <span class="span__text">Наша служба F1 доставит заказ вовремя.</span>
        </li>
    </ul>


    <div class="footer__order__bottom clearfix">
        <div id="jira" class="feedback__link"><a href="javascript:void(0)">Сообщить об ошибке</a></div>

        <div class="footerbottom">
            <div class="copy">© «Enter» 2011–2013. Все права защищены.<br />Указанная стоимость товаров и условия их приобретения действительны по состоянию на текущую дату.</div>
            <ul class="creditcard__list">
                <li class="creditcard__list__item"><img alt="" height="30" src="/images/counter2.gif" width="50"></li>
                <li class="creditcard__list__item"><img alt="" height="30" src="/images/counter3.gif" width="50"></li>
            </ul>
        </div>
    </div>
</div>