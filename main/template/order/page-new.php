...
<div class="bBuyingSteps clearfix" style="display:none" data-bind="style: { display: prepareData() ? 'block' : 'none'}">
	...
	<div class="bBuyingInfo" data-bind="visible: deliveryBoxes().length">
		...
		<form id="order-form" action="<?= $paypalECS ? $page->url('order.paypal.create', ['token' => $request->get('token'), 'PayerID' => $request->get('PayerID')]) : $page->url('order.create') ?>" method="post">
			...
			<!-- PayPal сумма заказа -->
			<div data-bind="css: { visible: paypalECS }" class="bBuyingLine mPaypalLine clearfix">
				...
				<div class="bPaypalTotal">
					...
					<div data-bind="css: {visible: paypalECS() && ( cartSum !== undefined ) && ( totalSum() !== cartSum )}" class="bPaypalTotalChanged"><strong>Сумма оплаты изменилась</strong></div>
				</div>
			</div>
			<!--/ PayPal сумма заказа -->
			...
			<div class="bBuyingLine clearfix">
				...
				<div class="bBuyingLine__eRight bInputList mRules">
					...
					<div>
                        ...
                        <!-- Сообщение о редиректе на сайт PayPal --><div class="bPaypalFootnote" data-bind="css: {visible: paypalECS() && ( cartSum !== undefined ) && ( totalSum() !== cartSum )}">Вы будете перенаправлены на сайт <img class="bPaypalImgIco" src="/css/bBuyingSteps/img/bPayPalIcoSmall.gif" /></div><!-- Сообщение о редиректе на сайт PayPal -->
					</div>
				</div>
			</div>
		</form>
