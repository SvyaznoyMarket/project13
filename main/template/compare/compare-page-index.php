<?php

return function (
    \Helper\TemplateHelper $helper
) { ?>

<div class="wrapw">
    <div class="cmprHd clearfix">
    	<div class="cmprHd_l"><a href=""><img src="/styles/compare/img/logo_cmpr.png" alt=""></a></div>

    	<div class="cmprHd_r">
    		<div class="cmprHd_t">
    			Сравнение товаров
    		</div>

    		<ul class="cmprHd_lst">
    			<li class="cmprHd_lst_i cmprHd_lst_i-act">Планшетные компьютеры <span class="cmprHd_lst_qn">12</span></li>
    			<li class="cmprHd_lst_i">Автомобили и электромотоциклы <span class="cmprHd_lst_qn">99</span></li>
    			<li class="cmprHd_lst_i">Планшетные компьютеры <span class="cmprHd_lst_qn">1200</span></li>
    			<li class="cmprHd_lst_i">Автомобили и электромотоциклы <span class="cmprHd_lst_qn">132</span></li>
    			<li class="cmprHd_lst_i">Сушильные машины <span class="cmprHd_lst_qn">12343</span></li>
    			<li class="cmprHd_lst_i">Автомобили и электромотоциклы <span class="cmprHd_lst_qn">122</span></li>
    		</ul>
    	</div>
    </div>
	
	<!-- скроллинге добавляем класс модификатор cmprCnt-scroll -->
	<div class="cmprCnt clearfix">

		<!-- левая колонка сравнения -->
	    <div class="cmprCnt_l" style="height: 549px;">
	        <div class="cmprCnt_cell cmprCnt_cell-hd">
	            <menu class="cmprCnt_mode">
	                <button class="cmprCnt_mode_btn btn1">Только отличия</button>
	                <button class="cmprCnt_mode_btn btn1 cmprCnt_mode_btn-act">Все характеристики</button>
	            </menu>
	        </div>
			
			<!-- параметры сравнения -->
	        <div class="cmprCnt_prod" style="height: 269px;">
	            <div style="overflow: hidden">
	                <div class="cmprCnt_cpt">
	                    <span class="cmprCnt_cpt_t">ОС, процессор и ещё очень много букв</span>
	                </div>

	                <div class="cmprCnt_cell">
	                    Платформа
	                </div>
	                <div class="cmprCnt_cell">
	                    Версия ОС
	                </div>
	                <div class="cmprCnt_cell">
	                    Модель процессора
	                </div>
	                <div class="cmprCnt_cell">
	                    Частота процессора
	                </div>
	                <div class="cmprCnt_cell">
	                    Двухъядерный процессор
	                </div>
	                <div class="cmprCnt_cell">
	                    Графический процессор
	                </div>
	            </div>

	            <div style="overflow: hidden">
	                <div class="cmprCnt_cpt">
	                    <span class="cmprCnt_cpt_t">Память</span>
	                </div>

	                <div class="cmprCnt_cell">
	                    Оперативная память
	                </div>
	                <div class="cmprCnt_cell">
	                    Встроенная память
	                </div>
	                <div class="cmprCnt_cell">
	                    Поддержка карт памяти
	                </div>
	                <div class="cmprCnt_cell">
	                    Поддержка карт памяти microSD/microSDHC
	                </div>
	                <div class="cmprCnt_cell">
	                    Максимальный поддерживаемый объем карт
	                </div>
	            </div>

	            <div style="overflow: hidden">
	                <div class="cmprCnt_cpt">
	                    <span class="cmprCnt_cpt_t">Беспроводная связь</span>
	                </div>

	                <div class="cmprCnt_cell">
	                    Bluetooth
	                </div>
	                <div class="cmprCnt_cell">
	                    WiFi
	                </div>
	                <div class="cmprCnt_cell">
	                    3G
	                </div>
	                <div class="cmprCnt_cell">
	                    LTE
	                </div>
	                <div class="cmprCnt_cell">
	                    GPS
	                </div>
	            </div>

	            <div style="overflow: hidden">
	                <div class="cmprCnt_cpt">
	                    <span class="cmprCnt_cpt_t">Интерфейсы</span>
	                </div>

	                <div class="cmprCnt_cell">
	                    HDMI
	                </div>
	                <div class="cmprCnt_cell">
	                    USB для подключения к ПК
	                </div>
	                <div class="cmprCnt_cell">
	                    USB для подключения флэшек
	                </div>

	                <div class="cmprCnt_cell">
	                    Наушники
	                </div>
	            </div>

	            <div style="overflow: hidden">
	                <div class="cmprCnt_cpt">
	                    <span class="cmprCnt_cpt_t">Камера</span>
	                </div>

	                <div class="cmprCnt_cell">
	                    Разрешение камеры
	                </div>
	                <div class="cmprCnt_cell">
	                    Автофокус
	                </div>
	                <div class="cmprCnt_cell">
	                    Вспышка
	                </div>
	                <div class="cmprCnt_cell">
	                    Встроенный микрофон
	                </div>
	                <div class="cmprCnt_cell">
	                    Фронтальная камера
	                </div>
	                <div class="cmprCnt_cell">
	                    Особенности камеры
	                </div>
	            </div>

	            <div style="overflow: hidden">
	                <div class="cmprCnt_cpt">
	                    <span class="cmprCnt_cpt_t">Мультимедиа</span>
	                </div>

	                <div class="cmprCnt_cell">
	                    Звуковая система
	                </div>
	                <div class="cmprCnt_cell">
	                    Форматы воспроизведения аудио
	                </div>
	                <div class="cmprCnt_cell">
	                    Форматы воспроизведения видео
	                </div>
	            </div>

	            <div style="overflow: hidden">
	                <div class="cmprCnt_cpt">
	                    <span class="cmprCnt_cpt_t">Особенности</span>
	                </div>

	                <div class="cmprCnt_cell">
	                    Трансформер
	                </div>
	                <div class="cmprCnt_cell">
	                    Датчик положения в пространстве
	                </div>
	                <div class="cmprCnt_cell">
	                    Аппаратная QWERTY-клавиатура
	                </div>
	                <div class="cmprCnt_cell">
	                    Наличие SIM-карт
	                </div>
	                <div class="cmprCnt_cell">
	                    Функция телефона
	                </div>
	                <div class="cmprCnt_cell">
	                    Количество SIM-карт
	                </div>
	                <div class="cmprCnt_cell">
	                    Тип SIM-карты
	                </div>
	            </div>

	            <div style="overflow: hidden">
	                <div class="cmprCnt_cpt">
	                    <span class="cmprCnt_cpt_t">Питание</span>
	                </div>

	                <div class="cmprCnt_cell">
	                    Тип аккумулятора
	                </div>
	                <div class="cmprCnt_cell">
	                    Емкость аккумулятора
	                </div>
	                <div class="cmprCnt_cell">
	                    Работа в режиме просмотра видео
	                </div>
	            </div>

	            <div style="overflow: hidden">
	                <div class="cmprCnt_cpt">
	                    <span class="cmprCnt_cpt_t">Дизайн</span>
	                </div>

	                <div class="cmprCnt_cell">
	                    Материал корпуса
	                </div>
	                <div class="cmprCnt_cell">
	                    Цвет
	                </div>
	            </div>

	            <div style="overflow: hidden">
	                <div class="cmprCnt_cpt">
	                    <span class="cmprCnt_cpt_t">Размеры и вес</span>
	                </div>

	                <div class="cmprCnt_cell">
	                    Размеры
	                </div>
	                <div class="cmprCnt_cell">
	                    Вес
	                </div>
	            </div>

	            <div style="overflow: hidden">
	                <div class="cmprCnt_cpt">
	                    <span class="cmprCnt_cpt_t">Комплектация</span>
	                </div>

	                <div class="cmprCnt_cell">
	                    Комплектация
	                </div>
	            </div>
	        </div>
	        <!--/ параметры сравнения -->
	    </div>
	    <!--/ левая колонка сравнения -->
		
		<!-- праваяя колонка сравнения -->
	    <div class="cmprCnt_r" style="width: 1194px; height: 549px;">
	    	<!-- описание товаров в сравнении -->
		    <div class="cmprCnt_thd" style="width: 1194px;">
				<div class="clearfix" style="width: 480px">
					<!-- первый товар -->
			        <div class="cmprCnt_col">
			        	<div class="cmprCnt_cell">
			        		<div class="clsr"></div>

			                <a class="cmprCnt_img" href="">
			                    <img src="http://fs04.enter.ru/1/1/163/d0/202993.jpg" class="descrImg_img">
			                </a>

			                <div class="cmprCnt_rating">
						        <img src="/images/reviews_star.png" class="cmprCnt_rating_img">
						        <img src="/images/reviews_star.png" class="cmprCnt_rating_img">
						        <img src="/images/reviews_star.png" class="cmprCnt_rating_img">
						        <img src="/images/reviews_star.png" class="cmprCnt_rating_img">
						        <img src="/images/reviews_star.png" class="cmprCnt_rating_img">

							    <span class="cmprCnt_rating_count">(16)</span>
							</div>
							
							<div class="cmprCnt_pt">
								<a href="" class="cmprCnt_cat">Планшетный компьютер</a>
					            <a href="" class="cmprCnt_n">Wexler TAB 7iD 8ГБ 3G черный Wexler TAB 7iD 8ГБ 3G черный</a>
				            </div>
							
			                <span class="cmprCnt_price cmprCnt_price-l">
			                    4 690
			                    <span class="rubl">p</span>
			                </span>
								
			                <span class="cmprCnt_price">
			                    4 690
			                    <span class="rubl">p</span>
			                </span>

							<div class="cmprCnt_buy"><a href="" class="cmprCnt_buy_lk btnBuy__eLink">Купить</a></div>
				        </div>
			        </div>
			        <!--/ первый товар -->
					
					<!-- второй товар -->
			        <div class="cmprCnt_col">
			        	<div class="cmprCnt_cell">
			        		<div class="clsr"></div>

			                <a class="cmprCnt_img" href="">
			                    <img src="http://fs04.enter.ru/1/1/163/d0/202993.jpg" class="descrImg_img">
			                </a>
							
			                <div class="cmprCnt_rating">
						        <img src="/images/reviews_star.png" class="cmprCnt_rating_img">
						        <img src="/images/reviews_star.png" class="cmprCnt_rating_img">
						        <img src="/images/reviews_star.png" class="cmprCnt_rating_img">
						        <img src="/images/reviews_star.png" class="cmprCnt_rating_img">
						        <img src="/images/reviews_star.png" class="cmprCnt_rating_img">

							    <span class="cmprCnt_rating_count">(16)</span>
							</div>
							
							<div class="cmprCnt_pt">
								<a href="" class="cmprCnt_cat">Планшетный компьютер</a>
					            <a href="" class="cmprCnt_n">Wexler TAB 7iD 8ГБ 3G черный</a>
				            </div>
							
			                <span class="cmprCnt_price cmprCnt_price-l">
			                    4 690
			                    <span class="rubl">p</span>
			                </span>
								
			                <span class="cmprCnt_price">
			                    4 690
			                    <span class="rubl">p</span>
			                </span>

							<div class="cmprCnt_buy"><a href="" class="cmprCnt_buy_lk btnBuy__eLink">Купить</a></div>
				        </div>
			        </div>
			        <!--/ второй товар -->
		        </div>
	        </div>
	        <!--/ описание товаров в сравнении -->
			
			<!-- характеристики товаров в сравнении -->
	        <div class="cmprCnt_descr" style="width: 1194px; height: 269px;">
	        	<div style="width: 480px">
	        		<!-- характеристики первого товара -->
	            	<div class="cmprCnt_col">
	            		<div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
		                    <div class="cmprCnt_cell">
		                        Android
		                    </div>
		                    <div class="cmprCnt_cell">
		                        4.1
		                    </div>
		                    <div class="cmprCnt_cell">
		                        Cortex A9
		                    </div>
		                    <div class="cmprCnt_cell">
		                        1200
		                    </div>
		                    <div class="cmprCnt_cell">
		                        да
		                    </div>
		                    <div class="cmprCnt_cell">
		                        PowerVR SGX531
		                    </div>
		                </div>

		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            1024
	                        </div>
	                        <div class="cmprCnt_cell">
	                            8
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            32 ГБ
	                        </div>
		                </div>
		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            нет
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
		                </div>
		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            нет
	                        </div>
	                        <div class="cmprCnt_cell">
	                            совместимый с USB
	                        </div>
	                        <div class="cmprCnt_cell">
	                            нет
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
		                </div>
		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            5 Мпикс.
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            0,3 Мпикс.
	                        </div>
		                </div>
		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            встроенные динамики
	                        </div>
	                        <div class="cmprCnt_cell">
	                            AAC, WAV, OGG, APE, MP3
	                        </div>
	                        <div class="cmprCnt_cell">
	                            WMV, MKV, MOV, MP4
	                        </div>
		                </div>
		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            нет
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            нет
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            2
	                        </div>
	                        <div class="cmprCnt_cell">
	                            miniSIM (обычная)
	                        </div>
		                </div>
		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            Li-Pol
	                        </div>
	                        <div lass="cmprCnt_cell">
	                            4000
	                        </div>
	                        <div class="cmprCnt_cell">
	                            до 6 ч.
	                        </div>
		                </div>
		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            Пластик
	                        </div>
	                        <div class="cmprCnt_cell">
	                            черный
	                        </div>
		                </div>
		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            19,3 х 11,9 х 0,98 см
	                        </div>
	                        <div class="cmprCnt_cell">
	                            316
	                        </div>
		                </div>
		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            планшет, USB-кабель, сетевой адаптер, инструкция
	                        </div>
		                </div>
		            </div>
		            <!--/ характеристики первого товара -->
					
					<!-- характеристики второго товара -->
		            <div class="cmprCnt_col">
	            		<div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
		                    <div class="cmprCnt_cell">
		                        Android
		                    </div>
		                    <div class="cmprCnt_cell">
		                        4.1
		                    </div>
		                    <div class="cmprCnt_cell">
		                        Cortex A9
		                    </div>
		                    <div class="cmprCnt_cell">
		                        1200
		                    </div>
		                    <div class="cmprCnt_cell">
		                        да
		                    </div>
		                    <div class="cmprCnt_cell">
		                        PowerVR SGX531
		                    </div>
		                </div>

		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            1024
	                        </div>
	                        <div class="cmprCnt_cell">
	                            8
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            32 ГБ
	                        </div>
		                </div>
		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            нет
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
		                </div>
		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            нет
	                        </div>
	                        <div class="cmprCnt_cell">
	                            совместимый с USB
	                        </div>
	                        <div class="cmprCnt_cell">
	                            нет
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
		                </div>
		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            5 Мпикс.
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            0,3 Мпикс.
	                        </div>
		                </div>
		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            встроенные динамики
	                        </div>
	                        <div class="cmprCnt_cell">
	                            AAC, WAV, OGG, APE, MP3
	                        </div>
	                        <div class="cmprCnt_cell">
	                            WMV, MKV, MOV, MP4
	                        </div>
		                </div>
		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            нет
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            нет
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            да
	                        </div>
	                        <div class="cmprCnt_cell">
	                            2
	                        </div>
	                        <div class="cmprCnt_cell">
	                            miniSIM (обычная)
	                        </div>
		                </div>
		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            Li-Pol
	                        </div>
	                        <div lass="cmprCnt_cell">
	                            4000
	                        </div>
	                        <div class="cmprCnt_cell">
	                            до 6 ч.
	                        </div>
		                </div>
		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            Пластик
	                        </div>
	                        <div class="cmprCnt_cell">
	                            черный
	                        </div>
		                </div>
		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            19,3 х 11,9 х 0,98 см
	                        </div>
	                        <div class="cmprCnt_cell">
	                            316
	                        </div>
		                </div>
		                <div style="overflow: hidden">
		                    <div class="cmprCnt_cpt">
		                    </div>
	                        <div class="cmprCnt_cell">
	                            планшет, USB-кабель, сетевой адаптер, инструкция
	                        </div>
		                </div>
		            </div>
		            <!-- характеристики второго товара -->
		        </div>
	    	</div>
	    	<!-- характеристики товаров в сравнении -->
	    </div>
	    <!--/ праваяя колонка сравнения -->
    </div>
</div>

    <footer class="footerw">
	    <p class="footerw_tx clearfix">&copy; ООО «Энтер» 2011&ndash;2014. ENTER&reg; ЕНТЕР&reg; Enter&reg;. Все права защищены. <a href="javascript:void(0)" class="footer__copy__link" id="jira">Сообщить об ошибке</a></p>
	</footer>
<? } ?>