...
<div class="bSearchSuggest">
    ...
    <div class="bSearchSuggest__eCategoryList">
        ...
        <a class="bSearchSuggest__eCategoryRes bSearchSuggest__eRes" href="<?= $category->getLink() ?>">
            <?= preg_replace('/' . preg_quote($searchQuery, '/') . '/iu', '<span class="bSearchSuggest__eSelected">' . htmlspecialchars('$0', ENT_QUOTES, \App::config()->encoding) . '</span>', $category->getName()) ?>
        </a>
        <? endforeach ?>
    </div>
    <? endif ?>
    ...
    <div class="bSearchSuggest__eProductList">
        ...
        <a class="bSearchSuggest__eGoodRes bSearchSuggest__eRes clearfix" href="<?= $product->getLink()?>">
            ...
            <p class="bSearchSuggest__eGoodTitleRes fl"><?= preg_replace('/' . preg_quote($searchQuery, '/') . '/iu', '<span class="bSearchSuggest__eSelected">' . htmlspecialchars('$0', ENT_QUOTES, \App::config()->encoding) . '</span>', $product->getName()) ?></p>
        </a>
        <? endforeach ?>
    </div>
    <? endif ?>
