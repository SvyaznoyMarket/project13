<?php
/**
 * @var $page      \View\DefaultLayout
 *
 */
?>

<div class="lts_sidebar bProductSection__eRight right">
    <aside>
        <? /* For Javascript */ ?>
        <div class="liveTex_stat">
            <div id="haveOnline">
                <p class="online hidden">Найдены операторы онлайн: <span id="count_opers">0</span>.</p>
                <p class="offline hidden">Нет операторов онлайн</p>
            </div>
        </div>

        <div class="bWidgetBuy mWidget">
            <div class="lts_menu">
                <? if (!empty($aside_menu)): ?>
                <h3>Навигация</h3>
                <ul>
                    <? foreach ($aside_menu as $item) { ?>
                        <li>
                            <a href="<?= $item['link'] ?>"><?= $item['name'] ?></a>
                        </li>
                    <? } ?>
                </ul>
            </div>
            <hr/>
            <? endif; ?>


            <h3>Используемые данные</h3>
            <form action="/livetex-statistics" method="get">
                <p class="info">Используйте поле "Дата дня" получения данных за день. Для указания интервала в несколько дней,
                    используйте поля "дата начала" и "дата окончания". Внимание, если поле дата дня не пустое,
                    то дата начала и дата окончания в полях ниже игнорируются — они используются, если поле "дата дня"
                    оставить пустым.
                </p>
                <? if ($stat_params): ?>
                <ul>
                    <? foreach($stat_params as $key => $item): ?>
                        <li class="li_<?= $key ?>">
                            <p>
                                <label for="<?= $key; ?>"><?= $item['descr']; ?></label>
                                <input type="text" name="<?= $key; ?>" value="<?= $item['value']; ?>" />
                            </p>
                        </li>
                    <? endforeach; ?>
                    <input type="submit" value="Обновить">
                </ul>
                <? endif; ?>
            </form>


            <hr/>
            <p>
                <a href="https://billing.livetex.ru/auth/" title="">Перейти в биллинговую систему LiveTex</a>
            </p>

        </div>
    </aside>
</div>