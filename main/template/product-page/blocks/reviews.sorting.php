...
<div class="reviews-sort jsReviewsSorting">
    ...
    <? foreach ($sorting->listByToken as $item): ?>
        <a href="#" class="js-review-update reviews-sort__btn reviews-sort__btn--val <?= $item->isActive ? 'active' : '' ?> <?= $item->direction ?>" data-url="<?= $helper->url('product.reviews', ['productUi' => $product->ui, 'sort' => $item->getValueAsString(true)]) ?>"><?= $item->name ?></a>
    <? endforeach ?>
</div>
