...
<? if ($reviewsData['num_reviews'] > 2): ?>
    <button class="js-review-update btn-type btn-type--normal" data-append="true" data-url="<?= $helper->url('product.reviews', ['productUi' => $product->ui, 'sort' => $sorting->getActive() ? $sorting->getActive()->getValueAsString() : null]) ?>" data-ui="<?= $product->getUi() ?>" data-total-num="<?= $reviewsData['num_reviews'] ?>">Показать больше отзывов</button>
<? endif ?>
        ...
        <div class="popup-form-success">
            ...
            <div class="popup-form-success__subscribtion">
                <form class="form-ctrl">
                    <div class="form-ctrl__group">
                        <input class="customInput customInput-defcheck jsCustomRadio js-customInput js-registerForm-subscribe" type="checkbox" name="subscribe" id="reviews_subscribe">
