  ...
  <? foreach($filters as $key => $filter) { ?>
    <? if(!empty($catalogJson['sub_category_filter_menu']) && mb_strtolower($catalogJson['sub_category_filter_menu']) == mb_strtolower($filter->getName())) { ?>
      ...
      <nav class="bBrandSubNavList">  
        <ul>
          <li class="bBrandSubNavList__eItem"><a class="bBrandSubNavList__eLink<?= empty($values) ? ' active' : '' ?>" href="<?= $category->getLink()?>?scrollTo=<?= $scrollTo ?>">Все</a></li>
          <? foreach ($filter->getOption() as $option) { $id = $option->getId() ?>
            <li class="bBrandSubNavList__eItem"><a class="bBrandSubNavList__eLink<?= in_array($id, $values) ? ' active' : '' ?>" href="<?= $category->getLink()?>?f<?= urlencode('[' . strtolower($filter->getId()) . '][]' ) . '=' . $id ?>&scrollTo=<?= $scrollTo ?>"><?= $option->getName() ?></a></li>
          <? } ?>
        </ul>
      </nav>
    <? } ?>
