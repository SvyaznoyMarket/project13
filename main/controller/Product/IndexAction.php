...
class IndexAction {
    ...
    public function execute($productPath, \Http\Request $request) {
        ...
        //if (false && self::$actionResponse) {
        if (self::$actionResponse) {
            return $this->executeByActionResponse($request);
        }
...
//        $result['creditIsAllowed'] = (bool)(($product->getPrice() * (($cart->getQuantityByProduct($product->getId()) > 0) ? $cart->getQuantityByProduct($product->getId()) : 1)) >= \App::config()->product['minCreditPrice']);
     ...
     */
    private function prepareKit($products, $restProducts, $mainProduct, $region, \EnterQuery\Delivery\GetByCart $deliveryQuery = null) {
        $result = [];
        ...
        $deliveryData = (new \Controller\Product\DeliveryAction())->getResponseData($deliveryItems, $region->getId(), $deliveryQuery);
    ...
    public function executeByActionResponse(\Http\Request $request) {
        ...
        if ((bool)$product->getKit()) {
            ...
            // Приготовим набор для отображения на сайте
            $kitProducts = $this->prepareKit($kit, [], $product, $region, $actionResponse->deliveryQuery);
        }
