...
class PromoXmlAction {
    ...
    /**
     * Создает промо-XML для google, yandex
     *
     */
    public static function generate() {
        ...
        $sourceDir = \App::config()->cmsDir . '/v1/promo/xml';
        $outputDir = \App::config()->appDir . '/web/promoxml';
        // $infoDir = $outputDir . '/info';
        if(!is_dir($outputDir)) mkdir($outputDir);
        // if(!is_dir($infoDir)) mkdir($infoDir);

        if(!is_dir($sourceDir)){
            throw new \Exception(sprintf('BuSeoReport: не найден каталог с исходными файлами %s', $sourceDir));
        }
        ...
        // если xml в процессе генерации, то выходим, иначе создаем лок-файл
        $lockFilepath = $outputDir . '/' . $dateStart->format('YmdH') . '.lock';
        ...
        foreach (scandir($sourceDir) as $file) {
            if(preg_match('/^(.+)\.csv$/', $file, $matches)) {
                ...
                $xmlGoogleFilepath = $outputDir . '/' . $dateStart->format('YmdH') . '_' . str_replace(' ', '_', $sourceFilename) . '_google.xml';
                $xmlYandexFilepath = $outputDir . '/' . $dateStart->format('YmdH') . '_' . str_replace(' ', '_', $sourceFilename) . '_yandex.xml';
                ...
                $productsCsv = [];
                if (($sourceCsv = fopen($sourceFilepath, "r")) !== FALSE) {
