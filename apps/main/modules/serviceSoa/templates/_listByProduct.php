...
<br>
<a class="bBacketServ__eMore" href="<?php echo url_for('cart_service_delete', array('service' => 'F1ID', 'product' => $product->id));?>">Отменить услугу</a>
</div>
</script>
          ...
          foreach ($list as $service) {  ?>
                <?php if (in_array($service['id'], $servListId)) { ?>
                    <div ref="<?php echo $service['token'];?>">
                        ...
                        <a class="bBacketServ__eMore" href="<?php echo url_for('cart_service_delete', array('service' => $service['id'], 'product' => $product->id));?>">Отменить услугу</a>
                    </div>        
                <?php } ?>
         <?php } ?>
