...
class OrderDefaultForm extends BaseOrderForm
{
  public function configure()
  {
    ...
    if (!$regionList = $this->getOption('regionList', false))
    {
      throw new Exception('Regions not provided');
    }

    // TEMP
    $deliveryTypeList = DeliveryTypeTable::getInstance()->getList();

    // region_id
    $this->widgetSchema['region_id'] = new myWidgetFormComponent(array('component' => array('order', 'field_region_id'), 'component_param' => array('regionList' => $regionList)));
    $this->validatorSchema['region_id'] = new sfValidatorChoice(array('choices' => $regionList->toValueArray('id'), 'required' => true));
    ...
    $this->widgetSchema['payment_method_id'] = new myWidgetFormComponent(array(
      'component' => array('order', 'field_payment_method_id'),
