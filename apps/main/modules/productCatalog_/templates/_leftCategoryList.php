        ...
        <ul>
            ...
            foreach ($list as $key => $item) {
                ...
                <li class="bCtg__eL<?php echo $item['level'] + 1;
                    ...
                    ?> ">
                    <a href="<?php echo url_for('productCatalog__category', array('productCategory' => $item['token_prefix'] ? ($item['token_prefix'] . '/' . $item['token']) : $item['token'])); ?>">
            ...
            <?php if ($currentDirectory->level > 0) { ?>
                <li class="bCtg__eL1 mBold">
                    <a href="<?php echo url_for('productCatalog__category', array('productCategory' => $root_info['token_prefix'] ? ($root_info['token_prefix'] . '/' . $root_info['token']) : $root_info['token'])); ?>">
            ...
            <?php if (isset($root_list['children'])) foreach ($root_list['children'] as $level1): ?>
                ...
                <li class="bCtg__eL2 <?php echo $classCur; ?>">
                    <a href="<?php echo url_for('productCatalog__category', array('productCategory' => $level1['token_prefix'] ? ($level1['token_prefix'] . '' . $level1['token']) : $level1['token'])); ?>">
                        <span>
                            <?php if ($currentDirectory->level > 0) { ?>
                                <div>-</div>
                            <?php } ?>
                    ...
                    <li class="bCtg__eL3 <?php echo $classCur1; ?>">
                        <a href="<?php echo url_for('productCatalog__category', array('productCategory' => $level2['token_prefix'] ? ($level2['token_prefix'] . '/' . $level2['token']) : $level2['token'])); ?>">
                    ...
                    <?php foreach ($tree[$level2['id']]['children'] as $level3) { ?>
                        ...
                        <li class="bCtg__eL4 <?php echo $classCur2; ?>">
                            <a href="<?php echo url_for('productCatalog__category', array('productCategory' => $level3['token_prefix'] ? ($level3['token_prefix'] . '/' . $level3['token']) : $level3['token'])); ?>">
