...
<!-- Filter -->
<form class="product_filter-block" action="<?php echo $url ?>" method="get"
    ...
    <dl class="bigfilter form bSpec">
        <h2>Выбираем:</h2>
        <?php include_component('productCatalog_', 'filter_selected', array('form' => $form, 'productCategory' => $productCategory)) ?>
        <?php $openNum = 0; ?>
        ...
        <dt<?php if (5 > $i) echo ' class="' . ((1 == $i) ? ' first' : '') . '"' ?>>
            ...
            <?php //include_partial('productCatalog_/filter_hint')  ?>
        </dt>

        <?php
        if ($name == 'price' || $name == 'brand') {
            $open = 'block';
        } elseif ($openNum < 5) {
            $openNum++;
            $open = 'block';
