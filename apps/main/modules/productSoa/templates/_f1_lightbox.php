  ...
  <table>
    ...
    <tr>
      <td class="bF1Block_eInfo"><?php echo $service['name'] ?><br>
        <a href="<?php echo url_for('service_show', array('service' => $service['site_token'])) ?>">Подробнее об
          услуге</a>
      </td>
      <td class="bF1Block_eBuy" ref="<?php echo $service['token'] ?>">
        ...
        <?php if ($showInCardButton && in_array($service['id'], $servListId)) { ?>
          <input data-f1title="<?php echo $service['name'] ?>" data-f1price="<?php echo $service['priceFormatted'] ?>"
                 ...
                 data-url="<?php echo url_for('cart_service_add', array('service' => $service['token'], 'product' => $productId)) ?>"
