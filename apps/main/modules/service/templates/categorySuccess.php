 ...
 * @var ServiceCategoryEntity $category
 * @var ServiceCategoryEntity[] $categoryList
 */
?>
<?php slot('title', $category->getName()) ?>

<?php slot('navigation') ?>
  <?php include_component('default', 'navigation', array('list'=>$category->getNavigation()));?>
<?php end_slot() ?>
          ...
          <div class="hf">
            <?php foreach($child->getServiceList() as $service): ?>
              <div class="font16 pb8">
                <a href="<?php echo url_for('service_show', array('service'=>$service->getToken())) ?>" >
