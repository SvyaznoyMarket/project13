<div class="content">
    <div class="vcardtitle">Круглосуточный контакт сENTER</div>
    <div class="vcard"><span class="tel">8 (800) 700 00 09</span></div>
    <div class="address">Звонок бесплатный. Радость в подарок :)</div>
</div>


<div class="links">
    <a href="<?php echo url_for('callback') ?>" class="link6">Обратная связь</a>
    <a href="<?php echo url_for('shop') ?>" class="link5">Наши магазины</a>
    <a href="<?php echo url_for('default_show', array('page' => 'about_company',)) ?>" class="link4">О компании</a>
    <a href="<?php echo url_for('default_show', array('page' => 'f1',)) ?>" class="link1">Сервис</a>
    <!--a class="link2">Связной</a>
    <a class="link3">Финансовые услуги</a-->
</div>

<div class="copy">
  <div class="social_network"><span class="gray font11">Посетите нас</span> <?php include_component('default', 'social_networks', array('view' => 'main')) ?></div>
  <div class="pb5">&copy; &laquo;Enter&raquo; 2011. Все права защищены. <a href="<?php echo url_for('default_show', array('page' => 'terms')) ?>">Условия продажи</a>&nbsp;<a href="<?php echo url_for('default_show', array('page' => 'legal')) ?>">Правовая информация</a></div>
</div>