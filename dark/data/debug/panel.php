...
?>
    <div draggable="true" ondblclick="$(this).remove()" style="position: fixed; top: 24px; left: 2px; width: 260px; overflow: hidden; z-index: 999; background: #000000; color: #00ff00; opacity: 0.8; padding: 4px 6px; border-radius: 5px; font-size: 10px; font-family: Courier New; box-shadow: 0 0 10px rgba(0,0,0,0.5);">
    <? foreach ($debug->getAll() as $item) { ?>
        <?= $item['name'] ?>: <? if (\Debug\Collector::TYPE_ERROR == $item['type']) { ?><span style="color: #ff0000;"><?= $item['value'] ?></span><? } else { ?><?= $item['value'] ?><? } ?>
        <br />
    <? } ?>
    </div>
<?
