...
<div class="bF1Info bBlueButton">
    ...
    <script type="text/html" id="f1look">
        <div ref="<%=fid%>">
            ...
            <a class="bBacketServ__eMore"
               href="<?php echo url_for('cart_service_delete', array('service' => 'F1ID', 'product' => $product->getId()));?>">Отменить услугу</a>
        </div>
    </script>
    ...
    <?php foreach ($listInCart as $service): ?>
        <div ref="<?php echo $service->getToken();?>">
            ...
            <a class="bBacketServ__eMore"
               href="<?php echo url_for('cart_service_delete', array('service' => $service->getId(), 'product' => $product->getId()));?>">Отменить услугу</a>
        </div>
        <?php endforeach ?>
    <?php } else { ?>
