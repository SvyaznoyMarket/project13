...
<dd style="display: <?= $isOpened ? 'block' : 'none' ?>;">
    <ul class="checkbox_list">
        ...
        <li<? if ($i > 4) { ?> class="hf" style="display: none"<? } ?>>
            <input name="<?= $name ?>[<?= $filter->getFilterId()?>][]" type="checkbox" value="<?= $id ?>"<? if (in_array($id, $values)) { ?> checked="checked"<? } ?> id="<?= $name ?>_<?= $filter->getFilterId() ?>_<?= $id ?>" class="hiddenCheckbox" />
