...
abstract class BaseProductTypeForm extends BaseFormDoctrine
{
  ...
  public function updateDefaultsFromObject()
  {
    ...
    if (isset($this->widgetSchema['tag_group_list']))
    {
      $this->setDefault('tag_group_list', $this->object->TagGroup->getPrimaryKeys());
    }
  ...
  public function saveTagGroupList($con = null)
  {
    if (!$this->isValid())
    {
    ...
    $existing = $this->object->TagGroup->getPrimaryKeys();
    $values = $this->getValue('tag_group_list');
    if (!is_array($values))
    {
      $values = array();
    }
    ...
    if (count($unlink))
    {
      $this->object->unlink('TagGroup', array_values($unlink));
    }

    $link = array_diff($values, $existing);
    if (count($link))
    {
      $this->object->link('TagGroup', array_values($link));
    }
  }
