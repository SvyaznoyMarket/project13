...
class RRClient implements \Core\ClientInterface {
    /** @var array */
    private $config;
    /** @var \Curl\Client */
    private $curl;
    ...
    private function getUrl($action, array $params = []) {
        $url = sprintf('%s1.0/Recomendation/%s/%s/', $this->config['apiUrl'], $action, $this->config['account'], http_build_query($params));
        if ((bool)$params) $url .= '?'.http_build_query($params);
        return $url;
    }
} 