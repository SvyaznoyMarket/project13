...
EOF;
  ...
  protected function makeManualGroup($group)
  {
    ...
    foreach ($products as $product)
    {
      $sql = "INSERT IGNORE INTO `similar_product` (`master_id`, `slave_id`) VALUES (".$product.", ".implode("), (".$product.", ", array_diff($products, array($product, ))).");";
  ...
  protected function makeAutoGroup($group)
  {
    ...
    unset($property);
    $products = $this->connection->query("SELECT `id`, `price` FROM `product` WHERE `type_id` = ".$group['product_type_id'])->fetchAll(Doctrine::FETCH_ASSOC);
    ...
    foreach ($products as $product)
    {
      ...
      $sql = "SELECT `product`.`id`, SUM(IF((".$sql_if_section."), 1, 0)) AS `matches` FROM `product` INNER JOIN `product_property_relation` ON `product`.`id` = `product_id` WHERE `product`.`id` <> ".$product['id'].(isset($group['price']) && $product['price'] ? (" AND `product`.`price` BETWEEN ".($product['price'] * (1 - $group['price']))." AND ".($product['price'] * (1 + $group['price']))) : "")." GROUP BY `product`.`id` HAVING `matches` >= ".$group['match'];
