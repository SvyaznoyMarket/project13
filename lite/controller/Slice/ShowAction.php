...
class ShowAction {
     ...
     */
    public function execute(\Http\Request $request, $sliceToken, $categoryId = null) {
        ...
        $category = $this->getCategory($categoryId, $slice, $region);
     ...
     * @return \Model\Product\Category\Entity
     */
    private function getCategory($categoryId, \Model\Slice\Entity $slice, \Model\Region\Entity $region = null) {
        ...
        if ($categoryId) {
            ...
            \App::scmsClient()->addQuery('category/get/v1', $params, [], function($data) use(&$category) {
    ...
    private function prepareSliceCategories(\Model\Slice\Entity $slice, \Model\Region\Entity $region, &$categories = []) {
        ...
        \RepositoryManager::productCategory()->prepareCollectionById($categoryIds, $region, function ($data) use (&$slice, &$categories) {
            ...
            foreach ($data as $item) {
                ...
                $category->setLink($router->generate('slice.category', ['sliceToken' => $slice->getToken(), 'categoryId' => $category->id]));
        ...
        \App::searchClient()->addQuery('category/tree', $params, [], function($data) use (&$category, &$region, $sliceToken, &$isSeoSlice) {
            ...
            if ($isSeoSlice) {
                ...
                $changeCategoryUrlToSliceUrl = function(\Model\Product\Category\Entity $category) use($sliceToken, $helper) {
                    $category->setLink($helper->url('slice.category', ['sliceToken' => $sliceToken, 'categoryId' => $category->id]));
                };
            }
