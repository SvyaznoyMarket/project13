...
class Action {
    public function index($tagToken, \Http\Request $request, $categoryId = null) {
        ...
        if (!$selectedCategory && $categoryId) {
            ...
            call_user_func(function() use($categoryId, $region, &$selectedCategory) {
                $params = [
                    'id' => $categoryId,
                ];
                ...
                \App::scmsClient()->addQuery('category/get/v1', $params, [], function($data) use(&$selectedCategory) {
        ...
        foreach ($categories as $category) {
            ...
            $categoriesByToken[$category->getLink()] = $category;
        ...
        if (!$selectedCategory && $categoryId) {
            if (isset($tagCategoriesById[$categoryId])) { // возьмём его из массива загруженных
                $selectedCategory = $tagCategoriesById[$categoryId];
            }

            // Без $selectedCategory дальше не пойдём в этом случае
            if (!$categoryId) {
                throw new \Exception\NotFoundException(sprintf('Категория @%s не найдена', $categoryId));
            }
        }
