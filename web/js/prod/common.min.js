...
};console.info("Init userbar module"),console.log(u),h.on("click",".jsUpsaleProduct",R),h.on("userLogged",E),h.on("basketUpdate",T),h.on("addtocart",y),h.on("getupsale",b),s.length&&(t=$(u.target),c.length&&c.on("click",g),t.length&&(r=t.offset().top+s.height(),f.on("scroll",_)))}(window.ENTER);