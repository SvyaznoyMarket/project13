	...
	//cnvCompatible=false
		if( ! cnvCompatible || liteversion ) {	
			...
			frontierctx.drawImage( document.getElementById( self.prefx + '1' ) , 
				Math.round( ( mvblockW - self.initres ) / 2 ), Math.round( ( mvblockH - self.initres ) / 2 ), 
				self.initres, self.initres)		
...
function gigaimage( worknode , zoom, /* zoomer node*/ zoo, overwritefn) {
	...
	this.destroy = function() {
		...
		jnode.unbind('mousewheel')
		jnode.unbind('touchstart')
		jnode.unbind('touchmove')
		jnode.unbind('touchend')				
		jnode.remove()
