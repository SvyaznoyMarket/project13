...
if(opts.reallyBig){$('body').append($('<div>').addClass('bPopupWrap').append($self)).append($overlay)}else{$('body').append($self).append($overlay)}
function setHeight(jnode){var jHeight=($(window).height()-80);if(jHeight<500)
jHeight=500
jnode.css('height',jHeight)}
if(opts.reallyBig){$self.css({marginLeft:'50px',marginRight:'50px'});setWrapPosition($self.parent());setHeight($self);}else{$self.css({position:'fixed'})
setSelfPosition();$self.css({left:'50%',marginLeft:($self.outerWidth()/2)*-1,zIndex:(opts.zIndex+3)});}
setOverlayHeight();$overlay.css({position:'fixed',width:'100%',height:'100%',top:0,left:0,zIndex:(opts.zIndex+2)}).css(opts.overlayCSS);if(opts.autofocus){var cusLoad=opts.onLoad
opts.onLoad=function(){cusLoad()
$self.find('input:text').first().focus()}}
if($overlay.is(":hidden")){$overlay.fadeIn(opts.overlaySpeed,function(){$self[opts.appearEffect](opts.lightboxSpeed,function(){setOverlayHeight();opts.onLoad()});});}else{$self[opts.appearEffect](opts.lightboxSpeed,function(){setOverlayHeight();opts.onLoad()});}
$(window).resize(setOverlayHeight).resize(function(){(opts.reallyBig)?setWrapPosition($self.parent()):setSelfPosition()}).resize(function(){if(opts.reallyBig)setHeight($self)}).scroll(function(){if(!opts.reallyBig)setSelfPosition()}).keydown(observeEscapePress);$('body').addClass('showModal');$self.find(opts.closeSelector).click(function(){removeModal(true);return false;});$overlay.click(function(e){e.preventDefault();return false;});function overlayclick(){$overlay.click(function(){if(opts.closeClick){removeModal(true);return false;}})}
setTimeout(overlayclick,500);$self.bind('close.lme',function(){removeModal(true)});$self.bind('resize.lme',function(){(opts.reallyBig)?setWrapPosition($self.parent()):setSelfPosition()});$overlay[0].removeModal=removeModal;function removeModal(removeO){$self[opts.disappearEffect](opts.lightboxDisappearSpeed,function(){if(removeO){removeOverlay();}
opts.destroyOnClose?$self.remove():$self.hide()
if(opts.reallyBig){$('.bPopupWrap').remove()}
$self.find(opts.closeSelector).unbind('click');$self.unbind('.lme');$(window).unbind('scroll',setSelfPosition);$(window).unbind('resize',setSelfPosition);$('body').removeClass('showModal');});}
...
function setWrapPosition($node){var s=$node[0].style;var topOffset=window.pageYOffset;if(!topOffset)
topOffset=document.documentElement.scrollTop;$node.css({position:'absolute',top:(topOffset+40)*1+'px',marginTop:0})
...
s.marginTop=0;}else{var top=(opts.modalCSS&&opts.modalCSS.top)?parseInt(opts.modalCSS.top):0;s.setExpression('top','((blah = document.documentElement.scrollTop ? document.documentElement.scrollTop : document.body.scrollTop) + '+top+') + "px"')}}else{if(opts.centered){$self.css({position:'fixed',top:'50%',marginTop:($self.outerHeight()/2)*-1})}else{$self.css({position:'fixed'}).css(opts.modalCSS);}}}}});};$.fn.lightbox_me.defaults={appearEffect:"fadeIn",overlaySpeed:300,lightboxSpeed:"fast",disappearEffect:"fadeOut",overlayDisappearSpeed:300,lightboxDisappearSpeed:"fast",closeSelector:".close",closeClick:true,closeEsc:true,destroyOnClose:false,onLoad:function(){},onClose:function(){},autofocus:false,classPrefix:'lb',zIndex:999,centered:false,modalCSS:{top:'40px'},overlayCSS:{background:'black',opacity:.4}}})(jQuery);
...
$wrapper.after('<a class="arrow back" title="Назад">&lt;</a><a class="arrow forward" title="Вперед">&gt;</a>');$('a.back',this).click(function(){return gotoPage(currentPage-1);});$('a.forward',this).click(function(){return gotoPage(currentPage+1);});$(this).bind('goto',function(event,page){gotoPage(page);});});};
;(function($){$.fn.typewriter=function(speed,callback){if(typeof callback!=='function')callback=function(){};var write=function(e,text,time){var next=$(e).text().length+1;if(next<=text.length){$(e).text(text.substr(0,next));setTimeout(function(){write(e,text,time);},time);}else{e.callback();}};return this.each(function(){this.callback=callback;var text=$(this).text();var time=speed/text.length;$(this).text('');write(this,text,time)});}}(jQuery));
...
var pasteEventName=getPasteEvent()+".mask",ua=navigator.userAgent,iPhone=/iphone/i.test(ua),android=/android/i.test(ua),caretTimeoutId;$.mask={definitions:{'a':"[A-Za-z]",'*':"[A-Za-z0-9]"},dataName:"rawMaskFn",placeholder:'_'};$.fn.extend({caret:function(begin,end){var range;if(this.length===0||this.is(":hidden")){return;}
...
;(function($){$.fn.goodsCounter=function(params){if(params==='destroy'){return this.each(function(){$(this).find('*').off('.goodsCounter');})}
return this.each(function(){var options=$.extend({},$.fn.goodsCounter.defaults,params),$self=$(this),plusBtn=$self.find(options.plusSelector),minusBtn=$self.find(options.minusSelector),input=$self.find(options.inputSelector),counterGroupName=$self.attr('data-spinner-for'),counterGroup=$('[data-spinner-for="'+counterGroupName+'"]'),timeout_id=0;var changeHandler=function changeHandler(count){clearTimeout(timeout_id);timeout_id=setTimeout(function(){$self.find('input').val(count);options.onChange.apply($self,[count]);},400);},plusHandler=function plusHandler(e){var nowCount=input.val();e.stopPropagation();if($self.hasClass('mDisabled')){return false;}
if((nowCount*1)+1>options.maxVal){return false;}
nowCount++;input.val(nowCount);changeHandler(nowCount);return false;},minusHandler=function minusHandler(e){var nowCount=input.val();e.stopPropagation();if($self.hasClass('mDisabled')){return false;}
if((nowCount*1)-1<1){return false;}
nowCount--;input.val(nowCount);changeHandler(nowCount);return false;},keyupHandler=function keyupHandler(e){var nowCount=input.val();e.stopPropagation();if($self.hasClass('mDisabled')){return false;}
nowCount=input.val();if((nowCount*1)<1){nowCount=1;}
if((nowCount*1)>options.maxVal){nowCount=options.maxVal;}
input.val(nowCount);changeHandler(nowCount);return false;},keydownHandler=function keydownHandler(e){e.stopPropagation();if(e.which===38){plusBtn.trigger('click');return false;}
else if(e.which===40){minusBtn.trigger('click');return false;}
else if(!(((e.which>=48)&&(e.which<=57))||((e.which>=96)&&(e.which<=105))||(e.which===8)||(e.which===46))){return false;}},updatespinner=function updatespinner(e,products){var
products=(products)?products:ENTER.config.clientCart.products,i=0,spinner,input;if(typeof products==='number'){spinner=$('[data-spinner-for="id-cartButton-product-'+products+'"]');spinner.addClass('mDisabled');input=spinner.find('input');input.attr('disabled','disabled');return;}
for(i=products.length-1;i>=0;i--){if(products[i].cartButton!=undefined){spinner=$('[data-spinner-for="'+products[i].cartButton.id+'"]');spinner.addClass('mDisabled');input=spinner.find('input');input.val(products[i].quantity).attr('disabled','disabled');}}};plusBtn.on('click.goodsCounter',plusHandler);minusBtn.on('click.goodsCounter',minusHandler);input.on('keydown.goodsCounter',keydownHandler);input.on('keyup.goodsCounter',keyupHandler);$('body').on('updatespinner.goodsCounter',updatespinner);});};$.fn.goodsCounter.defaults={plusSelector:'.bCountSection__eP',minusSelector:'.bCountSection__eM',inputSelector:'.bCountSection__eNum',maxVal:99,onChange:function(){}};})(jQuery);
if(typeof Object.create!=='function'){Object.create=function(obj){function F(){};F.prototype=obj;return new F();};}
(function($,window,document,undefined){var ElevateZoom={init:function(options,elem){var self=this;self.elem=elem;self.$elem=$(elem);self.imageSrc=self.$elem.data("zoom-image")?self.$elem.data("zoom-image"):self.$elem.attr("src");self.options=$.extend({},$.fn.elevateZoom.options,options);if(self.options.tint){self.options.lensColour="none",self.options.lensOpacity="1"}
if(self.options.zoomType=="inner"){self.options.showLens=false;}
self.$elem.parent().removeAttr('title').removeAttr('alt');self.zoomImage=self.imageSrc;self.refresh(1);$('#'+self.options.gallery+' a').click(function(e){self.$elem.addClass('mLoader');self.$elem.attr("src","");if(self.options.galleryActiveClass){$('#'+self.options.gallery+' a').removeClass(self.options.galleryActiveClass);$(this).addClass(self.options.galleryActiveClass);}
...
if(self.options.disableZoom){console.log('disableZoom');return false;}
...
self.currentImage=largeimage;self.$elem.remove('mLoader');self.$elem.attr("src",smallimage);},doneCallback:function(){var self=this;if(self.options.tint){self.zoomTintImage.attr("src",largeimage);self.zoomTintImage.attr("height",self.$elem.height());self.zoomTintImage.css({height:self.$elem.height()});self.zoomTint.css({height:self.$elem.height()});}
...
if(self.zoomTint){self.zoomTint.hide();}}};$.fn.elevateZoom=function(options){return this.each(function(){var elevate=Object.create(ElevateZoom);elevate.init(options,this);$.data(this,'elevateZoom',elevate);});};$.fn.elevateZoom.options={disableZoom:false,zoomLevel:1,easing:false,easingAmount:12,lensSize:200,zoomWindowWidth:400,zoomWindowHeight:400,zoomWindowOffetx:0,zoomWindowOffety:0,zoomWindowPosition:1,zoomWindowBgColour:"#fff",lensFadeIn:false,lensFadeOut:false,debug:false,zoomWindowFadeIn:false,zoomWindowFadeOut:false,zoomWindowAlwaysShow:false,zoomTintFadeIn:false,zoomTintFadeOut:false,borderSize:4,showLens:true,borderColour:"#888",lensBorderSize:1,lensBorderColour:"#000",lensShape:"square",zoomType:"window",containLensZoom:false,lensColour:"white",lensOpacity:0.4,lenszoom:false,tint:false,tintColour:"#333",tintOpacity:0.4,gallery:false,galleryActiveClass:"zoomGalleryActive",cursor:"default",responsive:false,onComplete:$.noop,onZoomedImageLoaded:function(){},onImageSwap:$.noop,onImageSwapComplete:$.noop};})(jQuery,window,document);
...
parsedShadows.push(normalizeShadow(parsedShadow));return parsedShadows;}});
;(function($){$.fn.customDropDown=function(params){return this.each(function(){var options=$.extend({},$.fn.customDropDown.defaults,params),$self=$(this),select=$self.find(options.selectSelector),value=$self.find(options.valueSelector);var selectChangeHandler=function selectChangeHandler(){var selectedOption=select.find('option:selected');value.html(selectedOption.val());options.changeHandler(selectedOption);};select.on('change',selectChangeHandler);});};$.fn.customDropDown.defaults={valueSelector:'.bDescSelectItem__eValue',selectSelector:'.bDescSelectItem__eSelect',changeHandler:function(){}};})(jQuery);
;(function($){$.fn.goodsSlider=function(params){var slidersWithUrl=0,slidersRecommendation=0,body=$('body'),reqArray=[],recommendArray=[];var
isRecommendation=function isRecommendation(type){return-1==$.inArray(type,["alsoBought","similar","alsoViewed"])?false:true;};this.each(function(){var $self=$(this),sliderParams=$self.data('slider');if(sliderParams.url!==null){slidersWithUrl++;}
if(sliderParams.type!==null&&isRecommendation(sliderParams.type)){slidersRecommendation++;}});var getSlidersData=function getSlidersData(event,url,type,callback){if(isRecommendation(type)){recommendArray.push({type:type,callback:callback});if(recommendArray.length===slidersRecommendation){$.ajax({type:'GET',url:url,success:function(res){var
i,type,callbF;try{for(i in recommendArray){type=recommendArray[i].type;callbF=recommendArray[i].callback;if('undefined'!==typeof(callbF)){if('undefined'!==typeof(type)&&'undefined'!==typeof(res.recommend)&&'undefined'!==typeof(res.recommend[type])){callbF(res.recommend[type]);}
else{callbF(res);}}}}
catch(e){console.warn('Error in RR recomendations');console.log(e);callback({'success':false});}},error:function(e){console.warn('Error in RR ajax response');console.log(e);callback({'success':false});}});}}
else{reqArray.push({type:'GET',url:url,callback:callback});if(reqArray.length===(slidersWithUrl-slidersRecommendation)){window.ENTER.utils.packageReq(reqArray);}}};body.bind('goodsliderneeddata',getSlidersData);var SliderControl=function(mainNode){var
options=$.extend({},$.fn.goodsSlider.defaults,params),$self=mainNode,sliderParams=$self.data('slider'),hasCategory=$self.hasClass('mWithCategory'),leftBtn=$self.find(options.leftArrowSelector),rightBtn=$self.find(options.rightArrowSelector),wrap=$self.find(options.sliderWrapperSelector),slider=$self.find(options.sliderSelector),item=$self.find(options.itemSelector),catItem=$self.find(options.categoryItemSelector),itemW=item.width()+parseInt(item.css('marginLeft'),10)+parseInt(item.css('marginRight'),10),elementOnSlide=parseInt(wrap.width()/itemW,10),nowLeft=0;var
nextSlide=function nextSlide(){if($(this).hasClass('mDisabled')){return false;}
leftBtn.removeClass('mDisabled');if(nowLeft+elementOnSlide*itemW>=slider.width()-elementOnSlide*itemW){nowLeft=slider.width()-elementOnSlide*itemW;rightBtn.addClass('mDisabled');}
else{nowLeft=nowLeft+elementOnSlide*itemW;rightBtn.removeClass('mDisabled');}
console.info(itemW);console.log(elementOnSlide);console.log(nowLeft);console.log(wrap.width());slider.animate({'left':-nowLeft});return false;},prevSlide=function prevSlide(){if($(this).hasClass('mDisabled')){return false;}
rightBtn.removeClass('mDisabled');if(nowLeft-elementOnSlide*itemW<=0){nowLeft=0;leftBtn.addClass('mDisabled');}
else{nowLeft=nowLeft-elementOnSlide*itemW;leftBtn.removeClass('mDisabled');}
slider.animate({'left':-nowLeft});return false;},reWidthSlider=function reWidthSlider(nowItems){leftBtn.addClass('mDisabled');rightBtn.addClass('mDisabled');if(nowItems.length>elementOnSlide){rightBtn.removeClass('mDisabled');}
slider.width(nowItems.length*itemW);nowLeft=0;leftBtn.addClass('mDisabled');slider.css({'left':nowLeft});wrap.removeClass('mLoader');body.trigger('markcartbutton');nowItems.show();},showCategoryGoods=function showCategoryGoods(){var nowCategoryId=catItem.filter('.mActive').attr('id'),showAll=(catItem.filter('.mActive').data('product')==='all'),nowShowItem=(showAll)?item:item.filter('[data-category="'+nowCategoryId+'"]');item.hide();reWidthSlider(nowShowItem);},selectCategory=function selectCategory(){catItem.removeClass('mActive');$(this).addClass('mActive');showCategoryGoods();},authFromServer=function authFromServer(res){var newSlider;if(!res.success){$self.remove();return false;}
newSlider=$(res.content)[0];$self.before(newSlider);$self.remove();$(newSlider).goodsSlider();},errorStatusCode=function errorStatusCode(){console.warn('Слайдер товаров: Неудача при получении данных с сервера');$self.remove();};if(sliderParams.url!==null){if(typeof window.ENTER.utils.packageReq==='function'){body.trigger('goodsliderneeddata',[sliderParams.url,sliderParams.type,authFromServer]);}
else{$.ajax({type:'GET',url:sliderParams.url,success:authFromServer,statusCode:{500:errorStatusCode,503:errorStatusCode,504:errorStatusCode}});}}
else{if(hasCategory){showCategoryGoods();}
else{reWidthSlider(item);}}
rightBtn.on('click',nextSlide);leftBtn.on('click',prevSlide);catItem.on('click',selectCategory);};return this.each(function(){var $self=$(this);new SliderControl($self);});};$.fn.goodsSlider.defaults={leftArrowSelector:'.bSlider__eBtn.mPrev',rightArrowSelector:'.bSlider__eBtn.mNext',sliderWrapperSelector:'.bSlider__eInner',sliderSelector:'.bSlider__eList',itemSelector:'.bSlider__eItem',categoryItemSelector:'.bGoodsSlider__eCatItem'};})(jQuery);
...
this.liveRegion.text(message);}});}(jQuery));(function($,undefined){$.widget("ui.menu",{version:"1.10.3",defaultElement:"<ul>",delay:300,options:{icons:{submenu:"ui-icon-carat-1-e"},menus:"ul",position:{my:"left top",at:"right top"},role:"menu",blur:null,focus:null,select:null},_create:function(){this.activeMenu=this.element;this.mouseHandled=false;this.element.uniqueId().addClass("ui-menu ui-widget ui-widget-content ui-corner-all").toggleClass("ui-menu-icons",!!this.element.find(".ui-icon").length).attr({role:this.options.role,tabIndex:0}).bind("click"+this.eventNamespace,$.proxy(function(event){if(this.options.disabled){event.preventDefault();}},this));if(this.options.disabled){this.element.addClass("ui-state-disabled").attr("aria-disabled","true");}
