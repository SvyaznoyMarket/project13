    ...
    function OrderModel() {
        ...
        function calculateDates( box ) {
            ...
            up: for( var linedate in box.caclDates ) { // Loop for T Interval
                    var datesUp = [];
                    for(i=0, l=box.itemList().length; i<l; i++) { // Loop for all intervals
                        var bidUp = box.token;
                        datesUp = box.itemList()[i].deliveries[bidUp].dates;
                        if( ! thereIsExactPropertie( datesUp, 'timestamp', box.caclDates[linedate].tstamp ) ) {
                    ...
                    box.caclDates[linedate].intervals = getIntervalsFromData( datesUp, 'timestamp', box.caclDates[linedate].tstamp );
                }
        } // fn calculateDates
...
// console.info( newboxes );
                ...
                for( var box in self.dlvrBoxes() ) {
                    ...
                    for( var i =0; i < procBox.itemList().length;  ) {
                        if( 'self_'+d.id in procBox.itemList()[i].deliveries ) {
                            if( procBox.itemList()[i].deliveries['self_'+d.id].dates.length > 1 ) {
                                selectedShopBoxShops[0].items.push( procBox.itemList()[i].token );
                            } else { // items which are 'one day' reserve-only
                                selectedShopBoxShops.push( { shop: d.id, items: [ procBox.itemList()[i].token ] } );
                            }
                            procBox.itemList.remove( procBox.itemList()[i] );
                        } else {
                            i++;
                        }
                    }
