...
this.setDimensionProps=function(b){b=b?b:o[a.zoom-1];c.attr("width",b).attr("height",b).css("left",Math.round((c.parent().innerWidth()-b)/2)).css("top",Math.round(($("body").height()-60-b)/2))};(!cnvCompatible||!n)&&this.setDimensionProps();this.zoomIn=function(){var a=c.parent(),b=a.offset();this.fixzoom(1,Math.round(a.width()/2)+b.left,Math.round(a.height()/2)+b.top)};this.zoomOut=function(){var a=c.parent(),b=a.offset();this.fixzoom(-1,Math.round(a.width()/2)+b.left,Math.round(a.height()/2)+b.top)};
c.parent();this.fixzoom=function(b,q,i){var g=this.zoom;if(!(a.zoom==1&&b<0||a.zoom==3&&b>0)){b>0?a.zoom++:a.zoom--;b/=Math.abs(b);g=o[a.zoom-1]/o[g-1];if(!cnvCompatible||!n){var j=c.parent().offset(),l=parseInt(c.css("left"),10),m=parseInt(c.css("top"),10);q-=j.left+l;i-=j.top+m;l-=b*Math.abs(Math.round(q*(1-g)));m-=b*Math.abs(Math.round(i*(1-g)));c.attr("width",o[a.zoom-1]).attr("height",o[a.zoom-1]).css({left:l,top:m})}else j=c.parent().offset(),q-=j.left+cnvimg.x,i-=j.top+cnvimg.y,cnvimg.x-=b*
Math.abs(Math.round(q*(1-g))),cnvimg.y-=b*Math.abs(Math.round(i*(1-g))),q=c.attr("ref"),h.clearRect(0,0,2500,2500),h.drawImage(document.getElementById(lkmv.prefx+q),cnvimg.x,cnvimg.y,o[a.zoom-1],o[a.zoom-1]);e&&(b>0?e.plus():e.minus())}};this.addZoom=function(b){cnvimg=b;c.bind({mousewheel:function(b,c){b.preventDefault();a.fixzoom(c,b.pageX,b.pageY)}});e.show()};this.noZoom=function(){c.unbind("mousewheel");e.hide()};this.addDrag=function(){function b(c){init={pageX:c.pageX,pageY:c.pageY};i=c.timeStamp;
m=true;a.cursorDrag()}c.bind({mousedown:function(a){a.preventDefault();b(a)}});c[0].addEventListener("touchstart",function(a){a.preventDefault();b(a)},false);c[0].addEventListener("touchmove",function(b){if(m)b.preventDefault(),a.action(b,{x:b.pageX-init.pageX,y:b.pageY-init.pageY},i,a.zoom),init.pageX=b.pageX,init.pageY=b.pageY},false);c[0].addEventListener("touchend",function(){m=false;a.cursorHand()},false)};this.addDrag();$(document).bind("mousemove.zoomer",function(b){if(m)b.preventDefault(),
a.action(b,{x:b.pageX-init.pageX,y:b.pageY-init.pageY},i,a.zoom),init.pageX=b.pageX,init.pageY=b.pageY});$(document).bind("mouseup.zoomer",function(){m=false;a.cursorHand()});this.dropping=function(a,b){var h=parseInt(c.css("left"),10),e=parseInt(c.css("top"),10);c.css("top",b.y+e).css("left",b.x+h)};this.action=j?j:this.dropping;this.destroy=function(){c.unbind("mousedown");c.unbind("mousewheel");c.unbind("touchstart");c.unbind("touchmove");c.unbind("touchend");c.remove();$(document).unbind(".zoomer");
