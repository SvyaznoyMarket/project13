...
a.data("url"));d.submit()}"order[delivery_type_id]"==$(b.target).attr("name")&&(a=c.find('[name="order[delivery_type_id]"]:checked'),a.length&&(h(a.next().find("strong").text()),f(a.val()),b=a.val(),$.post(c.data("updateFieldUrl"),{order:{delivery_type_id:b},field:"delivery_period_id"},function(a){if("undefined"!==typeof a.success&&a.success){var b=$('[name="order[delivery_period_id]"]');b.empty();$.each(a.data.content,function(a,c){b.append('<option value="'+a+'">'+c+"</option>")});b.find(":first").attr("selected",
