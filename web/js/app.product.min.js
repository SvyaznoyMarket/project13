...
q(c.price)+"</div>",a.splice(d,1));for(d in a)c=a[d],1==c.object.core_id&&(b+="<div>&mdash; Можем доставить "+k(c.text)+q(c.price)+"</div>",a.splice(d,1));b+="</li>"}h.html(b+"</ul>")}},"json")}$(".bDropMenu").each(function(){var a=$(this).find("span:first"),b=$(this).find("div");a.css("display","block");a.width()+60<b.width()?a.width(b.width()-70):b.width(a.width()+70)});$(".product_rating-form").live({"form.ajax-submit.prepare":function(){$(this).find("input:submit").attr("disabled",!0)},"form.ajax-submit.success":function(a,
b){!0==b.success&&$(".product_rating-form").effect("highlight",{},2E3)}});$(".product_comment-form").live({"form.ajax-submit.prepare":function(){$(this).find("input:submit").attr("disabled",!0)},"form.ajax-submit.success":function(a,b){$(this).find("input:submit").attr("disabled",!1);!0==b.success&&($($(this).data("listTarget")).replaceWith(b.data.list),$.scrollTo("."+b.data.element_id,500,{onAfter:function(){$("."+b.data.element_id).effect("highlight",{},2E3)}}))}});$(".product_comment_response-link").live({"content.update.prepare":function(){$(".product_comment_response-block").html("")},
"content.update.success":function(){$(".product_comment_response-block").find("textarea:first").focus()}});var m=!1;$(".order1click-link").bind("click",function(a){a.preventDefault();"undefined"!==typeof _gaq&&_gaq.push(["_trackEvent","QuickOrder","Open"]);m?$("#order1click-container").lightbox_me({centered:!0}):j($(this).attr("href"))});$("#order1click-form").bind("submit",function(a){a.preventDefault();var a=$(this),b=a.find("input:submit");b.attr("disabled",!0);b.val("Оформляю заказ...");b=a.serializeArray();
$.ajax({type:"POST",url:a.attr("action"),data:b,success:function(a){a.success?a.data&&($("#order1click-container").find("h2").html(a.data.title),$("#order1click-form").replaceWith(a.data.content),runAnalitics&&runAnalitics()):(a.data&&$("#order1click-form").html(a.data.form),a=$("#order1click-form").find("input:submit"),a.attr("disabled",!1),a.val("Оформить заказ"),$("#warn").length||(a=$('<span id="warn" style="color:red">').html("Не удалось оформить заказ. Приносим свои извинения! Повторите попытку или обратитесь с заказом в контакт cENTER&nbsp;8&nbsp;(800)&nbsp;700&nbsp;00&nbsp;09"),
$(".bFormB2").before(a)))}})});if($("#gMap").length){$("#gMap").bind({create:function(a,b,d,c){var f=$(this),a={zoom:11,center:new google.maps.LatLng(b.latitude,b.longitude),scrollwheel:!1,mapTypeId:google.maps.MapTypeId.ROADMAP,mapTypeControlOptions:{style:google.maps.MapTypeControlStyle.DROPDOWN_MENU}},i=new google.maps.Map(document.getElementById(f.attr("id")),a),a=new InfoBox({disableAutoPan:!1,maxWidth:0,pixelOffset:new google.maps.Size(-11,-108),zIndex:null,boxStyle:{opacity:0.85,width:"280px"},
closeBoxURL:"http://www.google.com/intl/en_us/mapfiles/close.gif",infoBoxClearance:new google.maps.Size(1,1),isHidden:!1,pane:"floatShadow",enableEventPropagation:!0}),e=function(){var a=d[this.id];f.trigger("showMarkers");f.trigger("infoWindow",[this,a])};f.data("markers",[]);$.each(d,function(a,b){var c=new google.maps.Marker({position:new google.maps.LatLng(b.latitude,b.longitude),map:i,title:b.name,icon:"/images/marker.png",id:b.id});google.maps.event.addListener(c,"click",e);f.data("markers").push(c)});
google.maps.event.addListener(i,"bounds_changed",function(){});google.maps.event.addListener(i,"click",function(){});google.maps.event.addListener(a,"closeclick",function(){f.trigger("showMarkers")});f.data("map",i);f.data("infoWindow",a);f.data("infoWindowTemplate",c)},move:function(){$(this).data("map")},infoWindow:function(a,b,d){var c=$(this),a=c.data("map"),f=c.data("infoWindow"),c=c.data("infoWindowTemplate");b.setMap(null);$.each(c.find("[data-name]"),function(a,b){b.innerHTML=d[$(b).data("name")]});
f.setContent(c.prop("innerHTML"));f.open(a,b)},showMarkers:function(){var a=$(this);$.each(a.data("markers"),function(b,d){null==d.map&&d.setMap(a.data("map"))})}});g=$("#gMap");g.trigger("create",[$("#map-center").data("content"),$("#map-markers").data("content"),$("#map-info_window-container")]);g.delegate(".shopchoose","click",function(a){r(a.target)});g[0].addEventListener("touchstart",function(a){a.target.className.match("shopchoose")&&r(a.target)},!1);var r=function(a){j($(a).parent().find(".shopnum").text())};
$(".bInShopLine__eButton a").bind("click",function(a){a.preventDefault();j($(this).attr("href"))});$(".bInShop__eCurrent a").click(function(){$.getJSON("/region/init",function(a){if(!a.success)return!1;var a=a.data,b=$("<div>").addClass("graying").css({opacity:"0.5"}),d=$('<div class="bCityPopupWrap">').html('<div class="hideblock bCityPopup"><i title="Закрыть" class="close">Закрыть</i><div class="title">Привет, из какого ты города?</div></div></div>');d.find(".close").click(function(){$(".graying").remove();
