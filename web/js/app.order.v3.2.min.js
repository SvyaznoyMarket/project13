...
onShopSelected:function(){},onMapClosed:function(b){var d=$(".bBuyingLine__eRadio:checked");regionMap.closeMap();DeliveryMap.onShopSelected.apply(this,[d.val(),b])},validate:function(b,d){$("#order-form");var e=!0;if("order_address"==b.attr("id")&&$('.bBuyingLine__eRadio[data-delivery-type="self"]:checked').length)return!1;b.is(":radio")&&!b.is(":checked")&&1<b.length?(e=!1,showError(b.first().parent().parent(),d,!1)):b.is(":checkbox")&&!b.is(":checked")&&1==b.length?(e=!1,showError(b.first().parent().parent(),
d,!1)):b.is(":text")&&!b.val()&&(e=!1,showError(b,d,!0));return e},onDeliveryBlockChange:function(){1==$(".order-delivery-holder:visible").length?$("#payment_method_online-field").show():($("#payment_method_online-field").hide(),$("#payment_method_online-field").find("input").attr("checked",!1),$("#payment_method_online-field").find(".mChecked").removeClass("mChecked"))}};window.regionMap=new MapWithShops($("#map-center").data("content"),$("#map-info_window-container"),"mapPopup",DeliveryMap.onMapClosed);
$("#order-loader-holder").html("");$("#order-form-part1").show();$("body").delegate(".bImgButton.mBacket","click",function(b){b.preventDefault();var d=$(this),e=d.data("token"),g=function(){$(this).remove();var c=DeliveryMap.data();$.each(c.deliveryTypes,function(a,b){$.each(b.items,function(a,h){h==e&&(c.deliveryTypes[b.token].items.splice(a,1),delete c.items[h])})});var a=$.inArray(e,c.unavailable);-1!==a&&c.unavailable.splice(a,1);DeliveryMap.data(c);DeliveryMap.render();DeliveryMap.onDeliveryBlockChange()};
$.ajax({async:!1,url:d.attr("href"),success:function(){d.closest(".order-item-container").hide("medium",g)}})});$("body").delegate(".bBuyingLine label","click",function(b){if($(b.target).is("input")&&("radio"==$(this).find("input").attr("type")&&(b=$('.mChecked input[name="'+$(this).find("input").attr("name")+'"]'),b.length&&b.each(function(b,d){$(d).parent("label").removeClass("mChecked")}),$(this).addClass("mChecked")),"checkbox"==$(this).find("input").attr("type")&&$(this).toggleClass("mChecked"),
...
this.render()}),DeliveryMap.renderUndeliveredMessage(b.val()));DeliveryMap.onDeliveryBlockChange()}});$("body").delegate(".order-shop-button","click",function(b){b.preventDefault();DeliveryMap.openShopMap($(this).data("delivery"))});$("body").delegate(".order-item_delivery-button","click",function(b){b.preventDefault();var b=DeliveryMap.data(),d=$(this),e=Templating.clone($(d.data("template"))),g=b.items[d.data("value")],c=null;$.each(b.deliveryTypes,function(a,b){$.each(b.items,function(b,f){f==
g.token&&(c=a)})});var a=e.find("a");$.each(g.deliveries,function(b,f){if(f.token!=c){var d=a.clone(),e=DeliveryMap.data().deliveryTypes[f.token].date,e={name:f.name+(e?" "+e:""),route:JSON.stringify({item:g.token,from:c,to:f.token})};Templating.assign(d,e);d.insertAfter(a)}});a.remove();e.appendTo(d.parent());e.bind({mouseleave:function(){$(this).remove()},click:function(a){d=$(a.target);if(d.is("a")){a=d.data("value");DeliveryMap.moveItem(a.item,a.from,a.to);$(this).remove();var c=DeliveryMap.data();
if(!c.deliveryTypes[a.to].date)c.deliveryTypes[a.to].date=c.items[a.item].deliveries[a.to].dates[0].value,DeliveryMap.data(c);DeliveryMap.render()}}});e.show()});$("body").delegate(".order-delivery_date-control","click",function(){var b=$(this);if(!$(b).hasClass("mDisabled")){var d=b.parent(),b=parseInt(b.data("value"));d.find('.order-delivery_date[data-week="'+b+'"]').length&&(d.find(".order-delivery_date").hide(),d.find('.order-delivery_date[data-week="'+b+'"]').show());var e=d.find('.order-delivery_date-control[data-direction="prev"]');
d.find('.order-delivery_date[data-week="'+(b-1)+'"]').length?(e.data("value",b-1),e.removeClass("mDisabled")):e.addClass("mDisabled");e=d.find('.order-delivery_date-control[data-direction="next"]');d.find('.order-delivery_date[data-week="'+(b+1)+'"]').length?(e.data("value",b+1),e.removeClass("mDisabled")):e.addClass("mDisabled")}});$("body").delegate(".order-delivery_date","click",function(){var b=$(this),d=b.closest("ul[data-interval-holder]").data("intervalHolder");if(!b.hasClass("bBuyingDates__eDisable")){var e=
b.closest(".order-delivery-holder"),g=b.data("value"),c=b.data("displayValue"),a=b.data("value"),c=b.data("displayValue"),e=b.closest(".order-delivery-holder"),g=e.data("value"),h=DeliveryMap.data().deliveryTypes[g];b.parent().find(".order-delivery_date").removeClass("bBuyingDates__eCurrent").addClass("bBuyingDates__eEnable");b.removeClass("bBuyingDates__eEnable").addClass("bBuyingDates__eCurrent");e.find("h2 [data-assign]").each(function(a,b){Templating.assign($(b),{displayDate:c})});var f=DeliveryMap.data();
if(d&&a!=h.date){var l=DeliveryMap.getDeliveryInterval(h,a),j=Object.keys(l).shift();f.deliveryTypes[g].interval=j;displayInterval=l[j]?"с "+l[j].start_at+" по "+l[j].end_at:"";e.find("h2 [data-assign]").each(function(a,c){Templating.assign($(c),{displayInterval:displayInterval})})}f.deliveryTypes[g].date=a;DeliveryMap.data(f);if(d){b.closest(".order-delivery-holder").find(".bBuyingDatePopup").remove();var h=DeliveryMap.data().deliveryTypes[g],d=$(b.closest("[data-interval-holder]").data("intervalHolder")),
k=Templating.clone($(d.data("template"))),l=DeliveryMap.getDeliveryInterval(h,a),p=k.find(".order-interval");$.each(l,function(c,b){intervalElement=p.clone();var f=b.start_at+","+b.end_at,d="с "+b.start_at+" по "+b.end_at;Templating.assign(intervalElement,{value:f,date:a,deliveryType:h.token});$.each(intervalElement.find("[data-assign]"),function(a,c){Templating.assign($(c),{name:d})});h.interval==f&&h.date==a&&intervalElement.addClass("bBuyingDatePopup__eOK");intervalElement.appendTo(k)});p.remove();
k.css({left:b.position().left,top:b.position().top});k.mouseenter(function(){clearTimeout($(this).data("timeoutId"))}).mouseleave(function(){var a=$(this);setTimeout(function(){a.remove()},50)});$.each(k.find("[data-assign]"),function(a,b){Templating.assign($(b),{date:c})});k.appendTo(d)}}});$("body").delegate(".order-interval","click",function(){var b=$(this),d=DeliveryMap.data();b.parent().find(".order-interval").each(function(a,c){$(c).removeClass("bBuyingDatePopup__eOK")});b.addClass("bBuyingDatePopup__eOK");
var e=b.data("date"),g=b.data("deliveryType"),c=b.closest(".order-delivery-holder"),a=b.data("value").split(","),a="с "+a[0]+" по "+a[1];c.find("h2 [data-assign]").each(function(c,b){Templating.assign($(b),{displayInterval:a})});d.deliveryTypes[g].date=e;d.deliveryTypes[g].interval=b.data("value");DeliveryMap.data(d);setTimeout(function(){b.closest(".order-delivery-holder").find(".bBuyingDatePopup").hide(50,function(){$(this).remove()})},150)});$('.bBuyingLine__eRadio"]:checked').length?(DeliveryMap.render(),
$("#order-form-part2").show("fast"),"self"==$('.bBuyingLine__eRadio"]:checked:first').data("deliveryType")&&$("#addressField").hide()):window.location.hash&&$(window.location.hash).click();$("#order-submit").data("locked",!1).data("text",$("#order-submit").text()).click(function(b){b.preventDefault();var d=$(this),e=d.closest("form"),b=$(e.data("validator")).data("value");if(d.data("locked"))return!1;e.find(".mRed").removeClass("mRed");e.find(".bFormError").remove();var g=!1;$.each(b,function(c,a){var b=
e.find('[name="'+c+'"]:visible');DeliveryMap.validate(b,a)||(g=!0)});g?$.scrollTo(".bFormError:first",300):(d.text("Оформляю заказ..."),b=e.serializeArray(),b.push({name:"delivery_map",value:JSON.stringify(DeliveryMap.data())}),d.data("locked",!0),$.ajax({url:e.attr("action"),timeout:6E4,async:!0,type:"POST",data:b,success:function(c){var a=$("#order-form");c.success?(d.text("Готово!"),d.attr("disabled",!0),window.location=c.data.redirect):c.error&&"invalid"==c.error.code&&(a.find(".mRed").removeClass("mRed"),
a.find(".bFormError").remove(),$("#order-message").html('<span class="red">'+c.error.message+"</span>"),$.each(c.errors,function(c,b){var d=a.find('[name="'+c+'"]:visible');showError(d,b,!0)}))},error:function(){d.text(d.data("text"))},complete:function(){d.data("locked",!1)}}))});$(".auth-link").bind("click",function(b){b.preventDefault();var d=$(this);$("#login-form, #register-form").data("redirect",!1);$("#auth-block").lightbox_me({centered:!0,onLoad:function(){$("#auth-block").find("input:first").focus()},
onClose:function(){$.get(d.data("updateUrl"),function(b){if(!0===b.success){var d=$(".order-form");$("#user-block").replaceWith(b.data.content);$.each(b.data.fields,function(c,a){var b=d.find('[name="'+c+'"]');2>b.val().length&&b.val(a)})}})}})})});function showError(b,d,e){e&&b.addClass("mRed");b.before('<span class="bFormError mb10 pt5">'+d+"</span>")}
