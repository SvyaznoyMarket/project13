...
d){if(b==d)return[];var e=this.data(),c=[],a=e.deliveryTypes[b].items.length;for(i=0;i<a;i++){var f=e.deliveryTypes[b].items[i];array_key_exists(d,e.items[f].deliveries)?e.deliveryTypes[d].items.push(f):c.push(f)}e.deliveryTypes[b].items=c;this.data(e);return c},getDeliveryPrice:function(b){if(!b)return 0;var d=this.data(),e=0;$.each(b.items,function(c,a){var f=d.items[a].deliveries[b.token].price;f>e&&(e=f)});return parseInt(e)},getDeliveryTotal:function(b){var d=this.data(),e=0;$.each(b.items,function(c,
a){e+=d.items[a].total});return parseInt(e)},getDeliveryDate:function(b){var d=this.data(),e=null;$.each(b.items,function(c,a){var f=[];$.each(d.items[a].deliveries[b.token].dates,function(a,c){f.push(c.value)});e=null===e?f:array_intersect(e,f)});return null===e?[]:array_values(e)},getDeliveryInterval:function(b,d){var e=this.data(),c={};$.each(b.items,function(a,f){$.each(e.items[f].deliveries[b.token].dates,function(a,f){f.value==d&&$.each(f.intervals,function(a,f){c[f.start_at+","+f.end_at]=f})})});
return c},render:function(){var b=this,d=this.data(),e=!0;$.each(d.deliveryTypes,function(a,c){if(c.items.length)return e=!1});if(e){var c=$("#order-form").data("cartUrl");window.location=c?c:"/";return!1}var a=!1;$.when(function(){var f=$.Deferred(),c=d.unavailable.length;c?($.each(d.unavailable,function(b,e){var g=d.items[e];if(!g||0==g.stock||"product"!=g.type||g.quantity<=g.stock)return b+1==c&&f.resolve(),!0;confirm('Вы заказали товар "'+g.name+'" в количестве '+g.quantity+" шт.\nДоступно только "+
...
{times:2},1E3,function(){$(this).find(".delivery-message").html("")}));DeliveryMap.onDeliveryBlockChange()};regionMap.openMap()},onShopSelected:function(){},onMapClosed:function(b){var d=$(".bBuyingLine__eRadio:checked");regionMap.closeMap();DeliveryMap.onShopSelected.apply(this,[d.val(),b])},validate:function(b,d){$("#order-form");var e=!0;if(-1!==$.inArray(b.attr("id"),["order_address_street","order_address_number"])&&!$('.bBuyingLine__eRadio[data-delivery-type="standart"]:checked').length)return console.info(1),
!0;-1!==$.inArray(b.attr("id"),["order_address_street","order_address_number"])&&!b.val()?(console.info(2),e=!1,$("#addressField").find("dd .bFormError").length||showError($("#addressField").find("dd > div:first"),d,!0)):b.is(":radio")&&!b.is(":checked")&&1<b.length?(console.info(3),e=!1,showError(b.first().parent().parent(),d,!1)):b.is(":checkbox")&&!b.is(":checked")&&1==b.length?(console.info(4),e=!1,showError(b.first().parent().parent(),d,!1)):b.is(":text")&&!b.val()&&(console.info(5),e=!1,showError(b,
d,!0));return e},onDeliveryBlockChange:function(){1==$(".order-delivery-holder:visible").length?$("#payment_method_online-field").show():($("#payment_method_online-field").hide(),$("#payment_method_online-field").find("input").attr("checked",!1),$("#payment_method_online-field").find(".mChecked").removeClass("mChecked"))},checkAddressFiled:function(){0<$('.order-delivery-holder[data-type="standart"]:visible').length?$("#addressField").show():$("#addressField").hide()}};window.regionMap=new MapWithShops($("#map-center").data("content"),
$("#map-info_window-container"),"mapPopup",DeliveryMap.onMapClosed);$("#order-loader-holder").html("");$("#order-form-part1").show();$("body").delegate(".bImgButton.mBacket","click",function(b){b.preventDefault();var d=$(this),e=d.data("token"),c=function(){$(this).remove();var a=DeliveryMap.data();$.each(a.deliveryTypes,function(c,f){$.each(f.items,function(c,b){b==e&&(a.deliveryTypes[f.token].items.splice(c,1),delete a.items[b])})});var c=$.inArray(e,a.unavailable);-1!==c&&a.unavailable.splice(c,
1);DeliveryMap.data(a);DeliveryMap.render();DeliveryMap.onDeliveryBlockChange()};$.ajax({async:!1,url:d.attr("href"),success:function(){d.closest(".order-item-container").hide("medium",c)}})});$("body").delegate(".bBuyingLine label","click",function(b){if($(b.target).is("input")&&("radio"==$(this).find("input").attr("type")&&(b=$('.mChecked input[name="'+$(this).find("input").attr("name")+'"]'),b.length&&b.each(function(b,c){$(c).parent("label").removeClass("mChecked")}),$(this).addClass("mChecked")),
"checkbox"==$(this).find("input").attr("type")&&$(this).toggleClass("mChecked"),b=$(this).find('input[type="radio"][data-delivery-type]'),b.length)){var d=$("#order-form").data("deliveryMapUrl");$("#order-form-part2").hide();$(".order-shop-button:first").hide();"self"==b.data("deliveryType")?$(".order-shop-button:first").show():($("#order-loader").clone().appendTo("#order-loader-holder").show(),DeliveryMap.getRemoteData(d,{deliveryTypeId:b.val()},function(){$("#order-loader-holder").html("");$("#order-form-part2").show("fast");
this.render()}),DeliveryMap.renderUndeliveredMessage(b.val()));DeliveryMap.onDeliveryBlockChange()}});$("body").delegate(".order-shop-button","click",function(b){b.preventDefault();DeliveryMap.openShopMap($(this).data("delivery"))});$("body").delegate(".order-item_delivery-button","click",function(b){b.preventDefault();var b=DeliveryMap.data(),d=$(this),e=Templating.clone($(d.data("template"))),c=b.items[d.data("value")],a=null;$.each(b.deliveryTypes,function(f,b){$.each(b.items,function(b,d){d==
c.token&&(a=f)})});var f=e.find("a");$.each(c.deliveries,function(b,d){if(d.token!=a){var e=f.clone(),g=DeliveryMap.data().deliveryTypes[d.token].date,g={name:d.name+(g?" "+g:""),route:JSON.stringify({item:c.token,from:a,to:d.token})};Templating.assign(e,g);e.insertAfter(f)}});f.remove();e.appendTo(d.parent());e.bind({mouseleave:function(){$(this).remove()},click:function(a){d=$(a.target);if(d.is("a")){a=d.data("value");DeliveryMap.moveItem(a.item,a.from,a.to);$(this).remove();var c=DeliveryMap.data();
...
c.remove&&g(this).removeClass(c.remove);d&&d.apply(this,arguments)})})}};g.fn.extend({_show:g.fn.show,_hide:g.fn.hide,__toggle:g.fn.toggle,_addClass:g.fn.addClass,_removeClass:g.fn.removeClass,_toggleClass:g.fn.toggleClass,effect:function(c,a,b,d){return g.effects[c]?g.effects[c].call(this,{method:c,options:a||{},duration:b,callback:d}):null},show:function(){return!arguments[0]||arguments[0].constructor==Number||/(slow|normal|fast)/.test(arguments[0])?this._show.apply(this,arguments):this.effect.apply(this,
...
a,b,d,e){return d*((a=a/e-1)*a*a+1)+b},easeInOutCubic:function(c,a,b,d,e){return 1>(a/=e/2)?d/2*a*a*a+b:d/2*((a-=2)*a*a+2)+b},easeInQuart:function(c,a,b,d,e){return d*(a/=e)*a*a*a+b},easeOutQuart:function(c,a,b,d,e){return-d*((a=a/e-1)*a*a*a-1)+b},easeInOutQuart:function(b,a,f,d,e){return 1>(a/=e/2)?d/2*a*a*a*a+f:-d/2*((a-=2)*a*a*a-2)+f},easeInQuint:function(b,a,f,d,e){return d*(a/=e)*a*a*a*a+f},easeOutQuint:function(b,a,f,d,e){return d*((a=a/e-1)*a*a*a*a+1)+f},easeInOutQuint:function(b,a,f,d,e){return 1>
(a/=e/2)?d/2*a*a*a*a*a+f:d/2*((a-=2)*a*a*a*a+2)+f},easeInSine:function(b,a,f,d,e){return-d*Math.cos(a/e*(Math.PI/2))+d+f},easeOutSine:function(b,a,f,d,e){return d*Math.sin(a/e*(Math.PI/2))+f},easeInOutSine:function(b,a,f,d,e){return-d/2*(Math.cos(Math.PI*a/e)-1)+f},easeInExpo:function(b,a,f,d,e){return 0==a?f:d*Math.pow(2,10*(a/e-1))+f},easeOutExpo:function(b,a,f,d,e){return a==e?f+d:d*(-Math.pow(2,-10*a/e)+1)+f},easeInOutExpo:function(b,a,d,e,g){return 0==a?d:a==g?d+e:1>(a/=g/2)?e/2*Math.pow(2,10*
(a-1))+d:e/2*(-Math.pow(2,-10*--a)+2)+d},easeInCirc:function(b,a,d,e,g){return-e*(Math.sqrt(1-(a/=g)*a)-1)+d},easeOutCirc:function(b,a,d,e,g){return e*Math.sqrt(1-(a=a/g-1)*a)+d},easeInOutCirc:function(b,a,d,e,g){return 1>(a/=g/2)?-e/2*(Math.sqrt(1-a*a)-1)+d:e/2*(Math.sqrt(1-(a-=2)*a)+1)+d},easeInElastic:function(b,a,d,e,g){var b=1.70158,h=0,k=e;if(0==a)return d;if(1==(a/=g))return d+e;h||(h=0.3*g);k<Math.abs(e)?(k=e,b=h/4):b=h/(2*Math.PI)*Math.asin(e/k);return-(k*Math.pow(2,10*(a-=1))*Math.sin((a*
