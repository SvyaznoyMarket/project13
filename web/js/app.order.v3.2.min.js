...
this.data(d);return!0},moveItemBlock:function(a,b){if(a==b)return[];var c=this.data(),d=[],e=c.deliveryTypes[a].items.length;for(i=0;i<e;i++){var f=c.deliveryTypes[a].items[i];array_key_exists(b,c.items[f].deliveries)?c.deliveryTypes[b].items.push(f):d.push(f)}c.deliveryTypes[a].items=d;this.data(c);return d},getDeliveryPrice:function(a){if(!a)return 0;var b=this.data(),c=0;$.each(a.items,function(d,e){var f=b.items[e].deliveries[a.token].price;f>c&&(c=f)});return parseInt(c)},getDeliveryTotal:function(a){var b=
this.data(),c=0;$.each(a.items,function(a,e){c+=b.items[e].total});return parseInt(c)},getDeliveryDate:function(a){var b=this.data(),c=null;$.each(a.items,function(d,e){var f=[];$.each(b.items[e].deliveries[a.token].dates,function(a,b){f.push(b.value)});c=null===c?f:array_intersect(c,f)});return null===c?[]:array_values(c)},getDeliveryInterval:function(a,b){var c=this.data(),d={};$.each(a.items,function(e,f){$.each(c.items[f].deliveries[a.token].dates,function(a,c){c.value==b&&$.each(c.intervals,
function(a,b){d[b.start_at+","+b.end_at]=b})})});return d},render:function(){var a=this,b=this.data();$("#order-submit").removeClass("disable");var c=!0;$.each(b.deliveryTypes,function(a,b){if(b.items.length)return c=!1});if(c){var d=$("#order-form").data("cartUrl");window.location=d?d:"/";return!1}var e=!1;$.when(function(){var a=$.Deferred(),c=b.unavailable.length;c?($.each(b.unavailable,function(d,g){var h=b.items[g];if(!h||0==h.stock||"product"!=h.type||h.quantity<=h.stock)return d+1==c&&a.resolve(),
!0;confirm('Вы заказали товар "'+h.name+'" в количестве '+h.quantity+" шт.\nДоступно только "+h.stock+" шт.\nЗаказать "+h.stock+"шт?")?$.ajax({url:h.deleteUrl}).done(function(){$.ajax({url:h.addUrl}).done(function(){b.items[g].quantity=b.items[g].stock;DeliveryMap.data(b);d+1==c&&a.resolve()})}):$.ajax({url:h.deleteUrl}).done(function(){d+1==c&&a.resolve()});e=!0}),e&&($("#order-form-part2").hide(),$("#order-loader-holder").html(""),$("#order-loader").clone().appendTo("#order-loader-holder").show())):
...
$.ajax({async:!1,url:b.attr("href"),success:function(){b.closest(".order-item-container").hide("medium",d)}})});$("body").delegate(".bBuyingLine label","click",function(a){$("#order-submit").addClass("disable");if($(a.target).is("input")&&("radio"==$(this).find("input").attr("type")&&(a=$('.mChecked input[name="'+$(this).find("input").attr("name")+'"]'),a.length&&a.each(function(a,b){$(b).parent("label").removeClass("mChecked")}),$(this).addClass("mChecked")),"checkbox"==$(this).find("input").attr("type")&&
$(this).toggleClass("mChecked"),a=$(this).find('input[type="radio"][data-delivery-type]'),a.length)){var b=$("#order-form").data("deliveryMapUrl");$("#order-form-part2").hide();$(".order-shop-button:first").hide();"self"==a.data("deliveryType")?$(".order-shop-button:first").show():($("#order-loader").clone().appendTo("#order-loader-holder").show(),DeliveryMap.getRemoteData(b,{deliveryTypeId:a.val()},function(){$("#order-loader-holder").html("");$("#order-form-part2").show("fast");this.render()}),
