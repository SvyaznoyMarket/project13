...
h.token+'"]').click(),$.ajax({url:h.addUrl,async:!1,success:function(){d.items[b].quantity=d.items[b].stock;DeliveryMap.data(d)}})):$('.bImgButton.mBacket[data-token="'+h.token+'"]').click();c=!0});if(c)return $("#order-form-part2").hide(),window.location.reload(),!1}$(".order-delivery-holder").each(function(a,c){c=$(c);b.renderDeliveryType(c);0==d.deliveryTypes[c.data("value")].items.length?c.hide():c.show()});var a=0;$.each(d.deliveryTypes,function(c,h){a+=b.getDeliveryTotal(h)+b.getDeliveryPrice(h)});
$(".order-total-container").find("[data-assign]").each(function(c,b){Templating.assign($(b),{total:printPrice(a)})});var h=$("#order-delivery-holder");$.each(d.deliveryTypes,function(a){h.append($('.order-delivery-holder[data-value="'+a+'"]'))});g=$("#order-form");g.find(".mRed").removeClass("mRed");g.find(".bFormError").remove()},renderDeliveryType:function(b){var d=this,e=this.data(),g=e.deliveryTypes[b.data("value")],c=b.find(".order-item-holder");c.html("");var a=b.find(".order-delivery_price");
a.html("");var h=d.getDeliveryPrice(g);if(0<h){var f=Templating.clone($(a.data("template")));f.find("[data-assign]").each(function(a,c){Templating.assign($(c),{price:h})});f.appendTo(a)}else a.html("Бесплатно");a=b.find(".order-delivery_total-holder");a.html("");var m=d.getDeliveryTotal(g)+d.getDeliveryPrice(g),f=Templating.clone($(a.data("template")));f.find("[data-assign]").each(function(a,c){Templating.assign($(c),{total:printPrice(m),name:"Итого"+("self"!=g.type?" с доставкой":"")})});f.appendTo(a);
var j=d.getDeliveryDate(g);$.each(b.find(".order-delivery_date"),function(a,c){var c=$(c),b=c.data("value"),h=-1!==$.inArray(b,j);c.removeClass("bBuyingDates__eDisable").removeClass("bBuyingDates__eEnable").removeClass("bBuyingDates__eCurrent");g.date==b&&c.addClass("bBuyingDates__eCurrent");h?c.addClass("bBuyingDates__eEnable"):c.addClass("bBuyingDates__eDisable")});$.each(g.items,function(a,b){d.renderItem(c,e.items[b])});b.find("h2 [data-assign]").each(function(a,c){Templating.assign($(c),{displayDate:b.find('.order-delivery_date[data-value="'+
g.date+'"]').data("displayValue")})});b.find(".order-delivery_date").removeClass("bBuyingDates__eCurrent");b.find('.order-delivery_date[data-value="'+g.date+'"]').addClass("bBuyingDates__eCurrent")},renderItem:function(b,d){d.totalFormatted=printPrice(d.total);var e=$(b.data("template")),e=Templating.clone(e);e.find("[data-assign]").each(function(b,c){Templating.assign($(c),d)});e.find(".order-item_delivery-button").remove();b.append(e)},renderUnavailable:function(){var b=this.data(),d=$("#order-unavailable");
d.find(".order-item-holder").html("");b.unavailable.length?($.each(b.unavailable,function(e,g){var c=b.items[g];if(!c)return!1;var a=Templating.clone($("#order-item-template"));c.totalFormatted=printPrice(c.total);a.find("[data-assign]").each(function(a,b){Templating.assign($(b),c)});d.find(".order-item-holder").append(a)}),d.show("fast")):d.hide("fast")},getUndeliveredItem:function(b){var d=this.data(),e={},g=Object.keys(d.items),c=[];$.each(d.deliveryTypes,function(a,c){if(c.id==b)return e=c,!1});
$.each(d.deliveryTypes,function(a,b){("standart"==e.type&&"standart"==b.type||b.token==e.token)&&$.each(b.items,function(a,b){c.push(b)})});return array_values(array_diff(g,c))},renderUndeliveredMessage:function(b){this.getUndeliveredItem(b).length?$("#order-message").html('<span class="red">Некоторые товары не могут быть получены выбранным способом доставки.</span>'):$("#order-message").html("<span>Отличный выбор!</span>")},openShopMap:function(b){this.onShopSelected=!b?function(b,e){$("#order-form-part2").hide();
$("#order-loader").clone().appendTo("#order-loader-holder").show();var g=$("#order-form").data("deliveryMapUrl");DeliveryMap.getRemoteData(g,{deliveryTypeId:b,shopId:e},function(){this.render();$("#order-loader-holder").html("");$("#order-form-part2").show("fast");DeliveryMap.renderUndeliveredMessage(b)},!0)}:function(d,e){var g=DeliveryMap.data(),c="self_"+e,a=DeliveryMap.moveItemBlock(b,c);if(!g.deliveryTypes[c].date)g=DeliveryMap.data(),g.deliveryTypes[c].date=DeliveryMap.getDeliveryDate(g.deliveryTypes[c]).shift(),
g.deliveryTypes[c].displayDate=$('.order-delivery-holder[data-value="'+c+'"]').find('.order-delivery_date[data-value="'+g.deliveryTypes[c].date+'"]').data("displayValue"),DeliveryMap.data(g);DeliveryMap.render();a.length&&($('.order-delivery-holder[data-value="'+b+'"]').find(".delivery-message").html('<div class="red">Невозможно переместить товары</div>'),$('.order-delivery-holder[data-value="'+b+'"]').effect("pulsate",{times:2},1E3,function(){$(this).find(".delivery-message").html("")}))};regionMap.openMap()},
...
d,!1)):b.is(":text")&&!b.val()&&(e=!0,showError(b,d,!0));return e}};window.regionMap=new MapWithShops($("#map-center").data("content"),$("#map-info_window-container"),"mapPopup",DeliveryMap.onMapClosed);$("#order-loader-holder").html("");$("#order-form-part1").show();$("body").delegate(".bImgButton.mBacket","click",function(b){b.preventDefault();var d=$(this),e=d.data("token");$.ajax({async:!1,url:d.attr("href"),success:function(){d.closest(".order-item-container").hide("medium",function(){$(this).remove()})}});
var g=DeliveryMap.data();$.each(g.deliveryTypes,function(c,a){$.each(a.items,function(c,b){b==e&&(g.deliveryTypes[a.token].items.splice(c,1),delete g.items[b])})});b=$.inArray(e,g.unavailable);-1!==b&&g.unavailable.splice(b,1);DeliveryMap.data(g);DeliveryMap.render()});$("body").delegate(".bBuyingLine label","click",function(b){if($(b.target).is("input")&&("radio"==$(this).find("input").attr("type")&&(b=$('.mChecked input[name="'+$(this).find("input").attr("name")+'"]'),b.length&&b.each(function(b,
d){$(d).parent("label").removeClass("mChecked")}),$(this).addClass("mChecked")),"checkbox"==$(this).find("input").attr("type")&&$(this).toggleClass("mChecked"),b=$(this).find('input[type="radio"][data-delivery-type]'),b.length)){var d=$("#order-form").data("deliveryMapUrl");$("#order-form-part2").hide();$(".order-shop-button:first").hide();"self"==b.data("deliveryType")?$(".order-shop-button:first").show():($("#order-loader").clone().appendTo("#order-loader-holder").show(),DeliveryMap.getRemoteData(d,
{deliveryTypeId:b.val()},function(){this.render();$("#order-loader-holder").html("");$("#order-form-part2").show("fast")}),DeliveryMap.renderUndeliveredMessage(b.val()))}});$("body").delegate(".order-shop-button","click",function(b){b.preventDefault();DeliveryMap.openShopMap($(this).data("delivery"))});$("body").delegate(".order-item_delivery-button","click",function(b){b.preventDefault();var b=DeliveryMap.data(),d=$(this),e=Templating.clone($(d.data("template"))),g=b.items[d.data("value")],c=null;
$.each(b.deliveryTypes,function(a,b){$.each(b.items,function(b,f){f==g.token&&(c=a)})});var a=e.find("a");$.each(g.deliveries,function(b,f){if(f.token!=c){var d=a.clone(),e=DeliveryMap.data().deliveryTypes[f.token].date,e={name:f.name+(e?" "+e:""),route:JSON.stringify({item:g.token,from:c,to:f.token})};Templating.assign(d,e);d.insertAfter(a)}});a.remove();e.appendTo(d.parent());e.bind({mouseleave:function(){$(this).remove()},click:function(a){d=$(a.target);if(d.is("a")){a=d.data("value");DeliveryMap.moveItem(a.item,
a.from,a.to);$(this).remove();var c=DeliveryMap.data();if(!c.deliveryTypes[a.to].date)c.deliveryTypes[a.to].date=c.items[a.item].deliveries[a.to].dates[0].value,DeliveryMap.data(c);DeliveryMap.render()}}});e.show()});$("body").delegate(".order-delivery_date-control","click",function(){var b=$(this);if(!$(b).hasClass("mDisabled")){var d=b.parent(),b=parseInt(b.data("value"));d.find('.order-delivery_date[data-week="'+b+'"]').length&&(d.find(".order-delivery_date").hide(),d.find('.order-delivery_date[data-week="'+
...
if(!b.hasClass("bBuyingDates__eDisable")){var e=b.closest(".order-delivery-holder"),g=b.data("value"),c=b.data("displayValue"),a=b.data("value"),c=b.data("displayValue"),e=b.closest(".order-delivery-holder"),g=e.data("value"),h=DeliveryMap.data().deliveryTypes[g];b.parent().find(".order-delivery_date").removeClass("bBuyingDates__eCurrent").addClass("bBuyingDates__eEnable");b.removeClass("bBuyingDates__eEnable").addClass("bBuyingDates__eCurrent");e.find("h2 [data-assign]").each(function(a,b){Templating.assign($(b),
{displayDate:c})});var f=DeliveryMap.data();if(d&&a!=h.date){var m=DeliveryMap.getDeliveryInterval(h,a),j=Object.keys(m).shift();f.deliveryTypes[g].interval=j;displayInterval=m[j]?"с "+m[j].start_at+" по "+m[j].end_at:"";e.find("h2 [data-assign]").each(function(a,c){Templating.assign($(c),{displayInterval:displayInterval})})}f.deliveryTypes[g].date=a;DeliveryMap.data(f);if(d){b.closest(".order-delivery-holder").find(".bBuyingDatePopup").remove();var h=DeliveryMap.data().deliveryTypes[g],d=$(b.closest("[data-interval-holder]").data("intervalHolder")),
k=Templating.clone($(d.data("template"))),m=DeliveryMap.getDeliveryInterval(h,a),p=k.find(".order-interval");$.each(m,function(c,b){intervalElement=p.clone();var f=b.start_at+","+b.end_at,d="с "+b.start_at+" по "+b.end_at;Templating.assign(intervalElement,{value:f,date:a,deliveryType:h.token});$.each(intervalElement.find("[data-assign]"),function(a,c){Templating.assign($(c),{name:d})});h.interval==f&&h.date==a&&intervalElement.addClass("bBuyingDatePopup__eOK");intervalElement.appendTo(k)});p.remove();
...
$("#order-form-part2").show("fast"));$("#order-submit").click(function(b){b.preventDefault();var d=$(this),e=d.closest("form"),b=$(e.data("validator")).data("value");e.find(".mRed").removeClass("mRed");e.find(".bFormError").remove();var g=!1;$.each(b,function(c,a){var b=e.find('[name="'+c+'"]:visible');g=DeliveryMap.validate(b,a)});g?$.scrollTo(".bFormError:first",300):(d.text("Оформляю заказ..."),b=e.serializeArray(),b.push({name:"delivery_map",value:JSON.stringify(DeliveryMap.data())}),$.ajax({url:e.attr("action"),
timeout:6E4,async:!1,type:"POST",data:b,success:function(c){var a=$("#order-form");c.success?(d.text("Готово!").attr("disabled",!0),window.location=c.data.redirect):c.error&&"invalid"==c.error.code&&(a.find(".mRed").removeClass("mRed"),a.find(".bFormError").remove(),$("#order-message").html('<span class="red">'+c.error.message+"</span>"),$.each(c.errors,function(c,b){var d=a.find('[name="'+c+'"]:visible');showError(d,b,!0)}))},complete:function(){d.text("Завершить оформление")}}))})});
...
g[b.trim(c).toLowerCase()]}b.effects={version:"1.7.3",save:function(c,a){for(var b=0;b<a.length;b++)null!==a[b]&&c.data("ec.storage."+a[b],c[0].style[a[b]])},restore:function(c,a){for(var b=0;b<a.length;b++)null!==a[b]&&c.css(a[b],c.data("ec.storage."+a[b]))},setMode:function(b,a){"toggle"==a&&(a=b.is(":hidden")?"show":"hide");return a},getBaseline:function(b,a){var h,f;switch(b[0]){case "top":h=0;break;case "middle":h=0.5;break;case "bottom":h=1;break;default:h=b[0]/a.height}switch(b[1]){case "left":f=
...
0,128],red:[255,0,0],silver:[192,192,192],white:[255,255,255],yellow:[255,255,0],transparent:[255,255,255]};b.easing.jswing=b.easing.swing;b.extend(b.easing,{def:"easeOutQuad",swing:function(c,a,h,f,d){return b.easing[b.easing.def](c,a,h,f,d)},easeInQuad:function(b,a,h,f,d){return f*(a/=d)*a+h},easeOutQuad:function(b,a,h,f,d){return-f*(a/=d)*(a-2)+h},easeInOutQuad:function(b,a,h,f,d){return 1>(a/=d/2)?f/2*a*a+h:-f/2*(--a*(a-2)-1)+h},easeInCubic:function(b,a,h,f,d){return f*(a/=d)*a*a+h},easeOutCubic:function(b,
a,h,f,d){return f*((a=a/d-1)*a*a+1)+h},easeInOutCubic:function(b,a,h,f,d){return 1>(a/=d/2)?f/2*a*a*a+h:f/2*((a-=2)*a*a+2)+h},easeInQuart:function(b,a,d,f,e){return f*(a/=e)*a*a*a+d},easeOutQuart:function(b,a,d,f,e){return-f*((a=a/e-1)*a*a*a-1)+d},easeInOutQuart:function(b,a,d,f,e){return 1>(a/=e/2)?f/2*a*a*a*a+d:-f/2*((a-=2)*a*a*a-2)+d},easeInQuint:function(b,a,d,f,e){return f*(a/=e)*a*a*a*a+d},easeOutQuint:function(b,a,d,f,e){return f*((a=a/e-1)*a*a*a*a+1)+d},easeInOutQuint:function(b,a,d,f,e){return 1>
...
e-b)*2*Math.PI/g))+d},easeOutElastic:function(b,a,d,f,e){var b=1.70158,g=0,k=f;if(0==a)return d;if(1==(a/=e))return d+f;g||(g=0.3*e);k<Math.abs(f)?(k=f,b=g/4):b=g/(2*Math.PI)*Math.asin(f/k);return k*Math.pow(2,-10*a)*Math.sin((a*e-b)*2*Math.PI/g)+f+d},easeInOutElastic:function(b,a,d,f,e){var b=1.70158,g=0,k=f;if(0==a)return d;if(2==(a/=e/2))return d+f;g||(g=e*0.3*1.5);k<Math.abs(f)?(k=f,b=g/4):b=g/(2*Math.PI)*Math.asin(f/k);return 1>a?-0.5*k*Math.pow(2,10*(a-=1))*Math.sin((a*e-b)*2*Math.PI/g)+d:0.5*
k*Math.pow(2,-10*(a-=1))*Math.sin((a*e-b)*2*Math.PI/g)+f+d},easeInBack:function(b,a,d,e,g,j){void 0==j&&(j=1.70158);return e*(a/=g)*a*((j+1)*a-j)+d},easeOutBack:function(b,a,d,e,g,j){void 0==j&&(j=1.70158);return e*((a=a/g-1)*a*((j+1)*a+j)+1)+d},easeInOutBack:function(b,a,d,e,g,j){void 0==j&&(j=1.70158);return 1>(a/=g/2)?e/2*a*a*(((j*=1.525)+1)*a-j)+d:e/2*((a-=2)*a*(((j*=1.525)+1)*a+j)+2)+d},easeInBounce:function(c,a,d,e,g){return e-b.easing.easeOutBounce(c,g-a,0,e,g)+d},easeOutBounce:function(b,
