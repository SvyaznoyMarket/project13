...
moveItemBlock:function(a,b){if(a==b)return[];var c=this.data(),d=[],e=c.deliveryTypes[a].items.length;for(i=0;i<e;i++){var f=c.deliveryTypes[a].items[i];array_key_exists(b,c.items[f].deliveries)?c.deliveryTypes[b].items.push(f):d.push(f)}c.deliveryTypes[a].items=d;this.data(c);return d},getDeliveryPrice:function(a){if(!a)return 0;var b=this.data(),c=0;$.each(a.items,function(d,e){var f=b.items[e].deliveries[a.token].price;f>c&&(c=f)});return parseInt(c)},getDeliveryTotal:function(a){var b=this.data(),
c=0;$.each(a.items,function(a,e){c+=b.items[e].total});return parseInt(c)},getDeliveryDate:function(a){var b=this.data(),c=null;$.each(a.items,function(d,e){var f=[];$.each(b.items[e].deliveries[a.token].dates,function(a,b){f.push(b.value)});c=null===c?f:array_intersect(c,f)});return null===c?[]:array_values(c)},getDeliveryInterval:function(a,b){var c=this.data(),d={};$.each(a.items,function(e,f){$.each(c.items[f].deliveries[a.token].dates,function(a,c){c.value==b&&$.each(c.intervals,function(a,b){d[b.start_at+
","+b.end_at]=b})})});return d},render:function(){var a=this,b=this.data(),c=!0;$.each(b.deliveryTypes,function(a,b){if(b.items.length)return c=!1});if(c){var d=$("#order-form").data("cartUrl");window.location=d?d:"/";return!1}var e=!1;$.when(function(){var a=$.Deferred(),c=b.unavailable.length;c?($.each(b.unavailable,function(d,g){var h=b.items[g];if(!h||0==h.stock||"product"!=h.type||h.quantity<=h.stock)return d+1==c&&a.resolve(),!0;confirm('Вы заказали товар "'+h.name+'" в количестве '+h.quantity+
" шт.\nДоступно только "+h.stock+" шт.\nЗаказать "+h.stock+"шт?")?$.ajax({url:h.deleteUrl}).done(function(){$.ajax({url:h.addUrl}).done(function(){b.items[g].quantity=b.items[g].stock;DeliveryMap.data(b);d+1==c&&a.resolve()})}):$.ajax({url:h.deleteUrl}).done(function(){d+1==c&&a.resolve()});e=!0}),e&&($("#order-form-part2").hide(),$("#order-loader-holder").html(""),$("#order-loader").clone().appendTo("#order-loader-holder").show())):a.resolve();return a.promise()}()).always(function(){if(e)return $("#order-form-part2").hide(),
...
b){Templating.assign($(b),{total:printPrice(c)})});var d=$("#order-delivery-holder");$.each(b.deliveryTypes,function(a){d.append($('.order-delivery-holder[data-value="'+a+'"]'))});var g=$("#order-form");g.find(".mRed").removeClass("mRed");g.find(".bFormError").remove();DeliveryMap.checkAddressFiled()})},renderDeliveryType:function(a){var b=this,c=this.data(),d=c.deliveryTypes[a.data("value")],e=a.find(".order-item-holder");e.html("");var f=a.find(".order-delivery_price");f.html("");var j=b.getDeliveryPrice(d);
if(0<j){var g=Templating.clone($(f.data("template")));g.find("[data-assign]").each(function(a,b){Templating.assign($(b),{price:j})});g.appendTo(f)}else f.html("Бесплатно");f=a.find(".order-delivery_total-holder");f.html("");var l=b.getDeliveryTotal(d)+b.getDeliveryPrice(d),g=Templating.clone($(f.data("template")));g.find("[data-assign]").each(function(a,b){Templating.assign($(b),{total:printPrice(l),name:"Итого"+("self"!=d.type?" с доставкой":"")})});g.appendTo(f);var h=b.getDeliveryDate(d);$.each(a.find(".order-delivery_date"),
function(a,b){var b=$(b),c=b.data("value"),e=-1!==$.inArray(c,h);b.removeClass("bBuyingDates__eDisable").removeClass("bBuyingDates__eEnable").removeClass("bBuyingDates__eCurrent");d.date==c&&b.addClass("bBuyingDates__eCurrent");e?b.addClass("bBuyingDates__eEnable"):b.addClass("bBuyingDates__eDisable")});$.each(d.items,function(a,d){b.renderItem(e,c.items[d])});a.find("h2 [data-assign]").each(function(b,c){Templating.assign($(c),{displayDate:a.find('.order-delivery_date[data-value="'+d.date+'"]').data("displayValue")})});
a.find(".order-delivery_date").removeClass("bBuyingDates__eCurrent");a.find('.order-delivery_date[data-value="'+d.date+'"]').addClass("bBuyingDates__eCurrent");var f=DeliveryMap.getDeliveryInterval(d,d.date),k=$(a.find("[data-interval-holder]").data("intervalHolder")),m=Templating.clone($(k.data("template")));k.html("");$.each(f,function(a,b){intervalElement=m.clone();var c=b.start_at+","+b.end_at,e="с "+b.start_at+" по "+b.end_at;$.each(intervalElement.find("[data-assign]"),function(a,b){Templating.assign($(b),
...
!a?function(a,c){$("#order-form-part2").hide();$("#order-loader").clone().appendTo("#order-loader-holder").show();var d=$("#order-form").data("deliveryMapUrl");DeliveryMap.getRemoteData(d,{deliveryTypeId:a,shopId:c},function(){$("#order-loader-holder").html("");$("#order-form-part2").show("fast");this.render();DeliveryMap.renderUndeliveredMessage(a);DeliveryMap.onDeliveryBlockChange()},!0)}:function(b,c){var d=DeliveryMap.data(),e="self_"+c,f=DeliveryMap.moveItemBlock(a,e);if(!d.deliveryTypes[e].date)d=
DeliveryMap.data(),d.deliveryTypes[e].date=DeliveryMap.getDeliveryDate(d.deliveryTypes[e]).shift(),d.deliveryTypes[e].displayDate=$('.order-delivery-holder[data-value="'+e+'"]').find('.order-delivery_date[data-value="'+d.deliveryTypes[e].date+'"]').data("displayValue"),DeliveryMap.data(d);DeliveryMap.render();f.length&&($('.order-delivery-holder[data-value="'+a+'"]').find(".delivery-message").html('<div class="red">Невозможно переместить товары</div>'),$('.order-delivery-holder[data-value="'+a+'"]').effect("pulsate",
...
$("#order-form-part2").show("fast"),DeliveryMap.checkAddressFiled()):window.location.hash&&$(window.location.hash).click();$("#order-submit").data("locked",!1).data("text",$("#order-submit").text()).click(function(a){a.preventDefault();var b=$(this),c=b.closest("form"),a=$(c.data("validator")).data("value");if(b.data("locked"))return!1;c.find(".mRed").removeClass("mRed");c.find(".bFormError").remove();var d=!1;$.each(a,function(a,b){var g=c.find('[name="'+a+'"]:visible');g.length&&!DeliveryMap.validate(g,
