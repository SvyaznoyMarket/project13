...
$("#order-loader").clone().appendTo("#order-loader-holder").show(),window.location.reload(),!1;b.renderUnavailable();$(".order-delivery-holder").each(function(a,c){c=$(c);b.renderDeliveryType(c);0==d.deliveryTypes[c.data("value")].items.length?c.hide():c.show()});var a=0;$.each(d.deliveryTypes,function(c,h){a+=b.getDeliveryTotal(h)+b.getDeliveryPrice(h)});$(".order-total-container").find("[data-assign]").each(function(c,b){Templating.assign($(b),{total:printPrice(a)})});var h=$("#order-delivery-holder");
$.each(d.deliveryTypes,function(a){h.append($('.order-delivery-holder[data-value="'+a+'"]'))});var f=$("#order-form");f.find(".mRed").removeClass("mRed");f.find(".bFormError").remove()})},renderDeliveryType:function(b){var d=this,e=this.data(),g=e.deliveryTypes[b.data("value")],c=b.find(".order-item-holder");c.html("");var a=b.find(".order-delivery_price");a.html("");var h=d.getDeliveryPrice(g);if(0<h){var f=Templating.clone($(a.data("template")));f.find("[data-assign]").each(function(a,c){Templating.assign($(c),
{price:h})});f.appendTo(a)}else a.html("Бесплатно");a=b.find(".order-delivery_total-holder");a.html("");var l=d.getDeliveryTotal(g)+d.getDeliveryPrice(g),f=Templating.clone($(a.data("template")));f.find("[data-assign]").each(function(a,c){Templating.assign($(c),{total:printPrice(l),name:"Итого"+("self"!=g.type?" с доставкой":"")})});f.appendTo(a);var j=d.getDeliveryDate(g);$.each(b.find(".order-delivery_date"),function(a,c){var c=$(c),b=c.data("value"),h=-1!==$.inArray(b,j);c.removeClass("bBuyingDates__eDisable").removeClass("bBuyingDates__eEnable").removeClass("bBuyingDates__eCurrent");
g.date==b&&c.addClass("bBuyingDates__eCurrent");h?c.addClass("bBuyingDates__eEnable"):c.addClass("bBuyingDates__eDisable")});$.each(g.items,function(a,b){d.renderItem(c,e.items[b])});b.find("h2 [data-assign]").each(function(a,c){Templating.assign($(c),{displayDate:b.find('.order-delivery_date[data-value="'+g.date+'"]').data("displayValue")})});b.find(".order-delivery_date").removeClass("bBuyingDates__eCurrent");b.find('.order-delivery_date[data-value="'+g.date+'"]').addClass("bBuyingDates__eCurrent")},
...
printPrice(c.total);a.find("[data-assign]").each(function(a,b){Templating.assign($(b),c)});d.find(".order-item-holder").append(a)}),d.show("fast")):d.hide("fast")},getUndeliveredItem:function(b){var d=this.data(),e={},g=Object.keys(d.items),c=[];$.each(d.deliveryTypes,function(a,c){if(c.id==b)return e=c,!1});$.each(d.deliveryTypes,function(a,b){("standart"==e.type&&"standart"==b.type||b.token==e.token)&&$.each(b.items,function(a,b){c.push(b)})});return array_values(array_diff(g,c))},renderUndeliveredMessage:function(b){this.getUndeliveredItem(b).length?
$("#order-message").html('<span class="red">Некоторые товары не могут быть получены выбранным способом доставки.</span>'):$("#order-message").html("<span>Отличный выбор!</span>")},openShopMap:function(b){this.onShopSelected=!b?function(b,e){$("#order-form-part2").hide();$("#order-loader").clone().appendTo("#order-loader-holder").show();var g=$("#order-form").data("deliveryMapUrl");DeliveryMap.getRemoteData(g,{deliveryTypeId:b,shopId:e},function(){$("#order-loader-holder").html("");$("#order-form-part2").show("fast");
this.render();DeliveryMap.renderUndeliveredMessage(b);DeliveryMap.onDeliveryBlockChange()},!0)}:function(d,e){var g=DeliveryMap.data(),c="self_"+e,a=DeliveryMap.moveItemBlock(b,c);if(!g.deliveryTypes[c].date)g=DeliveryMap.data(),g.deliveryTypes[c].date=DeliveryMap.getDeliveryDate(g.deliveryTypes[c]).shift(),g.deliveryTypes[c].displayDate=$('.order-delivery-holder[data-value="'+c+'"]').find('.order-delivery_date[data-value="'+g.deliveryTypes[c].date+'"]').data("displayValue"),DeliveryMap.data(g);DeliveryMap.render();
a.length&&($('.order-delivery-holder[data-value="'+b+'"]').find(".delivery-message").html('<div class="red">Невозможно переместить товары</div>'),$('.order-delivery-holder[data-value="'+b+'"]').effect("pulsate",{times:2},1E3,function(){$(this).find(".delivery-message").html("")}));DeliveryMap.onDeliveryBlockChange()};regionMap.openMap()},onShopSelected:function(){},onMapClosed:function(b){var d=$(".bBuyingLine__eRadio:checked");regionMap.closeMap();DeliveryMap.onShopSelected.apply(this,[d.val(),b])},
validate:function(b,d){$("#order-form");var e=!1;if("order_address"==b.attr("id")&&$('.bBuyingLine__eRadio[data-delivery-type="self"]:checked').length)return!1;b.is(":radio")&&!b.is(":checked")&&1<b.length?(e=!0,showError(b.first().parent().parent(),d,!1)):b.is(":checkbox")&&!b.is(":checked")&&1==b.length?(e=!0,showError(b.first().parent().parent(),d,!1)):b.is(":text")&&!b.val()&&(e=!0,showError(b,d,!0));return e},onDeliveryBlockChange:function(){1==$(".order-delivery-holder:visible").length?$("#payment_method_online-field").show():
($("#payment_method_online-field").hide(),$("#payment_method_online-field").find("input").attr("checked",!1),$("#payment_method_online-field").find(".mChecked").removeClass("mChecked"))}};window.regionMap=new MapWithShops($("#map-center").data("content"),$("#map-info_window-container"),"mapPopup",DeliveryMap.onMapClosed);$("#order-loader-holder").html("");$("#order-form-part1").show();$("body").delegate(".bImgButton.mBacket","click",function(b){b.preventDefault();var d=$(this),e=d.data("token"),g=
function(){$(this).remove();var c=DeliveryMap.data();$.each(c.deliveryTypes,function(a,b){$.each(b.items,function(a,h){h==e&&(c.deliveryTypes[b.token].items.splice(a,1),delete c.items[h])})});var a=$.inArray(e,c.unavailable);-1!==a&&c.unavailable.splice(a,1);DeliveryMap.data(c);DeliveryMap.render();DeliveryMap.onDeliveryBlockChange()};$.ajax({async:!1,url:d.attr("href"),success:function(){d.closest(".order-item-container").hide("medium",g)}})});$("body").delegate(".bBuyingLine label","click",function(b){if($(b.target).is("input")&&
("radio"==$(this).find("input").attr("type")&&(b=$('.mChecked input[name="'+$(this).find("input").attr("name")+'"]'),b.length&&b.each(function(b,d){$(d).parent("label").removeClass("mChecked")}),$(this).addClass("mChecked")),"checkbox"==$(this).find("input").attr("type")&&$(this).toggleClass("mChecked"),b=$(this).find('input[type="radio"][data-delivery-type]'),b.length)){var d=$("#order-form").data("deliveryMapUrl");$("#order-form-part2").hide();$(".order-shop-button:first").hide();"self"==b.data("deliveryType")?
($(".order-shop-button:first").show(),$("#addressField").hide()):($("#order-loader").clone().appendTo("#order-loader-holder").show(),$("#addressField").show(),DeliveryMap.getRemoteData(d,{deliveryTypeId:b.val()},function(){$("#order-loader-holder").html("");$("#order-form-part2").show("fast");this.render()}),DeliveryMap.renderUndeliveredMessage(b.val()));DeliveryMap.onDeliveryBlockChange()}});$("body").delegate(".order-shop-button","click",function(b){b.preventDefault();DeliveryMap.openShopMap($(this).data("delivery"))});
$("body").delegate(".order-item_delivery-button","click",function(b){b.preventDefault();var b=DeliveryMap.data(),d=$(this),e=Templating.clone($(d.data("template"))),g=b.items[d.data("value")],c=null;$.each(b.deliveryTypes,function(a,b){$.each(b.items,function(b,f){f==g.token&&(c=a)})});var a=e.find("a");$.each(g.deliveries,function(b,f){if(f.token!=c){var d=a.clone(),e=DeliveryMap.data().deliveryTypes[f.token].date,e={name:f.name+(e?" "+e:""),route:JSON.stringify({item:g.token,from:c,to:f.token})};
Templating.assign(d,e);d.insertAfter(a)}});a.remove();e.appendTo(d.parent());e.bind({mouseleave:function(){$(this).remove()},click:function(a){d=$(a.target);if(d.is("a")){a=d.data("value");DeliveryMap.moveItem(a.item,a.from,a.to);$(this).remove();var c=DeliveryMap.data();if(!c.deliveryTypes[a.to].date)c.deliveryTypes[a.to].date=c.items[a.item].deliveries[a.to].dates[0].value,DeliveryMap.data(c);DeliveryMap.render()}}});e.show()});$("body").delegate(".order-delivery_date-control","click",function(){var b=
...
d.find('.order-delivery_date[data-week="'+(b+1)+'"]').length?(e.data("value",b+1),e.removeClass("mDisabled")):e.addClass("mDisabled")}});$("body").delegate(".order-delivery_date","click",function(){var b=$(this),d=b.closest("ul[data-interval-holder]").data("intervalHolder");if(!b.hasClass("bBuyingDates__eDisable")){var e=b.closest(".order-delivery-holder"),g=b.data("value"),c=b.data("displayValue"),a=b.data("value"),c=b.data("displayValue"),e=b.closest(".order-delivery-holder"),g=e.data("value"),
h=DeliveryMap.data().deliveryTypes[g];b.parent().find(".order-delivery_date").removeClass("bBuyingDates__eCurrent").addClass("bBuyingDates__eEnable");b.removeClass("bBuyingDates__eEnable").addClass("bBuyingDates__eCurrent");e.find("h2 [data-assign]").each(function(a,b){Templating.assign($(b),{displayDate:c})});var f=DeliveryMap.data();if(d&&a!=h.date){var l=DeliveryMap.getDeliveryInterval(h,a),j=Object.keys(l).shift();f.deliveryTypes[g].interval=j;displayInterval=l[j]?"с "+l[j].start_at+" по "+l[j].end_at:
"";e.find("h2 [data-assign]").each(function(a,c){Templating.assign($(c),{displayInterval:displayInterval})})}f.deliveryTypes[g].date=a;DeliveryMap.data(f);if(d){b.closest(".order-delivery-holder").find(".bBuyingDatePopup").remove();var h=DeliveryMap.data().deliveryTypes[g],d=$(b.closest("[data-interval-holder]").data("intervalHolder")),k=Templating.clone($(d.data("template"))),l=DeliveryMap.getDeliveryInterval(h,a),p=k.find(".order-interval");$.each(l,function(c,b){intervalElement=p.clone();var f=
b.start_at+","+b.end_at,d="с "+b.start_at+" по "+b.end_at;Templating.assign(intervalElement,{value:f,date:a,deliveryType:h.token});$.each(intervalElement.find("[data-assign]"),function(a,c){Templating.assign($(c),{name:d})});h.interval==f&&h.date==a&&intervalElement.addClass("bBuyingDatePopup__eOK");intervalElement.appendTo(k)});p.remove();k.css({left:b.position().left,top:b.position().top});k.mouseenter(function(){clearTimeout($(this).data("timeoutId"))}).mouseleave(function(){var a=$(this);setTimeout(function(){a.remove()},
50)});$.each(k.find("[data-assign]"),function(a,b){Templating.assign($(b),{date:c})});k.appendTo(d)}}});$("body").delegate(".order-interval","click",function(){var b=$(this),d=DeliveryMap.data();b.parent().find(".order-interval").each(function(a,c){$(c).removeClass("bBuyingDatePopup__eOK")});b.addClass("bBuyingDatePopup__eOK");var e=b.data("date"),g=b.data("deliveryType"),c=b.closest(".order-delivery-holder"),a=b.data("value").split(","),a="с "+a[0]+" по "+a[1];c.find("h2 [data-assign]").each(function(c,
b){Templating.assign($(b),{displayInterval:a})});d.deliveryTypes[g].date=e;d.deliveryTypes[g].interval=b.data("value");DeliveryMap.data(d);setTimeout(function(){b.closest(".order-delivery-holder").find(".bBuyingDatePopup").hide(50,function(){$(this).remove()})},150)});$('.bBuyingLine__eRadio"]:checked').length&&(DeliveryMap.render(),$("#order-form-part2").show("fast"),"self"==$('.bBuyingLine__eRadio"]:checked:first').data("deliveryType")&&$("#addressField").hide());$("#order-submit").data("locked",
!1).data("text",$("#order-submit").text()).click(function(b){b.preventDefault();var d=$(this),e=d.closest("form"),b=$(e.data("validator")).data("value");if(d.data("locked"))return!1;e.find(".mRed").removeClass("mRed");e.find(".bFormError").remove();var g=!1;$.each(b,function(c,a){var b=e.find('[name="'+c+'"]:visible');g=DeliveryMap.validate(b,a)});g?$.scrollTo(".bFormError:first",300):(d.text("Оформляю заказ..."),b=e.serializeArray(),b.push({name:"delivery_map",value:JSON.stringify(DeliveryMap.data())}),
d.data("locked",!0),$.ajax({url:e.attr("action"),timeout:6E4,async:!0,type:"POST",data:b,success:function(c){var a=$("#order-form");c.success?(d.text("Готово!"),d.attr("disabled",!0),window.location=c.data.redirect):c.error&&"invalid"==c.error.code&&(a.find(".mRed").removeClass("mRed"),a.find(".bFormError").remove(),$("#order-message").html('<span class="red">'+c.error.message+"</span>"),$.each(c.errors,function(c,b){var d=a.find('[name="'+c+'"]:visible');showError(d,b,!0)}))},error:function(){d.text(d.data("text"))},
complete:function(){d.data("locked",!1)}}))})});function showError(b,d,e){e&&b.addClass("mRed");b.before('<span class="bFormError mb10 pt5">'+d+"</span>")}function array_intersect(b){var d={},e=arguments.length,g=e-1,c="",a={},h=0,f="";a:for(c in b){h=1;for(;h<e;){a=arguments[h];for(f in a)a[f]===b[c]&&h===g&&(d[c]=b[c]);continue a}}return d}
function array_diff(b){var d={},e=arguments.length,g="",c=1,a="",h={};a:for(g in b)for(c=1;c<e;c++){h=arguments[c];for(a in h)if(h[a]===b[g])continue a;d[g]=b[g]}return d}function array_values(b){var d=[],e="";if(b&&"object"===typeof b&&b.change_key_case)return b.values();for(e in b)d[d.length]=b[e];return d}function array_key_exists(b,d){return!d||d.constructor!==Array&&d.constructor!==Object?!1:b in d}
...
"function"==typeof h?h:f?f:null,e="string"==typeof h?h:null;return this.each(function(){var f={},h=b(this),g=h.attr("style")||"";if("object"==typeof g)g=g.cssText;if(c.toggle)h.hasClass(c.toggle)?c.remove=c.toggle:c.add=c.toggle;var o=b.extend({},document.defaultView?document.defaultView.getComputedStyle(this,null):this.currentStyle);c.add&&h.addClass(c.add);c.remove&&h.removeClass(c.remove);var n=b.extend({},document.defaultView?document.defaultView.getComputedStyle(this,null):this.currentStyle);
c.add&&h.removeClass(c.add);c.remove&&h.addClass(c.remove);for(var m in n)if("function"!=typeof n[m]&&n[m]&&-1==m.indexOf("Moz")&&-1==m.indexOf("length")&&n[m]!=o[m]&&(m.match(/color/i)||!m.match(/color/i)&&!isNaN(parseInt(n[m],10)))&&("static"!=o.position||"static"==o.position&&!m.match(/left|top|bottom|right/)))f[m]=n[m];h.animate(f,a,e,function(){"object"==typeof b(this).attr("style")?(b(this).attr("style").cssText="",b(this).attr("style").cssText=g):b(this).attr("style",g);c.add&&b(this).addClass(c.add);
...
(a-1))+d:f/2*(-Math.pow(2,-10*--a)+2)+d},easeInCirc:function(b,a,d,f,e){return-f*(Math.sqrt(1-(a/=e)*a)-1)+d},easeOutCirc:function(b,a,d,e,g){return e*Math.sqrt(1-(a=a/g-1)*a)+d},easeInOutCirc:function(b,a,d,e,g){return 1>(a/=g/2)?-e/2*(Math.sqrt(1-a*a)-1)+d:e/2*(Math.sqrt(1-(a-=2)*a)+1)+d},easeInElastic:function(b,a,d,e,g){var b=1.70158,j=0,k=e;if(0==a)return d;if(1==(a/=g))return d+e;j||(j=0.3*g);k<Math.abs(e)?(k=e,b=j/4):b=j/(2*Math.PI)*Math.asin(e/k);return-(k*Math.pow(2,10*(a-=1))*Math.sin((a*
