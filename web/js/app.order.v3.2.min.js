...
-1!==$.inArray(a.attr("id"),["order_address_street","order_address_number"])&&!a.val()?(c=!1,$("#addressField").find("dd .bFormError").length||showError($("#addressField").find("dd > div:first"),b,!0)):a.is(":radio")&&!a.is(":checked")&&1<a.length?(c=!1,showError(a.first().parent().parent(),b,!1)):a.is(":checkbox")&&!a.is(":checked")&&1==a.length?(c=!1,showError(a.first().parent().parent(),b,!1)):a.is(":text")&&!a.val()&&(c=!1,showError(a,b,!0));return c},onDeliveryBlockChange:function(){if($(".order-delivery-holder:visible").length)if(1==
$(".order-delivery-holder:visible").length)$("#payment_method_5-field").show(),$("#payment_method_6-field").show();else{var a=function(a){a.hide();a.find("input").is(":checked")&&(a.find("input").removeAttr("checked"),a.find(".mChecked").removeClass("mChecked"))};a($("#payment_method_5-field"));a($("#payment_method_6-field"))}},checkAddressFiled:function(){0<$('.order-delivery-holder[data-type="standart"]:visible').length?$("#addressField").show():$("#addressField").hide()}};var l=$("#mapPopup_shopInfo"),
...
a.closest(".bSelect").find('[data-event="onSelect"]').text(a.text());a=a.find("[data-value]").data();b.deliveryTypes[a.deliveryType].interval=a.value});$('#order-form-part1 .bBuyingLine__eRadio"]:checked').length?(DeliveryMap.render(),$("#order-form-part2").show("fast"),DeliveryMap.checkAddressFiled()):window.location.hash&&$(window.location.hash).click();$("#order-submit").data("locked",!1).data("text",$("#order-submit").text()).click(function(a){a.preventDefault();var b=$(this),c=b.closest("form"),
a=$(c.data("validator")).data("value");if(b.data("locked")||b.hasClass("disable"))return!1;c.find(".mRed").removeClass("mRed");c.find(".bFormError").remove();var d=!1;$.each(a,function(a,b){var f=c.find('[name="'+a+'"]:visible');f.length&&!DeliveryMap.validate(f,b)&&(d=!0)});d?$.scrollTo(".bFormError:first",300):(b.text("Оформляю заказ..."),blockDiv.block(),a=c.serializeArray(),a.push({name:"delivery_map",value:JSON.stringify(DeliveryMap.data())}),b.data("locked",!0),$.ajax({url:c.attr("action"),
timeout:6E4,async:!0,type:"POST",data:a,success:function(a){var c=$("#order-form");a.success?(b.text("Готово!"),b.attr("disabled",!0),k("credit_on",null,new Date(0),"/"),window.location=a.data.redirect):a.error&&("invalid"==a.error.code?(c.find(".mRed").removeClass("mRed"),c.find(".bFormError").remove(),$("#order-message").html('<span class="red">'+a.error.message+"</span>"),$.each(a.errors,function(a,b){var d=c.find('[name="'+a+'"]:visible');showError(d,b,!0)}),b.text("Завершить оформление"),blockDiv.unblock()):
(alert(a.error.message),window.location.reload()))},error:function(){b.text(b.data("text"));blockDiv.unblock()},complete:function(){b.data("locked",!1)}}))});PubSub.subscribe("authorize",function(a,b){$("#order_recipient_first_name").val(b.first_name);$("#order_recipient_last_name").val(b.last_name);$("#order_recipient_phonenumbers").val(b.phonenumber+"       ");$("#user-block").hide()});$(".auth-link").bind("click",function(a){a.preventDefault();$(this);$("#login-form, #register-form").data("redirect",
!1);$("#auth-block").lightbox_me({centered:!0,onLoad:function(){$("#auth-block").find("input:first").focus()}})});if("undefined"!==typeof $.mask)$.mask.definitions.n="[()0-9 -]",$("#order_recipient_phonenumbers").mask("8nnnnnnnnnnnnnnnnn",{placeholder:" ",maxlength:10}),$("#order_recipient_phonenumbers").val("8"),$.mask.definitions["*"]="[0-9*]",$("#order_sclub_card_number").mask("* ****** ******",{placeholder:"*"}),$("#order_sclub_card_number")[0].getAttribute("value")&&$("#order_sclub_card_number").val($("#order_sclub_card_number")[0].getAttribute("value")),
