...
$("#order_address_metro").autocomplete({source:f,appendTo:"#metrostations",minLength:2}).change(function(){for(var a=0,b=f.length;a<b;a++)if($(this).val()===f[a])return!0;$(this).val("")});if($(".bankWrap").length){var h=$(".bankWrap > .bSelect").data("value"),k=$("<div>").addClass("bSelect__eDropmenu"),m;for(m in h){var j=$("<div>").attr("ref",m).append($("<span>").text(h[m].name));j.click(function(){$(".bankWrap > .bSelect").find("span:first").text($(this).find("span").text());$("input[name=credit_bank_id]").val($(this).attr("ref"))});
