...
if(0<f){var n=Templating.clone($(g.data("template")));n.find("[data-assign]").each(function(a,b){Templating.assign($(b),{price:f})});n.appendTo(g)}else g.html("Бесплатно");g=a.find(".order-delivery_total-holder");g.html("");var h=b.getDeliveryTotal(d)+b.getDeliveryPrice(d),n=Templating.clone($(g.data("template")));n.find("[data-assign]").each(function(a,b){Templating.assign($(b),{total:printPrice(h),name:"Итого"+("self"!=d.type?" с доставкой":"")})});n.appendTo(g);var l=b.getDeliveryDate(d);$.each(a.find(".order-delivery_date"),
...
$("#order_address_metro").autocomplete({source:f,appendTo:"#metrostations",minLength:2}).change(function(){for(var a=0,b=f.length;a<b;a++)if($(this).val()===f[a])return!0;$(this).val("")});if($(".bankWrap").length){var h=$(".bankWrap > .bSelect").data("value"),k=$("<div>").addClass("bSelect__eDropmenu"),m;for(m in h){var j=$("<div>").attr("ref",m).append($("<span>").text(h[m].name));j.click(function(){$(".bankWrap > .bSelect").find("span:first").text($(this).find("span").text());$('input[name="order[credit_bank_id]"]').val($(this).attr("ref"))});
<<<<<<< HEAD
k.append(j)}$(".bankWrap > .bSelect").append(k)}});function showError(f,h,k){k&&f.addClass("mRed");f.before('<span class="bFormError mb10 pt5">'+h+"</span>")}function array_intersect(f){var h={},k=arguments.length,m=k-1,j="",a={},b=0,c="";a:for(j in f){b=1;for(;b<k;){a=arguments[b];for(c in a)a[c]===f[j]&&b===m&&(h[j]=f[j]);continue a}}return h}
function array_diff(f){var h={},k=arguments.length,m="",j=1,a="",b={};a:for(m in f)for(j=1;j<k;j++){b=arguments[j];for(a in b)if(b[a]===f[m])continue a;h[m]=f[m]}return h}function array_values(f){var h=[],k="";if(f&&"object"===typeof f&&f.change_key_case)return f.values();for(k in f)h[h.length]=f[k];return h}function array_key_exists(f,h){return!h||h.constructor!==Array&&h.constructor!==Object?!1:f in h}
=======
k.append(j)}$(".bankWrap > .bSelect").append(k)}$("#order_delivery_type_id_1").click();setTimeout(function(){$("#order_recipient_first_name").val("TEST");$("#order_recipient_last_name").val("TEST");$("#order_recipient_phonenumbers").val("89253333333       ");$("#payment_method_6-field input").click();$("#order_agreed").click()},2E3)});function showError(f,h,k){k&&f.addClass("mRed");f.before('<span class="bFormError mb10 pt5">'+h+"</span>")}
function array_intersect(f){var h={},k=arguments.length,m=k-1,j="",a={},b=0,c="";a:for(j in f){b=1;for(;b<k;){a=arguments[b];for(c in a)a[c]===f[j]&&b===m&&(h[j]=f[j]);continue a}}return h}function array_diff(f){var h={},k=arguments.length,m="",j=1,a="",b={};a:for(m in f)for(j=1;j<k;j++){b=arguments[j];for(a in b)if(b[a]===f[m])continue a;h[m]=f[m]}return h}function array_values(f){var h=[],k="";if(f&&"object"===typeof f&&f.change_key_case)return f.values();for(k in f)h[h.length]=f[k];return h}
function array_key_exists(f,h){return!h||h.constructor!==Array&&h.constructor!==Object?!1:f in h}
>>>>>>> ee0e9f6507bfab02d0f2c4f8b087535301398a1c
if(!Object.keys)Object.keys=function(){var f=Object.prototype.hasOwnProperty,h=!{toString:null}.propertyIsEnumerable("toString"),k="toString,toLocaleString,valueOf,hasOwnProperty,isPrototypeOf,propertyIsEnumerable,constructor".split(","),m=k.length;return function(j){if("object"!==typeof j&&"function"!==typeof j||null===j)throw new TypeError("Object.keys called on non-object");var a=[],b;for(b in j)f.call(j,b)&&a.push(b);if(h)for(b=0;b<m;b++)f.call(j,k[b])&&a.push(k[b]);return a}}();
