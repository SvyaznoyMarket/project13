...
d){var e=this.data(),g=[],c=e.deliveryTypes[b].items.length;for(i=0;i<c;i++){var a=e.deliveryTypes[b].items[i];array_key_exists(d,e.items[a].deliveries)?e.deliveryTypes[d].items.push(a):g.push(a)}e.deliveryTypes[b].items=g;this.data(e);return g},getDeliveryPrice:function(b){var d=this.data(),e=0;$.each(b.items,function(g,c){var a=d.items[c].deliveries[b.token].price;a>e&&(e=a)});return parseInt(e)},getDeliveryTotal:function(b){var d=this.data(),e=0;$.each(b.items,function(b,c){e+=d.items[c].total});
return parseInt(e)},getDeliveryDate:function(b){var d=this.data(),e=null;$.each(b.items,function(g,c){var a=[];$.each(d.items[c].deliveries[b.token].dates,function(c,b){a.push(b.value)});e=null===e?a:array_intersect(e,a)});return null===e?[]:array_values(e)},getDeliveryInterval:function(b,d){var e=this.data(),g={};$.each(b.items,function(c,a){$.each(e.items[a].deliveries[b.token].dates,function(a,c){c.value==d&&$.each(c.intervals,function(a,c){g[c.start_at+","+c.end_at]=c})})});return g},render:function(){var b=
this,d=this.data(),e=!0;$.each(d.deliveryTypes,function(a,c){if(c.items.length)return e=!1});if(e){var g=$("#order-form").data("cartUrl");window.location=g?g:"/"}b.renderUnavailable();if(d.unavailable.length){var c=!1;$.each(d.unavailable,function(a,b){var h=d.items[b];if(!h||0==h.stock||"product"!=h.type)return!0;confirm('Вы заказали товар "'+h.name+'" в количестве '+h.quantity+" шт. Доступно только "+h.stock+" шт. Заказать "+h.stock+"шт?")?($('.bImgButton.mBacket[data-token="'+h.token+'"]').click(),
...
b){Templating.assign($(b),{total:printPrice(a)})});var h=$("#order-delivery-holder");$.each(d.deliveryTypes,function(a){h.append($('.order-delivery-holder[data-value="'+a+'"]'))});g=$("#order-form");g.find(".mRed").removeClass("mRed");g.find(".bFormError").remove()},renderDeliveryType:function(b){var d=this,e=this.data(),g=e.deliveryTypes[b.data("value")],c=b.find(".order-item-holder");c.html("");var a=b.find(".order-delivery_price");a.html("");var h=d.getDeliveryPrice(g);if(0<h){var f=Templating.clone($(a.data("template")));
f.find("[data-assign]").each(function(a,c){Templating.assign($(c),{price:h})});f.appendTo(a)}else a.html("Бесплатно");a=b.find(".order-delivery_total-holder");a.html("");var m=d.getDeliveryTotal(g)+d.getDeliveryPrice(g),f=Templating.clone($(a.data("template")));f.find("[data-assign]").each(function(a,c){Templating.assign($(c),{total:printPrice(m),name:"Итого"+("self"!=g.type?" с доставкой":"")})});f.appendTo(a);var j=d.getDeliveryDate(g);$.each(b.find(".order-delivery_date"),function(a,c){var c=$(c),
...
var g=$("#order-form").data("deliveryMapUrl");DeliveryMap.getRemoteData(g,{deliveryTypeId:b,shopId:e},function(){this.render();$("#order-loader-holder").html("");$("#order-form-part2").show("fast");DeliveryMap.renderUndeliveredMessage(b)},!0)}:function(d,e){var g=DeliveryMap.data(),c="self_"+e,a=DeliveryMap.moveItemBlock(b,c);if(!g.deliveryTypes[c].date)g=DeliveryMap.data(),g.deliveryTypes[c].date=DeliveryMap.getDeliveryDate(g.deliveryTypes[c]).shift(),g.deliveryTypes[c].displayDate=$('.order-delivery-holder[data-value="'+
c+'"]').find('.order-delivery_date[data-value="'+g.deliveryTypes[c].date+'"]').data("displayValue"),DeliveryMap.data(g);DeliveryMap.render();a.length&&($('.order-delivery-holder[data-value="'+b+'"]').find(".delivery-message").html('<div class="red">Невозможно переместить товары</div>'),$('.order-delivery-holder[data-value="'+b+'"]').effect("pulsate",{times:2},1E3,function(){$(this).find(".delivery-message").html("")}))};regionMap.openMap()},onShopSelected:function(){},onMapClosed:function(b){var d=
$(".bBuyingLine__eRadio:checked");regionMap.closeMap();DeliveryMap.onShopSelected.apply(this,[d.val(),b])},validate:function(b,d){$("#order-form");var e=!1;b.is(":radio")&&!b.is(":checked")&&1<b.length?(e=!0,showError(b.first().parent().parent(),d,!1)):b.is(":checkbox")&&!b.is(":checked")&&1==b.length?(e=!0,showError(b.first().parent().parent(),d,!1)):b.is(":text")&&!b.val()&&(e=!0,showError(b,d,!0));return e}};window.regionMap=new MapWithShops($("#map-center").data("content"),$("#map-info_window-container"),
"mapPopup",DeliveryMap.onMapClosed);$("#order-loader-holder").html("");$("#order-form-part1").show();$("body").delegate(".bImgButton.mBacket","click",function(b){b.preventDefault();var d=$(this),e=d.data("token");$.ajax({async:!1,url:d.attr("href"),success:function(){d.closest(".order-item-container").hide("medium",function(){$(this).remove()})}});var g=DeliveryMap.data();$.each(g.deliveryTypes,function(c,a){$.each(a.items,function(c,b){b==e&&(g.deliveryTypes[a.token].items.splice(c,1),delete g.items[b])})});
b=$.inArray(e,g.unavailable);-1!==b&&g.unavailable.splice(b,1);DeliveryMap.data(g);DeliveryMap.render()});$("body").delegate(".bBuyingLine label","click",function(b){if($(b.target).is("input")&&("radio"==$(this).find("input").attr("type")&&(b=$('.mChecked input[name="'+$(this).find("input").attr("name")+'"]'),b.length&&b.each(function(b,d){$(d).parent("label").removeClass("mChecked")}),$(this).addClass("mChecked")),"checkbox"==$(this).find("input").attr("type")&&$(this).toggleClass("mChecked"),b=
$(this).find('input[type="radio"][data-delivery-type]'),b.length)){var d=$("#order-form").data("deliveryMapUrl");$("#order-form-part2").hide();$(".order-shop-button:first").hide();"self"==b.data("deliveryType")?$(".order-shop-button:first").show():($("#order-loader").clone().appendTo("#order-loader-holder").show(),DeliveryMap.getRemoteData(d,{deliveryTypeId:b.val()},function(){this.render();$("#order-loader-holder").html("");$("#order-form-part2").show("fast")}),DeliveryMap.renderUndeliveredMessage(b.val()))}});
$("body").delegate(".order-shop-button","click",function(b){b.preventDefault();DeliveryMap.openShopMap($(this).data("delivery"))});$("body").delegate(".order-item_delivery-button","click",function(b){b.preventDefault();var b=DeliveryMap.data(),d=$(this),e=Templating.clone($(d.data("template"))),g=b.items[d.data("value")],c=null;$.each(b.deliveryTypes,function(a,b){$.each(b.items,function(b,f){f==g.token&&(c=a)})});var a=e.find("a");$.each(g.deliveries,function(b,f){if(f.token!=c){var d=a.clone(),
e=DeliveryMap.data().deliveryTypes[f.token].date,e={name:f.name+(e?" "+e:""),route:JSON.stringify({item:g.token,from:c,to:f.token})};Templating.assign(d,e);d.insertAfter(a)}});a.remove();e.appendTo(d.parent());e.bind({mouseleave:function(){$(this).remove()},click:function(a){d=$(a.target);if(d.is("a")){a=d.data("value");DeliveryMap.moveItem(a.item,a.from,a.to);$(this).remove();var c=DeliveryMap.data();if(!c.deliveryTypes[a.to].date)c.deliveryTypes[a.to].date=c.items[a.item].deliveries[a.to].dates[0].value,
...
b-1),e.removeClass("mDisabled")):e.addClass("mDisabled");e=d.find('.order-delivery_date-control[data-direction="next"]');d.find('.order-delivery_date[data-week="'+(b+1)+'"]').length?(e.data("value",b+1),e.removeClass("mDisabled")):e.addClass("mDisabled")}});$("body").delegate(".order-delivery_date","click",function(){var b=$(this),d=b.closest("ul[data-interval-holder]").data("intervalHolder");if(!b.hasClass("bBuyingDates__eDisable")){var e=b.closest(".order-delivery-holder"),g=b.data("value"),c=b.data("displayValue"),
a=b.data("value"),c=b.data("displayValue"),e=b.closest(".order-delivery-holder"),g=e.data("value"),h=DeliveryMap.data().deliveryTypes[g];b.parent().find(".order-delivery_date").removeClass("bBuyingDates__eCurrent").addClass("bBuyingDates__eEnable");b.removeClass("bBuyingDates__eEnable").addClass("bBuyingDates__eCurrent");e.find("h2 [data-assign]").each(function(a,b){Templating.assign($(b),{displayDate:c})});var f=DeliveryMap.data();if(d&&a!=h.date){var m=DeliveryMap.getDeliveryInterval(h,a),j=Object.keys(m).shift();
f.deliveryTypes[g].interval=j;displayInterval=m[j]?"с "+m[j].start_at+" по "+m[j].end_at:"";e.find("h2 [data-assign]").each(function(a,c){Templating.assign($(c),{displayInterval:displayInterval})})}f.deliveryTypes[g].date=a;DeliveryMap.data(f);if(d){b.closest(".order-delivery-holder").find(".bBuyingDatePopup").remove();var h=DeliveryMap.data().deliveryTypes[g],d=$(b.closest("[data-interval-holder]").data("intervalHolder")),k=Templating.clone($(d.data("template"))),m=DeliveryMap.getDeliveryInterval(h,
a),p=k.find(".order-interval");$.each(m,function(c,b){intervalElement=p.clone();var f=b.start_at+","+b.end_at,d="с "+b.start_at+" по "+b.end_at;Templating.assign(intervalElement,{value:f,date:a,deliveryType:h.token});$.each(intervalElement.find("[data-assign]"),function(a,c){Templating.assign($(c),{name:d})});h.interval==f&&h.date==a&&intervalElement.addClass("bBuyingDatePopup__eOK");intervalElement.appendTo(k)});p.remove();k.css({left:b.position().left,top:b.position().top});k.mouseenter(function(){clearTimeout($(this).data("timeoutId"))}).mouseleave(function(){var a=
$(this);setTimeout(function(){a.remove()},50)});$.each(k.find("[data-assign]"),function(a,b){Templating.assign($(b),{date:c})});k.appendTo(d)}}});$("body").delegate(".order-interval","click",function(){var b=$(this),d=DeliveryMap.data();b.parent().find(".order-interval").each(function(a,c){$(c).removeClass("bBuyingDatePopup__eOK")});b.addClass("bBuyingDatePopup__eOK");var e=b.data("date"),g=b.data("deliveryType"),c=b.closest(".order-delivery-holder"),a=b.data("value").split(","),a="с "+a[0]+" по "+
a[1];c.find("h2 [data-assign]").each(function(c,b){Templating.assign($(b),{displayInterval:a})});d.deliveryTypes[g].date=e;d.deliveryTypes[g].interval=b.data("value");DeliveryMap.data(d);setTimeout(function(){b.closest(".order-delivery-holder").find(".bBuyingDatePopup").hide(50,function(){$(this).remove()})},150)});$('.bBuyingLine__eRadio"]:checked').length&&(DeliveryMap.render(),$("#order-form-part2").show("fast"));$("#order-submit").click(function(b){b.preventDefault();var d=$(this),e=d.closest("form"),
b=$(e.data("validator")).data("value");e.find(".mRed").removeClass("mRed");e.find(".bFormError").remove();var g=!1;$.each(b,function(c,a){var b=e.find('[name="'+c+'"]:visible');g=DeliveryMap.validate(b,a)});g?$.scrollTo(".bFormError:first",300):(d.text("Оформляю заказ..."),b=e.serializeArray(),b.push({name:"delivery_map",value:JSON.stringify(DeliveryMap.data())}),$.ajax({url:e.attr("action"),timeout:6E4,async:!1,type:"POST",data:b,success:function(c){var a=$("#order-form");c.success?(d.text("Готово!").attr("disabled",
!0),window.location=c.data.redirect):c.error&&"invalid"==c.error.code&&(console.info(),$("#order-message").html('<span class="red">'+c.error.message+"</span>"),$.each(c.errors,function(c,b){var d=a.find('[name="'+c+'"]:visible');showError(d,b,!0)}))},complete:function(){d.text("Завершить оформление")}}))})});function showError(b,d,e){e&&b.addClass("mRed");b.before('<span class="bFormError mb10 pt5">'+d+"</span>")}
