...
150)});$('.bBuyingLine__eRadio"]:checked').length&&(DeliveryMap.render(),$("#order-form-part2").show("fast"),"self"==$('.bBuyingLine__eRadio"]:checked:first').data("deliveryType")&&$("#addressField").hide());$("#order-submit").click(function(b){b.preventDefault();var d=$(this),e=d.closest("form"),b=$(e.data("validator")).data("value");e.find(".mRed").removeClass("mRed");e.find(".bFormError").remove();var g=!1;$.each(b,function(c,a){var b=e.find('[name="'+c+'"]:visible');g=DeliveryMap.validate(b,a)});
g?$.scrollTo(".bFormError:first",300):(d.text("Оформляю заказ..."),b=e.serializeArray(),b.push({name:"delivery_map",value:JSON.stringify(DeliveryMap.data())}),$.ajax({url:e.attr("action"),timeout:6E4,async:!0,type:"POST",data:b,success:function(c){var a=$("#order-form");c.success?(d.text("Готово!").attr("disabled",!0),window.location=c.data.redirect):c.error&&"invalid"==c.error.code&&(a.find(".mRed").removeClass("mRed"),a.find(".bFormError").remove(),$("#order-message").html('<span class="red">'+
c.error.message+"</span>"),$.each(c.errors,function(c,b){var d=a.find('[name="'+c+'"]:visible');showError(d,b,!0)}))},complete:function(){d.text("Завершить оформление")}}))})});function showError(b,d,e){e&&b.addClass("mRed");b.before('<span class="bFormError mb10 pt5">'+d+"</span>")}function array_intersect(b){var d={},e=arguments.length,g=e-1,c="",a={},h=0,f="";a:for(c in b){h=1;for(;h<e;){a=arguments[h];for(f in a)a[f]===b[c]&&h===g&&(d[c]=b[c]);continue a}}return d}
