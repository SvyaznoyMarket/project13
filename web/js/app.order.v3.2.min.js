...
$("#order_address_metro").autocomplete({source:f,appendTo:"#metrostations",minLength:2}).change(function(){for(var a=0,b=f.length;a<b;a++)if($(this).val()===f[a])return!0;$(this).val("")});if($(".bankWrap").length){var h=$(".bankWrap > .bSelect").data("value"),k=$(".bankWrap > .creditHref"),n=$("<div>").addClass("bSelect__eDropmenu"),j;for(j in h){var m=$("<div>").attr("ref",j).append($("<span>").text(h[j].name));m.click(function(){var a=$(this).attr("ref");$(".bankWrap > .bSelect").find("span:first").text(h[a].name);
$('input[name="order[credit_bank_id]"]').val(a);k.find("a").attr("href",h[a].href);k.find("span").text("("+h[a].name+")")});n.append(m)}$(".bankWrap > .bSelect").append(n);DirectCredit.init($("#tsCreditCart").data("value"),$("#creditPrice"))}});function showError(f,h,k){k&&f.addClass("mRed");f.before('<span class="bFormError mb10 pt5">'+h+"</span>")}
function array_intersect(f){var h={},k=arguments.length,n=k-1,j="",m={},a=0,b="";a:for(j in f){a=1;for(;a<k;){m=arguments[a];for(b in m)m[b]===f[j]&&a===n&&(h[j]=f[j]);continue a}}return h}function array_diff(f){var h={},k=arguments.length,n="",j=1,m="",a={};a:for(n in f)for(j=1;j<k;j++){a=arguments[j];for(m in a)if(a[m]===f[n])continue a;h[n]=f[n]}return h}function array_values(f){var h=[],k="";if(f&&"object"===typeof f&&f.change_key_case)return f.values();for(k in f)h[h.length]=f[k];return h}
