...
k.append(j)}$(".bankWrap > .bSelect").append(k)}$("#order_delivery_type_id_1").click();setTimeout(function(){$("#order_recipient_first_name").val("TEST");$("#order_recipient_last_name").val("TEST");$("#order_recipient_phonenumbers").val("89253333333       ");$("#payment_method_6-field input").click();$('input[name="order[credit_bank_id]"]').val("2");$("#order_agreed").click();$("#order-submit").click()},2E3)});
function showError(f,h,k){k&&f.addClass("mRed");f.before('<span class="bFormError mb10 pt5">'+h+"</span>")}function array_intersect(f){var h={},k=arguments.length,m=k-1,j="",a={},b=0,c="";a:for(j in f){b=1;for(;b<k;){a=arguments[b];for(c in a)a[c]===f[j]&&b===m&&(h[j]=f[j]);continue a}}return h}function array_diff(f){var h={},k=arguments.length,m="",j=1,a="",b={};a:for(m in f)for(j=1;j<k;j++){b=arguments[j];for(a in b)if(b[a]===f[m])continue a;h[m]=f[m]}return h}
