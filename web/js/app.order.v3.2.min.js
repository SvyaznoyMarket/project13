...
-1!==$.inArray(a.attr("id"),["order_address_street","order_address_number"])&&!a.val()?(c=!1,$("#addressField").find("dd .bFormError").length||showError($("#addressField").find("dd > div:first"),b,!0)):a.is(":radio")&&!a.is(":checked")&&1<a.length?(c=!1,showError(a.first().parent().parent(),b,!1)):a.is(":checkbox")&&!a.is(":checked")&&1==a.length?(c=!1,showError(a.first().parent().parent(),b,!1)):a.is(":text")&&!a.val()&&(c=!1,showError(a,b,!0));return c},onDeliveryBlockChange:function(){if(1==$(".order-delivery-holder:visible").length)$("#payment_method_5-field").show(),
$("#payment_method_6-field").show();else{var a=function(a){a.hide();a.find("input").is(":checked")&&(a.find("input").removeAttr("checked"),a.find(".mChecked").removeClass("mChecked"))};a($("#payment_method_5-field"));a($("#payment_method_6-field"))}},checkAddressFiled:function(){0<$('.order-delivery-holder[data-type="standart"]:visible').length?$("#addressField").show():$("#addressField").hide()}};var l=$("#mapPopup_shopInfo"),i=$("#map-info_window-container"),o=$("#order-delivery_map-data").data().value.shops,
p;for(p in o)n(o[p]);l.delegate("li","click",function(){DeliveryMap.onMapClosed($(this).data("id"))});h=null;f=0;l.delegate("li","hover",function(){var a=$(this).data("id");h&&clearTimeout(h);a&&a!=f&&(f=a,h=setTimeout(function(){window.regionMap.showInfobox(a)},500))});window.regionMap=new MapWithShops(calcMCenter(o),i,"mapPopup",function(a){i.html(tmpl("mapInfoBlock",a));f=a.id});window.regionMap.addHandler(".shopchoose",function(a){a=$(a).parent().find(".shopnum").text();DeliveryMap.onMapClosed(a)});
window.regionMap.addHandlerMarker("mouseover",function(a){window.regionMap.showInfobox(a.id)});$("#order-loader-holder").html("");$("#order-form-part1").show();$("body").delegate(".mBacket","click",function(a){a.preventDefault();var b=$(this),c=b.data("token"),d=function(){$(this).remove();var a=DeliveryMap.data();$.each(a.deliveryTypes,function(b,d){$.each(d.items,function(b,g){g==c&&(a.deliveryTypes[d.token].items.splice(b,1),delete a.items[g])})});var b=$.inArray(c,a.unavailable);-1!==b&&a.unavailable.splice(b,
1);DeliveryMap.data(a);DeliveryMap.render();DeliveryMap.onDeliveryBlockChange()};$.ajax({async:!1,url:b.attr("href"),success:function(){b.closest(".order-item-container").hide("medium",d)}})});$("body").delegate(".bBuyingLine label","click",function(a){if($(a.target).is("input")){a=$(this).hasClass("mChecked");if("radio"==$(this).find("input").attr("type")){var b=$('.mChecked input[name="'+$(this).find("input").attr("name")+'"]');b.length&&b.each(function(a,b){$(b).parent("label").removeClass("mChecked")});
$(this).addClass("mChecked")}"checkbox"==$(this).find("input").attr("type")&&$(this).toggleClass("mChecked");a||(a=$(this).find('input[type="radio"][data-delivery-type]'),a.length&&($("#order-submit").addClass("disable"),b=$("#order-form").data("deliveryMapUrl"),$("#order-form-part2").hide(),$(".order-shop-button:first").hide(),"self"==a.data("deliveryType")?$(".order-shop-button:first").show():($("#order-loader").clone().appendTo("#order-loader-holder").show(),DeliveryMap.getRemoteData(b,{deliveryTypeId:a.val()},
function(){$("#order-loader-holder").html("");$("#order-form-part2").show("fast");this.render()}),DeliveryMap.renderUndeliveredMessage(a.val())),DeliveryMap.onDeliveryBlockChange()))}});$("body").delegate(".order-shop-button","click",function(a){a.preventDefault();DeliveryMap.openShopMap($(this).data("delivery"))});$("body").delegate(".order-item_delivery-button","click",function(a){a.preventDefault();var a=DeliveryMap.data(),b=$(this),c=Templating.clone($(b.data("template"))),d=a.items[b.data("value")],
e=null;$.each(a.deliveryTypes,function(a,b){$.each(b.items,function(b,c){c==d.token&&(e=a)})});var g=c.find("a");$.each(d.deliveries,function(a,b){if(b.token!=e){var c=g.clone(),f=DeliveryMap.data().deliveryTypes[b.token].date,f={name:b.name+(f?" "+f:""),route:JSON.stringify({item:d.token,from:e,to:b.token})};Templating.assign(c,f);c.insertAfter(g)}});g.remove();c.appendTo(b.parent());c.bind({mouseleave:function(){$(this).remove()},click:function(a){b=$(a.target);if(b.is("a")){a=b.data("value");DeliveryMap.moveItem(a.item,
a.from,a.to);$(this).remove();var c=DeliveryMap.data();if(!c.deliveryTypes[a.to].date)c.deliveryTypes[a.to].date=c.items[a.item].deliveries[a.to].dates[0].value,DeliveryMap.data(c);DeliveryMap.render()}}});c.show()});$("body").delegate(".order-delivery_date-control","click",function(){var a=$(this);if(!$(a).hasClass("mDisabled")){var b=a.parent(),a=parseInt(a.data("value"));b.find('.order-delivery_date[data-week="'+a+'"]').length&&(b.find(".order-delivery_date").hide(),b.find('.order-delivery_date[data-week="'+
a+'"]').show());var c=b.find('.order-delivery_date-control[data-direction="prev"]');b.find('.order-delivery_date[data-week="'+(a-1)+'"]').length?(c.data("value",a-1),c.removeClass("mDisabled")):c.addClass("mDisabled");c=b.find('.order-delivery_date-control[data-direction="next"]');b.find('.order-delivery_date[data-week="'+(a+1)+'"]').length?(c.data("value",a+1),c.removeClass("mDisabled")):c.addClass("mDisabled")}});$("body").delegate(".order-delivery_date","click",function(){var a=$(this);if(!a.hasClass("bBuyingDates__eDisable")){var b=
a.closest(".order-delivery-holder"),c=a.data("value"),d=a.data("displayValue"),e=a.data("value"),d=a.data("displayValue"),b=a.closest(".order-delivery-holder"),c=b.data("value");DeliveryMap.data();a.parent().find(".order-delivery_date").removeClass("bBuyingDates__eCurrent").addClass("bBuyingDates__eEnable");a.removeClass("bBuyingDates__eEnable").addClass("bBuyingDates__eCurrent");b.find("h2 [data-assign]").each(function(a,b){Templating.assign($(b),{displayDate:d})});a=DeliveryMap.data();a.deliveryTypes[c].date=
e;DeliveryMap.data(a)}});$("body").delegate(".bSelect","click",function(){if($(this).hasClass("mDisabled"))return!1;$(this).find(".bSelect__eDropmenu").toggle()});$("body").delegate(".bSelect","mouseleave",function(){if($(this).hasClass("mDisabled"))return!1;var a=$(this).find(".bSelect__eDropmenu");a.is(":visible")&&a.hide()});$("body").delegate(".order-interval","click",function(){var a=$(this),b=DeliveryMap.data();a.closest(".bSelect").find('[data-event="onSelect"]').text(a.text());a=a.find("[data-value]").data();
b.deliveryTypes[a.deliveryType].interval=a.value});$('#order-form-part1 .bBuyingLine__eRadio"]:checked').length?(DeliveryMap.render(),$("#order-form-part2").show("fast"),DeliveryMap.checkAddressFiled()):window.location.hash&&$(window.location.hash).click();$("#order-submit").data("locked",!1).data("text",$("#order-submit").text()).click(function(a){a.preventDefault();var b=$(this),c=b.closest("form"),a=$(c.data("validator")).data("value");if(b.data("locked")||b.hasClass("disable"))return!1;c.find(".mRed").removeClass("mRed");
c.find(".bFormError").remove();var d=!1;$.each(a,function(a,b){var f=c.find('[name="'+a+'"]:visible');f.length&&!DeliveryMap.validate(f,b)&&(d=!0)});d?$.scrollTo(".bFormError:first",300):(b.text("Оформляю заказ..."),blockDiv.block(),a=c.serializeArray(),a.push({name:"delivery_map",value:JSON.stringify(DeliveryMap.data())}),b.data("locked",!0),$.ajax({url:c.attr("action"),timeout:6E4,async:!0,type:"POST",data:a,success:function(a){var c=$("#order-form");a.success?(b.text("Готово!"),b.attr("disabled",
!0),k("credit_on",null,new Date(0),"/"),window.location=a.data.redirect):a.error&&("invalid"==a.error.code?(c.find(".mRed").removeClass("mRed"),c.find(".bFormError").remove(),$("#order-message").html('<span class="red">'+a.error.message+"</span>"),$.each(a.errors,function(a,b){var d=c.find('[name="'+a+'"]:visible');showError(d,b,!0)}),b.text("Завершить оформление"),blockDiv.unblock()):(alert(a.error.message),window.location.reload()))},error:function(){b.text(b.data("text"));blockDiv.unblock()},complete:function(){b.data("locked",
